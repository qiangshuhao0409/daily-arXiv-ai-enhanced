...
<details>
  <summary>Details</summary>
Motivation: 为了缓解平台化带来的中心化和垄断趋势，研究旨在通过民主化平台赋能本地社区，支持自我组织的社交协调。

Method: 开发了一个开源的全栈架构，包括元平台、基础平台、支持库和插件，支持两阶段的平台实例化和定制化。
...
<details>
  <summary>Details</summary>
Motivation: 随着空中基础设施的快速发展，无人机作为空中基站（ABSs）可以扩展地面网络服务，提供按需连接并增强应急通信能力。然而，多小区间的干扰问题限制了网络效率。

Method: 提出了一种流体空中网络，利用ABS旋转优化波束成形权重，减少干扰。通过分析二维MIMO阵列的波束成形增益，设计了一种低复杂度算法来确定最佳旋转角度。

Result: 仿真结果表明，在干扰受限区域，提出的ABS旋转方案能显著减少干扰，多小区总速率比固定方向ABSs提升约10%。

Conclusion: ABS旋转是一种有效的干扰管理策略，可显著提升无人机辅助网络的性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式人工智能的发展，语义通信成为高效可靠通信的新范式。多用户场景下，如何满足用户多样化且重叠的需求是关键挑战。

Method: 构建意图感知共享知识库，区分公共和私有语义信息，采用ControlNet增强的扩散模型生成个性化图像，设计语义效率评分（SES）作为优化目标，使用强化学习算法解决联合优化问题。

Result: 仿真结果表明，所提方案在多用户语义提取和波束成形中表现有效。

Conclusion: SS-MGSC框架通过结合意图感知和个性化分发，显著提升了多用户语义通信的效率和用户体验。
...
<details>
  <summary>Details</summary>
Motivation: 现有反向散射标签设计仅支持单传感器，增加了空间开销，而多路复用方法依赖时钟或复杂调制链，导致成本高、体积大且易受时序漂移影响。

Method: mmBack利用共享参考信号同步传感器输入，设计电压分配方案通过单一振荡器和RF开关复用多传感器数据，接收端采用频率跟踪算法和有限状态机解复用。

Result: mmBack的ASIC功耗为25.56uW，支持5个5kHz带宽或3个18kHz带宽的传感器流，信号重构平均SNR超过15dB。

Conclusion: mmBack提供了一种高效、低功耗的多传感器同步采集和复用解决方案，克服了现有设计的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 探讨GenAI在网络优化中的潜力与局限性，填补现有文献中对生成模型关键限制的不足。

Method: 通过分类网络优化问题为一次性优化和马尔可夫决策过程（MDP），回顾关键文献并分析GDMs和LPTMs的应用。

Result: 提出了GDMs在两种优化设置下的理论泛化界限，并揭示了GenAI的局限性，如约束满足困难和概念理解有限。

Conclusion: 强调GenAI在网络优化中的潜力与挑战，提出未来研究方向，如生成与优化的结合。
...
Method: 复制并改进了原始研究中的两个争议性基准测试（Towers of Hanoi和River Crossing），引入逐步提示和协作对话方法。

Result: 发现LRMs在复杂性适中（如8个盘子的河内塔）时表现不佳，但在可解问题（如100对代理的River Crossing）中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM在基准测试中表现优异，但在实际临床环境中未能显著提升诊断效果，因此需要探索如何改进AI系统以更好地支持临床决策。

Method: 通过文献综述，分析了LLM在临床中的局限性，并提出了结合统计学习和专家规则知识的混合方法，如PEIRS和ATHENA-CDS。

Result: 研究发现，纯数据驱动的AI缺乏透明度和因果推理能力，而混合方法能更好地融入临床工作流程并提高可解释性。

Conclusion: 未来的AI决策支持系统应注重解释性，结合神经符号或混合AI，并通过改善人机交互来提升临床实践效果。
...
<details>
  <summary>Details</summary>
Motivation: 随着GenAI的快速发展，AI代理的能力显著提升，但其在智能制造中的定义、能力边界和应用仍不明确。

Method: 系统回顾AI和AI代理技术的发展，分析LLM-Agents、MLLM-Agents和Agentic AI的核心概念与技术进展，探讨其在智能制造中的应用与集成。

Result: 这些新兴AI范式为智能制造的信息处理、环境感知和自主决策开辟了新途径。

Conclusion: 研究揭示了这些技术在智能制造中的潜力，但也指出了定义模糊和实际应用中的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 道德领域的复杂性和模糊性使得评估道德机器的表现标准难以明确，需要一种形式化的方法来描述和验证伦理决策模型。

Method: 使用模糊规则描述伦理决策模型，并通过模糊Petri网进行验证和验证。

Result: 通过医学领域的案例研究验证了所提方法的有效性。

Conclusion: 该方法为伦理决策模型的描述和验证提供了一种可行的形式化工具。
...
Method: 结合LLMs进行作业转录和评分，提供与评分标准对齐的分数、转录文本和置信度评分。

Result: 在20多所机构的实际课程中应用，评分时间平均减少65%，高置信度预测与教师评分一致率达95.4%。

Conclusion: Pensieve是一个高效且可靠的AI辅助评分工具，适用于多学科STEM课程。
...
<details>
  <summary>Details</summary>
Motivation: 提高客户服务质量和响应时间，以增强客户忠诚度和市场份额。

Method: 集成基于LLM的智能体与模糊逻辑，处理短信客户请求。
...
Method: 采用分层框架Agent-as-tool，分离工具调用与推理过程，使模型专注于语言推理。

Result: 在少量样本（180个）上进行微调后，性能显著提升，在Bamboogle任务中表现优异，超过Search-R1。

Conclusion: Agent-as-tool框架有效解决了工具调用与推理过程的耦合问题，提升了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 专利蕴含丰富的技术知识，但访问和解读这些信息仍具挑战性，希望通过LLMs和代理技术挖掘专利中的创新产品概念。

Method: 设计了Agent Ideate框架，结合开源LLMs和基于代理的架构，在计算机科学、自然语言处理和材料化学三个领域进行实验。

Result: 代理方法在创意质量、相关性和新颖性上均优于独立LLMs。

Conclusion: 结合LLMs与代理工作流可显著提升创新流程，释放专利数据中未开发的商业创意潜力。
...
<details>
  <summary>Details</summary>
Motivation: 针对城市地区最后一公里配送效率的需求增长，探索利用店内顾客作为配送员的众包配送模式。

Method: 提出MDP模型处理订单和配送员的不确定性，结合NeurADP进行订单分配和DDQN进行动态定价。

Result: 实验显示，NeurADP + DDQN策略比固定定价节省6.7%成本，比短视基线节省18%，灵活延迟和多目的地路由进一步降低成本。

Conclusion: 动态前瞻性策略在众包配送系统中具有优势，为城市物流运营商提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探讨现有答案集语义的强制性条件是否过于严格，以及是否可以提出更通用的原则来定义答案集语义。

Method: 方法包括：1）分析现有条件的局限性；2）改进Gelfond答案集原则，引入新的支持性和最小性原则；3）定义新的答案集语义；4）评估现有语义；5）计算复杂性分析。

Result: 结果表明，现有条件可能过于严格，改进后的原则能更灵活地定义答案集语义，并支持新的语义定义。

Conclusion: 结论是改进的Gelfond答案集原则为答案集语义提供了更通用的框架，并能够更合理地评估现有语义。
...
### [33] [Cross-Attention Message-Passing Transformers for Code-Agnostic Decoding in 6G Networks](https://arxiv.org/abs/2507.01038)
...
<details>
  <summary>Details</summary>
Motivation: 传统解码器缺乏灵活性和可扩展性，无法满足6G网络异构通信场景的需求。

Method: 提出了CrossMPT（跨注意力消息传递Transformer）、FCrossMPT（基础版CrossMPT）和CrossED（CrossMPT集成解码器），利用Transformer架构实现与编码无关的解码。

Result: CrossMPT在单神经解码器中达到最佳性能，FCrossMPT和CrossED进一步提升了泛化能力和解码性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有BACC方法虽能容忍近似误差，但缺乏对不可信节点的安全性研究。

Method: 采用编码理论方法，提出SBACC，并引入新的评估点选择策略，使DCT编码适用于错误检测和纠正。

Result: 推导了SBACC在无不可信节点时的准确性界限，并优化了DCT编码的纠错能力与计算准确性之间的权衡。

Conclusion: SBACC在安全性和准确性方面优于BACC，为分布式计算提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态指纹定位中模态贡献随条件变化及空间模糊问题，特别是在非视距场景下。

Method: 通过空间上下文聚类和动态融合多模态信息，利用混合专家模型自适应选择可靠线索，并引入辅助损失稳定训练。

Result: 在三个真实城市布局和三个载波频段上，模型实现了亚米级MSE，并将非视距误差减半。

Conclusion: SCADF-MoE首次利用大规模多模态混合专家模型，实现了频率鲁棒的ISAC定位。
...
### [47] [Coding for Quasi-Static Fading Channel with Imperfect CSI at the Transmitter and Quantized Feedback](https://arxiv.org/abs/2507.01464)
*Yuhan Yang,Mei Han,Haonan Zhang,Haoheng Yuan,Fan Cheng,Bin Dai*
...
<details>
  <summary>Details</summary>
Motivation: 经典的SK方案对CSI敏感，难以直接应用于具有不完美CSI和量化反馈的衰落信道，因此需要设计一种适应此类信道的SK型方案。

Method: 在收发器的SK型编码-解码器中引入模格函数和辅助信号，以消除不完美CSI引起的解码错误。

Result: 成功设计出适用于准静态衰落信道且具有不完美CSI和量化反馈的SK型方案，解码错误被完美消除。

Conclusion: 该研究为在不完美CSI和量化反馈条件下设计高效衰落信道编码方案提供了可行方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统JSDM方案在多用户场景中需要复杂的矩阵分解以实现有效信道矩阵的块对角化，增加了系统开销。RIS能够重塑无线环境，为简化传输方案提供了新思路。

Method: 通过将RIS策略性地放置在基站的离散傅里叶变换（DFT）方向上，建立正交视距链路，设计简单的预波束成形方案。基于用户分组，优化RIS反射波束方向以减少组间干扰，并推导信道互相关系数的近似值用于RIS-用户关联。

Result: 数值结果表明，该RIS定制的JSDM设计不仅实现了有效信道的块对角化，还显著提高了多用户传输的总频谱效率。

Conclusion: RIS的引入为简化JSDM传输方案提供了有效途径，显著提升了系统性能，为未来无线通信系统的设计提供了新思路。
...
### [50] [Reconfigurable Intelligent Surface aided Integrated-Navigation-and-Communication in Urban Canyons: A Satellite Selection Approach](https://arxiv.org/abs/2507.01766)
*Tianwei Hou,Da Guan,Xin Sun,Anna Li,Wenqiang Yi,Yuanwei Liu,Arumugam Nallanathan*

Main category: cs.IT

TL;DR: 该研究探讨了利用STAR-RIS辅助的MEO卫星网络在城市峡谷中提供全球定位和通信服务的方法，提出了两种算法（NPA和CPA）优化性能，并分析了其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决城市峡谷中卫星-用户直接链路受阻的问题，同时提供定位和通信服务。

Method: 采用SC和SIC技术，开发了NO-INAC和CO-INAC两种模式，并提出了NPA和CPA算法优化性能。

Result: STAR-RIS可辅助完成城市峡谷用户的定位服务，但无法覆盖室内用户。

Conclusion: STAR-RIS辅助的INAC网络在城市峡谷中有效，但需进一步改进以覆盖室内场景。
...
### [51] [A Hybrid Ensemble Learning Framework for Image-Based Solar Panel Classification](https://arxiv.org/abs/2507.01778)
*Vivek Tetarwal,Sandeep Kumar*

Main category: cs.IT

TL;DR: 论文提出了一种新型的双集成神经网络（DENN），用于基于图像特征分类清洁与脏污的太阳能板，其性能优于现有集成方法。


<details>
  <summary>Details</summary>
Motivation: 随着太阳能系统的普及，需要有效的维护技术以保持其性能，而自动区分清洁与脏污的太阳能板是一个主要挑战。

Method: 采用双集成神经网络（DENN），结合多种集成模型的优势，以提高分类准确性和鲁棒性。

Result: DENN在Deep Solar Eye数据集上达到了最先进的准确率，优于其他现有方法。

Conclusion: 该研究展示了混合集成学习技术在自动化太阳能板检测中的潜力，为解决实际问题提供了可扩展的解决方案。
...
### [52] [Symbiotic Backscatter Communication: A Design Perspective on the Modulation Scheme of Backscatter Devices](https://arxiv.org/abs/2507.01782)
*Yinghui Ye,Shuang Lu,Liqin Shi,Xiaoli Chu,Sumei Sun*

Main category: cs.IT

TL;DR: 研究了低复杂度调制方案（MASK/MPSK）在共生反向散射通信（SBC）中的应用，推导了主发射器（PT）和反向散射设备（BD）的速率，并优化了调制相位以最大化PT速率。


<details>
  <summary>Details</summary>
Motivation: 现有研究假设BD信号为CSCG分布，但低成本的BD难以实现高复杂度的CSCG信号生成，因此探讨更实际的低复杂度调制方案。

Method: 采用MASK和MPSK调制方案，推导PT和BD的速率表达式，并通过优化相位设计最大化PT速率。

Result: 理论分析表明，BD是否提升PT速率取决于调制相位，而BD速率与相位无关。仿真显示低阶ASK优于低阶PSK。

Conclusion: 优化后的MASK/MPSK相位可提升PT速率，低阶ASK在直接链路不显著弱于反向散射链路时表现更优。
...
### [53] [ASTARS empowered Satellite Positioning Approach for Urban Canyons and Indoor Environments](https://arxiv.org/abs/2507.01783)
*Yu Zhang,Xin Sun,Tianwei Hou,Anna Li,Sofie Pollin,Yuanwei Liu,Arumugam Nallanathan*

Main category: cs.IT

TL;DR: 提出了一种基于ASTARS的卫星定位方法，用于解决城市峡谷和室内环境中卫星导航信号丢失的问题，通过反射和传输信号实现高精度定位。
...
Method: 部署ASTARS反射和传输信号，改进载波相位观测方程，结合网络时间同步和距离校正算法，使用最小二乘法确定位置。

Result: 在GNSS信号被遮挡的区域，定位误差不超过4米，优于传统NLoS方法的7米误差。

Conclusion: ASTARS方法显著提高了非视距区域的定位精度，误差控制在较低水平。
...
### [54] [Joint Power Control and Precoding for Cell-Free Massive MIMO Systems With Sparse Multi-Dimensional Graph Neural Networks](https://arxiv.org/abs/2507.01876)
*Yukun Ma,Jiayi Zhang,Ziheng Liu,Guowei Shi,Bo Ai*
...
<details>
  <summary>Details</summary>
Motivation: CF mMIMO系统在实际部署中面临高计算复杂度和复杂处理的优化挑战。

Method: 采用SP-MDGNN稀疏化AP与UE之间的连接，并引入WMMSE算法进行性能与复杂度的权衡分析。

Result: 稀疏方法在性能和复杂度之间取得了最优平衡，显著降低了计算复杂度，仅带来轻微性能下降。

Conclusion: SP-MDGNN框架为大规模网络中CF mMIMO系统的实际部署提供了可行方案。
