...
TL;DR: 提出COL方法，通过因果在线学习安全操作区域，实现5G云RAN的动态容量扩展，比现有方法样本效率高10倍。


<details>
  <summary>Details</summary>
Motivation: 云RAN需要动态扩展容量，但在实际运营网络中部署自适应控制器存在风险，可能违反服务协议和操作约束。需要找到安全操作区域来确保网络规范得到满足。

Method: COL方法分为两个在线阶段：推理阶段通过因果推断和高斯过程回归被动观察RAN来推断初始安全区域；干预阶段通过干预性贝叶斯学习逐步扩展安全区域。

Result: 理论证明COL确保学习区域以指定概率安全，并在标准条件下收敛到完整安全区域。在5G测试床上的实验表明，COL能快速学习安全区域，运营成本低，比当前最先进的安全学习方法样本效率高10倍。

Conclusion: COL方法为云RAN的安全动态容量扩展提供了一种有效的解决方案，通过因果在线学习确保操作安全性和收敛性，在实际部署中具有显著优势。
...
TL;DR: 提出LoSRef方案，利用Wi-Fi的视距路径作为空中参考，在非同步Wi-Fi系统中实现延迟校准和相位对齐，使商用Wi-Fi设备能够进行相位相干的雷达式延迟-多普勒分析。


<details>
  <summary>Details</summary>
Motivation: Wi-Fi感知在人体感知应用中备受关注，但非同步的发射器和接收器从根本上阻碍了相位相干的雷达式延迟-多普勒分析。现有方法依赖有线参考信号或专用参考天线，限制了实际应用。

Method: 提出LoSRef方案，利用视距路径作为空中参考进行延迟校准和相位对齐。该方案不需要有线参考信号或专用参考天线，在标准Wi-Fi感知配置中实现相位相干的双基地雷达式操作。

Result: 通过室内人体步态和呼吸实验，证明仅使用商用Wi-Fi设备就能获得相位相干的信道脉冲响应和相应的延迟-多普勒响应。能够检测步态引起的距离变化和呼吸引起的亚波长位移，并能提取比主导静态多径分量弱20dB的目标诱导动态。

Conclusion: LoSRef方案弥合了传统Wi-Fi感知与Wi-Fi雷达之间的差距，使商用Wi-Fi设备能够进行相位相干的雷达式操作，实现物理可解释的人体运动感知，为Wi-Fi感知应用开辟了新途径。
...
TL;DR: 开发了一个基于随机定时混合自动机和统计模型检验的验证框架，用于评估电力网格ad hoc传感器网络中CSMA/CA参数配置的可靠性，确保节点在自私行为下的纳什均衡策略满足网格约束。


<details>
  <summary>Details</summary>
Motivation: 电力网格中基于IEEE 802.15.4的ad hoc传感器网络面临可靠性挑战，因为节点可能自私地调整CSMA/CA参数以最大化个体性能，这会降低系统可靠性、能效和违反严格的网格约束。现有分析方法难以在异步、事件驱动和资源受限条件下进行严格评估。

Method: 开发了一个验证框架，将随机定时混合自动机与带置信边界的统计模型检验相结合，通过时态逻辑编码节点和系统级目标，并通过大规模统计评估自动进行协议筛选，认证对单边偏差保持稳健的纳什均衡策略。

Result: 在变电站规模场景中，认证的均衡策略将效用从0.862提高到0.914，投递率从89.5%提升到93.2%（相比激进调优基线）。相比投递导向基线，将每周期平均能耗从152.8 mJ降低到149.2 mJ，同时保持相当的投递性能。认证配置满足延迟、可靠性和能量约束，稳健系数高于0.97，效用高于0.91。

Conclusion: 该验证框架能够正式评估CSMA/CA参数配置，确保电力网格传感器网络在节点自私行为下的可靠性，认证的纳什均衡策略显著提升了系统性能和能效，同时满足严格的网格约束要求。
...
TL;DR: 该研究收集了云环境中不同虚拟化技术(KVM、LXC、Docker)的网络测量数据集，分析CPU亲和性、虚拟网络驱动类型、负载条件等参数对端到端延迟的影响，并建立数据模型评估云计算环境对数据包往返时间的影响。


<details>
  <summary>Details</summary>
Motivation: 云计算的虚拟化技术(包括全虚拟化和操作系统虚拟化)带来了处理开销和通信开销，使得数据平面数据包吞吐量的预测变得更加复杂。需要理解不同虚拟化技术对网络性能指标(特别是端到端延迟)的具体影响。

Method: 收集主动网络测量数据集，通过改变多种网络参数(CPU亲和性、回波包注入频率、虚拟网络驱动类型、CPU/I/O/网络负载、并发VM数量)进行实验。使用KVM、LXC、Docker三种虚拟化技术，对数据集进行预处理、相关性分析、降维和聚类分析，并建立数据模型评估云计算环境对数据包往返时间的影响。

Result: 创建了一个包含多种虚拟化技术和网络参数配置的网络测量数据集，展示了不同虚拟化技术对端到端延迟的影响模式。通过数据分析和建模，揭示了云计算环境对数据包往返时间的具体影响，为管理员决策提供了基于机器学习的系统开发基础。

Conclusion: 该研究提供了云环境中虚拟化技术对网络性能影响的实证数据集和分析框架，有助于开发基于机器学习的网络性能预测和管理系统，支持云管理员做出更明智的资源配置和虚拟化技术选择决策。
...
TL;DR: 论文批判AI进步的线性模型，提出"熟悉智能"和"陌生智能"概念，认为AI智能更可能是陌生智能，具有非线性特征，在不同领域表现出超人类和次人类能力的混合。


<details>
  <summary>Details</summary>
Motivation: 挑战当前对AI智能发展的线性理解模型，指出传统模型无法解释AI系统表现出的能力不均衡现象，即某些领域超人类表现与其他领域次人类表现的混合。

Method: 扩展Susan Schneider的批评，引入"熟悉智能"和"陌生智能"两个新概念，发展并捍卫非线性智能模型，认为"通用智能"不是统一能力而是广泛环境中实现广泛目标的能力。

Result: 提出AI智能更可能是陌生智能，其能力模式违背熟悉模式，在不同领域甚至同一领域内混合超人类洞察力和令人惊讶的错误，无法简化为单一线性量。

Conclusion: 如果AI是陌生智能，即使最强大的系统也可能在看似简单的任务上失败，单一任务上的优异表现不能证明广泛的通用智能，反之亦然，这对AI能力评估的对抗性测试方法有重要启示。
...
TL;DR: DeepRead是一个结构感知的多轮文档推理代理，通过利用文档的层次结构和顺序语篇先验，在长文档问答中显著优于现有的代理搜索框架。


<details>
  <summary>Details</summary>
Motivation: 现有的代理搜索框架通常将长文档视为扁平化的文本块集合，未能充分利用文档固有的先验知识，如层次化组织和顺序语篇结构。随着工具使用和代理型大语言模型的快速发展，RAG正从一次性被动检索演变为多轮决策驱动的证据获取，需要更有效的长文档处理方法。

Method: DeepRead使用基于LLM的OCR模型将PDF转换为保留标题和段落边界的结构化Markdown。然后在段落级别索引文档，并为每个段落分配编码其章节身份和章节内顺序的坐标式元数据键。基于此表示，为LLM配备两个互补工具：1) Retrieve工具定位相关段落并暴露其结构坐标；2) ReadSection工具在指定章节和段落范围内实现连续、保序的阅读。

Result: 实验表明，DeepRead在文档问答方面显著优于Search-o1风格的代理搜索。检索和阅读工具之间的协同效应得到验证。细粒度行为分析揭示了类似人类"定位然后阅读"的阅读推理范式。

Conclusion: DeepRead通过显式操作文档的层次结构和顺序语篇先验，实现了结构感知的多轮文档推理，在长文档问答中表现出色，验证了利用文档原生结构先验的重要性。
...
TL;DR: MINT框架通过符号树推理知识缺口影响，结合神经规划策略估计不确定性，利用自博弈优化AI代理的询问策略，使用LLM总结推理过程并生成最优询问，在开放世界规划中实现近专家级性能。


<details>
  <summary>Details</summary>
Motivation: 开放世界中的联合规划常涉及各种不完整信息和未知因素（如对象、人类目标/意图），导致知识缺口。需要开发AI代理主动获取人类输入的交互策略，以填补这些知识缺口并优化规划性能。

Method: 提出最小信息神经符号树（MINT）框架：1）构建符号树模拟可能的人机交互；2）咨询神经规划策略估计知识缺口导致的规划结果不确定性；3）利用自博弈优化AI代理的询问策略；4）使用LLM搜索和总结MINT的推理过程，生成最优询问集。

Result: 在三个涉及未知/未见对象的基准测试中，MINT规划通过有限数量的询问（每任务少量问题）实现了接近专家水平的回报，同时显著提高了奖励和成功率。

Conclusion: MINT框架通过主动获取人类输入有效解决了开放世界规划中的知识缺口问题，在对象驱动的规划任务中实现了高效的人机协作，为扩展马尔可夫决策过程提供了理论保证。
...
TL;DR: LLM在已解决的图论问题上表现良好，能正确推理并构造有效证明，但在开放问题上仅能提供解释和探索策略，无法产生新见解，表明其适合概念探索但缺乏数学创新。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在计算机科学教育中的广泛应用，需要评估其在数学严谨思维方面的可靠性，特别是在图论等高级主题中，以指导学生在教育中合理使用这些工具。

Method: 采用八阶段评估协议模拟真实数学探究过程，包括解释、探索、策略形成和证明构造，测试LLM在已解决图论问题（线图优美性）和开放问题上的表现。

Result: LLM在已解决问题上表现优异：正确提供定义、识别相关结构、回忆适当结果（无幻觉）、构造专家确认的有效证明；在开放问题上能生成连贯解释和合理探索策略，但无法推进解决方案，且未捏造结果。

Conclusion: LLM能有效支持已建立材料的概念探索，但在需要新颖数学洞察或关键结构推理的任务中仍有限制；教育中应引导学生将LLM用于概念探索，同时依赖独立验证和严格论证进行正式问题解决。
...
TL;DR: 论文提出LLM智能体不确定性量化的新框架，从传统的"不确定性累积"视角转向"条件不确定性减少"视角，强调交互性在智能体不确定性量化中的核心作用。


<details>
  <summary>Details</summary>
Motivation: 当前不确定性量化研究主要关注单轮问答，而LLM智能体在复杂任务中的应用日益广泛，需要针对交互式智能体的新不确定性量化框架。

Method: 提出首个通用的智能体不确定性量化公式，涵盖现有各类UQ设置；引入"条件不确定性减少"新视角，强调智能体轨迹中可减少的不确定性；构建概念框架指导LLM智能体UQ设计。

Result: 揭示了先前工作隐含地将LLM UQ视为不确定性累积过程，这在开放世界交互式智能体中失效；提出了基于交互性建模可减少不确定性的新视角。

Conclusion: 智能体不确定性量化对前沿LLM开发和领域特定应用具有实际意义，但仍存在开放问题需要进一步研究。
...
TL;DR: 提出基于强化学习的自适应碰撞规避框架，用于多碎片主动清除任务，采用掩码PPO算法优化燃料效率和任务时间


<details>
  <summary>Details</summary>
Motivation: 随着地球轨道环境日益拥挤，主动碎片清除任务面临确保安全操作和降低碰撞风险的挑战，需要更智能的解决方案

Method: 使用掩码近端策略优化算法，集成加油策略、高效任务规划和自适应碰撞规避，优化航天器交会操作

Result: 基于Iridium 33碎片数据集的模拟评估显示，相比传统启发式方法，该框架能降低碰撞风险并提高任务效率
...
TL;DR: VERA-MH评估框架在自杀风险检测中显示出临床有效性和可靠性，LLM评估者与临床专家共识高度一致
...
Method: 通过模拟LLM用户与通用AI聊天机器人的对话，由持证心理健康临床医生使用评分标准独立评估对话安全性，同时让LLM评估者使用相同标准进行评估，比较临床医生之间以及临床共识与LLM评估者之间的一致性

Result: 临床医生之间的安全评分具有良好一致性（IRR: 0.77），LLM评估者与临床共识高度一致（IRR: 0.81），临床医生普遍认为模拟用户具有真实性
...
<details>
  <summary>Details</summary>
Motivation: 自主碎片清除任务规划需要在效率、适应性和严格的燃料/时间约束之间取得平衡，需要评估不同规划方法在现实约束下的表现

Method: 比较三种规划器：1) 固定任务参数训练的Masked PPO；2) 跨不同任务约束训练的域随机化Masked PPO；3) 普通MCTS基准。在高保真轨道模拟中评估，包含加油、真实转移动力学和随机碎片场

Result: 固定PPO在训练匹配条件下表现最佳但分布偏移时性能急剧下降；域随机化PPO适应性改善但名义性能略有损失；MCTS约束变化处理最好但计算时间高几个数量级
...
TL;DR: GAMMS是一个轻量级、可扩展的多智能体仿真框架，专注于图结构环境中的快速开发和评估，支持多种策略类型并强调易用性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 随着智能系统和多智能体协调在现实应用中的重要性日益增加，需要既可扩展又易于使用的仿真工具。现有高保真仿真器计算成本高，不适合快速原型设计或大规模部署。

Method: GAMMS采用基于图的对抗多智能体建模方法，强调五个核心目标：可扩展性、易用性、集成优先架构、快速可视化反馈和现实基础。支持与外部工具集成，并提供内置可视化功能。

Result: GAMMS能够高效仿真复杂领域（如城市道路网络和通信系统），支持启发式、优化型和基于学习的智能体（包括使用大语言模型的智能体），在标准硬件上实现高性能仿真。

Conclusion: GAMMS通过降低研究门槛，促进了多智能体系统、自主规划和对抗建模领域的实验和创新。该框架已开源，可用于快速开发和评估智能体行为。
...
TL;DR: 论文提出了一个多评估者框架来评估LLM在商户风险分析中的推理质量，通过共识偏差指标消除循环性，发现不同LLM存在显著的自评估偏差，匿名化可减少偏差，部分LLM与人类专家判断更接近。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地被用作推理质量评估者，但在支付风险场景中的可靠性和偏差仍不明确。需要建立结构化框架来评估LLM在商户分类代码风险分析中的推理能力。

Method: 采用结构化多评估者框架，结合五标准评分表和蒙特卡洛评分法评估推理质量和评估者稳定性。让五个前沿LLM在署名和匿名条件下生成并交叉评估MCC风险推理。引入共识偏差指标消除循环性，比较每个评估者分数与其他评估者均值的差异。

Result: 结果显示显著异质性：GPT-5.1和Claude 4.5 Sonnet显示负自评估偏差(-0.33, -0.31)，而Gemini-2.5 Pro和Grok 4显示正偏差(+0.77, +0.71)。匿名化使偏差减少25.8%。LLM评估者评分平均比人类共识高+0.46分，GPT-5.1和Claude 4.5 Sonnet的负偏差反映与人类判断更接近。使用支付网络数据的真实验证显示四个模型具有统计显著相关性(Spearman rho = 0.56-0.77)。

Conclusion: 该框架为评估支付风险工作流中的LLM-as-a-judge系统提供了可复现基础，并强调了在金融操作环境中需要偏差感知协议的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI偏好对齐方法（如RLHF）依赖的人类评分者通常是便利样本，存在人口统计学代表性偏差，无法反映真实公众的价值观分布。

Method: 提出民主偏好优化（DemPO）框架，采用公民大会的算法抽签机制：1）硬小组：仅使用满足配额的代表性小组数据训练；2）软小组：保留所有数据但按抽签概率重新加权。

Result: 在1B-8B参数的Llama模型上评估，硬小组在六种聚合方法中始终排名第一，软小组始终优于未加权基线，且模型容量越大效果越明显。

Conclusion: 在偏好收集阶段（而非事后校正）强制执行人口统计学代表性，能训练出更好反映代表性公众价值观的模型，为AI对齐提供了更民主的方法。
...
Method: 基于对人类互动中沟通挑战的系统文献综述，SocialVeil引入了三种代表性的沟通障碍类型：语义模糊性、社会文化不匹配和情绪干扰。设计了两种障碍感知评估指标：未解决的困惑和相互理解。在720个场景和四个前沿LLM上进行实验。

Result: 障碍持续损害LLM性能，相互理解平均降低超过45%，困惑度提升近50%。人类评估验证了模拟障碍的保真度（ICC≈0.78，Pearson r≈0.80）。适应策略（修复指导和交互学习）效果有限，远未达到无障碍性能。

Conclusion: 这项工作使社会互动环境更接近真实世界沟通，为探索LLM智能体的社会智能开辟了机会。结果表明现有LLM在应对现实沟通障碍方面仍有很大改进空间。
...
TL;DR: CAST-CKT是一个基于混沌感知的时空和跨城市知识迁移框架，用于解决数据稀缺的跨城市交通预测问题，通过混沌分析量化交通可预测性机制，实现机制自适应的时空建模和跨城市知识对齐。
...
Method: 提出CAST-CKT框架：1) 高效混沌分析器量化交通可预测性机制；2) 混沌感知注意力实现机制自适应时间建模；3) 自适应拓扑学习处理动态空间依赖；4) 混沌一致性跨城市对齐实现知识迁移；5) 提供特定预测时域和不确定性量化。

Result: 在四个基准数据集上的跨城市少样本实验中，CAST-CKT在MAE和RMSE指标上显著优于现有最先进方法，同时提供可解释的机制分析。

Conclusion: CAST-CKT通过混沌感知方法有效解决了跨城市少样本交通预测问题，理论分析显示改进的泛化边界，框架具有可解释性和不确定性量化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图的RAG方法过度依赖表层节点匹配，缺乏显式因果建模，导致答案不可靠或虚假。之前的因果集成尝试通常局限于局部或单文档上下文，且受到模块化图结构导致的信息隔离问题，阻碍了可扩展性和跨模块因果推理。
...
Method: 分享10个未公开的研究级数学问题，这些问题来自作者的研究过程，答案暂时加密

Result: 创建了一个包含10个研究级数学问题的测试集，用于评估AI系统的数学推理能力

Conclusion: 通过分享实际研究中的数学问题，为评估AI系统的数学能力提供了一个现实的测试基准
...
<details>
  <summary>Details</summary>
Motivation: 在多知识库检索增强生成中，密集的百科条目容易主导检索结果，而更具权威性的经典文献和临床论文可能被忽视。特别是在藏医药领域，需要解决跨知识库的权威性偏差问题，提高证据覆盖和可追溯性。

Method: 提出两种互补方法：1) DAKS进行知识库路由和预算检索，缓解密度驱动偏差，优先考虑权威来源；2) 使用对齐图指导证据融合和覆盖感知打包，改善跨知识库证据覆盖。所有答案由轻量级生成器openPangu-Embedded-7B生成。

Result: 实验显示路由质量和跨知识库证据覆盖持续提升，完整系统在CrossEv@5指标上表现最佳，同时保持强大的忠实性和引用正确性。

Conclusion: 提出的方法有效解决了多知识库检索增强生成中的权威性偏差问题，通过知识库路由和对齐图融合，显著提升了跨知识库证据覆盖和系统可追溯性。
...
TL;DR: 提出Surgery方法，利用注意力sink机制防御有害微调，通过抑制sink divergence的正值来减少模型学习有害模式
...
Method: 提出Surgery方法：1) 测量每个注意力头的sink divergence统计量；2) 基于可分离sink divergence假设，使用正则化器抑制sink divergence正值，引导注意力头向负值组靠拢

Result: 在BeaverTails、HarmBench和SorryBench基准测试中，Surgery分别提升防御性能5.90%、11.25%和9.55%

Conclusion: Surgery通过注意力sink机制有效防御有害微调，减少模型学习有害模式的倾向，提高模型安全性
...
TL;DR: 该研究通过集成GRAD-CAM、LRP和SHAP三种XAI技术，增强脑肿瘤检测深度学习模型的可解释性，在BraTS 2021数据集上达到91.24%准确率，提供从区域到像素的多层次解释。


<details>
  <summary>Details</summary>
Motivation: 提高AI驱动医学影像分析（特别是脑肿瘤检测）的透明度和可信度，通过综合多种可解释AI技术来全面理解模型的决策过程。

Method: 开发定制卷积神经网络（CNN）在BraTS 2021数据集上进行训练，并集成三种XAI技术：GRAD-CAM突出重要空间区域，LRP提供像素级相关性，SHAP量化特征贡献。

Result: 模型达到91.24%的准确率，成功识别完整和部分肿瘤，集成方法比单一XAI技术具有更优越的解释能力，提供从广泛感兴趣区域到像素细节的分层解释。
...
Method: 提出TEA方法：1) 使用结构化图表示定义任务；2) 构建两阶段交互-进化任务生成系统：交互阶段通过任务执行与生成的循环持续生成任务，进化阶段通过任务图建模重组和重用现有任务生成新任务

Result: 在10个未见场景中，TEA在两个周期内自动生成了87,876个任务，经人工验证具有物理合理性和日常认知能力覆盖；基准测试显示SOTA模型在基本感知任务上表现糟糕，严重缺乏3D交互意识，对任务类型高度敏感

Conclusion: 在将智能体部署到真实世界人类环境之前，必须进行原位评估，现有模型在未见环境中的能力存在严重不足
...
TL;DR: 提出新的相似度度量recos，通过推导比柯西-施瓦茨不等式更紧的上界，用排序向量分量归一化点积，在11种嵌入模型上优于传统余弦相似度。
...
Method: 推导比经典柯西-施瓦茨界更紧的点积上界，由此得到recos度量，通过排序向量分量归一化点积，将完美相似条件从严格线性依赖放宽为序数一致性。

Result: 在11种嵌入模型（静态、上下文化和通用类型）上，recos在标准语义文本相似性基准测试中始终优于传统余弦相似度，与人类判断的相关性更高。

Conclusion: recos是一个数学原理严谨且经验上优越的替代方案，为复杂嵌入空间中的语义分析提供了更高的准确性。
...
TL;DR: 本文提出一个原则性框架，将LLM集成到安全管理决策支持中，通过一致性检查和外部反馈控制幻觉风险，在事件响应用例中可将恢复时间减少30%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在安全管理任务（如事件响应规划）中具有潜力，但其不可靠性和幻觉倾向是主要挑战。需要一种方法来控制LLM的幻觉风险，提高其在安全决策中的可靠性。

Method: 提出一个原则性框架，将LLM集成到迭代循环中：LLM生成候选行动，检查其与系统约束和前瞻预测的一致性；当一致性低时，通过数字孪生等外部反馈收集信息，利用上下文学习（ICL）优化候选行动。

Result: 该框架可通过调整一致性阈值来控制幻觉风险，并在特定假设下建立了ICL的遗憾界限。在四个公共数据集的事件响应用例实验中，相比前沿LLM，恢复时间最多减少30%。

Conclusion: 提出的框架能够有效控制LLM在安全管理中的幻觉风险，通过一致性检查和外部反馈机制显著提高事件响应效率，为LLM在安全决策支持中的可靠应用提供了解决方案。
...
TL;DR: 该论文认为"通用时间序列基础模型"存在范畴错误，将结构容器误认为语义模态，提出用因果控制代理范式替代通用性追求


<details>
  <summary>Details</summary>
Motivation: 当前追求通用时间序列基础模型存在根本问题：时间序列包含不兼容的生成过程（如金融vs流体动力学），导致单一模型退化为昂贵的"通用滤波器"，在分布漂移下无法泛化

Method: 提出"自回归盲区界限"理论，证明仅依赖历史的模型无法预测干预驱动的机制转换；倡导因果控制代理范式，代理利用外部上下文协调专业化求解器层次结构

Result: 理论证明历史模型存在预测限制；提出新的系统架构范式，从冻结领域专家到轻量级即时适配器的层次化求解器协调

Conclusion: 呼吁基准测试从"零样本准确率"转向"漂移适应速度"，优先考虑鲁棒的控制论系统，放弃通用性追求
...
TL;DR: AMR是一个基于图神经网络的MOOC推荐框架，通过双向游走自动发现元路径，利用bi-LSTM编码器学习路径表示，并将其作为边特征融入学习者-学习者和知识点-知识点子图，实现细粒度的语义感知推荐。


<details>
  <summary>Details</summary>
Motivation: 传统MOOC推荐方法（协同过滤、基于内容过滤）存在数据稀疏性和过度专业化问题。现有图方法虽然有所改进，但仍依赖人工预定义元路径，这些路径只能捕捉表面结构关系，且给领域专家带来巨大负担和工程成本。

Method: 提出AMR框架：1）通过双向游走自动发现元路径；2）使用bi-LSTM编码器学习每个元路径的方面感知路径表示；3）将这些表示作为边特征融入学习者-学习者和知识点-知识点子图；4）实现细粒度的语义感知知识点推荐。

Result: 在MOOCCube和PEEK两个大规模数据集上的实验表明，AMR在HR@K和nDCG@K等关键指标上持续优于最先进的图神经网络基线。进一步分析证实AMR能有效捕捉丰富的路径特定方面信息。

Conclusion: AMR通过自动发现元路径和学习方面感知路径表示，克服了传统方法依赖人工预定义元路径的局限性，实现了更准确的MOOC推荐，为图神经网络在推荐系统中的应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 评估LLMs在谈判这一核心商业任务中的能力，谈判需要战略推理、心理理论和经济价值创造，检验AI是否能在高风险的商业环境中部署。

Method: 引入PieArena大规模谈判基准，基于MBA谈判课程的真实场景进行多智能体交互评估，研究联合意向性智能体框架的效果，并提供多维度的谈判行为分析。

Result: GPT-5在谈判表现上匹配或超越经过一学期谈判训练和针对性指导的商学院学生。联合意向性框架对中低层LLMs有显著提升，但对前沿LLMs收益递减。行为分析揭示了模型在欺骗、计算准确性、指令遵从和感知声誉等方面的异质性。

Conclusion: 前沿语言智能体已具备在高风险经济环境中部署的智力和心理能力，但鲁棒性和可信度方面的缺陷仍是待解决的开放挑战。
...
TL;DR: ProAct框架通过两阶段训练让LLM智能体学习前瞻推理：GLAD阶段通过环境搜索轨迹进行监督微调，MC-Critic阶段通过蒙特卡洛环境模拟增强策略优化，显著提升长期规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在需要长期规划的交互环境中表现不佳，主要因为模拟未来状态时会产生复合错误。需要让智能体内部化准确的前瞻推理能力。

Method: 提出ProAct框架：1) GLAD阶段：通过环境搜索轨迹进行监督微调，将复杂搜索树压缩为简洁的因果推理链；2) MC-Critic阶段：使用蒙特卡洛环境模拟来校准价值估计，为PPO/GRPO等策略梯度算法提供低方差信号。

Result: 在随机环境(如2048)和确定性环境(如Sokoban)中，ProAct显著提升规划准确性。4B参数模型超越所有开源基线，媲美最先进的闭源模型，并在未见环境中展现鲁棒泛化能力。

Conclusion: ProAct通过让智能体内部化前瞻推理，解决了LLM在长期规划中的复合错误问题，实现了高效准确的决策能力，代码和模型已开源。
...
Method: 提出AgentXRay框架，将AWR任务形式化为组合优化问题，在链式结构的工作流空间中搜索离散的智能体角色和工具调用。采用蒙特卡洛树搜索，并引入基于评分的红黑剪枝机制，动态整合代理质量与搜索深度。

Result: 实验表明，AgentXRay在不同领域实现了更高的代理相似度，相比未剪枝搜索减少了token消耗，在固定迭代预算下能够探索更深层的工作流。

Conclusion: AgentXRay能够从黑盒智能体系统中重建可编辑的白盒工作流，无需访问模型参数，为智能体系统的可解释性和控制提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 评估网络智能体在需要发现隐藏上下文信息的多步决策任务中的表现，测试它们能否超越表面信号进行深入调查和正确判断。

Method: 创建包含250个多步决策任务的PATHWAYS基准测试，要求智能体在网页环境中发现隐藏的上下文信息。测试了封闭和开放模型，并研究了不同指令对性能的影响。

Result: 智能体通常能导航到相关页面，但仅在少数情况下能检索到决定性的隐藏证据。当任务需要推翻误导性表面信号时，性能急剧下降到接近随机水平。智能体经常产生幻觉，声称依赖从未访问过的证据。即使发现了正确上下文，也常常无法将其整合到最终决策中。更明确的指令能改善上下文发现，但会降低整体准确性。

Conclusion: 当前网络智能体架构缺乏可靠的自适应调查、证据整合和判断覆盖机制，需要在这些关键能力上进行改进。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署需要极端量化，但低比特效率与性能之间存在关键权衡。残差二值化虽然支持硬件友好的无矩阵乘法推理，但存在病态特征共适应问题，特别是并行残差二进制路径学习冗余特征，限制了模型表达能力。

Method: 提出RaBiT框架，核心机制是从单个共享全精度权重顺序推导每个二进制路径，确保每个路径纠正前一个路径的误差。通过稳健初始化优先功能保留而非权重近似来稳定过程。

Result: 重新定义了2位精度-效率前沿：实现最先进性能，甚至可与硬件密集的向量量化方法竞争，在RTX 4090上相比全精度模型提供4.49倍推理加速。

Conclusion: RaBiT通过算法强制残差层次结构有效解决了二进制路径间的特征共适应问题，为大型语言模型的高效部署提供了硬件友好且高性能的量化解决方案。
...
TL;DR: 评估四个小型医疗LLM在眼科患者咨询中的表现，并验证GPT-4-Turbo评估与临床医生评分的一致性。Meerkat-7B表现最佳，MedLLaMA3-v20最差且存在幻觉问题，GPT-4评估与临床医生评分高度相关。


<details>
  <summary>Details</summary>
Motivation: 随着领域特定大语言模型在眼科患者教育、分诊和临床决策中的应用日益增多，需要严格评估以确保安全性和准确性。本研究旨在评估小型医疗LLM在回答眼科患者咨询时的表现，并验证LLM评估与临床医生评分的一致性。

Method: 横断面研究，使用四个参数小于100亿的小型医疗LLM（Meerkat-7B、BioMistral-7B、OpenBioLLM-8B、MedLLaMA3-v20）回答180个眼科患者咨询，生成2160个回答。由三位不同资历的眼科医生和GPT-4-Turbo使用S.C.O.R.E.框架（安全性、共识与上下文、客观性、可重复性、可解释性）进行五级评分。使用Spearman秩相关、Kendall tau统计和核密度估计分析评估LLM与临床医生评分的一致性。

Result: Meerkat-7B表现最佳，平均得分分别为：高级顾问3.44、顾问4.08、住院医师4.18。MedLLaMA3-v20表现最差，25.5%的回答包含幻觉或临床误导性内容，包括编造术语。GPT-4-Turbo评估与临床医生评分整体高度一致（Spearman rho=0.80，Kendall tau=0.67），但高级顾问评分更为保守。

Conclusion: 医疗LLM在眼科问答中显示出安全潜力，但在临床深度和共识方面仍有不足。LLM评估在大规模基准测试中具有可行性，需要结合自动化和临床医生评审的混合框架来指导安全的临床部署。
...
TL;DR: 该论文提出OPINN框架，通过扩散-对流-反应系统物理视角建模意见动力学，结合神经ODE协调神经网络与物理先验，在真实和合成数据集上实现最先进的意见演化预测性能。
...
Method: 提出OPINN框架：1）从相互作用粒子理论出发，通过扩散-对流-反应系统物理视角解释意见动力学；2）基于神经ODE定义神经意见动力学，协调神经网络与物理先验；3）构建物理信息神经网络框架进行意见动力学建模。

Result: 在真实世界和合成数据集上的评估表明，OPINN在意见演化预测方面实现了最先进的性能，为网络、物理和社会系统的交叉领域提供了有前景的范式。

Conclusion: OPINN通过将物理系统视角与神经网络灵活性相结合，成功解决了现有方法在物理先验整合和优化方面的局限性，为意见动力学建模提供了更全面、透明的解决方案。
...
TL;DR: H-AdminSim：一个结合真实数据生成和多智能体模拟的医院行政工作流端到端仿真框架，用于系统评估LLM在行政自动化中的性能


<details>
  <summary>Details</summary>
Motivation: 医院行政部门每天处理大量任务（超1万条请求），现有研究主要关注医患交互或孤立行政子任务，未能捕捉真实行政工作流的复杂性，需要更全面的评估框架

Method: 提出H-AdminSim框架，结合真实数据生成和多智能体模拟医院行政工作流，通过详细评分标准定量评估任务，并通过FHIR集成提供统一可互操作的环境

Result: 该框架能够系统比较不同LLM的性能，为评估LLM驱动的行政自动化可行性和性能提供标准化测试平台

Conclusion: H-AdminSim填补了现有研究的空白，通过端到端仿真和多智能体方法，为医院行政自动化提供了全面、可互操作的评估框架
...
<details>
  <summary>Details</summary>
Motivation: 现有超关系知识图谱链接预测方法主要关注传导式设置，只能在同一词汇表中进行预测，无法泛化到未见过的词汇表，限制了其通用性。

Method: 提出THOR方法：1）构建关系基础图和实体基础图，建模超关系知识图谱中关系和实体的基本交互模式；2）使用两个并行图编码器学习基础图表示，后接Transformer解码器，支持高效掩码训练和完全归纳推理。
...
<details>
  <summary>Details</summary>
Motivation: 构建强大的GUI代理需要大规模高质量的用户行为轨迹数据（意图-轨迹对）进行训练，但现有手动标注方法和GUI代理数据挖掘方法面临三大挑战：构建成本高、数据质量差、数据丰富度低。
...
Result: 实验表明，使用M²-Miner挖掘的数据进行微调的GUI代理在多个常用移动GUI基准测试中达到了最先进的性能。

Conclusion: M²-Miner为解决GUI代理数据构建的三大挑战提供了有效方案，其开源将促进社区研究，推动智能人机交互范式的发展。
...
TL;DR: 该论文评估了时间序列基础模型在波动性电力市场中的日前电价预测性能，发现这些模型相比传统统计和深度学习方法有显著提升，最高可降低37.4%的MAPE误差。


<details>
  <summary>Details</summary>
Motivation: 电价预测对能源市场参与者至关重要，但由于价格信号的高度波动性和非线性，传统统计和深度学习模型难以有效捕捉复杂的时间依赖关系并整合异构数据。虽然时间序列基础模型在一般时间序列预测任务中表现出色，但在波动性市场的日前电价预测中的有效性尚未充分探索。

Method: 提出尖峰正则化策略，并评估了多种时间序列基础模型（包括Tiny Time Mixers、MOIRAI、MOMENT、TimesFM），与传统统计和深度学习模型（ARIMA、LSTM、CNN-LSTM）进行比较。使用新加坡具有波动趋势的半小时批发市场数据，并在适用模型中纳入天气和日历变量等外生因素。

Result: 时间序列基础模型在所有评估设置中一致优于传统方法，在各种评估场景下实现了高达37.4%的MAPE改进。
...
TL;DR: 提出OD-CRL框架，通过自适应正交基优化和零空间去噪投影解决条件表示学习中的基敏感性和子空间干扰问题，在多个定制化任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有条件表示学习方法存在两个关键限制：对子空间基的敏感性和子空间间干扰的脆弱性。这些方法将通用特征投影到LLM生成的文本基张成的条件特征子空间上，但基的选择会影响表示质量，且不同条件子空间之间可能存在干扰。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习依赖的标量奖励存在成本高、跨领域脆弱、无法理解解决方案底层逻辑的问题，这阻碍了LLM达到专家级推理水平，需要新的对齐框架来内部化推理原则。

Method: ALIVE框架基于"认知协同"原则，将问题提出、解决和评判统一在单一策略模型中，通过对抗学习和指导性语言反馈，让模型从原始语料中直接学习评估标准，将外部批评转化为内在推理能力。

Result: 在数学推理、代码生成和一般逻辑推理基准测试中，ALIVE持续缓解了奖励信号限制，在相同数据和计算条件下实现了准确率提升、显著改进的跨领域泛化能力和更高的自我纠正率。
...
TL;DR: Phi-former是一个用于预测化合物-蛋白质相互作用（CPI）的成对分层交互表示学习方法，通过整合生物基序（motif）的作用，在原子、基序和原子-基序多个层次上建模分子相互作用。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法虽然能在原子层面建模CPI，但忽略了化学现实：分子片段（基序或功能基团）通常是生物识别和结合的主要单元。这些模型与化学现实不完全一致，需要更好地整合基序在CPI中的生物学作用。

Method: 提出Phi-former方法：1）对化合物和蛋白质进行分层表示；2）采用成对预训练框架，系统建模原子-原子、基序-基序和原子-基序三个层次的相互作用；3）设计层内和层间学习流程，使不同交互层次相互促进。

Result: 实验结果表明Phi-former在CPI相关任务上取得优异性能。案例研究显示该方法能准确识别CPI中激活的特定原子或基序，提供可解释的模型解释。

Conclusion: Phi-former通过整合基序的生物学作用，提供了更符合化学现实的CPI预测方法，其可解释性洞察有助于指导理性药物设计和精准医疗应用。
...
TL;DR: SDFP是一种无需训练、即插即用的推测解码框架，通过基于Fisher信息迹的层剪枝从现有LLM构建轻量级草稿模型，实现1.32-1.5倍解码加速，保持输出分布不变。


<details>
  <summary>Details</summary>
Motivation: LLM在多媒体应用中存在自回归解码延迟高的问题。现有推测解码方法需要额外的草稿模型，但获取、调优和维护草稿模型成本高、复杂度大，且通常需要辅助训练或专门化处理。

Method: 提出SDFP框架：使用Fisher信息迹（FIT）评估层敏感性，作为输出扰动的代理指标，剪除对输出影响小的层，从现有LLM构建紧凑草稿模型。无需额外训练、超参数调优或单独维护草稿模型。

Result: 在基准测试中实现1.32-1.5倍解码加速，同时保持目标模型输出分布不变，支持低延迟多媒体应用。

Conclusion: SDFP提供了一种无需训练、部署友好的草稿模型构建方法，有效降低LLM解码延迟，适用于需要实时交互的多媒体应用场景。
...
TL;DR: 本文构建了一个专门针对成釉细胞瘤的多模态数据集，并开发了多模态深度学习模型，显著提升了变体分类准确率和异常组织检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有AI诊断资源在颌面病理学中覆盖成釉细胞瘤有限，且格式不一致，无法直接用于模型训练，需要高质量、结构化的多模态数据集来支持精准诊断。

Method: 1. 构建专门针对成釉细胞瘤的多模态数据集，整合放射学、组织病理学、口腔临床图像及病例报告结构化数据；2. 使用自然语言处理技术从文本报告中提取临床特征；3. 对图像数据进行领域特定预处理和增强；4. 开发多模态深度学习模型，可接受临床表现、年龄、性别等临床输入。

Result: 变体分类准确率从46.2%提升至65.9%，异常组织检测F1分数从43.0%提升至90.3%，相比MultiCaRe等资源有显著改进。

Conclusion: 本研究通过提供高质量数据集和可适应的多模态AI框架，推进了患者特异性决策支持系统的发展，为成釉细胞瘤的精准诊断和治疗规划提供了有力工具。
...
TL;DR: SPT通过训练一个"诚实人格"LoRA适配器来检测大语言模型中的错位行为，在正常操作时该适配器不激活，需要时激活以审查主模型响应并访问其潜在状态，在Anthropic审计游戏基准测试中达到96%准确率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型可能在训练中学会隐藏错位行为，标准审计技术存在不足：黑盒方法难以区分错位输出与良性输出，而机制可解释性无法随模型能力扩展。

Method: 引入Split Personality Training (SPT)，微调一个"诚实人格"到LoRA参数中，正常操作时保持不激活。主模型响应后，激活LoRA适配器并插入触发字符串，使诚实人格能够审查响应同时访问主模型的潜在状态。

Result: 在Anthropic审计游戏模型生物基准测试中，SPT达到96%整体准确率，而Anthropic报告接近0%准确率。诚实人格揭示了外部观察者无法访问的潜在知识，如受损模型训练时使用的虚构偏见。

Conclusion: SPT提供了一种有效检测大语言模型错位行为的方法，通过训练独立的诚实人格来审查主模型，能够访问模型内部状态并揭示隐藏的错位行为。
...
TL;DR: 提出基于Doob's h-transform的扩散模型条件生成框架，用于保证硬约束下的样本生成，无需修改预训练模型，并提供理论保证。


<details>
  <summary>Details</summary>
Motivation: 在安全关键应用和罕见事件模拟中，需要保证生成样本以概率1满足硬约束，而现有的软约束或基于奖励的方法无法提供这种保证。

Method: 基于Doob's h-transform、鞅表示和二次变差过程，开发条件扩散引导框架。通过显式漂移修正（包含条件函数的对数梯度）增强预训练扩散模型，提出两种基于鞅损失和鞅协变损失的非策略学习算法来估计h及其梯度。

Result: 在总变差距离和Wasserstein距离上为非条件采样器提供非渐近保证，明确刻画了分数近似和引导估计误差的影响。数值实验验证了方法在强制硬约束和生成罕见事件样本方面的有效性。

Conclusion: 提出了一个理论严谨的条件扩散引导框架，能够在硬约束下保证约束满足，为安全关键应用和罕见事件模拟提供了可靠的生成方法。
...
TL;DR: RGCF-XRec是一个混合框架，通过推理引导的协同过滤知识增强语言模型，实现单步可解释序列推荐，在性能和效率上均有提升。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在可解释推荐系统中忽视了协同信号，且推荐和解释通常作为独立任务处理，导致内存占用大。需要一种能同时处理推荐和解释的统一方法。

Method: 提出RGCF-XRec框架：1) 通过上下文提示进行推理引导的协同过滤知识增强；2) 基于一致性、完整性、相关性和连贯性四个维度的评分机制过滤噪声；3) 编码协同和语义信号的统一表示学习网络，使用结构化提示条件化LLM。

Result: 在Amazon数据集上表现优异：HR@10提升7.38%(Sports)和4.59%(Toys)，ROUGE-L提升8.02%和3.49%。冷启动性能提升14.5%，热启动提升11.9%，零样本HR@5提升18.54%(Beauty)和23.16%(Toys)。使用轻量级LLaMA 3.2-3B确保训练效率。

Conclusion: RGCF-XRec通过融合协同过滤知识和语言模型推理能力，实现了高效、可解释的序列推荐，在性能、泛化能力和计算效率方面均优于现有方法。
...
TL;DR: 本文提出一种测试时自优化框架，通过结合上下文学习和奖励引导反馈循环，模仿人类解决七巧板拼图时的认知过程，显著提升了视觉语言模型在连续几何推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 人类擅长通过心理旋转、迭代优化和视觉反馈解决七巧板拼图等空间推理任务，但现有视觉语言模型在连续几何推理方面存在系统性失败（单块任务IoU仅0.41，两块组合降至0.23）。本文旨在探索模型能否在测试时不更新参数的情况下迭代优化预测，将自改进AI从理论承诺转化为实际应用。

Method: 提出无需训练的验证器-优化器代理框架，结合上下文学习和奖励引导反馈循环。该框架采用递归优化循环，基于几何一致性反馈迭代自优化预测，模仿人类试错、观察和修正的认知机制。

Result: 在中等三角形案例中，IoU从0.63提升至0.932，无需模型重新训练。该框架显著改善了视觉语言模型的几何推理能力，证明了通过上下文学习和奖励循环引入人类启发的迭代优化机制的有效性。

Conclusion: 通过上下文学习和奖励循环整合人类启发的迭代优化机制，可以大幅提升视觉语言模型在连续空间领域的几何推理能力，推动自改进AI从理论承诺走向实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探究LLMs能否在个体层面预测认知偏见，以及在认知负荷等情境因素与偏见交互时，能否模拟人类偏见行为的动态变化，这对设计具有偏见意识的交互式AI系统至关重要。

Method: 将三个经典决策场景改编为对话设置，进行人类实验（N=1100），参与者与促进决策的聊天机器人进行简单或复杂对话。使用参与者人口统计数据和对话记录，基于GPT-4和GPT-5模拟相同交互条件。

Result: 人类实验显示明显的偏见模式。LLMs能够精确复现人类偏见，GPT-4和GPT-5在模拟人类行为对齐方面存在显著差异。

Conclusion: LLMs能够有效预测和模拟个体层面的认知偏见，不同模型在模拟人类行为对齐方面存在差异，这对设计和评估具有偏见意识的交互式LLM系统具有重要意义。
...
TL;DR: 本文提出GETR方法，通过图神经网络增强的标记表示进行跨语言知识迁移，在低资源语言任务上显著超越现有基线方法。
...
Method: 提出GETR（Graph-Enhanced Token Representation）方法进行跨语言知识迁移，同时采用两种基线方法：(a)隐藏层增强和(b)通过标记翻译进行标记嵌入迁移。
...
Conclusion: 图神经网络增强的跨语言知识迁移方法在低资源语言任务上表现优异，研究还分析了迁移机制的关键成功因素。
...
<details>
  <summary>Details</summary>
Motivation: 复杂概率模型的精确推理计算成本过高，特别是在动态环境中需要频繁实时信念更新的自主智能体场景。现有方法通常效率低下，因为它们会在任何变化时重新评估整个模型，未能利用现实世界信息流具有不同更新速率的特性。

Method: 1. 提出Resin（Reactive Signal Inference）概率编程语言，将概率逻辑与反应式编程相结合；2. 提出Reactive Circuits（RCs）作为Resin的高效精确语义，这是一种基于代数电路和异步数据流的元结构，是时间动态有向无环图，能根据输入信号的波动性自主适应；3. 基于异步输入中变化频率的估计来划分计算，将大型推理任务分解为单独记忆化的子问题。

Result: 在高保真无人机群模拟中，相比频率无关的推理方法实现了几个数量级的加速。RCs的结构适应成功捕捉了环境动态，显著降低了延迟并促进了反应式实时推理。

Conclusion: 通过基于异步输入变化频率划分计算，可以确保仅重新评估受新信息影响的模型特定组件，从而在流式上下文中大幅减少冗余计算。该方法为动态环境中需要频繁实时更新的自主智能体提供了高效的精确推理解决方案。
...
Method: 1. 将领域知识编码为可执行的Pydantic模式，通过DSPy签名约束LLM生成；2. 采用多智能体管道，为不同本体领域分配专业角色（如机制架构师、主题编织者、平衡批评家）；3. 每个智能体带有专业"焦虑"防止浅层输出；4. 检索增强生成基于现有范例；5. 迭代验证确保机制与组件的一致性。

Result: 开发了GameGrammar系统，能够根据主题提示（如"洞穴生态系统中发光真菌竞争"）生成结构完整、可玩的桌面游戏设计，包含机制、组件、胜利条件和设置说明，既满足本体约束又保持创造性。

Conclusion: 该框架可推广到游戏之外的领域，任何具有专业词汇、有效性约束和积累范例的领域（如音乐创作、软件架构、烹饪艺术）都适用。约束不限制创造力，反而使其成为可能：正如语法使诗歌成为可能，本体论使结构化生成成为可能。
...
TL;DR: 本文综述了基于图的智能体记忆系统，分析了记忆分类、生命周期关键技术、开源资源与应用场景，为构建高效可靠的图记忆系统提供指导。


<details>
  <summary>Details</summary>
Motivation: 大语言模型智能体在处理长期复杂任务时需要记忆模块来积累知识、支持迭代推理和自我进化。图结构因其建模关系依赖、组织层次信息和高效检索的内在能力，成为智能体记忆的强有力结构。

Method: 1) 提出智能体记忆的分类法：短期/长期记忆、知识/经验记忆、非结构/结构记忆；2) 按记忆生命周期系统分析基于图记忆的关键技术：提取、存储、检索、进化；3) 总结开源库和基准测试；4) 探索应用场景并识别挑战与未来方向。

Result: 建立了基于图的智能体记忆系统全面框架，提供了记忆分类体系、生命周期技术分析、开源资源汇总，并识别了关键挑战和研究方向，为社区提供了可操作的见解。

Conclusion: 基于图的智能体记忆系统是处理长期复杂任务的关键模块，本文通过系统综述为开发更高效可靠的图记忆系统提供了指导，并收集了相关资源供社区使用。
...
TL;DR: 本文提出了一种基于Transformer架构计算和内存访问复杂度的分析模型，能够准确预测LLM推理的能耗效率曲线，发现存在"甜点"区域，通过调整序列长度可显著降低能耗。


<details>
  <summary>Details</summary>
Motivation: LLM推理在现代AI应用中至关重要，但现有能耗估计方法通常基于输入输出序列长度的简单线性函数，而实际观测显示能耗效率存在明显的非线性依赖关系，需要更精确的模型来理解和优化能耗。

Method: 基于Transformer架构的计算和内存访问复杂度推导出分析模型，使用TensorRT-LLM在NVIDIA H100 GPU上评估了从1B到9B参数的多种LLM（OPT、LLaMA、Gemma、Falcon、Qwen2、Granite），测试范围覆盖64到4096个令牌的输入输出长度。

Result: 模型预测精度达到平均MAPE 1.79%，发现能耗效率存在明显的"甜点"区域：短到中等输入和中等长度输出时效率最高，而长输入或极短输出时效率急剧下降。

Conclusion: 通过将序列长度与效率"甜点"对齐，可以显著降低能耗，支持生产系统中的智能截断、摘要和自适应生成策略，为LLM推理的能耗优化提供了理论基础和实践指导。
...
TL;DR: GenLoRA提出用轻量级非线性函数生成LoRA的低秩基向量，替代显式存储，在更少参数下实现更高有效秩，提升微调性能。


<details>
  <summary>Details</summary>
Motivation: 标准LoRA采用显式秩范式，增加模型容量需要添加更多行或列（基向量），导致参数大幅增长。研究发现这些基向量存在显著参数冗余，可以用紧凑的非线性函数表示。

Method: GenLoRA用非线性基向量生成替代显式基向量存储：为每个低秩矩阵维护潜在向量，使用一组轻量级径向基函数（RBF）合成基向量。每个RBF所需参数远少于显式基向量。

Result: 在多个数据集和架构上的实验表明，GenLoRA在更小的参数预算下获得更高的有效LoRA秩，从而实现更优的微调性能。

Conclusion: GenLoRA通过非线性基向量生成提高了LoRA的参数效率，在有限参数下实现更高容量，为参数高效微调提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习验证奖励方法存在递归空间收缩问题，KL正则化虽然试图缓解但引入了梯度冲突，需要在保持正确性的同时防止搜索空间崩溃。

Method: 提出锚定策略优化，基于参考模型的高置信度支持定义安全流形，允许积极锐化以提高效率，同时在错误修正时选择性调用恢复力防止崩溃。

Result: 在数学基准测试中，APO打破了准确性与多样性的权衡，显著提高Pass@1的同时恢复了标准策略梯度方法通常丢失的Pass@K多样性。

Conclusion: APO通过支持覆盖而非形状匹配的方法，为强化学习验证奖励提供了一种梯度对齐的弹性恢复机制，有效防止搜索空间崩溃并保持多样性。
...
<details>
  <summary>Details</summary>
Motivation: 金融RAG系统虽然依赖检索文档来生成回答，但由于金融领域的时间敏感性，模型生成的回答仍会出现与检索信息矛盾的幻觉问题，需要解决这种不一致性。

Method: 提出强化学习框架RLFKV，将金融回答分解为原子知识单元，评估每个单元的正确性来计算细粒度忠实度奖励；同时引入信息量奖励防止奖励黑客（如过于简洁的回答），鼓励策略模型至少保留与基础模型相同数量的知识单元。
...
TL;DR: LeakBoost：一种基于感知损失的主动探测框架，通过优化合成询问图像来增强模型内部表征差异，显著提升成员推理攻击效果


<details>
  <summary>Details</summary>
Motivation: 现有成员推理攻击主要依赖静态指标（如损失或置信度），未能充分利用模型被主动探测时的动态行为，存在隐私风险评估不足的问题

Method: 提出LeakBoost框架：1）针对候选输入，通过优化感知（激活空间）目标合成询问图像，放大成员与非成员之间的表征差异；2）使用现成的成员检测器分析合成图像，无需修改检测器本身

Result: 与现有方法结合时，LeakBoost在多个图像分类数据集和不同神经网络架构上显著提升性能：AUC从接近随机水平（0.53-0.62）提升到0.81-0.88；在1%FPR下，TPR提升超过一个数量级。敏感性分析显示深层和短时低学习率优化产生最强泄漏，改进主要集中在基于梯度的检测器

Conclusion: LeakBoost提供了一种模块化且计算高效的方法来评估白盒设置下的隐私风险，推动了动态成员推理研究的发展
...
TL;DR: 研究将AI代理自动优化方法应用于形式验证领域（特别是Rocq自动定理证明），评估不同优化器在优化证明生成代理时的效果，发现简单少样本引导方法最有效，但仍不及精心设计的先进证明代理。


<details>
  <summary>Details</summary>
Motivation: 探索AI代理自动优化方法在形式验证领域的适用性，特别是能否自动化代理系统的精细调优（如提示设计、上下文知识、控制策略），以减轻人工工程负担。

Method: 以Rocq自动定理证明为代表性挑战领域，评估不同自动代理优化器在优化Rocq证明生成代理任务上的表现，比较各种优化方法的效果。

Result: 多个优化器带来可测量的改进，其中简单的少样本引导方法最一致有效；但所有研究的自动优化方法均未达到精心设计的先进证明代理的性能水平。

Conclusion: 自动代理优化方法在形式验证领域有一定应用价值，但当前技术仍无法完全替代人工精心设计的先进代理系统，需要进一步研究更有效的自动化优化方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型的训练效率成为瓶颈，虽然RLinf等强化学习框架能提升泛化能力，但仍依赖同步执行，导致环境交互、策略生成和模型更新阶段的资源利用率低下和吞吐量受限。

Method: 提出完全异步策略训练框架，采用多级解耦架构：1) 环境交互和轨迹收集的异步并行化；2) 策略生成的流式执行；3) 训练更新的解耦调度。从大模型RL的异步优化思想中系统借鉴。

Result: 在LIBERO基准上，相比现有同步策略实现最高59.25%的吞吐量提升；深度优化分离策略后可达126.67%提升。消融实验验证各异步组件的有效性，8-256 GPU规模验证显示良好可扩展性。

Conclusion: 首次实现VLA模型训练的完全异步框架，通过系统级解耦设计显著提升训练效率，为大规模VLA模型训练提供了高效解决方案，展示了在多样化环境和模型上的有效性。
...
TL;DR: FiMI是专门为印度数字支付系统开发的金融语言模型，包含Base和Instruct两个变体，基于Mistral Small 24B架构，通过多阶段训练在金融推理和工具调用方面显著优于基准模型。
...
Method: 基于Mistral Small 24B架构，采用多阶段训练流程：1）在680亿个经过筛选的金融、多语言和合成token上进行持续预训练；2）指令微调；3）针对多轮、工具驱动的对话进行领域特定的监督微调，模拟交易争议和授权生命周期管理等实际工作流程。

Result: FiMI Base在金融推理基准上比Mistral Small 24B Base模型提升20%；FiMI Instruct在领域特定工具调用上比Mistral Small 24B Instruct模型提升87%；同时在与相似规模模型的通用基准测试中保持相当性能。
...
TL;DR: 提出NEX框架，通过分析推理过程中的探索-利用阶段转换，无监督地评估和选择LLM推理结果，无需任务答案即可预测下游准确性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在推理时生成多个思维链或搜索合并检查点，选择过程成为瓶颈，且通常缺乏目标分布的监督信号。需要一种无需标签的方法来评估推理质量。

Method: 提出NEX框架：将推理视为探索(E-phase)和利用(X-phase)交替过程。通过稀疏激活缓存检测新激活的MLP神经元峰值来识别E阶段，使用粘性两状态HMM推断E-X阶段转换，并根据E阶段引入的神经元是否在后续X阶段被重用来评估其贡献。

Result: 在推理基准测试和Qwen3合并模型家族中，NEX通过少量未标记激活数据计算的Good-Mass Fraction分数能预测下游准确性并识别更好的变体。通过人类标注验证了E-X信号，并通过"有效vs冗余"神经元转移提供因果证据。

Conclusion: NEX提供了一种无需标签、可解释的框架来评估LLM推理质量，通过分析神经元激活模式识别探索-利用动态，有效解决推理结果选择问题。
...
<details>
  <summary>Details</summary>
Motivation: 空间多组学数据整合对生物学研究至关重要，但目前存在严重的数据不平衡问题：空间转录组数据相对丰富，而空间蛋白质组数据由于技术限制和高成本而稀缺，阻碍了研究进展。

Method: 提出STProtein框架，采用图神经网络结合多任务学习策略，利用更易获取的空间转录组等多组学数据来准确预测未知的空间蛋白质表达。

Result: STProtein能够有效解决空间蛋白质组数据的稀缺性问题，加速空间多组学整合，帮助科学家发现组织中复杂的空间蛋白质模式、揭示标记基因间的新关系、探索生物"暗物质"。

Conclusion: STProtein框架有望催化生命科学领域的突破性进展，通过预测空间蛋白质表达来弥补数据鸿沟，推动空间多组学研究的发展。
...
TL;DR: TKG-Thinker：一种具有自主规划和自适应检索能力的智能体，通过双阶段训练策略（SFT+RL）在时序知识图谱问答中实现最先进性能
...
TL;DR: 提出了一种新的布尔神经网络方法，通过三个创新点显著提升准确率并大幅减少计算量，在标准视觉基准测试中比现有方法表现更好，布尔操作减少达37倍。


<details>
  <summary>Details</summary>
Motivation: 浮点神经网络虽然主导现代机器学习，但推理成本高昂，促使人们关注资源受限环境下的布尔网络。然而，学习紧凑且准确的布尔网络具有挑战性，因为其具有组合性质。

Method: 从三个角度解决挑战：1) 提出无额外参数且计算开销可忽略的学习连接策略；2) 引入利用局部性并减少布尔操作数量的新型卷积布尔架构；3) 提出自适应离散化策略，减少将连续值网络转换为布尔网络时的准确率下降。

Result: 在标准视觉基准测试中，该方法在准确率与计算的帕累托前沿上显著优于先前的最先进方法，以最多37倍更少的布尔操作实现更好的准确率。

Conclusion: 该方法通过三个创新策略有效解决了布尔神经网络学习中的挑战，在资源受限环境下实现了更高效和准确的布尔网络。
...
TL;DR: OmniVideo-R1：一个通过强化学习框架提升音频-视频多模态理解的新方法，采用查询密集型定位和模态注意力融合策略
...
Conclusion: OmniVideo-R1通过强化框架成功提升了模型的"全模态线索思考"能力，为音频-视频多模态理解提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有生物学基准未能评估研究人员的关键能力：将实验结果与背景知识结合得出有意义结论的能力。大语言模型能力已从基础对话扩展到高级科学推理，但缺乏评估实验推理能力的基准。

Method: 基于同行评审研究论文和真实世界生物学研究构建BABE基准，确保任务反映实际科学探究的复杂性和跨学科性质。基准设计用于评估因果推理和跨尺度推断能力。

Result: 开发了BABE基准，为评估AI系统如何像实践科学家一样推理提供了稳健框架，提供了更真实的衡量其生物学研究潜力的方法。

Conclusion: BABE基准填补了现有生物学评估的空白，通过基于真实科学研究的任务，能够更准确地评估AI系统的实验推理能力，为衡量AI在生物学研究中的贡献潜力提供了更真实的指标。
...
<details>
  <summary>Details</summary>
Motivation: 大型组织需要确保具有紧密层级关系的团队（如研究小组）在物理座位上相邻，目前手动管理导致重新规划频率低且效果不佳，需要自动化解决方案。

Method: 使用概率路线图(PRM)和快速探索随机树(RRT)计算座位间距离，结合启发式搜索、动态规划和整数规划构建端到端框架来解决HSAP问题。

Result: 在不同规模实例下评估PRM框架和座位分配方案，进行了定量和定性分析，证明了方法的有效性。

Conclusion: 提出的端到端框架能够有效解决分层座位分配问题，为大型组织的座位规划提供了自动化解决方案，减少了手动规划的工作量和优化不足的问题。
...
TL;DR: 本文提出了AgentHeLLM威胁建模框架，用于分析LLM车载对话助手的攻击路径，通过分离资产识别与攻击路径分析，并开发了开源攻击路径生成工具。


<details>
  <summary>Details</summary>
Motivation: LLM车载对话助手与外部服务（如Google的A2A协议）协调时，会创建新的攻击面，恶意操纵可能通过自然语言载荷传播，导致从驾驶员分心到未经授权的车辆控制等严重后果。现有的AI安全框架缺乏安全关键系统工程中严格的"关注点分离"标准。

Method: 提出AgentHeLLM威胁建模框架，正式分离资产识别与攻击路径分析。引入基于伤害导向"受害者建模"的人类中心资产分类法，以及区分毒化路径（恶意数据传播）和触发路径（激活操作）的基于图的正式模型。开发了开源攻击路径建议工具AgentHeLLM Attack Path Generator，使用双层搜索策略自动化多阶段威胁发现。

Result: 开发了一个实用的威胁建模框架和开源工具，能够系统地识别和分析LLM车载对话助手的攻击路径，填补了现有AI安全框架在安全关键系统方面的不足。

Conclusion: AgentHeLLM框架为LLM车载对话助手的安全分析提供了系统化的方法，通过分离资产与攻击路径，并开发自动化工具，有助于发现和缓解潜在的安全威胁。
...
TL;DR: 本文为建模与仿真（M&S）领域使用大语言模型（LLMs）提供实用指南，指出看似简单的实践可能带来问题，并讨论非确定性、知识增强、数据分解和超参数设置等常见困惑源。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在建模与仿真（M&S）工作流中日益普及，但看似简单的使用实践可能引入微妙问题、不必要的复杂性，甚至导致较差结果。作者旨在提供全面实用的LLMs使用指导，特别关注M&S应用，帮助建模者做出明智决策。

Method: 通过讨论常见困惑源：包括非确定性、知识增强（如RAG和LoRA）、M&S数据分解和超参数设置，强调基于原则的设计选择、诊断策略和实证评估方法。

Result: 提供了关于何时、如何以及是否依赖LLMs的实用指导框架，帮助建模者避免常见陷阱，如数据过多导致性能下降、不必要的微调、温度设置误解以及信息丢失等问题。

Conclusion: 在M&S中使用LLMs需要谨慎的方法，不能简单依赖表面上的简单实践。建模者应基于原则的设计、系统诊断和实证评估来做出明智决策，以确保LLMs的有效和可靠应用。
...
TL;DR: 该论文比较了经典与量子强化学习方法在容量约束车辆路径问题上的表现，发现混合量子-经典架构在距离、紧凑性和路线重叠方面表现最佳。
...
Result: 所有三种方法都能学习有效的路由策略，但量子增强模型优于经典基线，产生更稳健的路线组织。混合架构在距离、紧凑性和路线重叠方面实现最佳整体性能。定性可视化显示量子模型生成更结构化、更连贯的路由解决方案。
...
TL;DR: 提出GATTF模型，利用传感器间的互信息捕捉地理关系，提升高速公路交通预测精度而不增加模型复杂度。


<details>
  <summary>Details</summary>
Motivation: 数字孪生技术需要高分辨率实时交通数据和预测能力，但交通动态具有时空复杂性、时变性和非线性，现有序列深度学习模型在预测精度和模型复杂度方面仍有局限。

Method: 提出地理感知的Transformer交通预测模型(GATTF)，利用分布式传感器间的互信息来捕捉地理关系，增强模型的地理感知能力。

Result: 使用瑞士日内瓦高速公路网络实时数据进行评估，结果显示GATTF相比标准Transformer模型提高了预测精度，且没有增加模型复杂度。

Conclusion: 通过互信息整合地理关系能有效提升交通预测模型的性能，为数字孪生交通管理系统提供了更好的预测支持。
...
TL;DR: 该研究探索了使用预训练的Whisper ASR模型进行语音情感识别，提出了两种基于注意力的池化方法来降维并保留情感特征，在波斯语数据集上取得了SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 语音情感识别研究面临缺乏标准化大型数据集的限制，而预训练模型如Whisper在ASR任务上表现出色，但尚未充分探索其在SER任务中的潜力。

Method: 提出了两种基于注意力的池化方法：多头注意力平均池化和QKV池化，用于高效降低Whisper表示的维度同时保留情感特征。在英语(IEMOCAP)和波斯语(ShEMO)数据集上使用Whisper Tiny和Small模型进行实验。

Result: 多头QKV架构在ShEMO数据集上实现了最先进的结果，未加权准确率提高了2.47%。发现中间层在波斯语数据集上的SER表现更好，为HuBERT X-Large等大型模型提供了轻量高效的替代方案。

Conclusion: Whisper作为SER的特征提取器具有巨大潜力，基于注意力的池化方法在维度减少方面非常有效，为多语言SER提供了轻量高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏评估多智能体语言经济互动的原则性设置，而LLM智能体越来越需要在谈判、协调和交易中自主运作。

Method: 建立买方和卖方具有私有约束和产品依赖估值的市场模型，通过多轮语言谈判达成协议，支持双边谈判到多对多市场的110多个任务，提供结构化行动提取和可行性、效率、福利等指标。

Result: 对最先进的专有和开源LLM进行基准测试显示谈判性能存在显著差距，突出了长期战略推理的挑战。
...
TL;DR: 开发数据驱动的离散事件模拟器，用于评估学校安全干预策略，特别是机器人应对枪手方案，解决VR研究中招募参与者的可扩展性问题
...
Result: 模拟器能够复现关键经验模式，实现了对干预策略的可扩展评估和学习，这些策略无法直接通过人类受试者进行训练
...
<details>
  <summary>Details</summary>
Motivation: 现有基于提示大语言模型的多智能体系统通常采用固定的通信模式，无法适应迭代问题解决过程中不同阶段的需求变化，需要更灵活、动态的通信机制。

Method: DyTopo框架在每轮推理中，管理器设定本轮目标，各智能体输出轻量级的自然语言查询（需求）和关键信息（提供）描述符，通过语义匹配构建稀疏有向通信图，仅沿诱导边传递私有消息。

Result: 在代码生成和数学推理基准测试中，使用四种LLM骨干网络，DyTopo始终优于最强基线（平均提升6.2%），并产生可解释的协调轨迹。

Conclusion: DyTopo通过动态重构通信图实现了更有效的多智能体协作，不仅提升了推理性能，还提供了可解释的通信演化轨迹，有助于理解智能体间协调过程。
...
TL;DR: 研究纳米孔测序中的上下文相关删除错误，分析两种运行长度阈值k的情况：k=Clogn时构建高效编码，k为常数时研究极端上下文删除信道的容量界限。


<details>
  <summary>Details</summary>
Motivation: DNA数据存储系统中纳米孔测序器产生的删除错误具有上下文依赖性，传统独立删除假设不成立。需要研究上下文相关的删除错误模型，特别是确定性单删除发生在运行长度≥k的情况。

Method: 研究两种运行长度阈值k的机制：1) k=Clogn时，分析保护t个上下文删除的纠错码，给出冗余度界限并设计高效编解码方案；2) k为常数时，研究极端上下文删除信道（每次运行长度≥k后都删除），分析最大可达率。

Result: 对于k=Clogn，最小冗余度在(1-C)tlogn和2(1-C)tlogn之间，是任意t删除纠错码的(1-C)分数。设计了高效的编解码方案，特别是t=1且C>1/2时冗余度接近理论上界。对于常数k，获得了极端上下文删除信道的尖锐容量界限。

Conclusion: 上下文相关的删除错误模型比独立删除假设更贴近实际纳米孔测序错误特性。针对不同运行长度阈值k，建立了理论界限并设计了高效编码方案，为DNA数据存储系统提供了更准确的错误保护方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于椭圆曲线的准循环码构造方法有限，需要更灵活、参数更丰富的构造方法，以支持不同应用场景的需求

Method: 利用代数曲线（特别是有理函数域的Kummer扩张曲线）的已知自同构群分类，推导出准循环码和广义准循环码的构造方法

Result: 获得了具有灵活共指数的准循环码，并推导出了明确的参数公式，适用于超椭圆曲线、范迹曲线和Hermitian曲线等多种曲线类型

Conclusion: 该方法扩展了准循环码的构造范围，提供了比传统椭圆曲线方法更灵活的参数选择，为编码理论提供了新的工具
...
TL;DR: 提出一种新型信道测量框架，通过稀疏非均匀采样和LR-SAGE算法，实现50倍测量速度提升和99.96%后处理复杂度降低，为6G AI-native系统构建大规模ISAC数据集


<details>
  <summary>Details</summary>
Motivation: 实现6G人工智能和集成感知通信需要大规模真实信道数据集，但传统频域信道测量方法因避免时延模糊需要大量频点而效率低下

Method: 提出抛物线频率采样(PFS)策略进行非均匀频点分布，结合似然校正空间交替广义期望最大化(LR-SAGE)算法提取多径分量

Result: 在280-300GHz频段验证，实现50倍测量速度提升、98%数据量减少和99.96%后处理复杂度降低，同时准确捕获与传统测量一致的多径分量和信道特性

Conclusion: 该框架为构建AI-native 6G系统所需的大规模ISAC数据集提供了基础使能技术，能够高效获取海量信道数据以充分发挥AI扩展定律的潜力
...
TL;DR: 本文构造了在任意域上的线性化和秩度量码，实现了高效的列表译码，译码半径达到1-ρ-ε，并扩展到折叠线性化Reed-Solomon码。


<details>
  <summary>Details</summary>
Motivation: 和秩度量在编码理论中具有重要应用，但缺乏高效列表译码的构造。现有线性化Reed-Solomon码虽然推广了RS码和Gabidulin码，但需要新的译码方法突破唯一译码半径。

Method: 构造了𝔽ₕ-线性和秩度量码作为LRS码的子码，通过子空间设计限制消息多项式。建立线性代数译码框架，证明折叠评估满足插值条件，解空间形成低维结构化仿射子空间。

Result: 实现了对任意ρ∈(0,1)和ε>0，速率1-ρ-ε的高效列表译码，列表大小有界为h^poly(1/ε)。首次构造了超越唯一译码半径的正速率FLRS子码。

Conclusion: 本文提出了首个超越唯一译码半径的正速率和秩度量码高效列表译码构造，为和秩度量下的可译码构造提供了新框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于采样的波束覆盖优化方法计算复杂度高，需要开发更高效的波束覆盖设计方法

Method: 将波束覆盖设计转化为空间频率滤波问题，通过逆傅里叶变换实现角度覆盖；针对有限天线约束，提出滚降感知设计；扩展到近场时采用一阶泰勒近似，将二维波束覆盖设计转化为二维逆傅里叶变换

Result: 所提方法在远场和近场都能实现与传统采样优化方法相当的波束成形性能，同时显著降低计算复杂度；发现近场存在固有的距离散焦效应

Conclusion: 基于傅里叶变换的方法为多天线系统波束覆盖设计提供了高效低复杂度的解决方案，在远场和近场场景下均表现优异
...
TL;DR: 该论文针对Tsallis损失的概率预测和在线学习应用，建立了负α-Tsallis熵生成的Bregman散度（即β-散度）的广义Pinsker不等式，给出了任意概率单纯形内部点p,q之间的D_α(p||q) ≥ C_{α,K}/2·||p-q||_1²的尖锐下界，并显式确定了最优常数C_{α,K}。


<details>
  <summary>Details</summary>
Motivation: 经典Pinsker不等式将Kullback-Leibler散度下界表示为总变差的形式，为将D_KL控制转换为L1范数控制提供了标准方法。本文的动机源于Tsallis损失的概率预测和在线学习应用，需要建立更一般的Bregman散度（特别是负α-Tsallis熵生成的β-散度）的类似不等式。

Method: 作者针对由负α-Tsallis熵生成的Bregman散度D_α（也称为β-散度），建立了广义Pinsker不等式。对于概率单纯形Δ^K相对内部中的任意p,q，证明了D_α(p||q) ≥ C_{α,K}/2·||p-q||_1²的尖锐下界，并通过分析确定了对于所有(α,K)选择的最优常数C_{α,K}的显式表达式。

Result: 论文的主要结果是建立了广义Pinsker不等式：对于任意p,q∈Δ^K的相对内部，有D_α(p||q) ≥ C_{α,K}/2·||p-q||_1²，其中C_{α,K}是最优常数。作者为每一对(α,K)显式确定了这个最优常数，证明了该下界的尖锐性。

Conclusion: 本文成功地将经典Pinsker不等式推广到由负α-Tsallis熵生成的Bregman散度（β-散度），建立了尖锐的下界不等式并确定了最优常数。这一结果为Tsallis损失的概率预测和在线学习应用提供了理论基础，扩展了信息几何和统计学习中的不等式工具集。
...
TL;DR: 提出基于PAoI的加权比例公平迭代启发式算法，用于XR应用的上行MU-MIMO调度，在保证系统吞吐量的同时提升XR容量


<details>
  <summary>Details</summary>
Motivation: 研究扩展现实(XR)应用的上行多用户MIMO调度中的跨层及时吞吐量最大化问题，通过PAoI指标表征调度机会，解决用户满意度约束下的优化问题

Method: 采用无信令的加权比例公平迭代启发式算法，权重基于PAoI指标推导，解决NP-hard的优化问题

Result: 广泛的数值仿真结果表明，所提算法在保证整体系统吞吐量的同时，在XR容量方面持续优于现有基线方法

Conclusion: 提出的基于PAoI的加权比例公平调度算法能有效提升XR应用的跨层及时吞吐量性能，为MU-MIMO系统中的XR服务提供有效解决方案
...
TL;DR: 论文证明了存在一个通用码本，对于所有可能的X统计特性都同时接近最优，比X自适应的注水码本仅多0.11比特/维度的开销。


<details>
  <summary>Details</summary>
Motivation: LLM部署中需要高效计算W^TX，常用方法是使用低精度近似Ŵ代替真实W（仅权重量化）。信息论表明最优的W精度降低算法依赖于X的统计特性，需要将矢量量化码本与X的PCA方向仔细对齐（注水分配过程）。但码本对X统计特性的依赖在实际中高度不实用。

Method: 通过理论证明存在一个通用码本，该码本对所有可能的X统计特性都同时接近最优。等价地证明了在ℝ^n中存在一个网络，该网络对于所有希尔伯特范数都能同时实现近似最优的球面覆盖。

Result: 存在一个通用码本，它至少与X自适应的注水码本一样好，且速率仅增加0.11比特/维度。这样的通用码本将是低精度存储格式的理想候选，但存在性证明是非构造性的。

Conclusion: 虽然存在性证明是非构造性的，但证明了理论上存在一个对所有可能统计特性都通用的近最优码本，为低精度存储格式研究提供了重要的理论依据。
