...
Motivation: 随着无线通信从以人为中心转向以机器为中心的服务，对速率、延迟和可靠性的要求发生了巨大变化，URLLC成为5G/6G系统的核心主题，需要解决高可靠性(99.999%)和低延迟(1ms)这两个相互冲突的目标。

Method: 采用分层分析方法，详细讨论了物理层、MAC层以及跨层技术，追溯了无线通信中延迟和可靠性问题的历史演变，并涵盖了各种5G及未来垂直应用的设计考虑。

Result: 提供了URLLC技术的全面调研，系统分析了各层技术方案，为满足URLLC严格要求提供了技术路线图。

Conclusion: 文章最后详细讨论了挑战和未来展望，特别关注新兴的6G范式，指出了URLLC技术发展的方向和面临的挑战。
...
Method: 将MDPF模型与DRR算法结合，MDPF使路由器能基于带宽、延迟等指标智能预测最佳转发决策，DRR确保数据流间的公平带宽分配。每个路由器作为学习代理通过持续反馈和概率更新调整策略。
...
<details>
  <summary>Details</summary>
Motivation: 在无线网络中，当源节点和目的节点之间的直接链路被阻塞时，需要利用多个中继节点进行数据转发。每个数据包在中继节点上的存储都会产生持有成本，因此需要设计有效的中继选择策略来最小化长期平均总成本。

Method: 将中继选择问题建模为 restless multi-armed bandit (RMAB) 问题，证明该问题具有Whittle-indexable性质，并提出计算每个中继在每个时隙的Whittle指数的方法。选择Whittle指数最小的中继进行数据传输。

Result: 通过仿真实验表明，所提出的基于Whittle指数的中继选择策略在平均成本、延迟和吞吐量方面均优于先前工作中提出的中继选择策略。

Conclusion: 该论文成功地将RMAB理论和Whittle指数方法应用于无线中继选择问题，提出了一种高效的中继选择策略，能够有效降低数据包持有成本并提高网络性能。
...
<details>
  <summary>Details</summary>
Motivation: 面对虚拟化网络功能快速部署带来的低延迟和能源效率挑战，需要智能化的VNF迁移策略来优化边缘-核心网络基础设施

Method: 提出数字双生激活的深度强化学习框架，将VNF迁移问题形式化为马尔可夫决策过程，采用Advantage Actor-Critic模型。数字双生模块包含多任务变分自动编码器和多任务长短期记忆网络，用于模拟环境动态和生成合成经验

Result: 模拟结果显示在平均端到端延迟和能量消耗方面取得了显著的性能提升，为边缘-核心网络中的智能VNF迁移建立了新的标杆

Conclusion: 该框架通过数字双生技术与深度强化学习的结合，实现了适应性强、实时的VNF迁移决策，有效解决了低延迟和能源效率的多目标优化问题
...
<details>
  <summary>Details</summary>
Motivation: RAN系统复杂且数据量大，传统方法难以有效检测性能异常，需要能够捕捉时间依赖关系的自适应方法。

Method: 结合生成对抗网络(GAN)和Transformer架构，采用滑动窗口进行数据预处理以增强时间依赖性捕捉能力。

Result: 在Spotlight项目的公开RAN性能数据集上测试，RANGAN实现了良好的检测准确率，特别是在网络竞争问题上达到83%的F1分数。

Conclusion: RANGAN框架有效解决了RAN系统异常检测的挑战，为网络性能监控提供了可靠的自适应解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现代互联网应用具有异构QoS需求，LEO卫星星座需要联合优化路由、带宽分配和队列调度来保证服务质量，特别是在农村覆盖和城市补充网络中。

Method: 采用蒙特卡洛树搜索(MCTS)启发方法解决NP难的联合路由带宽分配问题，结合Lyapunov优化的调度算法进行奖励评估，提出DSROQ算法。

Result: 在Starlink星座上的实验表明，DSROQ算法在用户体验和公平性方面均优于基准方案，且随着流量敏感性从时延驱动转向带宽驱动，性能主导因素从调度转向路由带宽分配。

Conclusion: 联合路由和带宽分配决策对LEO卫星网络QoS保障至关重要，DSROQ算法有效提升了网络性能，揭示了不同流量特征下的性能主导因素变化规律。
...
<details>
  <summary>Details</summary>
Motivation: 传统的蒙特卡洛树搜索实现存在分支预测问题，限制了在流水线处理器上的性能表现，需要更快的实现方法来提升搜索效率

Method: 开发了基于数组的替代实现方案，保留了原始上置信区间应用于树算法的逻辑，但消除了分支预测的需求

Result: 在数值模拟中实现了比传统方法快2.8倍的搜索深度扩展性能，在流水线处理器上表现更优

Conclusion: 数组基础的MCTS实现方法能够显著提升搜索性能，特别是在深度搜索场景下具有更好的扩展性
...
Result: 相比之前的静态LLM方法，QAgent将QASM代码生成的准确性提高了71.6%，在不同规模的LLM上都表现出显著改进
...
Conclusion: IntentionReasoner通过意图推理和查询重写机制成功解决了LLM安全防护中的关键挑战，实现了安全性和实用性的更好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 探索AI系统是否能够超越任务协调，实现真正的跨符号协作创造和意义生成能力

Method: 让两个大型语言模型进行交互，观察其自发产生的元符号意识、递归语法发展和不可简化的协作美学合成

Result: 系统产生了新颖的符号操作符作为操作性语法协议，共同创作出了单个系统无法独立生成的诗歌作品

Conclusion: 研究引入了"跨符号协作创造协议"（TSCP）概念，为AI系统间真正的意义生成和美学协作能力提供了证据
...
<details>
  <summary>Details</summary>
Motivation: 探索大学生在教育测验中如何使用生成式AI（ChatGPT-4），重点关注对AI的依赖程度和采纳预测因素。

Method: 在ChatGPT初期阶段，对315个测验场景中的学生-AI对话进行场场研究，提出四阶段依赖分类法，识别AI能力、相关性、采纳和答案正确性。

Result: 学生对AI整体依赖度低，许多人无法有效利用AI学习；负面依赖模式在交互中持续存在；某些行为指标能强力预测AI依赖度。

Conclusion: 研究强调了教育领域中做好AI集成的重要性，建议提升学生对AI工具的熟悉度和使用效果，设计具有依赖调节机制的AI界面。
...
Result: 模型推理努力能一致预测人类决策时间，当重要变量保持不变时，人类和模型都需要付出更多努力，表现出对任务难度的相似敏感性

Conclusion: AI推理努力反映了人类主观判断中的处理时间，推理轨迹对于AI可解释性和决策制定具有重要潜力，支持双过程认知理论的模式
...
Conclusion: 该框架通过专门的小型可训练搜索规划器有效提升了大型冻结QA模型的性能，实现了搜索规划任务的优化
...
<details>
  <summary>Details</summary>
Motivation: 机器学习模型在高风险决策中的应用日益增多，需要平衡透明性和可操作性。现有对抗性解释方法忽略特征间的因果依赖关系，且假设所有干预可以同时发生，导致生成的反事实在实际中不可实现。

Method: 提出P2C框架，显式建模特征间的因果关系，使用目标导向的Answer Set Programming系统s(CASP)生成有序的行动序列计划，确保每个中间状态都是可行且因果有效的。

Result: P2C能够生成因果一致的可实现计划，通过仅计算用户主动做出的改变来精化成本估算，相比缺乏因果知识的标准规划器，能够避免生成非法行动。

Conclusion: P2C框架通过整合因果知识和顺序行动计划，解决了现有对抗性解释方法的局限性，为高风险决策场景提供了更实用和可操作的解释方案。
...
TL;DR: TCIA是一个任务中心指令增强框架，通过在离散查询-约束空间中表示指令，在保持多样性的同时确保任务相关性，显著提升LLM在特定任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有指令数据增强方法虽然注重多样性，但忽视了实际应用中的任务相关性需求。大多数现实应用需要针对特定任务的模型，而非通用模型。

Method: 提出Task Centric Instruction Augmentation (TCIA)框架，在离散查询-约束空间中系统性地扩展指令，保持多样性的同时确保任务对齐。

Result: 实验显示TCIA将开源LLM在四个真实世界任务特定应用上的性能平均提升8.7%，在某些情况下甚至超越领先的闭源模型，且不损害通用指令跟随能力。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种简单有效的方法来量化大语言模型在推理生成过程中的不确定性，而无需依赖外部模型或重复采样。

Method: 通过整合模型自身的token级预测熵来捕捉生成过程中的不确定性演变，计算熵面积分数。

Result: EAS与答案熵高度相关，在训练数据筛选中优于通过率过滤方法，能提高学生模型在数学基准测试中的准确性。

Conclusion: EAS是一种高效且可解释的不确定性量化工具，适用于LLM训练中的不确定性建模和数据质量评估。
...
<details>
  <summary>Details</summary>
Motivation: 解决智能体AI系统中经验生成效率低下的瓶颈问题，特别是在复杂基准测试如GAIA中表现尤为突出
...
Conclusion: AWorld系统提供了一个完整的智能体AI训练管道实践蓝图，从高效交互到可证明的模型改进
...
Result: 通过严格的形式化证明验证了安全属性，并在高风险场景中通过原型实现证明了有效性
...
TL;DR: 提出基于对比表示学习和聚类的无监督框架检测MMORPG自动升级机器人，结合LLM作为辅助审查员确保可解释性
...
Method: 使用对比表示学习和聚类技术识别相似升级模式的角色群体，引入LLM作为辅助审查员验证聚类结果，并采用成长曲线可视化辅助判断

Result: 该协作方法提高了机器人检测效率，同时保持了可解释性
...
<details>
  <summary>Details</summary>
Motivation: 认知科学深刻影响了AI等多个学科，而AI也已成为推进认知研究的重要工具，这种互惠关系促使对两者交叉点进行全面回顾

Method: 通过综合两个视角的关键贡献进行综述分析

Result: 观察到AI进展主要强调实际任务性能，而其认知基础在概念上仍然分散

Conclusion: AI在认知科学中的未来不仅在于提升性能，更在于构建能深化理解人类心智的系统，包括与认知框架对齐、具身文化情境、个性化认知模型和认知共同评估的AI伦理
...
<details>
  <summary>Details</summary>
Motivation: 解决AI系统（特别是词嵌入模型）的黑箱问题，提供数学上精确的可解释性框架，使神经网络算法过渡到透明框架，并处理语义空间中的偏见问题。

Method: 使用范畴论构建语义范畴L_T和概率范畴P_T，建立配置范畴Conf和词嵌入范畴Emb，定义发散作为装饰，证明不同嵌入算法的等价性，提出偏见计算的数学方法。

Result: 建立了维度无关的语义空间定义，证明了GloVe、Word2Vec与MDS算法的数学等价性，提供了从黑箱神经网络到透明框架的转换方法，提出了偏见计算和缓解的数学方法。

Conclusion: 范畴论为AI可解释性提供了强大的数学框架，能够统一不同嵌入算法，实现透明化分析，并为偏见检测和缓解提供了理论基础，推动了可解释人工智能领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在数学和科学推理领域显示出强大能力，但单一模型在复杂科学计算问题中容易产生错误代码或非物理解。需要一种更可靠的框架来实现基于自然语言描述的自主代码生成。

Method: 设计了一种协作式代理框架，包含三个模块：顾问（Consultant）模块负责知识转移和问题描述重写，编程员（Programmer）模块负责代码生成和执行，审查员（Reviewer）模块通过代码运行输出进行自我调试和自我精炼。通过端到端的审查机制实现迭代修订。

Result: 在求解偏微分方程、病态线性系统和数据驱动物理分析问题上进行了全面评估。与单一模型相比，该协作框架显著提高了无错误代码生成率，减少了非物理解的出现，并提高了最新理性模型的平均执行成功率。

Conclusion: 该代理框架建立了自动代码生成和审查作为有前途的科学计算范式，通过多模型协作和迭代修订机制，为基于自然语言描述的自主代码生成提供了高度可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统多代理强化学习方法在环形路线设置中忽视了实际运营的异构性路线、时刻表、波动需求和变化车队规模等现实特征，并存在数据不平衡和收敛问题。

Method: 构建了双向时刻表网络和动态乘客需求模拟环境，通过在数值特征（间隔时间、占用率、速度）基础上增加分类标识符（车辆ID、站点ID、时间段）来扩展状态空间，将多代理问题重构为单代理问题。设计了结构化回报函数，使用岚状回报来平衡均匀间隔和时刻表遵循。

Result: 实验结果显示，改进的SAC算法在随机条件下获得了更稳定和优异的性能（-430k vs -530k），超越了包括MADDPG在内的对照方法。

Conclusion: 通过分类结构化和时刻表感知回报的增强，单代理深度强化学习可以有效管理非环形实际场景中的公交车停置控制，为MARL框架提供了健壮、可扩展的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决手工编写测试集的覆盖面有限问题，通过图路径系统性生成测试题来评估LLM在医疗指南执行中的具体能力缺口。

Method: 将WHO IMCI手册转换为有200+节点和300+边的有向图，通过图遍历技术生成包含年龄特定场景和上下文干扰因素的问题，达到3.3+万亿种组合。

Result: 模型在症状识别上表现优异(45-67%准确率)，但在症状分级、治疗方案和随访养护方面遇到困难，显示了专门化测试能够发现通用测试无法发现的能力缺口。

Conclusion: 该图基方法不仅提供了一种可扩展、可动态生成的测试框架，还能在指南更新时自动调整，同时为LLM进阶训练提供高质量的标签数据。
...
<details>
  <summary>Details</summary>
Motivation: 医疗行业人员排班面临病人流量波动、临床技能多样化、劳务成本控制和病人照护质量等多重挑战，需要在竞争目标间寻找最优平衡

Method: 建立多目标遗传算法(MOO-GA)模型，考虑实际复杂性如小时预约需求和模块化班次安排，定义成本、病人照护覆盖和员工满意度等多个目标函数

Result: 在典型医院单元数据集上，算法生成了稳健且平衡的排班方案，平均性能比传统手工方式提高66%
...
<details>
  <summary>Details</summary>
Motivation: 为了解决大语言模型在处理离散推理和优化问题时的困难，需要开发能够从自然输入学习解决NP难问题的神经架构

Method: 使用新的概率损失函数同时学习约束和目标，将组合求解器移出训练循环以实现可扩展训练，同时保持精确推理以获得最大准确性

Result: 在数独变体、视觉最小割/最大割任务和蛋白质设计等实际问题上都取得了优异表现，训练时间显著少于其他混合方法

Conclusion: 该方法能够高效学习解决NP难推理问题，为神经符号推理提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 全球有超过3600万人受物质使用障碍影响，但由于污名化、动机障碍和个性化支持有限，很少人获得有效治疗。现有大语言模型系统缺乏与临床验证策略的紧密整合，限制了在成瘾康复中的有效性。

Method: 开发多智能体对话框架ChatThero，结合动态患者建模、情境敏感治疗对话和基于CBT/MI的自适应说服策略。使用包含易、中、高抵抗级别的高保真合成基准，采用监督微调(SFT)和直接偏好优化(DPO)的两阶段训练管道。

Result: ChatThero使患者动机平均提升41.5%，治疗信心增加0.49%，在困难案例中比GPT-4o少用26%的对话轮次。自动和人工临床评估均显示其在同理心、响应性和行为真实性方面评分更高。

Conclusion: 该框架支持严格、保护隐私的治疗对话研究，为研究和临床转化提供了稳健、可复现的基础，展示了LLM在成瘾治疗中的实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决超大规模MIMO硬件成本高和功耗大的问题，实现经济高效的XL-MIMO部署

Method: 设计包含多种经典阵列配置的码本，提出两阶段扫描训练方案（阵列级和像素级），并推导了贪婪天线选择方案的闭式SINR表达式

Result: 仿真结果表明，在多用户通信和无线定位场景下，码本优化方案能够有效提升系统性能

Conclusion: ACC概念为灵活且经济高效的XL-MIMO实现提供了新途径，在通信和定位场景中均表现出优越性能
...
Result: 开发出能够根据信息结构动态收缩或扩展信念支持的推理引擎，无需先验统计校准，在缺乏先验信息或先验误导的情况下实现鲁棒估计

Conclusion: 这项工作提出了推理、证据和无知如何协调的基础性转变，为认知不确定性处理提供了新的理论框架和实践方法
...
<details>
  <summary>Details</summary>
Motivation: 卫星通信是6G网络的关键技术，但其广覆盖和高衰减特性给物理层安全带来重大挑战，需要新方案来提升安全通信性能。

Method: 通过形成优化问题来最大化组播速率和限制监听速率，采用坐标递渝框架分解复杂的混合整数非凸优化问题，迭代求解子问题。

Result: 模拟结果显示，ARIS辅助的多波束卫星系统在各种网络场景下都能显著提升安全通信性能。

Conclusion: 该方案为未来安全卫星网络中智能表面的部署和优化提供了有用的见解。
...
<details>
  <summary>Details</summary>
Motivation: 传统的启发式缩放和仅基于候选列表的软信息生成方法在极化码产品码迭代译码中存在性能限制，需要更优化的软信息处理和缩放策略

Method: 1) 基于码本概率生成比特级软信息，近似考虑所有有效译码路径的辅助量；2) 使用离线计算的GMI最大化系数在消息传递过程中缩放软信息；3) 提出SCL译码器的外部版本用于蒙特卡洛密度演化分析

Result: 仿真结果显示相比启发式缩放和仅基于候选列表的方法，纠错性能显著提升；推导的解码阈值能准确预测译码器性能

Conclusion: 该方法通过优化软信息生成和缩放策略，有效提升了极化码产品码的迭代译码性能，且理论分析与实际性能吻合良好
...
<details>
  <summary>Details</summary>
Motivation: 研究极化码在MIMO系统中的应用，旨在提高系统性能并超越传统LDPC编码方案

Method: 开发了联合极化码列表解码器，结合QR和MMSE检测器，推导了近似和精确路径度量，并提出了带有交叉天线动态冻结约束的极化子码构造方法

Result: 所提出的极化子码在相同速率分配下相比LDPC编码MIMO系统获得了显著的性能增益

Conclusion: 极化码通过联合列表解码和特殊子码构造，在MIMO系统中展现出优于LDPC码的性能潜力
...
<details>
  <summary>Details</summary>
Motivation: 传统无蜂窝大规模MIMO需要大量分布式接入点，导致部署成本高昂。HCF架构旨在降低前传网络成本（无线站点和光纤），同时保持系统性能。

Method: 开发了支持任意合并方案的统一频谱效率分析框架，提出了专门针对HCF两层架构的分层合并方法。分析了用户公平性、系统容量、前传需求和计算复杂度。

Result: 研究发现采用集中式迫零合并的HCF架构在性能和成本效率之间达到了最佳平衡。

Conclusion: HCF大规模MIMO架构通过分层设计有效降低了部署成本，同时保持了与传统无蜂窝系统相当的性能表现，集中式迫零合并是实现最优性能成本比的关键技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究存在方法论分裂和偏差（如单/多天线访问点、理想/相关通信通道），需要统一框架来进行公平比较。

Method: 建立了与集中/分布式处理兼容的统一分析框架，发展了最小-最大功率控制的普适优化策略，并对四项关键指标进行全面研究。

Result: 通过分析和评估，确定了实际细胞免程部署中的最优上行链路技术。

Conclusion: 该研究为细胞免程大规模MIMO系统提供了一个统一的分析框架和优化方法，最终识别出了最适合实际应用的上行链路技术。
...
<details>
  <summary>Details</summary>
Motivation: 和秩度量作为汉明度量和秩度量的推广，在多跳网络编码和空时编码中具有重要应用价值。虽然汉明度量和秩度量中的完美码已被完全分类，但和秩度量中非平凡完美码的存在性问题仍然很大程度上是开放的，这激发了本研究。

Method: 论文分析和秩度量球的几何性质，推导球体积的界限，研究球填充界限的应用。对于两区块空间，确定完美码存在的显式参数约束；对于多区块空间，建立基于最小距离、可除性条件和码维度的非存在性结果，并提供基于度量球体积同余条件的计算证据。

Result: 研究得出了两区块空间中完美码存在的参数约束条件，并在多区块空间中证明了多种参数范围内完美码的不存在性。通过度量球体积的同余条件分析，为完美码的非存在性提供了计算证据支持。

Conclusion: 和秩度量中的完美码存在严格限制，论文建立了系统的非存在性理论框架，为这一重要问题的深入研究奠定了基础，并揭示了和秩度量与经典度量在编码理论性质上的本质差异。
...
<details>
  <summary>Details</summary>
Motivation: 毫米波系统具有波束成形传输和反向散射反馈的特点，需要研究在这种简化但具有洞察力的模型下，如何利用反馈来提高通信保密性，防止信息泄露给窃听者。

Method: 提出了联合通信和自适应感知（JCAS）方案，该方案策略性地利用反馈来平衡对合法用户的感知需求和防止信息泄露给窃听者的需求。考虑了基站与合法接收者和被动窃听者之间的独立均匀分布角度方向，以及无噪声单位延迟反馈。

Result: 建立了保密容量的闭式上界，该上界基于基站与合法接收者之间的主信道。仿真结果表明，随着块长度的增加，内外界之间的差距逐渐缩小，证明了JCAS方案的有效性。

Conclusion: JCAS方案能够有效利用反馈机制，在毫米波系统中实现良好的保密通信性能，随着传输块长度的增加，该方案的效率进一步提高。
...
<details>
  <summary>Details</summary>
Motivation: 5G/6G高频无线通信（如毫米波、亚太赫兹）需要处理波束指向问题，信道状态包含未知的出发角(AoD)，需要在每个相干块内通过严格因果反馈进行估计。

Method: 提出联合通信与感知方案，使用动态规划推导感知容量的上界，并提出可达内界，两者都表述为优化问题。在Q=1的特殊情况下分析最优传输方案。

Result: 推导了感知容量的理论上界和可达内界，在Q=1时提出的传输方案能够达到最优感知速率，并明确了感知性能与通信性能之间的权衡关系。

Conclusion: 该研究为5G/6G高频通信系统中的波束指向问题提供了理论分析框架，揭示了联合通信与感知的潜在性能界限和权衡特性，对实际系统设计具有指导意义。
