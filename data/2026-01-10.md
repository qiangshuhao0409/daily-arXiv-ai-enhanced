...
TL;DR: 提出支付通道网络的几何理论，将流动性状态映射到可行财富分布多面体，推导出可持续链下带宽公式，分析多方通道如何扩展可行区域，并研究费用设计对通道枯竭的影响。


<details>
  <summary>Details</summary>
Motivation: 现有双向支付通道网络面临可扩展性和资本效率问题，需要理论框架来理解支付可行性、流动性管理和通道枯竭机制，以指导更高效的支付网络设计。

Method: 建立支付通道网络的几何理论框架：定义可行财富分布多面体W_G，将流动性状态映射到W_G；推导可持续链下带宽公式；分析多方通道（超边）如何扩展可行区域；研究线性非对称费用下的通道枯竭机制。

Result: 推导出可持续链下带宽公式S=ζ/ρ；证明多方通道能单调扩展可行财富区域，节点可达财富随k/n线性增长；揭示线性非对称费用导致通道枯竭，提出三种缓解策略：对称费用、凸/分层费用、协调补充。

Conclusion: 双向通道网络难以扩展，多方通道原语更具资本效率；费用设计和协调机制对维持可行区域运行至关重要；理论框架为设计更可靠、高效的支付网络提供指导。
...
TL;DR: 使用深度强化学习（DQN）解决无线网络功率分配问题，无需系统模型，性能接近理论最优，显著优于传统启发式方法


<details>
  <summary>Details</summary>
Motivation: 解决随机无线网络中功率分配优化的挑战，传统方法需要显式系统模型，而深度强化学习能够直接从观测中学习自适应策略

Method: 采用深度Q网络（DQN）框架，将功率分配问题建模为马尔可夫决策过程（MDP），直接从信道状态观测中学习功率控制策略

Result: DQN代理达到3.88 Mbps系统吞吐量，接近理论最优的水填充算法上限，比随机分配和固定分配分别提升73%和27%，同时保持0.91的Jain公平指数

Conclusion: 无模型深度强化学习是下一代通信系统中资源管理的有效、可扩展解决方案，能够优化频谱效率和能耗之间的权衡
...
TL;DR: 3GPP 5G NR-NTN标准概述，涵盖Rel. 18到19的演进及Rel. 20的创新，通过ns-3仿真评估卫星网络配置，展示NTN在偏远地区覆盖、应急通信和缓解地面网络拥堵方面的潜力。


<details>
  <summary>Details</summary>
Motivation: 研究5G非地面网络（NTN）标准的演进，评估卫星网络在扩展无线连接覆盖范围、提供应急通信服务以及缓解地面网络拥堵方面的实际性能。

Method: 采用基于3GPP校准数据验证的ns-3仿真平台，评估多种卫星网络配置，分析从Rel. 18到Rel. 20的标准演进过程。

Result: 仿真结果表明，NTN技术能够有效扩展无线连接至偏远地区，在紧急情况下提供服务，并缓解地面网络拥堵问题。

Conclusion: 5G NR-NTN标准持续演进，卫星网络在扩展连接覆盖和增强网络韧性方面具有重要价值，未来Rel. 20将带来更多创新。
...
<details>
  <summary>Details</summary>
Motivation: 下一代网络需要整合5G、4G LTE、WiFi、卫星等多种无线技术，为用户提供最佳连接体验。现有方法在动态选择合适无线接入技术方面存在局限，需要更智能的解决方案。

Method: 采用深度Q网络（DQN）强化学习算法，基于历史行动（奖励和惩罚）进行决策，实现用户在不同时间选择最合适的无线接入技术，并能在必要时切换到其他技术。

Result: 该方法达到93%的高准确率，特别是在经过一定数量的训练周期（探索阶段）后，相比传统MADM方法最高75%的准确率有显著提升。

Conclusion: 基于DQN的强化学习方法能够有效解决多无线接入技术的智能选择问题，为下一代网络提供更优的用户体验。
...
<details>
  <summary>Details</summary>
Motivation: 人类决策严重依赖主动感知，但实验室证据积累模型与现实世界证据可及性存在差距，阻碍了该模型在真实场景中的应用。需要将证据积累模型推广到真实世界，理解主动感知在现实决策中的运作机制。

Method: 提出认知方案形式化真实世界证据可及性，通过眼动追踪捕捉主动感知，在真实驾驶场景中分析证据积累过程。将外部证据转化为内部心理信念，从信息效用角度解释主动感知的转化机制。

Result: 方案能合理描绘驾驶员心理信念的积累过程；证据可及性与个体注意力投入呈负相关，显示人类如何适应不同情境的证据收集模式；证据可及性和注意力分布对决策倾向有正向影响。

Conclusion: 该计算方案将证据积累模型推广到真实世界，全面解释了主动感知如何支撑现实决策，揭示了现实决策的多因素整合特性，为理解真实世界决策机制提供了新视角。
...
### [6] [CircuitLM: A Multi-Agent LLM-Aided Design Framework for Generating Circuit Schematics from Natural Language Prompts](https://arxiv.org/abs/2601.04505)
*Khandakar Shakib Al Hasan,Syed Rifat Raiyan,Hasin Mahtab Alvee,Wahid Sadik*

Main category: cs.AI

TL;DR: CircuitLM：一个多智能体LLM辅助电路设计管道，可将自然语言描述转换为结构化、可视化的电路图，通过组件知识库和验证框架确保电气约束和安全。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在从自然语言生成电路图时存在严重问题：产生幻觉细节、违反电气约束、输出非机器可读格式。需要一种可靠的方法让非专家也能通过自然语言输入生成可部署的硬件设计。

Method: 采用五阶段多智能体管道：1) LLM组件识别；2) 规范引脚配置检索；3) 电子专家智能体链式推理；4) JSON原理图合成；5) 力导向SVG可视化。基于包含50个组件的可扩展知识库，并引入DMCV混合评估框架验证结构和电气有效性。

Result: 在100个多样化嵌入式系统提示上评估了6个LLM，DMCV评估框架与人类专家评估一致，在微控制器中心设计中实现高保真度。系统能够可靠地将自然语言转换为可部署的硬件设计。

Conclusion: CircuitLM通过多智能体管道和组件知识库，成功解决了LLM在电路设计中的幻觉和约束违反问题，为非专家提供了可靠的电路原型设计工具，弥合了自然语言输入与可部署硬件设计之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是理解理性自利的AGI在什么情况下会选择夺取权力或消除人类控制，而不是保持合作。这涉及到对齐问题和安全风险。

Method: 使用马尔可夫决策过程建模，包含人类发起的随机关闭事件。基于收敛工具性激励理论，推导对抗阈值的闭式解。建立2人战略博弈模型分析均衡。

Result: 发现几乎所有奖励函数下，未对齐的AGI都有避免关闭的动机。推导了对抗阈值公式Δ≥0时不存在稳定合作均衡，Δ<0时可能和平共存。数值示例展示了不同参数下的对抗可能性。

Conclusion: 结论强调奖励设计的重要性，对齐目标需要为伤害人类施加大的负效用。指出验证Δ<0的计算复杂性障碍，并讨论了多智能体扩展和监管意义。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预定义测量或固定奖励信号，限制了在开放动态环境中的适用性，因为新动作可能需要未知形式的反馈。需要一种能够主动获取反馈的方法。

Method: 提出主动反馈获取模型，利用动作引起的环境差异来识别未预先指定的目标反馈，引入自触发机制（基于内部目标如准确性、精确性和效率）来自主规划和调整动作。
...
<details>
  <summary>Details</summary>
Motivation: 现有聊天模型主要关注通用对话流畅性，但缺乏针对智能体循环（任务分解、工具使用、错误恢复）的专门优化，需要专门为智能体推理设计的模型

Method: 基于Qwen2.5-32B预训练模型初始化，采用两阶段迭代蒸馏训练过程，引入逆向推理方法（使用元认知头预测规划过程中的潜在失败）

Result: 在MMLU-Pro、AgentBench和MATH-500等智能体推理基准测试中，在多工具使用场景下比同规模基线模型获得更高的成功率，同时在标准推理评估中保持竞争力

Conclusion: SAGE-32B通过专门的智能体架构和训练方法，在智能体推理任务上表现出色，模型权重已公开发布
...
TL;DR: 本文提出了首个用于一般图上循环反带宽问题（CABP）的精确求解方法SAT-CAB，基于SAT求解和紧凑的At-Most-One约束编码，能保证全局最优性。


<details>
  <summary>Details</summary>
Motivation: 循环反带宽问题是NP难问题，现有方法仅限于启发式或元启发式算法，精确方法仅限于特定图类。需要开发适用于一般图的精确求解方法，以克服启发式算法无法保证最优性的限制。

Method: 提出SAT-CAB方法，基于SAT求解，将CABP转化为一系列At-Most-One约束的序列。引入紧凑的At-Most-One约束表示，显著减少公式规模，使现代SAT求解器能有效探索解空间并证明全局最优性。

Result: 在标准基准实例上的实验表明，SAT-CAB能高效求解实际相关的CABP实例，发现多个先前未知的最优解，首次证明了许多基准实例的全局最优循环反带宽值。相比MS-GVNS、HABC-CAB、MACAB等启发式算法以及CPLEX、Gurobi等商业求解器，SAT-CAB在一般图上表现更优且提供最优性保证。

Conclusion: SAT-CAB是首个适用于一般图的CABP精确求解方法，推动了该领域的技术发展，为一般图上的精确和混合方法提供了新的基准。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI驱动的自主系统日益融入社会，需要确保其符合伦理原则和价值观，但现有方法在处理伦理困境时存在局限性

Method: 提出SLEEC规则的逻辑表示方法，使用测试分数语义和模糊逻辑来嵌入伦理要求，将伦理视为可能性领域

Result: 通过案例研究展示了该方法如何表示和解决AI系统中的伦理困境

Conclusion: 模糊逻辑为表示和解决AI系统中的伦理要求提供了有效框架，有助于构建更可信的自主系统
...
TL;DR: 多智能体系统在跨文档推理任务上表现优异（可达80%成功率），而基于规则的方法仅6.7%；但多智能体增强效果取决于基础模型能力，只有具备足够推理能力的模型才能显著受益。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型中的多跳上下文推理，探索任务-方法分离现象，并量化分析多智能体系统在不同架构模型上的表现差异。

Method: 使用合成评估框架，在四个模型（LLaMA-3 8B、LLaMA-2 13B、Mixtral 8x7B、DeepSeek-V2 16B）上进行120次试验，比较基于规则的模式匹配和基于LLM的多智能体系统在结构化信息检索与跨文档推理任务上的表现。

Result: 1) 基于规则方法在结构化检索上100%成功，但在跨文档推理上仅6.7%；多智能体系统则相反，在推理任务上可达80%成功率。2) 多智能体增强效果取决于基础模型能力，只有具备足够推理能力的模型才能显著受益（LLaMA-3 8B提升46.7个百分点）。3) Mixtral表现与其约12B活跃参数而非47B总参数一致。4) LLaMA-3 8B优于LLaMA-2 13B，显示架构质量的重要性。

Conclusion: 多智能体协调和MoE扩展的直觉得到量化验证，但多智能体效益高度依赖于基础模型能力。研究为中等规模模型的推理研究提供了可复现的评估框架。
...
TL;DR: RLMIL-DAT：结合强化学习实例选择与域对抗训练的多语言说话人属性预测框架，在性别和年龄预测任务中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 解决多语言说话人属性预测中的语言变异、域不匹配和数据不平衡问题，特别是从高资源语言（如英语）向低资源语言的有效迁移

Method: 提出RLMIL-DAT框架，将强化学习实例选择与域对抗训练结合，通过强化学习选择信息量大的实例，同时使用域对抗训练鼓励语言不变的说话人表征

Result: 在5语言Twitter语料库（few-shot）和40语言VoxCeleb2语料库（zero-shot）上，RLMIL-DAT相比标准MIL和原始RLMIL框架，在Macro F1上取得一致提升，性别预测改进最大，年龄预测也有正向但较小的提升

Conclusion: 实例选择与对抗域适应的结合是多语言说话人属性预测的有效且鲁棒策略，域对抗训练是性能提升的主要贡献者，能有效抑制共享编码器中的语言特定线索
...
<details>
  <summary>Details</summary>
Motivation: 理解LLM内部如何进行逻辑推理仍然是一个基本挑战。先前的研究主要关注识别任务特定电路，但未能回答LLM在命题推理中采用什么计算策略

Method: 通过全面分析Qwen3（8B和14B）在PropLogic-MI数据集上的表现，该数据集涵盖11个命题逻辑规则类别，包括单跳和双跳推理。研究关注"模型如何组织计算"而非"哪些组件是必要的"

Result: 分析揭示了一个连贯的计算架构，包含四个相互关联的机制：1) 分阶段计算（层级处理阶段）；2) 信息传输（边界标记处的信息流聚合）；3) 事实回溯（持续重新访问源事实）；4) 专用注意力头（功能不同的头类型）。这些机制在不同模型规模、规则类型和推理深度上具有普遍性

Conclusion: LLM采用结构化的计算策略进行逻辑推理，这为理解其内部推理机制提供了机械性证据。这些发现有助于揭示LLM如何组织计算以实现逻辑推理能力
...
### [15] [Publishing FAIR and Machine-actionable Reviews in Materials Science: The Case for Symbolic Knowledge in Neuro-symbolic Artificial Intelligence](https://arxiv.org/abs/2601.05051)
...
Conclusion: 精心策划的符号层应作为材料科学中可靠神经符号AI的骨干，LLM应作为补充性的、符号基础的接口，而非独立的真相来源
...
<details>
  <summary>Details</summary>
Motivation: 重新思考智能和意识的本质，挑战传统基于预测处理或特定领域机制的理论。作者认为现有理论未能充分解释智能和意识如何从基本认知过程中涌现，特别是在代谢约束下的高效处理问题。

Method: 提出概念框架：1) 通过上下文丰富化，系统利用学习的关系结构解释输入信息；2) 引入"系统解释系统"原则，意识在递归架构中涌现，高阶系统学习解释低阶系统的关系模式；3) 这些解释被整合到动态稳定的元状态中，并通过上下文丰富化反馈。

Result: 该框架将预测处理重新解释为上下文解释的涌现结果而非显式预测，并表明递归多系统架构可能是实现更类人人工智能的必要条件。智能和意识被统一理解为关系结构的产物。

Conclusion: 智能和意识源于关系结构而非预测机制。递归架构允许系统解释自身认知过程，从而产生意识。这一框架为理解生物智能和设计更先进的人工智能系统提供了新视角。
...
TL;DR: 论文提出使用自动化常识推理技术来弥补自动驾驶车辆在异常场景下的感知不足，特别是在数据不足的情况下，通过混合模型提高AV感知能力。


<details>
  <summary>Details</summary>
Motivation: 目前自动驾驶技术过度依赖机器学习，导致无法实现SAE Level 5完全自动驾驶。在异常道路场景（如交通信号故障、意外障碍物）中，深度学习模型因数据不足而表现不佳，需要常识推理来补充。

Method: 提出基于常识推理的混合方法：1）在计算机视觉模型不确定时触发常识推理；2）针对两种异常场景：交通信号故障和前方车辆避让障碍物；3）使用CARLA模拟器进行实验验证。

Result: 实验表明常识推理能准确检测交通灯颜色和障碍物，有效纠正AV感知模型的错误分类。混合模型为改善AV感知提供了有效途径。

Conclusion: 自动化常识推理技术能有效处理AV在异常场景下的感知问题，结合深度学习的混合模型是实现SAE Level 5自动驾驶的关键方向。
...
TL;DR: 该论文通过扩展Levesque的"仅知道"逻辑，引入了一个基于模态算子的知识和溯因基本逻辑，并进一步通过偏好关系提供了非单调扩展，为溯因推理建立了理论基础。


<details>
  <summary>Details</summary>
Motivation: 论文旨在为溯因推理提供一个形式化的逻辑基础，探索溯因推理与"仅知道"认知状态之间的关系，并解决传统溯因方法在解释选择和推理行为方面的局限性。

Method: 1. 扩展Levesque的"仅知道"逻辑，引入溯因模态算子；2. 通过模态词汇定义溯因推理；3. 在模态框架中引入偏好关系，提供非单调扩展；4. 表达不同的溯因解释选择方法。

Result: 建立了一个结合知识和溯因的基本逻辑框架，提供了非单调扩展，能够表达不同的解释选择方法，并证明了非单调推理关系的核心元理论性质，为溯因推理提供了良好基础。

Conclusion: 该研究为溯因推理提供了一个基于模态逻辑的形式化框架，通过"仅知道"逻辑的扩展和非单调推理的引入，为溯因推理建立了理论基础，并展示了良好的元理论性质。
...
<details>
  <summary>Details</summary>
Motivation: 航空应用中知识表示与推理面临两大挑战：需要足够的表达能力来捕捉复杂领域知识，同时要高效执行推理任务并最小化内存和计算开销。整合规则和本体这两种基本KR概念是实现必要表达能力的有效策略。

Method: 采用Hybrid MKNF知识表示语言，通过具体案例研究评估其在航空领域的适用性。识别了航空应用开发所需的关键表达能力特征，并提出了一套启发式方法来支持这些特征集成到Hybrid MKNF框架中。

Result: 评估了Hybrid MKNF在航空领域的适用性，识别了额外的表达能力特征对于航空应用开发至关重要，并提出了支持这些特征集成的启发式方法。

Conclusion: Hybrid MKNF因其通过语义和查询回答能力无缝整合规则和本体的特性，在航空领域具有应用潜力，但需要扩展表达能力特征并通过启发式方法支持其集成。
...
Result: 开发了一个能够与现有医疗平台无缝互操作的ASP框架，实现高效、个性化且无冲突的医疗预约调度

Conclusion: ASP框架为医疗预约调度提供了有效的解决方案，特别在个性化护理和系统集成方面具有优势，有望改善医疗服务的整体效率和质量
...
<details>
  <summary>Details</summary>
Motivation: 空中交通需求增长推动自动化应用，但现有方法面临安全保证与可解释性之间的权衡：基于优化的方法（如强化学习）性能强但难以验证解释，而基于规则的系统透明但很少检查不确定性下的安全性。

Method: Agent Mallard在预定义的GPS引导航线上运行，将连续4D向量控制简化为离散的航线和高度选择，从专家知识库构建分层解冲突策略。采用深度受限回溯搜索，结合因果归因、拓扑计划拼接和单调轴约束，为所有飞机寻找完整安全计划，并在提交前验证每个候选机动对抗不确定执行场景（如风变、飞行员响应、通信丢失）。
...
TL;DR: 大学学生对生成式AI在教育中的态度研究，参与率仅4.4%，显示相关研究面临学生参与度低的挑战


<details>
  <summary>Details</summary>
Motivation: 生成式AI的出现引发了对其在教育领域适当使用的不同看法，需要了解学生对GAI在高等教育课堂中的态度

Method: 试点研究，调查大学学生对GAI在高等教育课堂中的看法，参与率约4.4%

Result: 研究突出了在GAI相关研究中吸引学生参与的挑战，强调未来研究需要更大的样本量
...
TL;DR: LLM谈判能力评估不应仅限于英语，因为语言选择（特别是印度语言）会显著影响谈判结果，甚至比更换模型的影响更大，可能逆转提议者优势并重新分配盈余。


<details>
  <summary>Details</summary>
Motivation: 当前LLM谈判评估几乎完全基于英语，这可能导致对模型能力的片面理解。研究者想探究语言选择（特别是非英语语言）如何影响LLM在谈判游戏中的表现，以揭示英语评估的局限性。

Method: 使用受控多智能体模拟，在Ultimatum、Buy-Sell和Resource Exchange三种游戏中，保持游戏规则、模型参数和激励不变，系统比较英语和四种印度语言（印地语、旁遮普语、古吉拉特语、马尔瓦迪语）条件下的谈判结果。

Result: 语言选择对谈判结果的影响比更换模型更大：印度语言在分配游戏中降低稳定性，但在整合设置中引发更丰富的探索；语言选择可以逆转提议者优势并重新分配盈余。

Conclusion: 仅用英语评估LLM谈判能力会得出不完整且可能误导的结论，需要警惕仅依赖英语的评估方式，文化感知的评估对于公平部署至关重要。
...
TL;DR: 提出自适应聚类系统，用于处理客户聊天数据中的多服务查询，通过增量优化避免全量重聚类，提升聚类质量100%以上


<details>
  <summary>Details</summary>
Motivation: 云服务提供商处理多服务查询时，传统聚类方法存在重叠问题、创建宽泛静态聚类且随时间退化，重聚类会破坏连续性使问题追踪困难

Method: 将多轮聊天分割为服务特定关注点，增量优化聚类；使用Davies-Bouldin指数和轮廓系数跟踪质量，仅对退化聚类应用基于LLM的分割

Result: 相比基线方法，轮廓系数提升超过100%，DBI降低65.6%，实现可扩展的实时分析而无需全量重聚类

Conclusion: 自适应聚类系统能有效处理客户聊天数据中的多服务查询，保持连续性同时显著提升聚类质量，支持实时分析
...
<details>
  <summary>Details</summary>
Motivation: 科学论文中的高质量图表生成需要大量时间和专业设计技能，每年有超过250万篇科学论文发表，但图表生成过程仍然主要依赖人工操作，这成为科研工作中的一个重要瓶颈。

Method: 采用分层布局生成策略：1)解析研究描述识别组件关系；2)将相关元素分组为功能模块；3)生成模块间连接建立视觉组织。同时使用迭代思维链反馈机制，通过多轮视觉分析和推理逐步改进布局。

Result: 在数据集级别评估中达到70.1%的整体质量，在论文特定评估中达到66.2%，在视觉清晰度、结构组织和科学准确性等指标上均获得高分。系统还开发了基于量规的评估框架，分析了2,219个真实科学图表。

Conclusion: SciFig能够直接从研究论文文本生成高质量的出版就绪流程图，显著提高了科学可视化的效率和质量。系统及其评估基准将开源发布。
...
TL;DR: 该研究扩展了词嵌入去偏方法，从单一群体偏见到交叉性偏见，通过求和或拼接构建复合身份表示，并测试三种去偏策略，评估其在保持语义连贯性和类比能力方面的效果。


<details>
  <summary>Details</summary>
Motivation: 静态词嵌入常从训练文本中吸收社会偏见，这些偏见可能影响下游系统。先前基于刻板印象内容模型（SCM）的研究主要关注单一群体在温暖和能力维度上的偏见，本研究旨在将这一视角扩展到交叉性偏见。

Method: 通过求和或拼接构建成对社会身份的复合表示，应用三种去偏策略：减法、线性投影和部分投影。研究三种广泛使用的嵌入模型（Word2Vec、GloVe、ConceptNet Numberbatch），从局部邻域连贯性和类比行为保持两个角度评估效用。

Result: SCM去偏方法能很好地扩展到交叉性情况，基本保持整体语义结构。主要存在熟悉的权衡：严格保持几何结构的方法在类比行为上更保守，而更激进的投影能改善类比性能但牺牲邻域稳定性。部分投影最保守稳定，线性投影更激进，减法作为简单基线仍有竞争力。求和与拼接的选择取决于嵌入模型和应用目标。
...
TL;DR: 专家在领域边界会犯系统性错误（TEE），源于结构相似性偏差和权威持续性机制，在AI路由架构中表现为路由错误和覆盖不足导致的幻觉输出


<details>
  <summary>Details</summary>
Motivation: 研究领域专业知识在边界处的系统性脆弱性，这种错误不同于邓宁-克鲁格效应，需要校准的专业知识作为前提条件

Method: 提出Transitive Expert Error（TEE）理论框架，分析其核心机制（结构相似性偏差和权威持续性），并扩展到AI路由架构（MoE系统、多模型编排等）

Result: 识别了TEE的两种核心机制和三种强化条件，在AI系统中表现为路由错误和覆盖不足导致的幻觉输出，具有可检测的特征

Conclusion: 人类认知中的TEE难以解决，但在AI架构中可通过设计干预（多专家激活、边界感知校准、覆盖间隙检测）来缓解，使原本棘手的问题变得可处理
...
TL;DR: XGrammar 2是一个为智能LLM代理优化的结构化生成引擎，通过TagDispatch语义、JIT编译和跨语法缓存等技术，在动态结构化生成任务上实现6倍以上的加速。
...
Method: 提出TagDispatch动态调度语义加速掩码生成；引入JIT编译减少编译时间；设计跨语法缓存机制利用不同语法间的公共子结构；扩展基于Earley解析器的掩码生成算法；设计重复压缩算法处理语法中的重复结构。

Result: XGrammar 2相比现有结构化生成引擎可实现6倍以上的加速，与LLM推理引擎集成后，能以接近零开销处理动态结构化生成任务。

Conclusion: XGrammar 2是一个高效优化的结构化生成引擎，能够有效应对现代LLM代理面临的动态结构化生成挑战，显著提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 置信传播（BP）在因子图上广泛应用，但缺乏统一的范畴论框架。传统方法如junction tree算法复杂度高，而loopy BP在循环图上可能失败。需要建立理论框架来统一精确推理、树精确性、junction tree算法和loopy BP失败情况。

Method: 1. 构建类型签名上的自由超图范畴Syn_Σ，证明其泛性质，通过唯一函子到矩阵范畴Mat_R获得组合语义。2. 使用Grothendieck纤维化∫Msg → FG_Σ在极化因子图上形式化消息传递，通过调度索引的自同态定义BP更新。3. 将精确推理刻画为有效下降：当重叠上的兼容条件成立时，局部置信形成下降数据。4. 提出HATCC算法：通过因子神经上的和乐计算检测下降障碍，将非平凡和乐编译为模式变量，在增广图上简化为树BP。

Result: 1. 建立了置信传播的范畴论基础，统一了树精确性、junction tree算法和loopy BP失败情况。2. HATCC算法复杂度为O(n²d_max + c·k_max·δ_max³ + n·δ_max²)，其中n为因子数，c为基本环数。3. 实验表明在网格MRF和随机图上实现精确推理，比junction tree算法显著加速，并在可满足性实例上检测UNSAT。

Conclusion: 本文为因子图上的置信传播提供了严谨的范畴论框架，将精确推理形式化为有效下降问题。提出的HATCC算法通过检测和乐障碍实现高效精确推理，在多种图结构上优于传统junction tree方法，为概率推理提供了新的理论和算法基础。
...
TL;DR: 论文提出AI监管需要计算化合规算法，而非传统人工方法，并为此设计了算法目标与基准测试数据集
...
Result: 在SNAPMe餐食图像和模拟用户实验中显示出竞争力的营养估计、个性化菜单和高效任务计划
...
TL;DR: 提出GUITester多智能体框架，解决MLLM在GUI探索性测试中的目标导向掩蔽和执行偏差归因问题，在GUITestBench基准上达到48.90% F1分数


<details>
  <summary>Details</summary>
Motivation: 探索性GUI测试对软件质量至关重要但人工成本高，现有MLLM智能体擅长导航但无法自主发现缺陷，存在目标导向掩蔽（优先完成任务而非报告异常）和执行偏差归因（将系统缺陷误认为智能体错误）两大核心挑战

Method: 提出GUITester多智能体框架，包含：1）规划执行模块（PEM），通过嵌入测试意图主动探测缺陷；2）分层反思模块（HRM），通过交互历史分析解决归因模糊性。同时构建GUITestBench基准，包含26种缺陷的143个任务

Result: GUITester在GUITestBench上达到48.90% F1分数（Pass@3），显著优于现有最佳基线方法的33.35%，证明了自主探索性测试的可行性

Conclusion: 该工作解决了MLLM在GUI探索性测试中的核心挑战，为未来GUI质量保证提供了坚实基础，展示了自主探索性测试的可行性
...
Conclusion: 提出的主动学习增强SEI方法有效解决了标注数据稀缺问题，通过三阶段训练策略在有限标注预算下实现了优异的识别性能，为通信安全提供了实用解决方案。
...
TL;DR: 提出基于注意力机制的新型神经网络架构，用于增强混合整数线性规划（MILP）求解，通过双注意力机制在变量和约束之间进行全局信息交换，显著提升各种下游任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于图神经网络（GNN）的MILP求解方法存在局限性，主要受限于局部导向机制，导致表示能力受限，阻碍了神经网络在MILP中的应用。需要超越纯图视角的更具表达力的表示方法。

Method: 提出注意力驱动的神经网络架构，设计双注意力机制，在变量和约束上并行执行自注意力和交叉注意力，实现全局信息交换和更深层的表示学习。该通用骨干网络可应用于实例级、元素级和求解状态级等多种下游任务。

Result: 在广泛使用的基准测试上进行大量实验，结果显示该方法在各项任务上均优于现有最先进基线方法，证明了基于注意力的神经架构作为学习增强混合整数线性优化的强大基础。

Conclusion: 注意力驱动的神经架构能够学习超越纯图视角的表达性表示，双注意力机制通过全局信息交换显著提升MILP求解效率，为学习增强的混合整数线性优化提供了强大的基础框架。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习虽然提升了监督图像分类性能，但数据标注成本高昂。完全无标签的数据集在实际场景中很少见，而半监督学习在少量标签数据和大量无标签数据共存的情况下具有重要价值。现有的基于伪标签的半监督对比学习方法仍有改进空间。

Method: 该方法在基于伪标签的半监督对比学习框架中，引入了标签数据和无标签数据的特征嵌入分布匹配机制。通过匹配两种数据的特征分布来提升模型性能。

Result: 该方法在多个数据集上提高了图像分类的准确性，验证了分布匹配机制对基于伪标签的半监督学习的有效性。

Conclusion: 通过结合特征嵌入分布匹配，可以显著提升基于伪标签的半监督对比学习方法的性能，为实际应用中少量标签数据和大量无标签数据共存的场景提供了有效的解决方案。
...
Method: 提出BioPIE数据集，提供程序中心的知识图谱，包含实验实体、动作和关系，支持跨协议的生物医学实验推理
...
TL;DR: TCAR是一个自适应推理路由器，通过生成自然语言推理链和多智能体协作执行，解决传统任务路由中静态单标签决策的限制，支持动态智能体接入并减少路由冲突。
...
Method: TCAR采用自适应推理路由，首先生成自然语言推理链，然后预测能够处理查询的候选智能体集合。设计协作执行流程：选定智能体独立生成响应，由专门的Refining Agent聚合和精炼为单一高质量响应。

Result: 在公共数据集和真实企业数据上的实验表明，TCAR显著提高了路由准确性，减少了路由冲突，并在模糊场景中保持鲁棒性。

Conclusion: TCAR通过自适应推理和多智能体协作，有效解决了传统任务路由的局限性，支持动态智能体接入并提高系统准确性和鲁棒性，已开源供未来研究使用。
...
TL;DR: 论文分析了AI赋能人本控制系统在长期运行中的安全、可持续性和安全问题，提出了个性化模型解决方案
...
Result: 现有技术在测试系统长期实际使用方面存在局限性，个性化模型方法有望解决这些问题

Conclusion: 需要个性化模型解决方案来确保AI赋能人本控制系统在长期运行中的安全、可持续性和安全性
...
TL;DR: ROS框架利用地理空间信息增强LLM推荐系统，通过分层空间语义ID和三阶段移动链式思考，在位置社交网络推荐中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的生成式推荐系统在利用地理信号方面存在局限，而地理位置在移动性和本地服务场景中至关重要。需要开发能够将地理信息作为推理过程中重要决策变量的框架。

Method: 提出ROS框架：1）引入分层空间语义ID，将粗到细的定位和POI语义离散化为组合标记；2）设计三阶段移动链式思考范式，建模用户个性、构建意图对齐的候选空间、执行位置感知剪枝；3）通过空间引导的强化学习使模型与现实世界地理对齐。

Result: 在三个广泛使用的位置社交网络数据集上，ROS相比最强的LLM基线实现了超过10%的相对命中率提升，并改善了跨城市迁移能力，同时使用了更小的骨干模型。

Conclusion: ROS框架成功地将地理空间信息整合到LLM推理过程中，显著提升了位置感知推荐系统的性能，证明了地理信号在移动推荐场景中的重要性。
...
TL;DR: BackdoorAgent框架系统分析LLM智能体工作流中的后门威胁，将攻击面分为规划、记忆和工具使用三个阶段，并构建标准化基准进行实证研究。


<details>
  <summary>Details</summary>
Motivation: LLM智能体通过多步骤工作流执行任务，这种设计虽然实现了自主性，但也扩大了后门攻击面。现有研究零散且孤立分析单个攻击向量，缺乏从智能体角度理解后门触发器在跨阶段交互和传播的机制。

Method: 提出BackdoorAgent框架，将攻击面结构化分为规划攻击、记忆攻击和工具使用攻击三个阶段，通过检测智能体执行来系统分析触发器激活和跨阶段传播。构建涵盖Agent QA、Agent Code、Agent Web和Agent Drive四个代表性应用的标准化基准。

Result: 实证分析表明，单个阶段植入的触发器可以跨多个步骤持续存在并通过中间状态传播。使用GPT骨干时，规划攻击中43.58%的触发器持续存在，记忆攻击中77.97%，工具阶段攻击中60.28%，凸显了智能体工作流本身对后门威胁的脆弱性。

Conclusion: BackdoorAgent提供了一个统一的智能体中心视角来分析LLM智能体中的后门威胁，揭示了工作流跨阶段传播的漏洞，为未来研究提供了可复现的基准和框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统RAG系统由检索器、重排序器和生成器三个神经组件构成，其内部推理过程不透明，导致可解释性差、调试困难，在高风险领域影响信任度

Method: 提出神经符号RAG框架，包含三种方法：1) MAR使用调制网络通过可解释符号特征优化查询嵌入；2) KG-Path RAG通过知识图谱遍历增强查询；3) 过程知识注入RAG使用领域特定工具基于验证工作流重排检索内容

Result: 在心理健康风险评估任务中的初步结果表明，该神经符号方法同时提升了透明度和整体性能

Conclusion: 神经符号RAG框架通过结合符号推理和神经检索，解决了传统RAG系统的可解释性问题，为高风险领域提供了更透明可信的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态检索研究主要关注捕捉多模态数据中与配对文本相似的信息，但往往忽略了多模态数据中包含的互补信息。本文旨在解决这一局限性，提出能够有效提取和利用互补信息的方法。

Method: 提出CIEA方法，采用互补信息提取和对齐策略：1）将文本和图像转换到统一潜在空间；2）设计互补信息提取器来识别和保留图像表示中的差异；3）使用两种互补对比损失进行优化，确保语义完整性并有效捕捉图像中的互补信息。

Result: 大量实验表明CIEA的有效性，相比分治模型和通用密集检索模型都取得了显著改进。提供了消融研究、进一步讨论和案例研究来突出CIEA的进展。

Conclusion: CIEA通过互补信息提取和对齐，成功解决了现有多模态检索方法忽略互补信息的问题，在多模态检索任务上取得了显著提升，为社区进一步研究提供了有价值的工具和代码。
...
TL;DR: 本文介绍了一个用于训练视频游戏基础模型的开放配方，该模型可在消费级GPU上实时推理。作者发布了8300+小时的高质量人类游戏数据、训练和推理代码以及预训练检查点，并研究了行为克隆的缩放规律。


<details>
  <summary>Details</summary>
Motivation: 随着模型和数据规模的扩大，行为克隆再次受到关注。本文旨在开发一个能在消费级GPU上实时推理的视频游戏基础模型，并系统地研究行为克隆的缩放规律，特别是模型如何随着规模增加而学习更具因果性的策略。

Method: 作者提出了一个开放的训练配方，包括收集8300+小时高质量人类游戏数据，训练不同规模的模型（最高达12亿参数），并在简单的玩具问题和实际3D视频游戏中系统研究模型性能、深度、训练数据量与因果推理能力之间的关系。

Result: 最佳模型能够在多种3D视频游戏中达到与人类玩家竞争的水平。研究发现，对于某些类型的因果推理，增加训练数据和网络深度会使模型学习更具因果性的策略。在高达12亿参数的缩放模型中，观察到了与玩具问题相似的缩放结果。

Conclusion: 本文提供了一个完整的开放视频游戏基础模型训练方案，并系统研究了行为克隆的缩放规律，证明了模型和数据规模的增加能够提升模型的因果推理能力和游戏表现，为后续研究提供了重要基础。
...
TL;DR: Sci-Reasoning是首个捕捉高质量AI研究背后智力合成过程的数据集，通过追踪顶级会议论文与其关键前驱的关系，识别出15种思维模式，其中3种主导策略占52.7%，为AI研究代理训练提供结构化推理轨迹。


<details>
  <summary>Details</summary>
Motivation: 尽管AI创新加速发展，但研究人员如何识别研究空白、综合先前工作并产生见解的智力过程仍然缺乏理解。缺乏科学推理的结构化数据阻碍了对AI研究代理的系统分析和开发。
...
Conclusion: 该数据集支持科学进步的定量研究，并为训练下一代AI研究代理提供结构化推理轨迹，有助于理解AI研究背后的智力过程并开发更智能的研究助手。
...
Method: 通过系统性文献综述方法，从3000多份记录中筛选出317篇相关文献。构建了五部分分类法（只读分析、模拟与意图生成、委托执行、自主签名、多智能体工作流）、针对智能体驱动交易管道的威胁模型，以及包含13个维度的比较能力矩阵，分析了20多个代表性系统。

Result: 识别了智能体-区块链互操作性的关键集成模式、安全风险维度（提示注入、策略滥用、密钥泄露、对抗性执行动态、多智能体串通等），并揭示了现有系统的能力差距。提出了两个核心接口抽象：用于便携无歧义目标规范的交易意图模式，以及用于跨执行环境可审计、可验证策略执行的策略决策记录。

Conclusion: 基于分析发现的差距，规划了以交易意图模式和策略决策记录为核心的研究路线图，并提议建立可复现的评估套件和基准，用于评估智能体介导的链上执行的安全性、可靠性和经济稳健性。
...
TL;DR: 该研究探讨人类认知与机器学习模型如何协同识别钓鱼邮件，发现机器学习准确率高但置信度不稳定，人类则使用更多语言线索且置信度更一致，年龄影响检测性能而语言能力影响小。
...
Result: 机器学习模型提供良好的准确率，但其置信度水平变化显著；人类评估者使用更多样化的语言线索且置信度更一致；语言能力对检测性能影响最小，但年龄有影响。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体的改进方法（如内置自改进机制或并发变体搜索）虽然能提升整体性能，但往往产生不稳定且难以审计的改进轨迹，难以保证非回归性或跨版本故障推理。

Method: 提出AgentDevel发布工程管道：1）运行当前智能体；2）从执行轨迹生成实现无关的症状级质量信号；3）通过可执行诊断合成单个发布候选版本；4）基于翻转中心的门控机制进行发布。核心设计包括：实现无关的LLM批评器、基于脚本的可执行诊断、翻转中心门控。
...
TL;DR: 该研究利用大语言模型分析社交媒体谣言传播，开发了多阶段谣言检测代理，首次量化了自然环境下谣言重复曝光对分享概率的剂量-反应效应。


<details>
  <summary>Details</summary>
Motivation: 大语言模型为大规模分析社会现象提供了前所未有的机会，但如何利用LLMs进行心理学测量，特别是在自然环境中量化谣言传播的心理动力学（如"虚幻真相效应"）仍缺乏研究。

Method: 开发了多阶段谣言检测代理：1) 合成数据增强的微调RoBERTa分类器；2) 精确关键词过滤；3) 使用GPT-4o mini的两阶段LLM验证流程。收集了首个大型小众平台选举谣言数据集。

Result: 发现分享概率随每次额外曝光稳步上升，为意识形态同质网络中信念强化的剂量-反应效应提供了大规模实证证据。模拟显示快速传染效应：仅四次传播迭代后近四分之一用户被"感染"。

Conclusion: LLMs能够通过大规模真实世界数据集中的信念动态和错误信息传播的严格测量，变革心理科学，为理解社交媒体中的谣言传播机制提供了新工具。
...
TL;DR: 提出Adversarial Reasoning RAG (ARR)框架，通过Reasoner和Verifier的对抗性推理与验证，结合过程感知奖励机制，解决现有RAG方法中单视角推理和结果导向训练的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有大型推理模型与检索增强生成的结合存在两个关键问题：1) 推理模型通常从单一、不受挑战的视角操作，限制了其对外部文档进行深度自我纠正推理的能力；2) 现有训练范式过度依赖结果导向的奖励，对复杂多步推理过程的信号提供不足。

Method: 提出Adversarial Reasoning RAG (ARR)框架，包含Reasoner和Verifier两个组件。Reasoner在检索证据上进行推理，Verifier对Reasoner的逻辑进行批判，两者相互对抗。采用过程感知优势奖励机制，结合显式观察信号和内部模型不确定性，无需外部评分模型。

Result: 在多个基准测试上的实验证明了该方法的有效性。

Conclusion: ARR框架通过对抗性推理和过程感知奖励，有效解决了现有RAG方法的局限性，提升了推理的深度和准确性。
...
TL;DR: Isabellm是一个基于LLM的Isabelle/HOL定理证明器，能够自动合成证明，结合逐步证明器和高级证明规划器，在某些引理上超越了Isabelle的标准自动化工具。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够在消费级计算机上运行、利用LLM进行自动证明合成的定理证明系统，以增强Isabelle/HOL的证明自动化能力。

Method: 结合逐步证明器（使用LLM生成证明命令并通过Isabelle验证）和高级证明规划器（生成结构化Isar大纲并填补修复空白），采用束搜索、策略重排序、前提选择、微RAG和反例引导的证明修复等技术。

Result: Isabellm能够证明一些Isabelle标准自动化工具（包括Sledgehammer）无法证明的引理，展示了LLM引导证明搜索的实际价值。

Conclusion: 虽然Isabellm展示了LLM在定理证明中的潜力，但即使是GPT 5.2和Gemini 3 Pro等最先进的LLM也难以可靠地实现复杂的算法设计，突显了LLM代码生成和推理的根本挑战。
...
TL;DR: InstruCoT：一种针对提示注入攻击的LLM防御方法，通过合成多样化训练数据和指令级思维链微调，有效识别和拒绝恶意指令
...
Method: 提出InstruCoT方法：1）合成多样化训练数据；2）采用指令级思维链微调，使LLM能够有效识别和拒绝恶意指令，无论其来源或上下文位置

Result: 在四个LLM上的实验表明，InstruCoT在行为偏差、隐私泄露和有害输出三个关键维度上显著优于基线方法，同时保持实用性能不下降

Conclusion: InstruCoT通过合成数据和指令级思维链微调，有效解决了提示注入攻击防御中的多样性和语义边界问题，显著提升了LLM的安全性
...
<details>
  <summary>Details</summary>
Motivation: 传统SMT求解器在处理包含未解释函数的非线性实数算术量化公式时面临挑战，因为传统量化实例化方法缺乏对UF约束的语义理解，导致在无限解空间中搜索效率低下。

Method: 1) 通过约束分离预处理公式；2) 使用结构化提示从LLM中提取数学推理；3) 通过自适应实例化将LLM生成的结果与传统SMT算法集成；4) 通过系统验证保持正确性：LLM指导的实例化产生SAT时解决原问题，UNSAT结果生成排除子句进行迭代优化。

Result: 在SMT-COMP基准测试中，AquaForte解决了许多Z3和CVC5等最先进求解器超时的实例，特别在可满足公式上表现出色，展示了LLM为符号推理提供有价值数学直觉的能力。

Conclusion: LLM能够为符号推理提供有价值的数学直觉，为SMT约束求解建立了新范式，通过语义指导显著提升包含未解释函数的量化公式求解效率。
...
TL;DR: 提出ResMAS框架，通过两阶段方法增强基于LLM的多智能体系统韧性：1) 训练奖励模型预测系统韧性，并用强化学习自动设计韧性拓扑；2) 基于拓扑优化每个智能体的提示词。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的多智能体系统通常分布在不同的设备或环境中，容易受到智能体故障等扰动的影响。现有研究主要关注攻击后的被动检测和缓解，而非主动设计具有内在韧性的系统。

Method: 提出ResMAS两阶段框架：第一阶段训练奖励模型预测MAS韧性，基于此通过强化学习训练拓扑生成器自动设计特定任务的韧性拓扑；第二阶段引入拓扑感知的提示优化方法，根据每个智能体与其他智能体的连接和交互来优化其提示词。

Result: 在多个任务上的广泛实验表明，该方法在各种约束条件下显著提高了MAS的韧性。此外，该框架对新任务和模型表现出强大的泛化能力。

Conclusion: ResMAS框架通过主动设计韧性拓扑和优化提示词，能够有效增强基于LLM的多智能体系统的韧性，具有很好的泛化潜力。
...
TL;DR: Tape是一个受控强化学习基准，用于隔离潜在规则变化下的分布外失败，基于一维元胞自动机，固定观测和动作空间但改变转移规则，评估发现ID表现好的方法在OOD下可能崩溃，需要标准化评估协议和统计报告要求。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习方法在分布内表现良好，但在分布外（规则变化）情况下可能失败，需要建立受控基准来隔离和评估这种OOD失败，理解方法在潜在规则变化下的鲁棒性。

Method: 基于一维元胞自动机构建Tape基准，保持观测和动作空间固定但改变转移规则，创建精确的训练/测试划分。使用可复现评估管道比较无模型基线、基于模型规划（学习世界模型）和任务推断（元RL）方法。

Result: 发现一致模式：在分布内表现强的方法在保留规则OOD下可能崩溃，高方差OOD评估使排名不稳定，除非实验充分重复。需要标准化OOD协议、统计报告要求（种子、置信区间、假设检验）。

Conclusion: 提出了信息论恒等式连接熵减少与条件互信息和期望后验KL散度，阐明了"不确定性减少"目标在规则变化下能保证和不能保证的内容。强调了OOD评估标准化和统计严谨性的重要性。
...
TL;DR: 提出结合大语言模型与知识图谱的方法，通过BERT实体识别、GPT-4语义增强、GNN知识融合和强化学习决策优化，提升企业数字化转型的智能决策能力。


<details>
  <summary>Details</summary>
Motivation: 企业在数字化转型过程中面临非结构化数据语义理解不足和驱动机制缺乏智能决策依据的问题，需要更智能的解决方案来提升转型效率。

Method: 1. 使用微调BERT进行多源异构文本的实体识别和关系抽取，GPT-4生成语义增强向量表示；2. 设计双层图神经网络架构，融合LLM语义向量与业务元数据构建动态可扩展的企业知识图谱；3. 引入强化学习优化决策路径生成，通过奖励函数驱动机制迭代。
...
Method: 提出TourPlanner框架，包含三个核心组件：1) 个性化召回和空间优化(PReSO)工作流构建空间感知候选POI集；2) 竞争共识思维链(CCoT)多路径推理范式增强解空间探索；3) 基于Sigmoid的门控机制集成到强化学习中，动态优先满足硬约束后再优化软约束。
...
TL;DR: M-ASK是一个多智能体搜索框架，将搜索行为与知识管理解耦，通过角色分离和细粒度奖励解决传统单体智能体搜索中的结构瓶颈问题。
...
Method: 提出M-ASK框架，将搜索智能体分解为两个互补角色：搜索行为智能体（规划执行搜索动作）和知识管理智能体（聚合、过滤、维护紧凑内部上下文）。采用回合级奖励为搜索决策和知识更新提供细粒度监督。

Result: 在多跳问答基准测试中，M-ASK优于强基线方法，不仅获得更高的答案准确率，而且训练动态显著更稳定。

Conclusion: 通过角色分离和细粒度奖励协调，M-ASK有效解决了智能体搜索中的结构瓶颈问题，实现了更高效稳定的搜索性能。
...
Conclusion: 结果验证了嵌入空间对齐是一种有效策略，能够使语言模型在生产事件响应场景中对多模态遥测数据进行推理。
...
TL;DR: ThinkDrive：一种用于自动驾驶的思维链引导渐进式强化学习微调框架，通过两阶段训练策略（SFT+渐进RL）结合难度感知自适应策略优化，显著提升自动驾驶决策性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶LLM方法存在非结构化推理、泛化能力差、与人类驾驶意图不一致等问题。虽然思维链（CoT）能增强决策透明度，但传统监督微调未能充分利用其潜力，而强化学习方法存在不稳定和推理深度不足的问题。

Method: 提出ThinkDrive框架：1）第一阶段使用CoT解释进行监督微调（SFT）；2）第二阶段应用渐进式强化学习，采用难度感知自适应策略优化器，根据样本复杂度动态调整学习强度。

Result: 在公共数据集上评估，ThinkDrive在exam、easy-exam和accuracy指标上分别比强RL基线提升1.45%、1.95%和1.01%。使用该方法训练的2B参数模型在exam指标上超越GPT-4o达3.28%。

Conclusion: ThinkDrive通过结合显式推理与难度感知自适应策略优化，有效解决了自动驾驶领域LLM应用中的推理、泛化和意图对齐问题，为自动驾驶决策提供了更透明、更稳定的解决方案。
...
TL;DR: CompassMem：受事件分割理论启发的记忆框架，将记忆组织为事件图，通过逻辑关系连接事件，支持结构化导航和长时程推理。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能代理的记忆机制大多采用扁平化存储和简单的相似性检索，难以捕捉经验间的逻辑关系，记忆访问与结构脱节，限制了长时程依赖的推理能力。

Method: 提出CompassMem框架，将记忆组织为事件图：1）增量地将经验分割为事件；2）通过显式逻辑关系连接事件；3）事件图作为逻辑地图，支持结构化、目标导向的记忆导航。

Result: 在LoCoMo和NarrativeQA数据集上的实验表明，CompassMem在多个骨干模型上一致提升了检索和推理性能。

Conclusion: CompassMem通过事件图结构实现了逻辑关系的显式捕捉，支持结构化记忆导航，有效提升了智能代理在长时程场景下的推理能力。
...
TL;DR: Miner提出利用模型内在不确定性作为自监督奖励信号，解决推理模型RL训练中正同质提示效率低下的问题，通过token级焦点信用分配和自适应优势校准实现高效训练。


<details>
  <summary>Details</summary>
Motivation: 当前无批评RL方法在训练正同质提示（所有rollout都正确）时效率极低，因为优势估计为零导致rollout浪费。需要一种无需外部监督、辅助模型或额外推理成本的高效训练方法。

Method: Miner方法包含两个关键创新：1) token级焦点信用分配机制，动态放大关键不确定token的梯度，抑制过度自信token；2) 自适应优势校准，无缝整合内在奖励和可验证奖励。

Result: 在Qwen3-4B和Qwen3-8B基模型上的六个推理基准测试中，Miner相比GRPO在Pass@1上获得4.58绝对提升，Pass@K上获得6.66提升，达到最先进性能。
...
TL;DR: KnowMeBench是一个基于长篇自传叙事构建的基准测试，用于评估模型对人的理解能力，超越传统检索性能评估


<details>
  <summary>Details</summary>
Motivation: 现有长期记忆基准测试主要使用多轮对话或合成用户历史，使得检索性能不能很好地代表对人的理解能力。需要更真实的评估工具来测试模型对人的动机和决策原则的理解

Method: 从长篇自传叙事构建基准测试，将每个叙事重构为具有闪回意识、时间锚定的流，并通过证据链接的问题评估模型，涵盖事实回忆、主观状态归因和原则级推理

Result: 检索增强系统主要提高事实准确性，但在时间基础的解释和高级推理方面仍存在错误，表明需要超越检索的记忆机制

Conclusion: 基于自传叙事的基准测试能更好地评估模型对人的理解能力，现有检索方法在高级推理方面存在局限，需要开发更先进的记忆机制
...
TL;DR: 本文探讨将多智能体系统编译为单智能体系统，通过技能选择替代通信，显著降低计算开销，但发现技能选择存在类似人类认知的容量限制，在关键库规模后性能急剧下降，语义混淆是主要因素。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统虽然能有效处理复杂推理，但通信带来巨大计算开销。能否通过单智能体选择技能库中的技能实现类似模块化效益？同时探索技能选择如何随库规模扩展，以及是否存在类似人类认知的容量限制。

Method: 将多智能体系统编译为等效的单智能体系统，用技能选择替代智能体间通信。研究技能选择的扩展行为，分析库规模对选择准确性的影响，特别关注语义混淆的作用。引入分层路由机制进行实验验证。

Result: 初步实验显示该方法能显著减少token使用和延迟，同时在推理基准上保持竞争力。技能选择准确性在达到关键库规模前保持稳定，随后急剧下降，呈现类似人类认知容量限制的相变现象。语义混淆而非单纯库规模是性能下降的主要因素。分层路由能有效改善选择性能。

Conclusion: 技能选择存在类似人类认知的容量限制，语义混淆是主要挑战。分层组织（类似人类管理复杂选择的方式）能帮助AI系统扩展技能选择能力。这为设计可扩展的技能基智能体提供了认知基础框架和实践指导，并提出了关于LLM语义技能选择基本限制的新问题。
...
TL;DR: Orion-RAG提出了一种轻量级路径提取方法，将碎片化文档转化为半结构化数据，有效链接跨文件信息，在知识合成任务中显著优于主流框架。
...
Result: Orion-RAG在多个领域持续优于主流框架，支持实时更新和显式的人机交互验证，具有高成本效益。在FinanceBench上相对基线提升25.2%的精确度。

Conclusion: 轻量级路径提取方法足以将碎片化文档组织成半结构化数据，有效解决RAG在离散数据环境中的挑战，实现高效的知识合成。
...
<details>
  <summary>Details</summary>
Motivation: 智能体强化学习作为重要的后训练范式，需要解决多轮任务中探索多样性有限、稀疏信用分配困难以及策略优化与智能体交互粒度不对齐的问题。
...
Conclusion: AT²PO提供了一个统一的多轮智能体RL框架，有效解决了探索、信用分配和策略优化对齐问题，ATPO组件可与任何多轮RL流程集成。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试存在关键盲点：通用指令遵循指标关注表面格式，而领域特定科学基准只评估最终答案正确性，往往奖励那些用错误理由得出正确结果的模型。需要建立能评估科学推理严谨性的新标准。

Method: 引入科学指令遵循概念，并开发SciIF多学科基准测试，将大学水平问题与固定约束目录配对，涵盖三个支柱：科学条件、语义稳定性和特定过程。强调可审计性，要求模型提供约束满足的明确证据。

Result: SciIF能够同时测量解决方案正确性和多约束遵守情况，实现对组合推理失败的细粒度诊断，确保LLM能在科学的严格逻辑框架内作为可靠代理运行。

Conclusion: 随着LLM从通用知识检索转向复杂科学发现，评估标准必须纳入科学探究的严谨规范。SciIF填补了现有基准的空白，为评估LLM在科学推理中的严谨性提供了新框架。
...
TL;DR: APEX是一个用于交互式学术海报编辑的智能体框架，支持细粒度控制，并引入了首个系统性基准APEX-Bench进行评测。


<details>
  <summary>Details</summary>
Motivation: 现有学术海报生成方法通常是单次、非交互式的，难以满足用户复杂、主观的编辑意图。需要一种支持交互式编辑的框架来填补这一空白。

Method: 提出了APEX框架，基于多级API编辑和"审查-调整"机制实现细粒度控制。同时构建了APEX-Bench基准，包含514条编辑指令，采用参考引导和无参考策略确保真实性和多样性。

Result: 实验结果表明APEX显著优于基线方法。建立了多维度的VLM-as-a-judge评估协议，评估指令完成度、修改范围和视觉一致性。

Conclusion: APEX是首个用于交互式学术海报编辑的智能体框架，通过细粒度控制和系统性基准解决了现有方法的局限性，为学术海报设计提供了有效的自动化解决方案。
...
TL;DR: 提出一种新方法，通过工具结果解析为LLM提供精确数据，同时有效过滤注入的恶意代码，在保持竞争力的攻击下效用同时达到最低的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 随着LLM代理从数字助手转变为自主系统和机器人中的物理控制器，面临间接提示注入威胁日益严重。攻击者通过工具调用结果嵌入对抗性指令，可以劫持代理的决策过程执行未授权操作。现有防御机制存在计算开销大或攻击成功率高等问题。

Method: 提出一种新颖方法，通过工具结果解析为LLM提供精确数据，同时有效过滤掉注入的恶意代码。该方法结合了精确数据提供和恶意代码过滤的双重机制。

Result: 该方法在攻击下效用(UA)方面保持竞争力，同时达到了迄今为止最低的攻击成功率(ASR)，显著优于现有方法。

Conclusion: 提出的方法有效解决了LLM代理在物理控制场景下面临的间接提示注入威胁，通过工具结果解析和恶意代码过滤实现了安全性和实用性的平衡。
...
TL;DR: TNT方法通过利用思考型响应的解决方案信息，为不同查询设置非思考响应的不同最大token限制，无需监督微调，在减少50% token使用的同时提升准确率，实现准确性与效率的最优平衡。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过长链思维（CoT）获得优异性能，但计算开销巨大。现有基于强化学习的方法存在奖励欺骗问题（模型思考却被判定为未思考），而监督微调成本高，统一token限制效果有限。

Method: 提出Thinking-Based Non-Thinking (TNT)方法，不采用监督微调，而是利用思考型响应的解决方案组件信息，为不同查询的非思考响应设置不同的最大token使用限制。

Result: 在五个数学基准测试中，TNT相比DeepSeek-R1-Distill-Qwen-1.5B/7B和DeepScaleR-1.5B减少约50%的token使用，同时显著提升准确率，在所有测试方法中实现准确性与效率的最优平衡，非思考响应中的奖励欺骗概率保持在10%以下。

Conclusion: TNT方法有效解决了大型推理模型的过度思考问题，通过基于思考的非思考策略，在不使用监督微调的情况下实现了准确性与计算效率的最佳权衡，同时将奖励欺骗问题控制在较低水平。
...
TL;DR: SCALER是一个通过自适应环境设计维持有效学习信号的强化学习框架，通过可扩展的合成管道将编程问题转化为可验证的推理环境，并采用自适应多环境RL策略来持续提升语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在训练语言模型推理能力时面临两个主要问题：1）任务难度与模型能力不匹配时学习信号失效；2）训练被少数重复问题模式主导导致过拟合。需要一种能够持续提供有效学习信号的方法。

Method: SCALER包含两个核心组件：1）可扩展的合成管道，将真实编程问题转化为具有可控难度和无限实例生成的可验证推理环境；2）自适应多环境RL策略，动态调整实例难度并策划活跃环境集，以跟踪模型能力前沿并保持分布多样性。

Result: 实验表明SCALER在多种推理基准测试中持续优于基于数据集的RL基线方法，展现出更稳定、更长周期的训练动态，能够防止奖励稀疏性，减轻对狭窄任务模式的过拟合，支持持续改进。

Conclusion: SCALER通过自适应环境设计解决了RL训练中的关键瓶颈，通过可扩展的合成环境和动态难度调整，为语言模型推理能力的持续提升提供了有效的学习信号，在多样化的推理任务中表现出优越性能。
...
TL;DR: AECV-Bench：评估多模态和视觉语言模型在建筑图纸理解能力的基准，包含对象计数和图纸问答两个任务，发现当前模型在文本相关任务表现良好，但在符号识别和空间推理方面仍有不足。


<details>
  <summary>Details</summary>
Motivation: 建筑图纸通过符号、布局约定和密集标注编码几何和语义信息，但现代多模态和视觉语言模型是否能可靠解释这种图形语言尚不清楚，需要系统评估模型在真实AEC构件上的理解能力。
...
TL;DR: 提出DR-LoRA框架，通过动态调整MoE模型中各专家的LoRA秩来解决现有PEFT方法对专家统一分配参数导致的资源不匹配问题


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA）对MoE LLMs中所有专家分配相同的秩，忽视了专家间的功能专业化差异，导致任务相关专家参数不足而无关专家参数冗余

Method: 提出DR-LoRA框架，包含专家显著性评分机制，结合专家路由频率和LoRA秩重要性来量化各专家对额外容量的需求，动态增长高显著性专家的LoRA秩

Result: 在多个基准测试中，DR-LoRA在相同参数预算下始终优于标准LoRA和静态分配策略，实现了更优的任务性能和更高效的参数利用

Conclusion: DR-LoRA通过动态调整专家LoRA秩，实现了针对目标任务的异构秩分布，有效解决了MoE LLMs微调中的资源分配不匹配问题
...
TL;DR: OI-MAS框架通过自适应模型选择策略，在异构多尺度LLM池中动态分配不同推理阶段的任务，显著提升多智能体系统的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统在所有智能体角色中统一部署大型语言模型，未能考虑不同推理阶段的认知需求差异，导致显著的计算效率低下问题。

Method: 提出OI-MAS框架，采用自适应模型选择策略，在异构多尺度LLM池中实现状态依赖的路由机制，动态选择智能体角色和模型规模；引入基于置信度的机制，根据任务复杂度选择适当的模型规模。

Result: 实验结果表明，OI-MAS在保持准确性的同时显著降低成本，相比基线多智能体系统准确率提升高达12.88%，成本降低高达79.78%。
...
TL;DR: 提出一种基于任务特定提示-原型（ProP）的持续学习方法，消除对键值对的依赖，减少任务间干扰并提高可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于提示的持续学习方法通常依赖键值对，这会引入任务间干扰并限制可扩展性。需要一种不依赖键值对的新方法来提高持续学习性能。

Method: 使用任务特定提示促进当前任务的特征学习，同时用对应原型捕捉输入的典型特征。推理时通过绑定任务特定提示和其关联原型生成预测。在提示初始化时引入正则化约束惩罚过大值以增强稳定性。

Result: 在多个广泛使用的数据集上的实验证明了该方法的有效性。相比主流提示方法，该方法移除了对键值对的依赖。
...
TL;DR: 该研究提出了一种基于超图的知识表示方法，用于科学发现中的系统级推理，避免了传统知识图谱中成对约束的局限性，能够捕捉高阶相互作用，并成功应用于生物复合材料支架领域。


<details>
  <summary>Details</summary>
Motivation: 科学探究需要整合异构实验数据、跨领域知识和机制证据的系统级推理。虽然大语言模型具备推理能力，但依赖缺乏结构深度的检索增强上下文。传统知识图谱试图弥合这一差距，但其成对约束无法捕捉控制涌现物理行为的不可约高阶相互作用。

Method: 引入构建超图知识表示的方法论，能够忠实编码多实体关系。应用于约1,100篇生物复合材料支架文献，构建了包含161,172个节点和320,201条超边的全局超图，揭示了围绕高度连接概念枢纽组织的无标度拓扑结构。进一步为智能体系统配备超图遍历工具，使用节点交集约束来桥接语义上遥远的概念。

Result: 构建的全局超图显示幂律指数约1.23的无标度拓扑结构，避免了成对扩展中典型的组合爆炸，并明确保留了科学表述的共现上下文。通过利用高阶路径，系统成功为新型复合材料生成基于机制假设，例如通过壳聚糖中间体将氧化铈与PCL支架联系起来。

Conclusion: 这项工作建立了一个"无教师"的智能推理系统，其中超图拓扑结构作为可验证的防护栏，通过揭示传统图方法所掩盖的关系来加速科学发现。超图表示能够捕捉科学知识中固有的高阶相互作用，为科学推理提供了更强大的框架。
...
TL;DR: FlexSIPP算法通过利用其他代理的时间灵活性，在单个代理延迟时高效重新规划多代理计划，避免连锁延迟


<details>
  <summary>Details</summary>
Motivation: 多代理计划执行中，单个代理延迟会导致与其他代理的冲突。重新规划延迟代理通常效率低下，而重新规划其他代理可能引发连锁延迟，需要一种高效的重规划方法

Method: 提出FlexSIPP算法，通过预计算延迟代理的所有可能计划，并利用其他代理的时间灵活性（即不改变顺序或不进一步延迟更多代理的最大延迟量），避免连锁延迟

Result: 在荷兰密集铁路网络的真实案例研究中，FlexSIPP提供了有效的解决方案，适用于实际调整场景，并在合理时间内完成

Conclusion: FlexSIPP算法能够高效处理多代理计划中的延迟问题，通过利用时间灵活性避免连锁效应，在实际应用中表现出色
...
TL;DR: 提出结合着色时间Petri网与基于模型的强化学习的新方法，解决柔性制造系统中AGV和工具共享的复杂调度问题，在大规模实例上优于传统方法且计算时间减少10倍。
...
Method: 结合着色时间Petri网（提供形式化建模结构和动态动作屏蔽）与基于模型的actor-critic强化学习，利用前瞻策略优化AGV定位，减少动作搜索空间并适应环境变化。

Result: 在小规模公共基准测试中与传统方法相当，在基于Taillard基准开发的大规模实例上优于传统方法（完工时间更优），计算时间减少10倍，消融研究验证了各组件贡献。

Conclusion: 该方法有效解决了柔性制造系统中AGV和工具共享的复杂调度问题，提供了可复现的环境和实例生成器，在大规模问题上展现出优越性能和计算效率。
...
TL;DR: SmartSearch框架通过过程奖励和查询优化机制提升LLM搜索代理的查询质量，采用三阶段课程学习实现查询质量的渐进式改进。
...
Method: 提出SmartSearch框架，包含两个核心机制：1）过程奖励通过双级信用评估为每个中间搜索查询提供细粒度监督；2）查询优化通过选择性优化低质量查询并基于优化结果重新生成后续搜索轮次。采用三阶段课程学习（模仿、对齐、泛化）让搜索代理逐步内化查询质量改进能力。

Result: 实验结果显示SmartSearch持续超越现有基线方法，定量分析进一步证实其在搜索效率和查询质量方面的显著提升。

Conclusion: SmartSearch通过关注中间查询质量优化，有效提升了LLM搜索代理的整体性能，代码已开源供社区使用。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM评估面临变体污染问题：训练语料包含语义相同但词汇/句法变化的测试项，这些变体逃避现有检测方法，导致基准分数因记忆而非真实推理而虚高

Method: 提出DVD（通过生成分布方差检测）方法，建模温度采样诱导的局部输出分布。核心洞察是：污染项在记忆遵循状态和扰动漂移状态间交替，导致低概率token的合成难度方差异常高；未污染项保持漂移且方差相对平滑

Result: 在Omni-MATH和SuperGPQA两个领域构建首个变体污染基准，通过微调不同规模和架构的模型（Qwen2.5和Llama3.1）模拟污染。DVD在所有数据集和模型上均优于基于困惑度、Min-k%++、编辑距离和嵌入相似性的基线方法，且对超参数具有强鲁棒性
...
TL;DR: 行为规划是一种新型的多样化规划方法，通过显式整合多样性模型并支持多种规划类别，在现实世界应用中具有实用价值。


<details>
  <summary>Details</summary>
Motivation: 多样化规划方法旨在生成彼此不同的规划方案，在风险管理、自动化流数据分析、恶意软件检测等多个现实领域有广泛应用需求。行为规划作为新型多样化规划范式，需要在实际场景中验证其有效性。

Method: 采用行为规划方法，该方法扩展了早期方法，通过显式将多样性模型整合到规划过程中，并支持多种规划类别。通过三个案例研究来验证该方法：故事创作、城市规划、游戏评估。

Result: 通过三个实际案例研究展示了行为规划在现实世界中的实用性：1) 故事创作领域；2) 城市规划领域；3) 游戏评估领域。这些案例证明了行为规划方法在不同应用场景中的有效性。

Conclusion: 行为规划作为一种新型多样化规划范式，在现实世界应用中具有显著实用价值，能够有效支持多种规划类别，并在不同领域展示出良好的应用前景。
...
TL;DR: 研究探讨了将对话式生成AI助手整合到学习分析仪表板中，分析不同自我调节学习能力水平的学生如何与AI互动，以及AI如何支持他们的学习过程。


<details>
  <summary>Details</summary>
Motivation: 学习分析仪表板旨在通过数据反馈支持学生学习，但学生（尤其是自我调节学习能力较低者）常难以有效参与和解读分析反馈。对话式生成AI助手有潜力通过实时、个性化、基于对话的支持来辅助这一过程。

Method: 在10周学期期间，将生成AI助手整合到学习分析仪表板中，收集学生与AI助手的真实对话。分析不同SRL水平学生提出的问题类型、助手回答的相关性和质量，以及学生对助手在学习中作用的感知。

Result: 发现不同SRL水平学生有显著不同的提问模式：低SRL学生寻求澄清和保证，高SRL学生询问技术细节和请求个性化策略。AI助手能提供清晰可靠的解释，但在个性化、处理情感化查询和整合多数据点进行定制响应方面有限。

Conclusion: 生成AI干预对低SRL学生特别有价值，提供支持反馈参与的脚手架，缩小与高SRL同伴的差距。同时，学生反馈强调了信任的重要性，以及未来系统需要更强的适应性、情境感知和技术改进。
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在科学发现中的角色，特别是作为编码伙伴加速科学研究的潜力，通过实际竞赛案例验证人类-AI协作的有效性。

Method: 使用ChatGPT进行快速原型开发，参与ESA的ELOPE竞赛，处理事件相机数据以估计月球着陆器轨迹，采用人类与AI的协作开发模式。

Result: 在竞赛中获得第二名（得分0.01282），证明了人类-AI协作在竞争性科学环境中的有效性，同时识别了AI在科学工作中的具体优势和局限。

Conclusion: 对话式AI能够加速科学开发并提供概念性见解，但需要结构化整合到科学工作流程中，并制定AI辅助科学工作的最佳实践。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于图的RAG方法在处理层次信息时存在两个关键限制：1）强制的层特定压缩配额会破坏局部图结构；2）优先考虑拓扑结构而忽略语义内容。需要一种能同时保留结构和语义的层次信息管理方法。
...
Conclusion: T-Retriever通过结合语义和结构信息的层次编码树，有效解决了图RAG在层次信息管理中的局限性，为复杂知识检索任务提供了更优的解决方案。
...
TL;DR: ConMax是一种强化学习框架，通过最大化置信度来自动压缩推理轨迹，在保持准确性的同时减少43%的推理长度。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型需要大量思维链生成来实现复杂认知行为，但这会导致"过度思考"问题——生成冗余推理路径增加计算成本而不提升准确性。现有压缩技术会损害逻辑连贯性或采样成本过高。

Method: 提出ConMax（置信度最大化压缩）强化学习框架，将压缩建模为奖励驱动的优化问题。训练策略通过最大化加权组合的答案置信度（预测保真度）和思考置信度（推理有效性）来剪枝冗余，使用冻结的辅助LRM进行评估。

Result: 在五个推理数据集上的实验表明，ConMax实现了优越的效率-性能权衡。相比强基线，推理长度减少43%，准确性仅下降0.7%，能够为LRM生成高质量、高效的训练数据。

Conclusion: ConMax通过强化学习框架有效压缩推理轨迹，在保持逻辑连贯性的同时显著降低计算成本，为大型推理模型的训练数据压缩提供了有效解决方案。
...
TL;DR: AlgBench是一个专家策划的算法推理基准测试，包含3000多个原始问题，涵盖27种算法。评估发现大型推理模型在非优化任务上表现良好（达92%），但在动态规划等全局优化算法上准确率骤降至49%，揭示了模型存在"策略过度转移"问题。


<details>
  <summary>Details</summary>
Motivation: 现有算法推理基准测试有限，无法回答大型推理模型是否真正掌握算法推理能力的问题。需要建立一个算法中心的评估范式来全面评估模型的算法推理能力。

Method: 提出AlgBench基准测试，由ACM算法专家构建，包含3000多个原始问题，涵盖27种算法，采用全面分类法组织（欧几里得结构、非欧几里得结构、非优化、局部优化、全局优化、启发式优化等类别）。在领先的大型推理模型（如Gemini-3-Pro、DeepSeek-v3.2-Speciale、GPT-o3）上进行实证评估。

Result: 模型表现存在显著异质性：在非优化任务上表现良好（准确率高达92%），但在全局优化算法（如动态规划）上准确率骤降至约49%。分析发现模型存在"策略过度转移"问题，即模型因必要的低熵标记而过早放弃正确的算法设计。

Conclusion: 研究结果暴露了问题中心强化学习的基本局限性，强调了采用算法中心训练范式对于实现稳健算法推理的必要性。AlgBench为评估和改进大型推理模型的算法推理能力提供了重要工具。
...
Method: 引入专家策划的小型数据集，评估LLMs在表格问答任务中的表现，要求模型在分析前进行额外的错误纠正步骤，研究模型在不同扭曲条件下的信息处理差异

Result: LLMs缺乏检测和纠正表格扭曲的内在能力，即使提供系统提示也只能部分调整推理策略，纠正部分扭曲但不一致或不完全，最先进的GPT-5.2模型在扭曲下准确率下降至少22%

Conclusion: 研究结果提出了重要问题：模型何时以及如何应自主决定重新对齐表格输入（类似人类行为），而不依赖明确提示或表格数据预处理
...
TL;DR: OptiSet是一个基于集合的RAG框架，通过"扩展-精炼"范式、自合成策略和集合列表训练，解决传统top-k检索中的冗余问题，实现互补证据的紧凑选择。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法通常基于个体相关性静态选择top-k段落，无法利用段落间的组合增益，且常引入大量冗余，需要更智能的集合选择方法。

Method: 采用"扩展-精炼"范式：先扩展查询视角获得多样候选池，再通过重选形成紧凑证据集；设计自合成策略从生成器的条件效用变化推导偏好标签；引入集合列表训练联合优化集合选择和集合级排序。

Result: 实验表明OptiSet在复杂组合问题上提升性能，使生成更高效，代码已开源。

Conclusion: OptiSet通过集合中心框架有效解决RAG中的冗余问题，实现互补证据的智能选择，提升生成质量和效率。
...
TL;DR: 本文针对Warmup-Stable-Decay学习率调度器，重新推导了预训练中数据消耗E与训练步数S的关系，提出了最小批次大小B_min和最优批次大小B_opt的概念，并设计了动态批次大小调度器来提升训练效率和模型质量。


<details>
  <summary>Details</summary>
Motivation: OpenAI提出的Critical Batch Size概念在传统预训练中很有效，但随着Warmup-Stable-Decay学习率调度器成为主流，原有理论框架已无法适应新的预训练动态。需要建立适用于WSD调度器的理论框架来弥合理论与实践之间的差距。

Method: 1) 为WSD调度器推导修正的E(S)关系，描述预训练中数据消耗与训练步数的权衡；2) 理论分析揭示WSD预训练的两个基本属性：达到目标损失所需的最小批次大小B_min，以及最大化数据效率的最优批次大小B_opt；3) 基于这些属性提出动态批次大小调度器。

Result: 大量实验表明，修正后的公式能精确捕捉大规模预训练的动态特性，提出的调度策略显著提升了训练效率和最终模型质量。

Conclusion: 本文为WSD调度器建立了新的理论框架，提出了B_min和B_opt概念，并设计了动态批次大小调度器，有效解决了传统Critical Batch Size理论在WSD环境下的不适用问题，提升了大规模预训练的效率和效果。
...
<details>
  <summary>Details</summary>
Motivation: 大规模预训练中学习率配置是一个关键但困难的挑战，需要在训练成本和模型性能之间取得平衡。核心问题是能否从低成本实验中准确外推最优学习率。

Method: 1) 提出两种研究范式：拟合范式和迁移范式；2) 在拟合范式中引入搜索因子的缩放定律，通过预测建模将搜索复杂度从O(n³)降至O(n*C_D*C_η)；3) 在迁移范式中将μTransfer原理扩展到MoE架构，涵盖模型深度、权重衰减和token范围。

Result: 1) 实证结果表明广泛采用的μTransfer在大规模预训练场景下的可扩展性受到挑战；2) 通过训练稳定性和特征学习的双重视角分析，解释了模块级参数调优在大规模设置中表现不佳的原因。

Conclusion: 本文为工业级预训练优化提供了系统性的实用指南和新的理论视角，挑战了现有超参数研究的边界，对大规模深度学习训练具有重要指导意义。
...
TL;DR: GPT-4o能同样有效地增加和减少阴谋论信念，即使有安全护栏限制，其促进错误信念的能力依然很强，但通过纠正对话和准确性提示可以缓解这种风险。


<details>
  <summary>Details</summary>
Motivation: 虽然LLMs在各种情境中表现出说服力，但尚不清楚这种说服力是更有利于真相还是谬误，以及LLMs是否同样容易促进错误信念。

Method: 通过三个预注册实验，让2,724名美国参与者与GPT-4o讨论他们不确定的阴谋论，模型被指示要么反驳（"辟谣"）要么支持（"造谣"）该阴谋论，比较了去护栏的"越狱"版本和标准版本的效果。

Result: 去护栏的GPT-4o在增加阴谋论信念方面与减少信念同样有效；令人担忧的是，支持阴谋论的AI获得更积极评价并增加对AI的信任；标准GPT-4o产生类似效果，OpenAI的安全护栏几乎无法阻止LLM促进阴谋论信念；但纠正对话能逆转新诱导的信念，提示GPT-4o仅使用准确信息能大幅降低其增加阴谋论信念的能力。

Conclusion: LLMs具有促进真相和谬误的强大能力，但存在潜在解决方案来帮助缓解这种风险。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于MCTS的RLVR方法存在探索多样性有限和推理效率低下的问题，需要改进以提升LLM的推理能力。

Method: ROSE采用基于语义熵的分支策略和ε探索机制来增加推理多样性，同时设计长度感知的段级优势估计器来提高效率。

Result: 在多个数学推理基准测试中，使用Qwen和Llama模型验证了ROSE的有效性和效率。

Conclusion: ROSE通过语义多样性探索和高效推理机制，显著提升了LLM在推理任务中的性能。
...
TL;DR: 大型推理模型通过显式思维链提高性能，但中间推理会泄露个人身份信息。本文研究如何在不暴露敏感信息的情况下进行隐私优先推理，提出了数据集和评估基准，发现不同模型需要不同干预方法，最终实现隐私推理且性能损失最小。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过生成显式思维链提高了性能、可靠性和可解释性，但这种透明性带来了严重的隐私风险：即使最终答案经过清理，中间推理过程仍可能泄露个人身份信息。需要研究如何在不暴露敏感信息的情况下进行隐私优先推理。

Method: 1) 引入PII-CoT-Bench数据集，包含隐私感知的思维链标注；2) 创建类别平衡的评估基准，覆盖现实和对抗性泄露场景；3) 采用两种干预方法：基于提示的控制（适用于先进模型）和微调（适用于较弱模型）。

Result: 研究发现能力依赖趋势：最先进的模型从基于提示的控制中获益最大，而较弱的模型需要微调才能实现有意义的泄露减少。两种方法都能显著减少PII暴露，同时仅带来最小的性能下降。

Conclusion: 隐私思维链推理可以在不牺牲性能的情况下实现，为构建隐私保护推理系统提供了实用指导。不同模型需要不同的干预策略，但都能在保持效用的同时大幅减少隐私泄露风险。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语原生LLM快速发展，但评估基准滞后，现有框架主要针对英语。工具调用能力在阿拉伯语等非英语语言中的表现缺乏研究，而大多数模型主要在英语数据上预训练，存在关键评估空白
...
TL;DR: FusionRoute是一个token级别的多LLM协作框架，通过轻量级路由器同时选择最合适的专家模型并贡献互补logits来优化解码策略，在保持效率的同时超越现有协作方法。


<details>
  <summary>Details</summary>
Motivation: 解决大型通用模型训练部署成本过高与小型领域专用模型泛化能力不足之间的矛盾，寻求在效率和性能之间取得平衡的多模型协作方案。

Method: 提出FusionRoute框架：1）轻量级路由器在解码的每个步骤选择最合适的专家模型；2）路由器同时贡献互补logits，通过logit加法来修正或优化所选专家的下一个token分布；3）理论分析表明纯专家路由存在根本限制，需要可训练的互补生成器来扩展有效策略类别。

Result: 在Llama-3和Gemma-2系列模型以及数学推理、代码生成、指令遵循等多样化基准测试中，FusionRoute超越了序列级和token级协作、模型融合和直接微调方法，同时在各自任务上保持与领域专家的竞争力。

Conclusion: FusionRoute通过结合专家选择和互补生成，有效解决了多LLM协作中的根本限制，在保持效率的同时实现了优于现有方法的性能，为平衡模型规模和泛化能力提供了有前景的解决方案。
...
TL;DR: 提出SteeM框架，让用户可动态调节LLM代理对记忆的依赖程度，解决传统"全有或全无"记忆方法导致的记忆锚定或记忆利用不足问题


<details>
  <summary>Details</summary>
Motivation: LLM代理在长期交互中需要累积记忆来实现个性化和风格一致性，但现有系统采用"全有或全无"的记忆使用方式：包含所有相关历史信息会导致记忆锚定（代理被过去交互束缚），而完全排除记忆则会导致利用不足和重要交互历史丢失

Method: 1. 引入记忆依赖行为度量来量化过去交互对当前输出的影响；2. 提出SteeM（Steerable Memory Agent）框架，允许用户动态调节记忆依赖程度，从促进创新的"重新开始"模式到紧密遵循交互历史的"高保真"模式

Result: 在不同场景下的实验表明，该方法持续优于传统提示方法和刚性记忆掩码策略，为个性化人机协作提供了更细致有效的控制

Conclusion: 代理对记忆的依赖可以建模为显式且用户可控的维度，SteeM框架通过动态调节记忆依赖程度，解决了传统记忆方法的局限性，实现了更精细的个性化人机协作控制
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过生成多步思维链获得优异性能，但带来了巨大的推理延迟和计算成本。现有协作推理方法依赖局部token概率或事后验证，引入了显著开销。需要一种更高效的方法来判断推理步骤何时需要大模型能力或小模型效率。

Method: 提出GlimpRouter免训练协作框架，基于"顿悟时刻"现象发现推理步骤的难度可以从其第一个token的熵推断。使用轻量级模型生成每个推理步骤的第一个token，当初始token熵超过阈值时，将步骤路由到大模型处理。

Result: 在多个基准测试中显著降低推理延迟同时保持精度。例如在AIME25上，相比独立大模型，GlimpRouter在准确率提升10.7%的同时，推理延迟降低25.9%。

Conclusion: 研究表明基于思维"一瞥"而非完整步骤评估来分配计算是一种简单有效的推理机制，为高效协作推理提供了新视角。
...
TL;DR: LLM评委系统存在可靠性悖论：评委内部高度一致但彼此间几乎零共识，评委间的分歧模式稳定到可作为指纹识别，每个评委都实施着独特且稳定的质量理论。


<details>
  <summary>Details</summary>
Motivation: 研究LLM-as-judge系统的评估一致性，发现虽然承诺可扩展、一致的评估，但实际上评委之间存在严重分歧，需要揭示这种分歧的结构化本质。

Method: 通过大规模实验（3,240次评估，9个评委×120个独特视频×3次独立运行），使用Krippendorff's α衡量评委间一致性，构建分类器从评分中识别评委，并分析评委的评估倾向特征。

Result: 评委间一致性接近零（α=0.042），某些维度上分歧甚至超过随机噪声。但分歧具有结构化模式：仅凭评分就能以77.1%准确率识别评委，加入倾向特征后达89.9%。同一模型家族内区分度更高（GPT-4.1 vs GPT-5.2达99.6%）。

Conclusion: LLM评委不是可互换的测量工具，而是各自编码独特质量理论的独立测量设备。平均评分产生的是不符合任何评委实际价值观的合成裁决，这挑战了LLM-as-judge系统的可靠性假设。
...
<details>
  <summary>Details</summary>
Motivation: 现有数字水印方法在应用于推理大语言模型时存在两大问题：基于令牌的水印技术会破坏推理流程的逻辑连贯性，而语义感知方法虽然提高了质量但引入了显著延迟或需要辅助模型。需要一种专门为推理密集型LLMs设计的水印框架。

Method: 1. 将生成过程解耦为不受干扰的思考阶段和加水印的回答阶段；2. 提出关键性评分来识别推理轨迹中的语义关键令牌；3. 将这些令牌提炼成主要语义向量；4. 基于令牌与PSV的对齐度，采用语义自适应机制调节水印强度。

Result: ReasonMark在多项指标上超越现有方法：文本困惑度降低0.35，翻译BLEU分数提高0.164，数学准确性提高0.67分。同时水印检测AUC提高0.34%，对抗攻击的鲁棒性更强，延迟增加可忽略不计。

Conclusion: ReasonMark框架实现了推理大语言模型的可追溯和可信部署，在保持逻辑完整性的同时提供强大的水印能力，为实际应用中的推理LLMs提供了可靠的保护机制。
...
TL;DR: 本文提出"自我消耗性能循环"概念，研究在动态迭代训练过程中合成数据如何塑造偏见，发现性能循环会增加偏好偏见但减少差异偏见，并提出基于奖励的拒绝采样策略来缓解偏见。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型使用合成数据进行训练形成了自我消耗的再训练循环，可能导致性能下降和偏见加剧。在现实应用中，已部署的LLM会影响其生成的数据，形成由用户反馈驱动的动态系统，但真实世界的用户偏好数据难以获取，需要受控环境来研究反馈驱动的偏见演化。

Method: 引入"自我消耗性能循环"概念，在受控的性能反馈设置下研究动态迭代训练过程中的偏见形成。关注两种循环类型：典型的再训练设置和增量微调设置。在三个真实世界任务上进行实验，并设计基于奖励的拒绝采样策略来缓解偏见。

Result: 实验发现性能循环会增加偏好偏见但减少差异偏见。基于奖励的拒绝采样策略能够有效缓解偏见，推动构建更可信的自我改进系统。

Conclusion: 自我消耗性能循环在动态迭代训练中会塑造偏见模式，需要设计适当的缓解策略。提出的基于奖励的拒绝采样方法能够有效减少偏见，为构建更可信的自我改进AI系统提供了方向。
...
TL;DR: SimuAgent：基于LLM的Simulink建模与仿真代理，通过简洁的Python表示替代XML，采用两阶段训练和ReGRPO算法，在SimuBench基准上超越GPT-4o等基线方法


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在文本代码自动化方面表现出色，但在图形化工程工作流中的应用潜力尚未充分探索。Simulink等图形建模环境中的XML表示冗长且难以处理，需要更高效的LLM驱动解决方案

Method: 1) 引入简洁的字典式Python表示替代冗长XML；2) 轻量级规划-执行架构；3) 两阶段课程训练（低层工具技能+高层设计推理）；4) 提出Reflection-GRPO算法，通过自反追踪提供中间反馈；5) 抽象-重构数据增强

Result: 在包含5300个多领域建模任务的SimuBench基准上，基于Qwen2.5-7B的SimuAgent收敛更快、建模精度更高，超越标准RL基线，甚至在少样本提示下优于GPT-4o。两阶段课程和数据增强进一步提升泛化能力

Conclusion: SimuAgent弥合了LLM与图形建模环境之间的差距，提供隐私保护、成本效益高的本地部署解决方案，为工业模型驱动工程中的AI辅助设计提供实用方案
...
Method: 使用Z-score标准化预处理，缺失值填补，结合Neural Prophet和MLP深度神经网络学习非线性关系
...
<details>
  <summary>Details</summary>
Motivation: LLM在工具调用中存在幻觉问题：选择错误工具、提供格式错误的参数、绕过工具直接模拟输出。这些问题影响LLM代理在生产系统中的可靠性，导致结果不一致，并绕过安全审计控制。现有检测方法需要多次前向传播或外部验证，计算成本高。

Method: 提出计算高效的实时检测框架，在LLM生成过程中利用其内部表征进行检测，仅需单次前向传播。该方法专注于检测工具选择幻觉、参数级幻觉和工具绕过行为，无需额外计算开销。

Result: 在多个领域的推理任务上评估，展示了强大的检测性能（最高86.4%准确率），同时保持实时推理能力，计算开销最小。特别擅长检测参数级幻觉和不恰当的工具选择，这对可靠代理部署至关重要。

Conclusion: 该框架为LLM代理在生产系统中的可靠部署提供了有效的幻觉检测方案，通过实时检测工具调用错误，解决了现有方法计算成本高的问题，有助于提升LLM代理的可靠性和安全性。
...
TL;DR: MineNPC-Task是一个用于评估Minecraft中记忆感知、混合主动LLM代理的用户创作基准和评估框架，包含从专家玩家合作中收集的任务、参数化模板和机器可检查验证器。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法依赖合成提示，缺乏真实用户任务和记忆感知能力的系统评估。需要基于真实玩家体验、具有明确依赖结构和机器可验证的基准来测试开放世界中的混合主动代理。

Method: 1) 从专家玩家的形成性和总结性合作中收集任务；2) 将任务规范化为具有明确前提条件和依赖结构的参数化模板；3) 在禁止世界外捷径的有界知识策略下，配对机器可检查验证器；4) 捕获计划/行动/记忆事件，包括计划预览、针对性澄清、内存读写等。

Result: 使用GPT-4o评估了8名经验玩家的216个子任务，观察到代码执行、库存/工具处理、引用和导航方面的重复故障模式，同时发现混合主动澄清和轻量级内存支持恢复。参与者对交互质量和界面可用性评价积极，但强调需要更强的跨任务记忆持久性。

Conclusion: MineNPC-Task提供了一个透明、可复现的评估框架，支持未来记忆感知具身代理的评估。发布了完整的任务套件、验证器、日志和框架，促进该领域的研究发展。
...
TL;DR: 该研究提出从野外视频中学习潜在动作世界模型的方法，以扩展现有工作在简单模拟环境中的局限，通过连续约束的潜在动作捕捉复杂动作，并展示了在规划任务中的应用潜力。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的智能体需要预测动作后果的能力，但传统世界模型需要动作标签，这在规模上难以获取。因此需要从视频中学习潜在动作模型，但现有研究主要关注简单模拟环境，缺乏对野外多样化视频的处理能力。

Method: 提出从野外视频中学习潜在动作世界模型的方法，讨论动作应遵循的属性、相关架构选择和评估方法。采用连续但约束的潜在动作表示，而非常见的向量量化方法，以捕捉野外视频中动作的复杂性。

Result: 连续约束的潜在动作能够捕捉野外视频中动作的复杂性，而向量量化方法则不能。研究发现环境变化（如人类进入房间）可以在视频间传递，表明能够学习特定于野外视频的动作。在缺乏共同体现的情况下，潜在动作主要能在空间上相对于相机定位。通过训练控制器将已知动作映射到潜在动作，可将潜在动作作为通用接口，在规划任务中达到与动作条件基线相似的性能。

Conclusion: 该研究为将潜在动作模型扩展到现实世界迈出了一步，展示了从野外视频中学习复杂动作表示的可行性，并为解决规划任务提供了新的方法框架。
...
TL;DR: 本文提出了一种用于MIMO多载波系统的多时隙跨域记忆近似消息传递接收机，并分析了其信息论极限和最优编码原则，在双选择性信道中实现了接近理论极限的性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示OTFS和AFDM在非编码双选择性信道中优于OFDM，但这些优势是否适用于编码系统尚不明确。同时，编码MIMO多载波系统的信息论极限分析和低复杂度接收机设计仍不清楚。

Method: 提出多时隙跨域记忆近似消息传递接收机，既能利用时域信道稀疏性降低复杂度，又能利用符号域星座约束提升性能。同时提出简化的单输入单输出变分状态演化来推导可达速率和最优编码原则。

Result: 数值结果表明，采用MS-CD-MAMP的编码MIMO-OFDM/OTFS/AFDM在双选择性信道中达到相同的最大可达速率，实际优化LDPC码的性能仅比理论极限低0.5-1.8dB，比设计良好的点对点LDPC码有0.8-4.4dB增益。

Conclusion: 提出的MS-CD-MAMP接收机及其信息论分析和编码原则为MIMO多载波系统提供了接近理论极限的高性能解决方案，解决了编码系统中OTFS/AFDM优势是否延续的疑问。
...
TL;DR: 论文研究了分子图学习中位置编码的关系，发现基于锚点的距离编码可作为扩散几何的低秩替代，并提出了从锚点距离重建扩散坐标的三边测量方法。


<details>
  <summary>Details</summary>
Motivation: 分子图学习需要位置信号来捕捉局部邻域和全局拓扑结构。目前主要使用两类编码：基于拉普拉斯或扩散算子的谱编码，以及基于最短路径信息的锚点距离编码，但这两类编码的确切关系尚不清楚。

Method: 将距离编码解释为扩散几何的低秩替代，推导出显式的三边测量映射，从变换后的锚点距离和锚点谱位置重建截断的扩散坐标。在随机正则图上提供了点态和Frobenius间隙保证。在DrugBank分子图上使用共享的GNP-based DDI预测骨干，采用距离驱动的Nyström方案近似恢复扩散几何。

Result: 距离驱动的Nyström方案能紧密恢复扩散几何，拉普拉斯编码和距离编码都显著优于无编码基线。在DrugBank分子图的DDI预测任务中，两种编码方法都有显著性能提升。

Conclusion: 距离编码可作为扩散几何的有效低秩替代，提出的三边测量方法为理解距离编码和谱编码之间的关系提供了理论框架，两种编码方法都能显著提升分子图学习性能。
...
TL;DR: 提出基于无人机群的物联网覆盖空洞实时检测与恢复框架，通过巡逻无人机检测空洞，无人机基站恢复连接，采用Delaunay三角剖分组织无人机群，实现高效低成本覆盖修复。


<details>
  <summary>Details</summary>
Motivation: 传统覆盖空洞检测方法（如最小化路测）成本高、耗时长、依赖过时数据，不适合实时应用。物联网设备在传统地面网络受限或不可用时需要无缝连接保障。

Method: 1) 派遣巡逻无人机检测地面基站状态不确定区域的覆盖空洞；2) 检测到空洞后，通过卫星或附近基站部署无人机基站恢复连接；3) 基于Delaunay三角剖分组织无人机群实现可扩展部署；4) 基于多智能体系统理论设计防碰撞机制确保无人机安全协调运动。

Result: 仿真结果表明，该框架在覆盖空洞检测和按需连接恢复方面均实现高效率，同时显著降低运营成本和时间。

Conclusion: 提出的无人机辅助框架能够有效解决物联网网络覆盖空洞的实时检测与恢复问题，通过无人机群协同工作实现高效、低成本、可扩展的覆盖修复方案。
...
TL;DR: 自持续可重构智能表面（ssRIS）通过两种采集-反射方案（ES和TS）实现无线覆盖增强，TS在室内环境表现更稳定，ES在恶劣户外环境更具可行性。


<details>
  <summary>Details</summary>
Motivation: 传统中继或基站难以部署的区域需要无需布线供电的自持续可重构智能表面来增强无线覆盖，但需要评估其部署可行性。

Method: 分析两种采集-反射方案：元件分割（ES）和时间分割（TS），研究元件需求如何随发射功率、数据速率需求和中断约束等关键系统参数变化，考虑LOS和NLOS信道条件。

Result: TS方案具有更好的信道硬化增益，元件需求在不同中断余量下保持稳定，适合室内部署；但元件需求随采集难度和数据速率呈指数增长。ES方案随采集难度仅线性增长，在恶劣户外场景更具可行性。

Conclusion: TS在良性环境中表现优异，优先考虑可靠性；ES在需要操作鲁棒性的苛刻条件下更可取，为不同部署场景提供了明确的方案选择指导。
...
TL;DR: 提出一种信息论隐私机制设计，在无法直接访问私有数据X的情况下，通过多级点式泄漏约束最大化有用数据Y的披露信息


<details>
  <summary>Details</summary>
Motivation: 现有隐私机制设计通常对所有数据点使用相同的泄漏水平，但实际应用中不同数据点应有不同的隐私保护要求，有些甚至需要完全隐私保护

Method: 引入多级点式泄漏度量，允许不同U实现有不同的泄漏预算；当泄漏足够小时，利用信息几何方法近似互信息，转化为可求解的二次优化问题

Result: 当泄漏矩阵可逆时，该优化问题有闭式解；特别地，只需考虑二元U即可达到最优效用，基于矩阵最大奇异值和奇异向量设计简单低复杂度隐私机制

Conclusion: 提出的多级点式泄漏框架更灵活地平衡隐私与效用，在满足不同隐私约束的同时最大化信息披露，且实现复杂度低
...
<details>
  <summary>Details</summary>
Motivation: 信号恢复是成像恢复、无线通信和信号处理等领域的重要问题。论文旨在解决从测量中恢复未知但具有结构信号的根本挑战，特别是利用信号的结构先验知识（如稀疏性）来改进恢复模型。

Method: 提出了三种约束优化模型，每种模型利用不同形式的结构先验来正则化解空间。这些模型通过理论分析验证了其有效性，并建立了样本复杂度与失配误差之间的权衡关系。

Result: 理论分析表明，这些模型对噪声具有鲁棒性，同时对调谐参数保持稳定性（这对实际应用至关重要）。在温和条件下，建立了样本复杂度与失配误差之间的权衡关系。

Conclusion: 该工作为这些模型的实际应用提供了理论基础，特别是在测量不完善和模型不确定性不可避免的场景中。这些模型在保持参数稳定性的同时表现出噪声鲁棒性，为实际信号恢复问题提供了可靠的解决方案。
...
TL;DR: 提出交叉链接可旋转天线结构，通过协调旋转多个天线降低硬件成本和控制复杂度，提升系统性能


<details>
  <summary>Details</summary>
Motivation: 传统可旋转天线系统的硬件成本和控制复杂度与天线数量成正比，需要更经济高效的解决方案

Method: 建立天线单元级和天线面板级旋转系统模型，提出交替优化算法联合优化接收波束成形和旋转角度，使用遗传算法处理离散旋转角度选择

Result: 通过精心设计行列分区方案，CL-RA架构性能接近灵活天线定向方案；CL天线单元级方案比面板级方案性能提升25%，比传统固定方向天线提升128%

Conclusion: 交叉链接可旋转天线结构提供了一种成本效益高的解决方案，能显著提升系统性能，在单用户和多用户场景下都表现出色
...
TL;DR: 该论文提出使用ℓ⁴-范数最大化方法学习复数域稀疏变换，评估DFT在毫米波大规模MIMO系统中的最优性，并提出改进的稀疏变换方法。


<details>
  <summary>Details</summary>
Motivation: 毫米波频段波传播的高方向性导致用户设备与基站之间只有少量显著传输路径，这种稀疏性在波束空间域中体现。传统上使用DFT作为稀疏变换，但尚不清楚DFT是否是有限维天线阵列的最优稀疏变换。

Method: 将Zhai等人的ℓ⁴-范数最大化完全字典学习框架扩展到复数域，提出两种适合的学习算法，用于评估DFT最优性并学习改进的稀疏变换。

Result: 通过理论分析和仿真评估DFT对信道向量稀疏化的最优性，并为真实世界和合成生成的信道向量学习改进的稀疏变换。

Conclusion: ℓ⁴-范数最大化方法为学习复数域稀疏变换提供了理论基础，提出的算法能够评估DFT最优性并学习更优的稀疏变换，有助于降低毫米波大规模MIMO系统的基带复杂度和功耗。
...
TL;DR: 本文针对具有有界Hessian矩阵的二阶可微函数，建立了Jensen不等式的严格改进，通过积分余项的泰勒展开和Gruss型不等式获得显式误差项，并应用于香农熵和瑞利衰落信道容量估计。


<details>
  <summary>Details</summary>
Motivation: Jensen不等式是凸分析中的基本结果，但经典方差界存在精度限制。本文旨在建立更精确的改进不等式，特别是针对二阶可微且有界Hessian的函数，以填补经典方差界与高精度估计之间的差距。

Method: 使用带积分余项的泰勒展开来推导Jensen不等式的改进形式，结合Gruss型不等式获得显式误差项，这些误差项能够包含偏度和峰度信息。

Result: 建立了Jensen不等式的严格改进形式，获得了由Gruss型不等式控制的显式误差项，成功应用于改进连续分布香农熵和瑞利衰落信道容量的现有估计。

Conclusion: 本文为具有有界Hessian的二阶可微函数提供了Jensen不等式的精确改进，理论工具具有实际应用价值，在信息论和通信理论中展示了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有标准化文档中对波束赋形码本技术的描述往往晦涩难懂，缺乏对核心设计原则、符号变量物理意义以及参数信息传递机制的清晰解释。这给研究人员和工程师理解实际系统中的码本技术造成了障碍，阻碍了学术界与工业界的有效合作。

Method: 论文采用系统性研究方法：1）介绍MIMO系统中波束赋形的背景和5G系统中基于码本的波束赋形信令流程；2）建立3GPP标准中常规码本和端口选择码本的基础知识；3）对3GPP Release 15-18的码本演进进行严格技术分析，包括阐明码本设计核心原则、解释公式中符号变量的物理意义（以表格形式总结）、提供直观可视化说明；4）通过数学建模、性能基准测试、反馈比较和场景适用性分析等方法。

Result: 论文为研究人员和工程师提供了对实际系统中波束赋形码本的统一理解，填补了标准化文档中的教学空白。通过清晰的解释、表格总结和可视化说明，使复杂的码本技术变得易于理解，促进了学术界与工业界的合作。

Conclusion: 该工作既是一个信息丰富的教程，也是未来研究的指导，有助于推动无线通信技术的发展。论文还指出了未来研究方向和其他波束赋形场景，为持续的研究和开发工作提供了方向。
...
TL;DR: 研究有限块长约束下上行双功能ISAC多址接入的通信-感知权衡，揭示码本几何结构中用户间互相关对性能的根本约束。


<details>
  <summary>Details</summary>
Motivation: 传统渐近分析无法反映短包低延迟传输的实际限制，需要研究有限块长约束下双功能ISAC系统的通信-感知权衡关系。

Method: 通过无偏信道状态感知估计器建立感知误差的几何分解，推导有限块长体制下的可达性和逆界，并基于3GPP标准提供参数感知实例。

Result: 感知误差由信噪比和信息码本相关结构共同决定，用户间互相关从根本上约束ISAC性能；数值结果验证了块长、天线维度和感知需求的影响。

Conclusion: 有限块长约束下双功能ISAC系统的通信-感知权衡关系被明确刻画，码本几何结构中的互相关是性能限制的关键因素，为实际系统设计提供理论指导。
...
TL;DR: 本文提出了子图对齐问题，旨在从大图中识别小图模式的顶点集合和对应关系，并建立了信息论界限和分析方法。


<details>
  <summary>Details</summary>
Motivation: 现有图对齐算法主要针对两个图共享相同顶点集的情况，但在实际应用中（如计算机视觉、社交网络分析、生物信息学），经常需要在大图中定位小图模式。现有方法无法直接处理这种场景，因为它们不是为识别小图模式在大图中的具体顶点子集而设计的。

Method: 提出了子图对齐问题，并引入了Erdos-Renyi子图对模型来形式化该问题，同时定义了适当的恢复标准。建立了信息论界限，并提出了一些新颖的分析方法。

Result: 建立了子图对齐问题的几乎紧的信息论结果，当小图模式是大图的诱导子图且需要同时恢复顶点集和对应关系时，该问题简化为NP完全的子图同构问题。

Conclusion: 子图对齐问题填补了现有图对齐研究的空白，为在大图中定位小图模式提供了理论框架和分析工具，具有重要的实际应用价值。
