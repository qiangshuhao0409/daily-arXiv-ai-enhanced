...
Motivation: 大规模分布式训练对网络基础设施提出高要求，现有解决方案（如ECMP）在AI/ML工作负载下表现不佳，且现有分组喷洒方案存在缓冲区膨胀和尾延迟问题。

Method: PRIME采用伪随机轮询分组喷洒方法，利用拥塞信号动态调整负载，避免网络热点。

Result: PRIME在置换流量和网络退化场景下分别提升性能15%和27%。

Conclusion: PRIME通过动态负载均衡有效解决了现有方案的性能问题，适用于大规模分布式训练场景。
...
<details>
  <summary>Details</summary>
Motivation: 5G超密集网络中的带内干扰会显著降低SINR，影响协议操作和网络性能。

Method: 利用CNN处理gNB物理层的I/Q样本，实现快速干扰检测。

Result: 在真实环境中测试超过700万个NR UL时隙，干扰检测准确率超过91%，耗时低于650微秒。

Conclusion: InterfO-RAN是首个基于GPU加速的O-RAN dApp，能有效维护密集部署中的网络性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着低延迟服务需求的增长，现有研究忽略了分组化对排队延迟的影响，尤其是以秒为单位的平均排队延迟。

Method: 建立了分组化与平均排队延迟的数学关系，通过数值方法确定最优分组大小，并分析了网络参数的影响。

Result: 确定了最优平均排队延迟及其对应的分组大小，并通过仿真研究了分组化对延迟抖动的影响。

Conclusion: 研究为随机接入网络中的分组化策略提供了新视角，并重新评估了无连接与基于连接方案的权衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有仿真工具在模拟LoRaWAN网络时计算开销大、耗时长，需要一种轻量级且准确的替代方案。

Method: FAST-LoRa通过分析模型和高效矩阵运算简化计算，避免复杂的包级仿真。

Result: FAST-LoRa在复杂场景中仍能准确估计关键网络指标，计算时间减少三个数量级。

Conclusion: FAST-LoRa是一种高效的近似工具，适用于稳定流量模式和上行通信场景。
...
<details>
  <summary>Details</summary>
Motivation: 解决无线设备在不同网络条件下数据传输的及时性和能效问题。

Method: 采用唤醒无线电机制和定制MAC协议，支持不同通信类别的数据流量。

Result: 系统分析显示，该方法能减少30%的能耗，同时保持两种通信模式的可靠性。

Conclusion: 双模通信框架在能效和可靠性方面优于传统方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注平均指标值，而忽略了多跳网络中信息年龄的完整分布，特别是内容的信息性。

Method: 分析了单跳和多跳网络中VAoI的稳态分布，推导了随机、均匀和阈值调度策略下的闭式表达式。

Result: 得出了VAoI的稳态分布和平均值的闭式解，并确定了阈值调度中的最优阈值。

Conclusion: 数值验证了理论结果，为高效通信网络设计提供了依据。
...
<details>
  <summary>Details</summary>
Motivation: 由于计算资源和任务的物理属性多样化，复杂系统中任务与资源的匹配变得更具挑战性，需要一种新的机制来实现价值驱动的任务完成。

Method: 提出了一种超图辅助的TTR-matching框架，通过定义任务特定的信任资源超图和任务超图，设计超图匹配算法，实现任务与资源的精准匹配。

Result: 实验结果表明，TTR-matching框架在识别任务特定的可信合作者和最大化任务完成价值方面优于其他算法。

Conclusion: 该框架为复杂系统中的任务资源匹配提供了一种高效且价值驱动的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: KGC的事后解释性缺乏形式化和一致的评估，阻碍了研究的可重复性和跨研究比较。

Method: 提出一个通用框架，通过多目标优化统一现有的事后解释性算法，并改进评估协议。

Result: 框架统一了现有方法，并通过改进的评估协议（如MRR和Hits@k）验证了其有效性。
...
Method: 方法包括分析四个典型领域的工作流程，并提出一个包含数据准备级别和处理阶段的两维框架。
...
Method: 引入包含917项挑战的听觉图灵测试，评估GPT-4和Whisper等先进音频模型的表现。

Result: AI模型平均准确率仅6.9%，远低于人类的52%，暴露了其在复杂听觉场景处理中的不足。
...
Result: 过滤不一致预测显著提高了人类和LLM预测的准确性，但用户通常不遵循一致性。

Conclusion: 论证一致性对提升预测准确性有实际价值，需在群体预测中引入机制过滤不一致意见。
...
<details>
  <summary>Details</summary>
Motivation: 量化查询答案中各事实的贡献，为ontology-mediated查询提供更高效的解释方法。

Method: 利用数据库结果分析WSMS的计算复杂性，探索不同查询和ontology语言的组合复杂度。

Result: 在可重写为一阶逻辑的查询中WSMS计算高效，而在支持可达性查询的ontology语言中计算困难。DL-Lite方言中某些受限查询仍可高效计算。

Conclusion: WSMS在特定查询和ontology语言中高效，为实际应用提供了理论支持。
...
<details>
  <summary>Details</summary>
Motivation: 处理复杂实例时，现有方法在ReLU变量选择上效率不足，需要更优的解决方案。

Method: 提出SAS和GS评分方法，结合Hybrid MILP（先使用α,β-CROWN，再调用部分MILP）。

Result: SAS减少了约6倍的二进制变量，未解决实例数量降低40%，运行时间合理。

Conclusion: SAS和Hybrid MILP方法在效率和准确性上优于现有方法，适用于大规模网络。
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI科学家系统是否能够达到人类水平并推动科学发现的革命。
...
Result: 提出3级自主AI框架，强调人类监督的必要性。

Conclusion: AI需在人类监督下发展，避免完全自主带来的风险。
...
<details>
  <summary>Details</summary>
Motivation: 由于缺乏系统性的基准测试来评估数据科学代理的效能和局限性，作者希望通过实际应用观察来填补这一空白。

Method: 通过商业应用的使用情况，设计了反映真实用户交互的基准测试，评估了三种LLM模型（Claude-4.0-Sonnet、Gemini-2.5-Flash、OpenAI-o4-Mini）在三种方法（零样本上下文工程、多步上下文工程、SmolAgent）下的表现。

Result: 研究发现不同模型和方法之间存在明显的性能差异，并揭示了提示问题和温度参数对结果的影响。
...
Method: 提出QTAO提示框架，结合推理与行动；构建CRFD-25数据集；开发零-shot对话推荐系统。

Result: LLM4Rail能提供准确的个性化服务，如餐饮推荐。

Conclusion: LLM4Rail展示了LLM在铁路服务中的潜力，通过QTAO框架和CRFD-25数据集实现高效个性化服务。
...
Method: 采用双代理架构，结合推理和批判阶段，利用开源权重的LLM。

Result: 代理能够可靠地将自然语言查询转换为可执行的SQL语句。

Conclusion: 提出的双代理架构有效提升了查询生成的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动化指令合成方法在多样性和难度上存在不足，需要减少对人工标注的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在因果推理任务中的潜力，解决传统模型在数据扰动下的过拟合和性能随机性问题。

Method: 使用Corr2Cause基准测试，结合Tree-of-Thoughts和Chain-of-Thoughts方法，设计模块化上下文管道。

Result: 推理优先架构在因果发现任务中表现显著优于传统方法，模块化管道性能提升近三倍。

Conclusion: 高级推理模型在因果发现中具有巨大潜力，但需结合结构化上下文框架以最大化其能力，为跨领域任务提供通用蓝图。
...
Method: 提出因果解释方法，证明其形式化性质，引入对比性因果解释和完整因果解释（与原始图像置信度一致）。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于上下文学习（ICL）的LLM代理在复杂推理和工具使用任务中表现优异，但其性能高度依赖于演示示例的选择，缺乏通用且理论支持的选择标准。

Method: 提出DICE框架，通过因果视角将演示知识分解为可转移和不可转移部分，并提出逐步选择标准，确保代理性能提升。
...
Result: 实验结果表明，该方法实现了资源高效的信任评估，支持大规模系统的多跳协作。

Conclusion: 提出的方法有效平衡了评估开销与信任准确性，提升了分布式协作的资源利用率。
...
<details>
  <summary>Details</summary>
Motivation: 传统代理辅助记忆回忆方法受限于内存模块大小，无法获取完整记忆。受记忆理论启发，论文提出通过策略引导激活用户相关记忆。

Method: 设计了5W Recall Map分类记忆查询场景，定义15种策略模式，结合蒙特卡洛树搜索算法优化策略选择和响应生成，并微调开源大语言模型开发MemoCue代理。

Result: 在三个数据集上，MemoCue在回忆启发性能上优于基于LLM的方法17.74%，人类评估也验证了其优势。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法将个性化问题推荐建模为马尔可夫决策过程并使用强化学习解决，但存在探索效率低的问题，难以在训练中为每个学生找到最佳问题。

Method: 提出了Ranking Alignment Recommendation (RAR)，将协作思想融入探索机制，以在有限的训练轮次内实现更高效的探索。

Result: 实验表明，RAR显著提升了推荐性能，且该框架可应用于任何基于强化学习的问题推荐系统。

Conclusion: RAR通过改进探索机制，有效提升了个性化问题推荐的性能，具有普适性。
...
Method: 基于Infocom的交互式小说游戏设计TextQuests基准，禁止使用外部工具，专注于内在长期上下文推理能力。

Result: TextQuests为评估AI代理在复杂、探索性环境中的自主问题解决能力提供了有效工具。

Conclusion: TextQuests推动了AI代理在长期自主推理能力上的发展，并已公开发布。
...
Result: Seed-Prover在IMO问题中达到78.1%的证明率，显著优于先前方法，并在IMO 2025中完全解决了5/6的问题。

Conclusion: Seed-Prover和Seed-Geometry展示了形式化验证与长链推理在自动数学推理中的有效性。
...
Result: 在可验证推理任务中显著优于现有数据集（如s1k、OpenMathReasoning），在非可验证任务中优于人类或标准自指导提示。

Conclusion: CoT-Self-Instruct是一种高效的数据生成方法，显著提升LLMs性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI代理多为单一任务设计，缺乏通用性和扩展性，且受限于自回归LLM的固有缺陷。人类通过模拟行动结果进行推理，启发研究者设计更通用的AI代理。

Method: 提出SimuRA架构，基于世界模型进行规划模拟，利用LLM的概念丰富潜在空间实现灵活规划。

Result: 在航班搜索任务中，成功率从0%提升至32.2%，基于世界模型的规划比自回归规划优势高达124%。

Conclusion: SimuRA展示了世界模型模拟作为推理范式的优势，为训练通用AI代理提供了可能性，并发布了基于SimuRA的网页浏览代理供测试。
...
<details>
  <summary>Details</summary>
Motivation: 传统观点认为混合光-射频系统主要通过链路切换实现鲁棒性，但作者认为光学链路可以作为射频上行链路的功能性支持，提升可靠性。

Method: 提出了一种名为O-RF-CBF的新架构，通过光学下行链路反馈指导射频上行链路的自适应编码策略。

Result: 数值结果显示，O-RF-CBF在提升上行链路吞吐量方面显著优于传统O-RF系统。
...
TL;DR: 提出一种新型LPWAN收发器，通过CSS前导和CPFSK-DSSS组合实现高接收灵敏度和低复杂度。


<details>
  <summary>Details</summary>
Motivation: 传统LPWAN收发器在覆盖范围和速率间存在折衷，需改进。

Method: 采用CSS前导简化同步，CPFSK-DSSS组合增强覆盖，双峰检测和非相干解调提升灵敏度。

Result: 仿真和实测表明，该收发器在灵敏度和复杂度上优于传统方案。

Conclusion: 新型收发器满足LPWAN需求，兼具高性能和低成本。
...
<details>
  <summary>Details</summary>
Motivation: 研究压缩感知中随机线性测量的数量与平均信息维度的关系，以确定恢复ψ*-混合过程的基本极限。

Method: 引入相关维度率作为下界，分析随机线性测量数量与平均信息维度的关系。

Result: 若测量数小于平均信息维度，则几乎无损恢复不可能；反之则可能。

Conclusion: 平均信息维度是压缩感知中的精确阈值，相关维度率是其下界。
...
<details>
  <summary>Details</summary>
Motivation: 研究通用整数编码（UCI）的最小扩展因子，以优化压缩性能，填补现有理论空白。

Method: 提出ν码和Δδ码两类UCI，并通过理论分析证明其最小扩展因子的最优性。

Result: ν码的最小扩展因子为2.0386，Δδ码与ν码均为当前最优UCI。最优UCI的最小扩展因子范围为2 ≤ C_C* ≤ 2.0386。

Conclusion: ν码和Δδ码是目前最优的UCI，最小扩展因子范围进一步缩小至2到2.0386之间，且最优UCI的下界为2。
...
<details>
  <summary>Details</summary>
Motivation: 研究ZC序列的线性微频跳特性，探索其DFT和IDFT的直观计算方法。

Method: 利用微频跳模式计算ZC序列的DFT和IDFT，并引入广义二次高斯和计算累积和。

Result: 发现DFT可转换为带频移和相位偏移的线性微频跳符号，从而简化计算。

Conclusion: ZC序列的DFT可通过累积频点计算，类似于普通微频跳符号，为相关领域提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 研究多功能的MIMO ISAC系统如何在通信、感知和抗窃听威胁之间实现平衡。

Method: 推导了ESR和CRB，利用中心极限定理计算SNR和CRB的PDF。

Result: 表征了CRB-保密速率区域的边界，揭示了通信与感知之间的性能权衡。

Conclusion: 在随机ISAC网络中，实现了安全性和隐私性的平衡，同时优化了通信与感知性能。
...
<details>
  <summary>Details</summary>
Motivation: 通过优化IRS元素的位置和波束成形，提升ISAC系统的通信速率和感知精度，并扩大其覆盖范围。

Method: 1. 单用户场景下分析可移动元素的功能并设计联合波束成形和元素位置优化方案；2. 扩展到多用户场景并提出元素位置优化方案。

Result: 仿真结果表明，IRS元素的移动可提高通信速率和感知精度，并扩大ISAC的覆盖范围。

Conclusion: IRS的可移动元素对提升ISAC系统性能具有显著效果。
...
<details>
  <summary>Details</summary>
Motivation: 卫星通信在未来无线网络中日益重要，但面临链路预算有限和语义通信计算资源消耗增加的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 提升同时无线信息和能量传输（SWIPT）的效率，避免牺牲时频资源，同时支持能量接收器（ERs）和信息接收器（IRs）两组用户。

Method: 采用保护性部分零强迫预编码技术，优化AP选择、功率控制和BDRIS散射矩阵设计，提出启发式搜索、逐次凸逼近和深度强化学习算法。

Result: BDRIS在组或全连接架构下显著优于传统对角RIS，能量收集增益最高可达七倍。

Conclusion: BDRIS在SWIPT系统中具有显著优势，优化算法有效解决了复杂问题。
