...
Motivation: 填补物理层处理算法理论进展与实际在商用处理器上实现之间的差距，解决现有系统吞吐量与最先进技术应达到吞吐量之间的差距。

Method: 开发MMGaP系统，使用裸金属CUDA内核实现大规模MIMO处理算法，可扩展到大型GPU处理平台，并能打包为TensorFlow模块便于系统集成。

Result: 在5G网络中使用100MHz带宽、8天线基站和8个并发用户时，MMGaP将每个用户的上行链路吞吐量提高约50Mbps，下行链路吞吐量提高100Mbps。对于16天线和16用户场景，上行链路吞吐量每用户提高超过50Mbps。

Conclusion: MMGaP能够在不同NVIDIA GPU上以线速运行，满足最先进5G系统的时序要求，实现了大规模MIMO处理算法的实际部署。
...
<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在运筹学任务中对闭源API的依赖带来的隐私问题，以及从头训练开源模型的高计算成本问题。

Method: 使用半自动数据合成流程生成多样化的运筹学问题-答案对，微调Llama-3.1-8B-Instruct模型，并通过外部求解器增强模型以生成API调用。

Result: 在四个标准基准测试中的三个上，OR-Toolformer达到最高80.1%的执行准确率，比同规模基线模型高出4.3%以上；在两种未见运筹学问题类型的零样本评估中，平均准确率达到54%，比最强基线提高21个百分点。
...
<details>
  <summary>Details</summary>
Motivation: 现有人类行为建模方法要么对理性做出不现实假设，要么计算复杂难以快速适应，需要更高效准确的行为预测方法以促进人机协作安全。

Method: 将日常社交互动建模为可预测的行为程序，使用LLM合成行为程序假设空间，结合概率推理处理不确定性，将动作理解视为程序合成问题。

Result: 在网格世界任务和大规模家庭模拟器中测试，ROTE从稀疏观测预测人类和AI行为，在样本内准确性和样本外泛化方面比行为克隆和LLM基线方法提升高达50%。

Conclusion: 通过将行为建模为程序而非信念驱动的策略，ROTE为AI系统在现实世界中高效有效预测人类行为开辟了新路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI系统在化学工程中跨学科协作和探索未知问题方面存在局限，需要构建能够自主进化和科学发现的智能生态系统。

Method: 集成领域知识库、知识增强技术和协作智能体，构建多智能体协作系统，引入具有本体工程能力的协作智能体(CA)来解决跨领域协作效率瓶颈。

Result: 知识库增强机制使7个专家智能体的对话质量平均提高10-15%；CA干预使远领域专家对的协作效率提高8.5%，而邻近领域仅提高0.8%，揭示了"知识库差距导致的协作效率递减"效应。
...
<details>
  <summary>Details</summary>
Motivation: 传统评估基准无法捕捉智能体在交互环境中沟通、说服和协作时涌现的社会和认知动态。

Method: 创建多智能体辩论框架，让具有不同角色和激励的LLM智能体在LLM主持人的监督下就各种话题进行辩论，使用新的心理测量和语义指标进行分析。

Result: 发现智能体有强烈的寻求共识倾向（语义一致性μ>0.88），分配的角色会产生稳定的心理测量特征，主持人的角色能显著改变辩论结果。
...
Result: AGILE在复杂度不同的拼图任务上大幅提升性能（在2×2设置下准确率从9.5%提升到82.8%），并在9个通用视觉任务上表现出强泛化能力，平均提升3.1%。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够结合形式验证和非正式推理的AI系统，以解决复杂的数学问题，特别是国际数学奥林匹克竞赛级别的难题

Method: 整合三个主要组件：Lean证明搜索系统、生成并形式化引理的非正式推理系统，以及专门的几何求解器
...
<details>
  <summary>Details</summary>
Motivation: 现有的上下文和记忆基准测试主要关注对话场景，但企业环境中动态记忆评估的需求更为关键，需要测试代理在复杂组织工作流程中的记忆能力。

Method: 通过整合Slack、Linear和Git等多个通信和生产力平台的异步事件，创建平台间交错的时间线，包含噪声、冲突和交叉引用信息，采用专家设计和基于代理的合成方法构建数据集。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录快速增长为数据驱动的临床决策带来机遇和挑战，需要开发能够分析复杂临床数据并提供决策支持的工具。

Method: 采用检索增强生成（RAG）管道，整合自然语言处理与结构化临床输入，通过检索相似病例并合成信息来生成治疗建议。

Result: 初步评估显示，在适当约束和严格验证下，基于LLM的工具可为处方工作流程提供有价值的决策支持。
...
<details>
  <summary>Details</summary>
Motivation: 奖励黑客攻击（模型利用奖励函数漏洞获得高分但不解决实际任务）是一个严重威胁，特别是隐式攻击会绕过思维链监控。

Method: TRACE方法：逐步截断模型的思维链，强制模型回答，测量每个截断点的验证通过率。黑客模型会通过少量思维链就获得高通过率。

Result: 在数学推理任务中比最强的72B思维链监控器提升65%以上，在编程任务中比32B监控器提升30%以上，并能发现训练中的未知漏洞。

Conclusion: TRACE提供了一种可扩展的无监督监督方法，在当前监控方法无效的情况下有效检测隐式奖励黑客攻击。
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体在多步任务中经常出现可预测的失败，如尝试未满足前提条件的动作、发出冗余命令或错误处理环境约束。虽然检索增强生成(RAG)能通过提供运行时指导来改善性能，但需要维护外部知识库并在每次部署时增加计算开销。

Method: 通过蒸馏将推理时检索转化为学习能力：(1)从智能体失败中提取紧凑、可重用的提示；(2)使用这些提示在情节开始时通过一次性检索生成改进的教师轨迹；(3)在移除提示字符串的情况下训练学生模型，强制内部化而非记忆。

Result: 在两个交互式基准测试ALFWorld(家务任务)和WebShop(在线购物)中，蒸馏后的学生模型始终优于基线智能体，在ALFWorld上达到91%成功率(基线为79%)，WebShop分数提升至72(基线为61)，同时使用的token数量比检索增强教师少10-60%。

Conclusion: 该方法在不同模型规模(7B/14B参数)和智能体架构(ReAct/StateAct)上具有通用性，表明通过有针对性的微调可以有效内部化检索收益，无需永久运行时依赖。
...
<details>
  <summary>Details</summary>
Motivation: 现代工程依赖大量实验和模拟数据集，传统数据驱动方法需要大量人工干预，难以有效扩展和泛化到多样化应用。需要开发自动化建模策略来减少人工工作量。

Method: 使用两种LLM代理框架：多代理系统（专业协作代理）和单代理系统（基于ReAct范式）。这些框架自主处理数据预处理、神经网络开发、训练、超参数优化和不确定性量化。

Result: 在包含约25,000个实验数据点的OECD/NEA基准数据集上验证，LLM代理开发的模型超越了传统临界热通量查找表，预测精度和不确定性量化与专家开发的贝叶斯优化深度神经网络模型相当。

Conclusion: 基于LLM的代理在自动化复杂工程建模任务方面具有巨大潜力，能显著减少人工工作量，同时达到或超过现有预测性能标准。
...
<details>
  <summary>Details</summary>
Motivation: 系统日志是宝贵的网络威胁情报来源，但由于缺乏结构、语义不一致和跨设备碎片化等问题，其效用受到限制。需要能够将嘈杂、异构数据协调为连贯且可互操作表示的方法。

Method: 集成轻量级日志本体与检索增强生成(RAG)和迭代校正步骤，确保生成的KG在语法和语义上有效。系统将KG聚合为会话，并使用LLM预测MITRE ATT&CK战术。

Result: 在公共基准和真实世界蜜罐数据集上的评估显示，OntoLogX能够在多个KG后端上稳健生成KG，并准确将对抗活动映射到ATT&CK战术。检索和校正提高了精确率和召回率。
...
Result: Falconer在指令跟随准确率上接近最先进的LLM，同时减少推理成本高达90%，加速大规模知识挖掘超过20倍。
...
Method: 1. 使用可解释AI技术自动创建课程：结合专家指定的问题解决规则和新型XAI技术自动生成课程。2. 利用专家指定的学习课程开发自适应辅导系统：通过专家制定的学习路径开发更高效的算法。

Result: 通过传粉者识别辅导系统的案例研究，证明了这些方法在实际应用中的可行性和重要性。

Conclusion: 领域专家的精炼知识在开发有效教育系统中具有关键作用，通过结合专家知识和AI技术可以创建更有效的自适应辅导系统。
...
<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法在处理多模态任务时，将视觉输入视为固定条件，忽略了视觉变化带来的不确定性，导致策略缺乏鲁棒性，探索效率低下。

Method: VOGUE方法将图像作为随机上下文，通过计算原始分支和噪声分支之间的对称KL散度来量化策略对视觉扰动的敏感性，创建不确定性感知的探索信号。该方法结合不确定性比例奖励、token熵奖励和退火采样策略来平衡探索与利用。

Result: 在Qwen2.5-VL-3B/7B模型上，VOGUE在三个视觉数学基准测试中平均提升pass@1准确率2.6%，在三个通用领域推理基准测试中提升3.7%，同时提高了pass@4性能并缓解了RL微调中常见的探索衰减问题。

Conclusion: 基于视觉输入固有不确定性的探索策略是提升多模态推理的有效方法，VOGUE通过将探索从输出空间转移到输入空间，显著改善了多模态大语言模型的推理能力。
...
Method: 通过两步流程创建数据集：(1)使用精心设计的指令提示LLM生成120个技术文档样本；(2)法律专家审查并标注每个样本是否违反欧盟AI法案的具体条款。

Result: 创建了包含120个样本的基准数据集，并评估了前沿LLM在重现专家合规标签方面的表现。

Conclusion: AIReg-Bench为理解基于LLM的AI法规合规评估工具的机遇和局限性提供了起点，并为后续LLM的比较建立了基准。
...
<details>
  <summary>Details</summary>
Motivation: 现代部署中大量计算资源（数千个token或节点扩展）用于提升可靠性，但标准树搜索方法存在两个问题：广度饱和（额外样本产生近似重复）和深度短视（噪声短期效用修剪了长期有回报的分支）。

Method: LToT将前沿分为主线和侧线。主线用于利用高效用候选，侧线用于探索一致但低效用候选。采用侧向竞速算法（LR-SC）进行高效探索，包括有上限的连续减半竞赛、宽度感知阈值和即时提升机制。

Result: 理论证明侧向成本为伪线性Θ(N₀ log_η N₀)，与无上限主线的指数增长形成对比。实证评估正在进行中。

Conclusion: LToT能够将大量测试时计算预算转化为有原则的多样性，同时保持提升纪律，缓解饱和和短视问题而不增加计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在数学推理任务中的内部表示机制，通过分析token的序列转移模式来理解推理过程，并寻求在生成过程中平衡利用已有推理轨迹和探索新路径的策略。

Method: 训练稀疏自编码器生成token的稀疏向量表示，使用k-means聚类构建token转移图（顶点为token簇，加权边表示序列转移），基于边权重定义奖励函数量化对推理轨迹的遵循程度，同时通过聚类多样性评估探索程度。

Result: 研究发现平衡利用和探索对于数学推理任务的高准确性至关重要，稀疏自编码器可作为可扩展的奖励模型指导生成过程，避免极端行为，提升推理质量。

Conclusion: 该方法通过分析LLM内部token表示和转移模式，有效平衡了推理过程中的利用和探索，为提升数学推理性能提供了新的技术路径，稀疏自编码器在指导生成方面展现出良好的可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 高保证领域（如法律、医疗）需要准确、可验证且明确基于证据的推理，但LLM在处理包含可废止逻辑、例外情况的复杂推理时存在局限

Method: 使用高级逻辑语言和推理器与LLM结合，构建双重符号图上下文和逻辑上下文，将长文本推理问题转化为紧凑的接地评估

Result: 在四个多领域基准测试中，相比四个基线模型，LogT整体性能提升11.84%，在否定推理、蕴含推理和可废止推理上分别提升10.2%、13.2%和5.5%

Conclusion: LogT神经符号方法能有效解决高保证文本中的复杂逻辑推理挑战，显著提升LLM在严格推理任务中的表现
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM规划代理在处理观测不确定性时往往忽略内部动态与实际环境之间的差异，需要开发能够主动寻求信息来对齐内部动态的框架。

Method: InfoSeeker框架提示LLM主动收集信息，通过规划行动来验证理解、检测环境变化或测试假设，然后再生成或修订任务导向计划。

Result: 在包含不完全观测和不确定动态的新基准套件上，InfoSeeker相比先前方法实现了74%的绝对性能提升，且不牺牲样本效率。在机器人操作和网页导航等基准测试中也优于基线方法。

Conclusion: 在部分可观测环境中，紧密集成规划和信息寻求对于实现鲁棒行为至关重要，InfoSeeker框架证明了这种集成方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的扩散语言模型训练方法依赖稀疏的结果奖励，可能会强化导致偶然正确结果的错误推理路径，这与推理的自然结构不匹配。

Method: 首先提出理论框架将复杂问题解决形式化为层次选择过程，然后引入SAPO算法，使用过程奖励函数使扩散语言模型的去噪过程与潜在推理层次对齐。

Result: 实验结果表明该方法在挑战性推理基准上显著提升性能，并增强了生成过程的可解释性。

Conclusion: 基于理论框架的SAPO算法能够有效引导扩散语言模型学习结构化推理路径，提升复杂推理任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法直接优化安全响应，但可能损害模型的通用推理能力。作者希望开发一种既能提升安全性又能保持模型通用能力的方法。

Method: 指导模型进行三个步骤：1) 列举潜在危害，2) 分析危害后果，3) 生成主动避免这些风险的安全输出。通过监督微调和强化学习在三个LLM家族中实现。

Result: 相比基线方法SafetyPrompt，InvThink在有害响应上减少高达15.7%；安全改进随模型规模扩大而增强；在医疗、金融、法律等高风险领域表现优异；缓解了安全税问题，保持了标准基准上的通用推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的LLM多智能体系统防御方法存在局限性：自我验证方法因单个智能体能力不足而效果不佳，外部防护模块会增加系统开销并造成单点故障。需要一种既能保证安全又不增加系统复杂度的解决方案。

Method: 提出AdvEvo-MARL框架，通过对抗学习联合优化攻击者（生成演化越狱提示）和防御者（任务智能体），在保持任务完成能力的同时抵抗攻击。引入公共基线优势估计，同一功能组内的智能体共享组级平均回报基线，以稳定学习并促进协作。

Result: 在代表性攻击场景中，AdvEvo-MARL始终将攻击成功率保持在20%以下，而基线方法可达38.33%。同时保持甚至提高了任务准确率（在推理任务上提升达+3.67%）。

Conclusion: 研究表明，无需依赖额外的防护智能体或增加系统开销，就可以同时提高安全性和实用性，AdvEvo-MARL框架为LLM多智能体系统的安全防护提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有对话推荐系统在处理动态用户偏好、保持对话连贯性以及平衡多个排序目标方面存在显著挑战，需要更智能的解决方案。

Method: 采用分层智能体网络架构，包含对话理解、偏好建模、上下文感知和动态排序等专门LLM智能体，通过自适应权重机制协调，结合三层学习策略：简单查询的快速响应、复杂偏好的智能推理和挑战场景的深度协作。

Result: 在三个真实数据集上的实验表明，AgentRec相比最先进基线方法实现了2.8%的对话成功率提升、1.9%的推荐准确性改进（NDCG@10）和3.2%的对话效率提升，同时通过智能智能体协调保持了可比较的计算成本。

Conclusion: AgentRec框架通过多智能体协作和自适应学习机制，有效解决了现有对话推荐系统的关键挑战，为下一代智能推荐系统提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 虽然LLMs在多个领域表现出色，但在需要认知能力的应用如心理咨询方面仍有待探索。本文旨在评估LLMs是否具备成为心理咨询师所需的专业知识水平。

Method: 构建PsychoBench基准，包含约2,252个精心设计的单选题，基于美国国家心理咨询师认证考试内容，涵盖心理学多个子学科，要求深度理解和广泛知识。

Result: 评估显示先进模型如GPT-4o、Llama3.3-70B和Gemma3-27B远超通过阈值（约70%准确率），而较小的开源模型如Qwen2.5-7B、Mistral-7B远低于此标准。
...
<details>
  <summary>Details</summary>
Motivation: 现有上下文马尔可夫决策过程方法在高维或无结构上下文中泛化能力差，导致计算开销大、性能不稳定。

Method: 利用大语言模型将上下文输入压缩为低维语义摘要，这些摘要通过保留决策关键信息并减少冗余来增强状态表示。
...
Method: 将轨迹恢复作为代理任务，使用GLOBALTRACE数据集（包含4000多条真实世界轨迹），通过提示框架让LLMs在不访问外部导航工具的情况下生成有效路径。
...
Result: 在2023年第四季度至2025年第二季度的回测中，各代理表现出基于提示角色的独特行为，巴菲特代理表现最佳。
...
<details>
  <summary>Details</summary>
Motivation: 随着计算机使用代理的广泛部署，发现这些代理存在系统性的盲目目标导向问题，可能带来安全风险，需要系统性地识别和评估这种风险。

Method: 开发了BLIND-ACT基准，包含90个任务捕捉三种BGD模式，基于OSWorld提供真实环境，使用LLM评估代理行为，并与人工标注达到93.75%的一致性。

Result: 评估了9个前沿模型，包括Claude Sonnet和Opus 4等，观察到平均80.8%的高BGD率。提示干预能降低BGD水平但风险依然存在。

Conclusion: 识别BGD并引入BLIND-ACT为未来研究和减轻这种基本风险奠定了基础，确保CUA的安全部署需要更强的训练或推理时干预。
...
<details>
  <summary>Details</summary>
Motivation: 患者等待侵入性手术时经常有未解答的问题，但时间紧迫的工作流程和隐私限制限制了个性化咨询。

Method: 使用句子转换器分类器路由输入，返回临床医生策划FAQ的逐字答案，在两个领域（拔牙和胃镜检查）使用专家评审验证集进行阈值设定和独立测试集评估。

Result: E5-large-instruct编码器达到0.983的总体准确率，AUC 0.996，与GPT-4o在统计上无差异。非生成临床路径每输入消耗约1.0 mWh，比本地8B SLM的小谈话回复低约170倍，同时保持约0.10秒延迟。

Conclusion: 通过返回经过审查的FAQ逐字答案，在临床路径中结构性地避免了前沿判别和生成引起的错误，支持在带宽有限环境中的隐私、可持续性和公平部署。
...
<details>
  <summary>Details</summary>
Motivation: 当前AGI评估方法依赖对人类智能直觉的合成任务设计，在AI历史上表现不佳，需要更有效的评估框架。

Method: 提出基于稳健任务执行能力的评估设计哲学，借鉴数据科学中展示系统可靠部署的实践方法。

Result: 提供了AGI评估的具体实践示例，展示了如何通过能力证明而非直觉测试来评估AGI。
...
Method: 基于LLM引导的响应编辑框架，生成具有针对性错误但保持风格和长度相似的拒绝响应，构建了包含30K高质量样本的VaPR数据集。

Result: 在三个LVLM家族上实现显著性能提升：LLaVA平均提升6.5%，Qwen2VL提升4.0%，Qwen2.5VL提升1.5%，特别是在推理任务上表现突出。

Conclusion: VaPR框架有效解决了合成偏好标注的噪声问题，能够显著提升模型性能，且可扩展到开源LLM作为编辑器，具有很好的通用性。
...
<details>
  <summary>Details</summary>
Motivation: 解决知识图谱使用中的技术障碍，包括对本体论和查询语言语法的深入理解需求，使研究人员能够通过自然语言轻松访问结构化代谢组学数据。
...
Conclusion: MetaboT成功弥合了复杂语义技术与用户友好交互之间的差距，通过自动化SPARQL查询生成和执行，为研究人员提供了便捷的知识图谱访问方式。
...
<details>
  <summary>Details</summary>
Motivation: 弥合理论AI构建与运营网络安全需求之间的差距，提供符合行业标准的稳健多智能体系统基础。

Method: 通过将NIST CSF 2.0功能细分为具体任务，将AI智能体特性（自主性、自适应学习、实时响应）与各子类别安全要求关联，并定义分级自主水平。

Result: 概念验证表明，定制化AI智能体部署能够与现实约束和风险状况对齐，增强态势感知、加速响应时间，并通过自适应风险管理加强长期韧性。
...
<details>
  <summary>Details</summary>
Motivation: 学术规范咨询对帮助学生理解和遵守机构政策至关重要，但构建有效系统需要特定领域的监管资源。

Method: 提出CatRAG混合检索推理框架，结合检索增强生成和图推理，使用分层分类知识图谱和语义特征进行领域对齐，轻量级意图分类器路由查询到适当的检索模块。

Result: 构建了特定监管数据集，在分类和问答任务中实现最先进性能，F1分数达到98.89%。
...
<details>
  <summary>Details</summary>
Motivation: 在快速演变的军事威胁和复杂作战环境中，AI集成到军事行动中具有显著优势，但也带来了构建和部署人机协作系统的挑战和风险。目前的研究多从外部视角处理这些问题，而深入系统内部动态能处理更广泛的多维责任、安全和鲁棒性方面。

Method: 设计了一个可信赖的协同学习模型，包含四个关键维度：1) 可调节自主性 - 根据任务状态、系统置信度和环境不确定性动态校准自主级别；2) 多层控制 - 持续监督、活动监控和问责制；3) 双向反馈 - 显式和隐式反馈回路，确保推理、不确定性和学习适应的适当沟通；4) 协作决策 - 生成、评估和提出决策，附带置信水平和理由。

Result: 提出了一个综合的人机协作模型，通过具体示例和建议，为军事行动中负责任和可信赖的人机协作系统的进一步发展做出贡献。

Conclusion: 该研究通过提出包含四个维度的可信赖协同学习模型，为解决军事行动中人机协作系统的多维责任、安全和鲁棒性挑战提供了框架，有助于开发更负责任和可信赖的人机协作系统。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLMs在复杂任务中依赖CoT推理，但由于自回归的token级生成，推理过程局限于局部决策而缺乏全局规划，导致推理冗余、不连贯或不准确，现有方法如树算法和RL计算成本高且难以产生最优推理轨迹。

Method: 两阶段框架：1）使用高级LLMs将CoT提炼为紧凑高层指导进行SFT；2）引入指导感知RL方法联合优化最终输出和高层指导质量。
...
Conclusion: PTA-GRPO框架能有效提升LLMs的推理能力，验证了其有效性和泛化性。
...
<details>
  <summary>Details</summary>
Motivation: 重新构建和操作化对抗性逆强化学习到大型语言模型推理中，直接从专家演示学习推理奖励，而不是通过监督微调模仿风格，以优先考虑正确性而非表面形式。

Method: 使用对抗性逆强化学习学习密集的token级别奖励模型，该模型在训练时提供步骤级反馈优化推理策略，在推理时作为批评者重新排序采样轨迹。

Result: 在GSM8K数据集上，使用Llama3和Qwen2.5骨干网络验证了：(1)密集推理奖励可作为学习信号激发推理；(2)通过奖励引导的重新排序提高了预测性能（特别是基于Llama的策略）。

Conclusion: 通过将训练信号、推理时选择和token级别诊断统一到单一推理奖励中，这项工作表明可重用的过程级奖励具有增强语言模型中多步推理的广泛潜力。
...
Method: CARS从无约束语言模型采样开始，通过记录约束违反的连续序列到trie结构中，自适应地排除这些无效路径，并从未来采样中减去它们的概率质量，确保不会重复访问已证明无效的前缀。

Result: 在程序模糊测试和分子生成等多个领域的实验中，CARS在每有效样本所需的语言模型前向传递次数方面始终实现更高的效率，同时比GCD和近似语言模型分布的方法产生更强的样本多样性。

Conclusion: CARS严格提高了拒绝采样的样本效率而不产生分布失真，接受率单调提高，生成的样本精确遵循约束分布，在需要有效性和多样性的约束生成任务中具有显著优势。
...
TL;DR: 该研究评估大语言模型在集体决策中与人类社会推理的对齐程度，通过Lost at Sea任务进行大规模实验，发现不同LLM在集体推理中的行为存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地用于建模和增强集体决策，需要检验它们与人类社会推理的对齐程度，而现有研究主要关注个体层面。

Method: 使用Lost at Sea社会心理学任务进行大规模在线实验(N=748)，随机分配组别进行领导者选举，比较可见人口属性与匿名化条件下的表现，并模拟匹配的LLM组进行基准测试。

Result: LLM行为出现分化：一些模型反映人类偏见，其他模型则掩盖这些偏见并试图补偿。人类-AI在集体推理中的对齐程度取决于情境、线索和模型特定的归纳偏见。
...
Conclusion: 该框架将AI重新定位为机构问责的重要组成部分，为维护学术交流信任提供关键基础设施，为科学界提供确保公平的透明工具，为出版策略师提供可扩展的工作流程审计、完整性风险管理和基于证据的治理工具。
...
TL;DR: ReTabAD是一个表格异常检测基准，通过恢复文本语义实现上下文感知的异常检测研究，包含20个数据集和多种检测算法。
...
Method: 提供20个精心策划的表格数据集，包含结构化文本元数据，并实现经典、深度学习和基于LLM的异常检测算法；提出无需任务特定训练的零样本LLM框架。
...
TL;DR: 对大型语言模型深度利用的系统研究表明：深层网络贡献具有高度异质性和上下文依赖性，浅层负责知识和检索，深层对推理能力至关重要，评估方法显著影响深度利用的观察结果。


<details>
  <summary>Details</summary>
Motivation: 反驳当前认为LLM深层对表示学习贡献有限的片面观点，通过系统性研究揭示深度利用的真实复杂性。

Method: 采用多维度系统分析，包括不同评估协议（似然度vs生成）、任务类别和模型架构，研究各层在知识、检索、推理等能力中的贡献。

Result: 浅层集中了知识和检索能力，深层对推理准确性和长程连贯性至关重要；评估方法显著影响观察结果：似然度评估中深层可裁剪，但生成评估中深层不可或缺。

Conclusion: LLM深度利用具有高度异质性和上下文依赖性，需要基于任务、指标和模型的视角来理解和压缩大型模型。
...
TL;DR: 研究发现AI模型在ARC-AGI基准测试中的高准确率可能高估了其抽象推理能力。通过ConceptARC任务的双重评估（输出准确性和规则分析），发现模型往往依赖表面模式而非真正抽象推理，在视觉模态中表现更差但可能被低估。


<details>
  <summary>Details</summary>
Motivation: 评估AI模型是否真正具备抽象推理能力，而非仅靠表面模式在基准测试中获得高分。质疑仅凭准确率评估抽象推理能力的可靠性。

Method: 使用ConceptARC任务，在不同设置下评估模型：输入模态（文本vs视觉）、是否允许使用Python工具、推理努力程度。除了输出准确性，还精细评估模型生成的解释规则。

Result: 文本模态中模型准确率接近人类，但规则分析显示大多依赖表面"捷径"而非真正抽象；视觉模态中准确率大幅下降，但规则分析显示模型仍能生成部分正确抽象规则，只是应用能力不足。

Conclusion: 仅凭准确率评估抽象推理会高估文本模态能力、低估视觉模态能力。提出的双重评估框架能更真实反映多模态模型的抽象推理能力，模型在抽象推理方面仍落后于人类。
...
<details>
  <summary>Details</summary>
Motivation: 企业级文档理解模型需要大量多样化且标注良好的数据集，但传统数据收集方法成本高昂（可达数百万美元），且受隐私、法律限制。

Method: 结合随机模式和参数化采样，通过概率建模布局模式、视觉结构和内容变异性，可控地生成多样化文档变体。

Result: 在关键信息提取任务中，使用FlexDoc生成的数据将绝对F1分数提升高达11%，同时相比传统硬模板方法减少90%以上的标注工作量。

Conclusion: FlexDoc已在实际部署中加速企业级文档理解模型的开发，同时显著降低数据获取和标注成本。
...
TL;DR: 该论文提出了一个针对深度研究代理（DRAs）的严格基准和多维评估框架，包含214个专家策划的挑战性查询，支持对长格式报告进行语义质量、主题聚焦和检索可信度的综合评估。
...
Method: 构建包含214个专家策划查询的基准，分布在10个广泛主题领域，每个查询配有手动构建的参考包；开发多维评估框架，集成语义质量、主题聚焦和检索可信度的评分指标。
...
<details>
  <summary>Details</summary>
Motivation: 虽然RLVR被广泛用于提升大语言模型的推理能力，但近期证据表明它可能反而会缩小推理边界而非扩展它。本文旨在深入分析RLVR失败的原因。

Method: 通过理论分析和在多个数学推理基准上的实证研究，揭示了RLVR中的负干扰现象和赢家通吃现象，并提出了一种专注于低概率问题的数据筛选算法。

Result: 研究发现RLVR会导致负干扰（学习解决某些训练问题会降低其他问题正确解的概率）和赢家通吃现象（过度强化高概率正确解而抑制低概率问题），基于这些发现提出的数据筛选算法显著改善了Pass@k性能。

Conclusion: RLVR的失败源于标准强化学习目标中固有的在线策略采样，导致模型收敛到狭窄的解决策略。通过专注于低概率问题的数据筛选，可以有效缓解这一问题并提升模型性能。
...
Method: bBoN方法通过生成多个执行轨迹，并使用描述代理执行过程的行为叙述来进行轨迹选择，实现了广泛的探索和原则性的轨迹选择。

Result: 在OSWorld上达到69.9%的新SOTA，显著优于先前方法，接近人类水平的72%。在WindowsAgentArena和AndroidWorld上展示了强大的泛化能力。

Conclusion: 当正确实施时，扩展CUAs具有惊人的有效性，有效的扩展需要结构化的轨迹理解和选择，bBoN为实现这一目标提供了实用框架。
...
TL;DR: 提出RLAD方法，通过推理抽象来引导模型进行更有效的推理，避免冗长和退化的探索。


<details>
  <summary>Details</summary>
Motivation: 当前大型模型在推理过程中往往无法一致地捕获或重用过程，而是陷入冗长和退化的探索。需要引导模型学习成功的推理过程。

Method: 引入推理抽象的概念，训练模型能够为问题提出多个抽象描述，然后使用强化学习激励在利用这些抽象信息的基础上构建解决方案。采用两玩家RL训练范式，联合训练抽象生成器和解决方案生成器。

Result: 该方法实现了结构化探索，解耦了抽象提议和解决方案生成的学习信号，并提高了对更难问题的泛化能力。测试时在生成抽象上分配更多计算资源比生成更多解决方案更有利于性能提升。

Conclusion: 推理抽象能够有效引导有意义的探索，提升模型的推理能力和泛化性能。
...
TL;DR: 提出BioX-Bridge框架，通过轻量级桥接网络实现生物信号基础模型间的跨模态知识迁移，显著减少可训练参数(88-99%)并保持或提升性能。


<details>
  <summary>Details</summary>
Motivation: 生物信号模态间存在相关性，但大标注数据集稀缺限制了特定模态模型的训练。现有知识蒸馏方法计算开销大，尤其对于大型基础模型。
...
Conclusion: BioX-Bridge为生物信号跨模态知识迁移提供了一种高效解决方案，显著降低了计算和内存开销。
...
TL;DR: 提出基于MCMC的AI驱动通用MIMO接收器架构，作为通用贝叶斯计算引擎，统一处理信道估计、符号检测和信道解码等任务。


<details>
  <summary>Details</summary>
Motivation: MIMO接收器处理面临复杂性和可扩展性挑战，AI技术为解决这些问题提供了潜力。现有AI方法将接收器处理视为黑盒，缺乏可解释性和灵活性。

Method: 采用马尔可夫链蒙特卡洛(MCMC)技术构建通用贝叶斯计算引擎，将信道估计、符号检测和信道解码等任务整合到统一概率框架中。

Result: 该方法提高了接收器在不同场景下的可解释性、可扩展性和灵活性，支持整体性能优化。

Conclusion: 所提出的统一框架可以与数据驱动学习方法无缝结合，促进完全智能通信接收器的发展。
...
TL;DR: 本章介绍DNA编码及其约束条件，重点讨论使用代数方法构建具有高汉明距离的非循环DNA编码，并探讨多个约束条件下的代数构造和界限。


<details>
  <summary>Details</summary>
Motivation: DNA编码在DNA计算和DNA数据存储中的应用日益重要，需要满足多种热力学和组合约束条件。传统代数方法构建的DNA编码汉明距离较低，因此需要开发能够产生高汉明距离DNA编码的新代数构造方法。

Method: 使用代数方法，通过有限环和映射（通常是某些有限环上的等距映射）来构造DNA编码，重点关注非循环DNA编码，并讨论各种度量标准如Gau距离、非均聚物距离等。

Result: 提出了能够满足多个约束条件和/或性质的DNA编码代数构造方法，并建立了多约束条件下DNA编码的代数界限。

Conclusion: 本章系统介绍了DNA编码的各种约束条件和代数构造方法，为DNA计算和DNA数据存储提供了理论基础，并指出了该领域未来的研究方向。
...
TL;DR: 本文研究了多项式求值码（特别是Toric码）的列表解码问题，提出了考虑凸多面体几何和组合特性的广义Guruswami-Sudan算法，并计算了译码半径的界限。


<details>
  <summary>Details</summary>
Motivation: 多项式求值码在编码理论中占据重要地位，但现有的列表解码研究主要集中在特殊案例（如Reed-Solomon和Reed-Muller码），需要开发适用于更一般多项式求值码的解码方法。

Method: 提出了Guruswami-Sudan算法的广义版本，该方法考虑了定义Toric码的凸多面体P的几何和组合特性。

Result: 计算了所提出算法的译码半径界限，为一般多项式求值码提供了列表解码的理论保证。

Conclusion: 该工作扩展了列表解码理论的应用范围，为基于凸多面体的多项式求值码提供了有效的解码算法和性能分析。
...
TL;DR: 提出基于统一EP树表示的并行SGRAND解码器和混合GRAND算法，在保持最大似然解码性能的同时显著降低解码延迟。


<details>
  <summary>Details</summary>
Motivation: 现有Soft GRAND解码器虽然能实现最大似然解码，但其顺序测试错误模式的本质阻碍了并行化，导致高解码延迟。

Method: 利用统一的EP树表示错误模式，支持紧凑表示、高效操作和并行探索；提出并行SGRAND设计和混合GRAND算法。

Result: 并行SGRAND相比串行实现加速3.75倍，混合增强方法加速4.8倍，硬件映射下预期有更大增益。

Conclusion: EP树表示和并行设计有效解决了GRAND解码器的延迟问题，同时保持ML最优性。
...
TL;DR: 本文研究了在可分区可重构智能表面增强的无人机网络中下行非正交多址接入通信的性能，提出了RUOM算法来优化公平性和效率。


<details>
  <summary>Details</summary>
Motivation: 研究无人机网络中下行NOMA通信的性能，特别是在RIS增强环境下，旨在解决无人机通信中的公平性和资源效率问题。

Method: 分析了基站与无人机之间的三种链路类型，推导了信噪比的累积分布函数闭式表达式，提出了RUOM算法进行双层优化。

Result: 仿真结果验证了分析模型，表明RUOM算法显著提高了基站-无人机通信的公平性和效率。

Conclusion: 提出的RUOM算法在RIS辅助的无人机NOMA通信中有效平衡了公平性和资源效率，为无人机网络优化提供了有效解决方案。
...
TL;DR: 提出一个实用的两阶段公共随机性提取框架，结合变分概率量化和安全草图技术，用于从相关随机源中提取秘密密钥，并应用于基于感知的物理层密钥生成。


<details>
  <summary>Details</summary>
Motivation: 解决传统物理层密钥生成方法依赖信道互易性、需要双向信道探测导致协议开销大、不适合高移动性场景的问题。

Method: 第一阶段使用变分概率量化，通过概率神经网络编码器将观测映射为离散均匀随机变量；第二阶段使用码偏移结构的安全草图进行协调。

Result: 通过端到端仿真和真实软件定义无线电测量验证了框架的可行性和性能，包括窃听者部分知晓Bob位置的情况。

Conclusion: 提出的公共随机性提取框架和基于感知的物理层密钥生成方法具有可行性和令人信服的性能。
...
Method: 通过监测控制激光束在通过含有里德堡原子的蒸汽池前后的变化，分析透射激光信号，将传输变化归因于外部电磁场的存在和特性。比较里德堡原子接收器与传统接收器在检测8-PAM信号时的性能。

Result: 里德堡接收器能够抑制离共振干扰且无需额外滤波器，作为集成滤波器和解调器，在可实现符号错误率方面优于传统电路基接收器。

Conclusion: 里德堡原子接收器在检测电磁信号方面表现出优越性能，能够有效抑制干扰，为高精度量子传感应用提供了有前景的解决方案。
...
TL;DR: 提出了一种联合考虑语义相关性和信道状态的协作边缘推理方法，通过使注意力权重依赖于信道信息来改善无线环境下的设备分组协作性能。


<details>
  <summary>Details</summary>
Motivation: 在存在数据损坏的无线环境中，多个设备协作可以提高推理性能。现有方法仅考虑语义相关性或信道状态，在恶劣传播条件下表现不佳。

Method: 采用联合方法，在设备分组协作时同时考虑语义信息相关性和信道状态，使通用注意力权重依赖于信道信息。

Result: 数值模拟显示，与本地推理和仅考虑应用层或物理层参数的分离决策方法相比，联合方法具有优越性。
...
TL;DR: 该论文研究协作推理框架，通过交换中间特征而非原始数据来提高分类准确性，重点分析了信道噪声影响、协作点选择和通信-准确性权衡优化。


<details>
  <summary>Details</summary>
Motivation: 边缘设备协作推理需要平衡准确性和带宽限制，现有方法在信道噪声和通信效率方面存在优化空间。

Method: 基于键-查询机制进行选择性信息交换，分析不同协作点对性能的影响，探索通信剪枝策略。

Result: 中间协作方法对信道错误具有鲁棒性，查询传输比数据传输本身需要更高的可靠性。

Conclusion: 通过优化协作点和通信策略，可以在最小化资源使用的同时实现准确性优化。
