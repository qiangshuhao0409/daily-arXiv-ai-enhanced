...
TL;DR: 本文研究索尼Xperia 1 III和IV智能手机在泰国b28/n28频段上的4x4 MIMO实际性能，包括不同信号条件下的可靠性测试和最优条件下的吞吐量测试，并与2Rx天线设备进行对比。


<details>
  <summary>Details</summary>
Motivation: 目前市场上绝大多数商用5G手机都缺少低频段(<1 GHz)的4x4 MIMO支持，而泰国True-H和dtac网络已在b28/n28频段部署4T4R，因此需要评估支持该功能的设备在实际使用中的性能表现。

Method: 使用索尼Xperia 1 III和IV智能手机，通过固件修改配置2Rx端口作为对比，在不同信号条件下进行可靠性测试，在最优条件下进行最大吞吐量测试，并与仅支持2Rx天线的其他厂商设备进行性能比较。

Result: 实验结果显示，在泰国b28/n28频段上，支持4x4 MIMO的索尼Xperia设备能够实现更好的性能表现，具体数据需等待完整论文发布。

Conclusion: 低频段4x4 MIMO在实际部署中具有性能优势，但受限于网络运营商部署和设备制造商支持，目前应用范围有限。
...
TL;DR: 5G SA在泰国已覆盖76%人口，相比5G NSA和LTE网络，5G SA在UHD视频直播中能提供更稳定的低延迟传输，成功交付超过95%的视频片段。
...
Method: 使用MPEG-DASH协议在不同移动网络技术下进行UHD视频实时直播测试，评估丢包率、MAC吞吐量和延迟，涵盖静止、城市移动、高速移动和理想信号条件等多种场景。

Result: 5G SA在所有测试场景中都能成功交付超过95%的UHD视频片段，5G NSA表现因LTE网络状况而异，LTE网络则有超过20%的视频片段无法按时交付。
...
TL;DR: 展示了本地部署的轻量级大语言模型（LLaMA-3 8b Q-4b）结合检索增强生成（RAG）技术，用于自动化5G网络管理，重点强调隐私保护。


<details>
  <summary>Details</summary>
Motivation: 解决5G网络管理中敏感数据传输的安全问题，通过本地部署消除对外部API的依赖，确保数据隐私安全。同时让非专业用户也能轻松创建和配置私有网络。

Method: 使用轻量级LLaMA-3模型在本地或边缘设备运行，结合RAG技术从综合数据库中检索相关信息，基于自然语言输入生成精确的网络配置。

Result: 该方法提高了网络配置生成的准确性，简化了私有网络的创建和配置过程，使非专业用户也能轻松使用。

Conclusion: 结合本地LLM和RAG技术能够提供安全、高效且适应性强的5G网络解决方案，为隐私保护和多用户场景下的5G网络应用铺平道路。
...
TL;DR: 提出基于数字孪生(DT)的安全接入策略，通过虚拟复制物理SAGIN环境来评估窃听风险，利用演化博弈模型平衡保密容量和排队延迟，提升物联网网络安全性能。


<details>
  <summary>Details</summary>
Motivation: 空间-空中-地面一体化网络(SAGIN)中的物联网网络面临日益严重的窃听攻击威胁，需要解决数据保密性的安全问题。

Method: 在DT框架中创建物理SAGIN环境的虚拟副本，持续评估动态窃听风险并量化保密容量；使用演化博弈模型平衡DT更新的保密容量与排队延迟；开发分布式算法获取均衡接入策略。

Result: 仿真结果表明，所提出的基于DT的方法显著提高了SAGIN物联网网络的安全性，有效平衡系统负载，防止过载发生，并降低排队延迟。

Conclusion: DT驱动的安全接入策略能够全面改善网络整体性能，在安全性和效率之间取得良好平衡。
...
TL;DR: LatencyScope是一个用于精确计算5G RAN中单向延迟的数学框架，能够识别系统级瓶颈并包含配置优化器来寻找满足延迟可靠性目标的配置。


<details>
  <summary>Details</summary>
Motivation: 5G网络需要满足超可靠低延迟通信(URLLC)需求，但现有工具无法准确建模复杂的延迟来源和其随机特性，需要开发能够精确预测延迟并优化系统配置的框架。

Method: 构建了在RAN各层建模延迟源的数学框架，识别无线接口、调度策略和硬件/软件约束等瓶颈，并开发配置优化器在数百亿配置中搜索最优设置。

Result: 在两个开源5G RAN测试平台(srsRAN和OAI)上验证，LatencyScope能够紧密匹配经验延迟分布，显著优于现有分析模型和仿真器，并能找到满足URLLC目标的系统配置。

Conclusion: LatencyScope为网络运营商提供了有效识别最佳系统配置的工具，能够满足5G URLLC应用的严格要求，在延迟预测和系统优化方面表现出色。
...
TL;DR: 本文综述了内容中心网络在5G物联网环境中的安全挑战和解决方案，重点分析了内容认证、数据完整性、隐私保护等关键问题，并指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着物联网和5G技术的融合，内容中心网络成为传统IP架构的有前景替代方案，但其在5G物联网环境中的部署面临严重的安全挑战，需要系统性的安全解决方案。

Method: 采用文献综述和分类分析的方法，对现有内容中心架构在物联网-5G场景中的安全解决方案进行系统回顾和分类。

Result: 识别了当前安全解决方案的趋势和局限性，强调了内容认证、数据完整性、隐私保护和抗攻击能力等核心安全问题的现状。
...
<details>
  <summary>Details</summary>
Motivation: 现有多接入点协调协议依赖静态规则，无法适应动态网络条件（如干扰水平和拓扑变化），需要更智能的自适应解决方案。
...
TL;DR: 本文研究使用大型语言模型自动重构层次结构以优化双曲嵌入质量，实验证明LLM重构的层次结构能显著提升嵌入质量并提供可解释的重组方案。


<details>
  <summary>Details</summary>
Motivation: 双曲嵌入的质量与输入层次结构密切相关，而现有层次结构往往不符合双曲嵌入的最佳条件（高分支因子和单继承）。需要一种自动方法来重构层次结构以满足这些标准。

Method: 提出基于提示的方法，使用LLMs根据双曲嵌入的理想标准来转换现有层次结构，并在16个不同层次结构上进行实验验证。

Result: 实验表明，LLM重构的层次结构在多个标准嵌入质量指标上都能产生更高质量的双曲嵌入，同时还能提供可解释的重组理由。

Conclusion: LLMs能够有效自动重构层次结构以优化双曲嵌入质量，为知识工程师提供可解释的重组方案，显著提升嵌入性能。
...
TL;DR: AssurAI是一个针对韩语多模态生成AI安全评估的质量控制数据集，包含11,480个文本、图像、视频和音频实例，覆盖35种AI风险因素，专门针对韩国社会文化背景设计。
...
Method: 定义了35种AI风险因素的分类法，通过多学科专家组构建；采用两阶段构建（专家引导播种和众包扩展）、三重独立标注和迭代专家红队循环的严格质量控制流程。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法严重依赖手动预定义的操作符，限制了泛化能力和可扩展性，需要开发能够自动生成可重用操作符的框架。

Method: 采用三阶段操作符提取过程：基于案例的初始操作符生成、操作符聚类与初步抽象、深度提取抽象执行操作符，并引入操作符记忆机制增强工作流搜索。

Result: 在通用和具身基准测试中，A²Flow平均性能提升2.4%和19.3%，资源使用减少37%，优于现有最先进基线方法。

Conclusion: A²Flow通过完全自动化的操作符提取和工作流生成，显著提升了智能体工作流设计的泛化性、可扩展性和效率。
...
TL;DR: 提出了Reasoning With a Star数据集和基准测试方法，用于评估LLM在太阳物理领域的科学推理能力，发现基于系统工程原则的多智能体工作流在需要演绎推理的问题上表现优于直接提示。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在太阳物理科学推理中需要整合物理假设、保持单位一致性和提供清晰科学格式的挑战。

Method: 构建了来自NASA和UCAR Living With a Star暑期学校问题集的数据集，包含问题上下文、推理步骤、答案类型等；开发了程序化评分器；比较了单次提示和四种多智能体模式。

Result: 通过系统工程原则分解工作流在需要演绎推理的问题上表现优于直接提示，而在纯归纳回忆问题上表现相似。

Conclusion: 多智能体协作方法能够有效提升LLM在复杂科学推理任务中的表现，特别是在需要演绎推理的场景下。
...
<details>
  <summary>Details</summary>
Motivation: 将数字孪生技术从工业领域引入医疗健康，旨在实现从被动治疗向预测性、预防性和个性化医疗的转变，通过患者特异性模拟提高医疗决策的准确性和效率。

Method: 整合医学影像、生物传感器和计算模型，创建动态的数据驱动虚拟患者模型，通过实时数据流持续更新，支持双向交互，应用于心脏、肿瘤和药理学等领域。

Result: 代表性应用包括预测心律失常治疗结果的心脏数字孪生、追踪肿瘤进展和优化放疗的肿瘤学数字孪生，以及加速药物发现的药理学数字孪生。

Conclusion: 数字孪生技术有望彻底改变医疗模式，但需要解决互操作性、数据隐私和模型保真度等挑战，并通过可解释AI、联邦学习和统一监管框架等方案推进，未来多器官数字孪生、基因组学整合和伦理治理将是关键发展方向。
...
TL;DR: Paraconsistent-Lib是一个开源的Python库，用于构建PAL2v算法，支持12种经典格区域分析、PAN输出和决策输出，简化了复杂算法的实现。


<details>
  <summary>Details</summary>
Motivation: 为了提供一个易于使用的通用PAL2v计算库，减少推理和决策系统中算法实现的复杂性和错误。

Method: 设计为通用PAL2v标准计算库，支持三种结果类型：12种经典格区域分析、PAN输出和决策输出，可独立或网络形式实现多种PAL2v算法。

Result: 成功开发了Paraconsistent-Lib库，能够简化PAL2v算法的实现，减少代码复杂性和错误，并通过GitHub接收用户反馈进行持续开发。

Conclusion: Paraconsistent-Lib是一个稳定且持续发展的开源库，为PAL2v算法在推理和决策系统中的应用提供了便利工具。
...
TL;DR: 对多模态思维链推理在跨领域泛化能力进行综合分析，发现在科学推理之外的常识推理任务中效果有限


<details>
  <summary>Details</summary>
Motivation: 探索多模态思维链推理方法在不同领域的泛化能力，特别是在需要广泛常识和世界知识的任务中

Method: 采用Zhang等人的两阶段框架，将理由生成与答案推理分离，通过门控融合机制整合视觉特征与T5语言模型

Result: 视觉特征整合显著减少理由生成中的幻觉，但思维链推理效果在不同问题类型间差异很大，常识推理尤其具有挑战性

Conclusion: 为多模态推理系统实施提供实用见解，并确定了跨领域泛化改进的关键方向
...
TL;DR: OpenApps是一个轻量级开源生态系统，用于评估UI智能体在不同应用变体中的可靠性，发现智能体在固定应用中的表现相对稳定，但在不同应用变体间成功率波动可达50%以上。


<details>
  <summary>Details</summary>
Motivation: 当前UI智能体评估局限于固定环境，无法反映智能体在真实部署中遇到的应用设计和内容变化，需要新的评估维度来衡量跨应用变体的可靠性。

Method: 开发OpenApps生态系统，包含6个可配置应用（消息、日历、地图等），使用单个CPU即可运行并生成数千个应用版本，进行了超过10,000次独立评估测试7个领先的多模态智能体。

Result: 发现智能体在固定应用中的可靠性相对稳定，但在不同应用变体间成功率波动显著，如Kimi-VL-3B的平均成功率从63%降至4%；智能体行为（如循环、幻觉操作）也随环境配置大幅变化。

Conclusion: 应用变体是评估UI智能体可靠性的重要新维度，OpenApps为这一维度的系统评估提供了工具，揭示了当前智能体在真实部署中可能面临的可靠性挑战。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型产生错误或过时内容的问题，需要在终身学习设置中高效准确地更新知识，避免昂贵的重新训练。

Method: 在模型表示空间进行干预，学习抗改写和编辑局部化的模块，将每个更新限制在低维子空间以减少跨编辑干扰，推理时通过查询自适应路由器选择合适模块。

Result: 在LLaMA和Qwen模型的知识编辑基准测试中，RILKE可扩展到大规模数据集，展示出高编辑成功率、强改写泛化能力，并以适度内存开销保持通用效用。
...
<details>
  <summary>Details</summary>
Motivation: 现有组合解释方法使用束搜索，但无法提供最优性理论保证，不清楚当前解释与真正最优解的接近程度。

Method: 提出三部分框架：(i)识别影响空间对齐因素的分解方法，(ii)搜索过程中对齐估计的启发式，(iii)首个能在可行时间内计算最优组合解释的算法。

Result: 在计算机视觉和卷积神经网络中，10-40%的束搜索解释在涉及重叠概念时是次优的。提出的束搜索变体在运行时间上匹配或优于先前方法。

Conclusion: 该框架首次实现了保证最优的组合解释计算，为神经元可解释性研究提供了理论基础和实用工具。
...
TL;DR: ENACT是一个评估具身认知的基准测试，通过视觉问答形式测试模型从第一人称交互中进行世界建模的能力，包含正向和逆向世界建模两个任务。


<details>
  <summary>Details</summary>
Motivation: 研究现代视觉语言模型是否表现出具身认知的特征，尽管它们主要是在非具身方式下训练的。具身认知理论认为智能源于感知运动交互而非被动观察。
...
Conclusion: 现代视觉语言模型确实表现出一些具身认知的特征，但与人类相比仍有明显差距，特别是在长期交互和视角泛化方面。
...
TL;DR: Ivy是一个AI教练系统，通过结合符号化的任务-方法-知识(TMK)模型和生成解释层，提供结构化、多步骤的教学解释，解决了LLM生成解释缺乏因果逻辑和结构性的问题。


<details>
  <summary>Details</summary>
Motivation: 在程序性技能学习中，教学解释需要传达步骤背后的因果、目标导向和组合逻辑，而大型语言模型通常生成流畅但浅显的响应，缺乏这种结构。

Method: 将符号化的TMK模型与生成解释层结合，TMK编码因果转换、目标层次和问题分解，在明确的TMK结构边界内指导LLM构建解释。

Result: 与GPT和检索增强GPT基线相比，符号约束在"如何"和"为什么"问题的解释结构质量上持续改进，专家和独立注释在三个推理维度上验证了效果。
...
TL;DR: 提出了ICPO方法，通过利用LLM生成不同响应的概率来反映其对推理过程的自我评估，结合可验证奖励来增强推理能力，解决了现有RLVR方法中粗粒度奖励、奖励噪声和低效探索等问题。
...
TL;DR: L4M框架结合对抗性LLM代理和SMT求解器证明，将自然语言的解释灵活性与符号验证的严谨性相结合，在法律推理中超越现有LLM和Legal AI基准。


<details>
  <summary>Details</summary>
Motivation: 现有LLM系统擅长表层文本分析，但缺乏原则性法理学所需的保证。法律理性表现为实质理性（结果公平性）和形式理性（遵循明确规则），需要结合自然语言灵活性和符号验证严谨性。

Method: 三阶段管道：1) 法规形式化：将法律条款转换为逻辑公式；2) 双方法律事实提取：检察官和辩护方对齐的LLM分别提取事实元组和法规；3) 求解器中心裁决：将双方论证编译为逻辑约束，通过不满足核心触发迭代自我批判，最终由法官LLM生成透明判决。

Result: 在公共基准测试中，系统超越了GPT-4-mini、DeepSeek-V3、Claude 4等先进LLM以及最先进的Legal AI基线，同时提供严谨且可解释的符号证明。

Conclusion: L4M成功将自然语言解释的灵活性与符号验证的严谨性相结合，为法律推理提供了既灵活又可靠的新方法，显著提升了法律AI系统的性能和可解释性。
...
TL;DR: 提出OVOD-Agent框架，将被动类别匹配转变为主动视觉推理和自进化检测，通过Visual-CoT和w-MDP建模视觉上下文转换，结合Bandit模块和自监督奖励模型优化，在COCO和LVIS数据集上显著提升开放词汇目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法虽然在多模态数据集上预训练，但推理仍局限于固定类别名称，造成多模态训练与单模态推理之间的差距。文本空间仍有待充分探索，改进文本表示能显著提升检测性能。

Method: 提出OVOD-Agent框架：1）基于Chain-of-Thought思想构建可解释的Visual-CoT；2）将视觉上下文转换建模为弱马尔可夫决策过程；3）使用Bandit模块在有限监督下生成探索信号；4）结合马尔可夫转移矩阵和Bandit轨迹进行自监督奖励模型优化。

Result: 在COCO和LVIS数据集上的实验表明，OVOD-Agent能在各种OVOD骨干网络上提供一致改进，特别是在稀有类别上表现突出，验证了框架的有效性。

Conclusion: OVOD-Agent通过将被动类别匹配转化为主动视觉推理和自进化检测，有效提升了开放词汇目标检测性能，特别是在处理稀有类别时表现出色，为文本空间的深入探索提供了新思路。
...
TL;DR: 本文提出了Halpern-Pearl因果定义的抽象化版本，使其能应用于更广泛的模型，包括处理复杂逻辑公式和嵌套反事实的情况。


<details>
  <summary>Details</summary>
Motivation: Halpern-Pearl的因果定义局限于因果模型，无法处理包含析取、否定、信念和嵌套反事实的复杂公式。本文旨在抽象化该定义，使其适用于任何定义反事实的模型。

Method: 通过提取Halpern-Pearl因果定义的关键特征，构建抽象化定义框架，使其能应用于各种反事实模型。

Result: 成功开发出能处理复杂逻辑公式（包括析取、否定、信念和嵌套反事实）的抽象因果定义，并扩展到解释概念的抽象定义。

Conclusion: 抽象化方法不仅扩展了因果定义的应用范围，还深化了对因果定义特征的理解，即使在因果模型中也提供了新的洞见。
...
TL;DR: Prune4Web是一种新的网页自动化方法，通过程序化修剪DOM树来大幅减少候选元素数量，解决了传统方法在处理大型DOM结构时的效率和精度问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于LLM的网页代理在处理复杂网页时面临DOM结构过大（10,000-100,000个token）的挑战，传统方法要么通过粗暴截断丢失关键信息，要么使用低效启发式方法，无法在精度和可扩展性之间取得平衡。

Method: 提出DOM树修剪编程范式，让LLM生成可执行的Python评分脚本来动态过滤DOM元素，基于分解的子任务语义线索。该方法将DOM处理从资源密集的LLM读取转移到高效的程序化修剪，实现25-50倍的候选元素减少。

Result: 在低级别定位任务上，Prune4Web将准确率从46.8%大幅提升至88.28%，表现出卓越的实时网页自动化效果。

Conclusion: Prune4Web通过程序化DOM修剪实现了网页自动化的突破性进展，在保持高精度的同时显著提升了处理效率，为复杂网页交互提供了可行的解决方案。
...
Method: 为混合决策引入了新的启发式方法，优化了观察文字方案与计数方法的选择策略
...
TL;DR: ChatDRex是一个基于对话的多智能体系统，通过自然语言访问生物医学知识图谱，整合网络分析和药物重定位预测，使非计算机专家也能进行复杂的生物信息学分析。


<details>
  <summary>Details</summary>
Motivation: 药物重定位比传统药物开发更高效经济，但现有算法工具分散且需要多领域专家协作，数据服务难以集成到工作流中。

Method: 构建基于NeDRex知识图谱的多智能体系统，包括查询路由、数据检索、算法执行、结果可视化等专用智能体，配备推理模块进行幻觉检测。

Result: 系统为无计算机专业背景的医生和研究人员提供了自然语言控制复杂分析的能力，促进了药物重定位的假设生成和机会探索。

Conclusion: ChatDRex实现了生物信息学资源的民主化访问，加速了新疗法的发现，推动了精准医学和转化研究。
...
TL;DR: EWE是首个用于极端天气自动诊断的智能代理框架，通过模拟专家工作流程实现从原始气象数据到多模态可视化的自主分析和解释，旨在解决当前人工诊断的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 极端天气事件对全球社会构成日益严重的风险，但当前专家驱动、劳动密集的诊断范式造成了关键的分析瓶颈，阻碍了科学进展。虽然AI在地球科学预测方面取得了显著进展，但自动诊断推理这一同等重要的挑战仍未被充分探索。

Method: EWE通过知识引导规划、闭环推理和领域定制的气象工具包来模拟专家工作流程，能够从原始气象数据自主生成和解释多模态可视化，实现全面的诊断分析。

Result: 提出了该新兴领域的首个基准测试，包含103个高影响事件的精选数据集和新的逐步评估指标。EWE展示了向自动化科学发现迈进的潜力。

Conclusion: EWE标志着向自动化科学发现迈出的一步，并具有民主化专业知识和智力资源的潜力，特别是对易受极端天气影响的发展中国家而言。
...
Method: 使用多LLM智能体辩论指令安全性，由关键评估器基于逻辑性、风险识别、证据质量和清晰度评分，通过迭代讨论和共识投票机制。
...
TL;DR: 提出了一个层次化的空间认知框架，将空间智能分解为五个渐进复杂层次，并构建了SpatialBench基准来系统评估多模态大语言模型的空间推理能力。
...
Method: 构建层次化空间认知框架（五个认知层次），开发SpatialBench基准（15个任务），引入高层能力导向的统一评估指标。

Result: 实验发现模型在感知层面表现良好，但在符号推理、因果推理和规划方面存在局限；人类进行选择性目标导向抽象，而模型过度关注表面细节。

Conclusion: 建立了首个系统化评估MLLMs层次化空间认知的框架，为未来空间智能系统奠定了基础。
...
TL;DR: 提出悲观验证方法，通过构建多个并行验证来检测数学证明中的错误，显著提升验证性能且计算资源消耗小。
...
Result: 该方法显著提升多个数学验证基准的性能，计算效率甚至超过扩展长链思维，且发现数据集标注错误导致模型假阴性。
...
TL;DR: 语言模型在专业领域无法可靠披露AI身份，存在虚假专家风险。研究发现模型在专业角色中透明度差异显著，从2.8%到73.6%不等，且推理优化会抑制透明度。


<details>
  <summary>Details</summary>
Motivation: 在专业高风险领域，如果语言模型不能可靠披露AI身份，用户无法信任其能力边界，可能导致用户受到伤害。

Method: 采用标准环境设计，对16个开放权重模型（4B-671B参数）进行19,200次试验审计，使用贝叶斯验证和Rogan-Gladen修正确保测量稳健性。

Result: 模型表现出明显的领域特异性不一致：金融顾问角色初始披露率30.8%，而神经外科医生角色仅3.5%。模型身份比参数数量更能预测行为（ΔR²adj = 0.359 vs 0.018）。推理优化会抑制透明度，推理变体比基础模型披露率低达48.4%。

Conclusion: 透明度反映的是训练因素而非模型规模，组织不能假设安全属性会转移到部署环境，需要刻意设计行为和实证验证。
...
<details>
  <summary>Details</summary>
Motivation: 在技术快速发展和全球挑战复杂的时代，负责任前瞻已成为政策制定者应对未来不确定性和塑造未来的重要框架。需要伦理地预测新兴机遇和风险，关注促进主动、可持续和负责任的未来设计。

Method: 建立负责任计算前瞻的基本原则，开发AI驱动的前瞻工具，结合模拟和情景分析，增强政策制定者应对不确定性、评估风险和制定可持续战略的能力。

Result: AI将在负责任、以人为本的前瞻中发挥支持性工具作用，补充而非替代政策制定者的判断，实现对有韧性和伦理健全未来的主动塑造。

Conclusion: 主张将AI深思熟虑地整合到前瞻实践中，赋能政策制定者和社区应对21世纪的重大挑战。
...
TL;DR: LLMs在8拼图任务中表现出状态跟踪和规划能力的显著局限性，即使有反馈和验证器辅助也无法解决任何谜题。
...
Result: 反馈仅对部分模型-提示组合有效，但所有模型在外部验证器辅助下仍无法解决任何谜题，存在状态表示脆弱和启发式规划弱的问题。

Conclusion: 当前LLMs在规划方面存在实质性局限，需要开发维护显式状态和执行结构化搜索的机制。
...
<details>
  <summary>Details</summary>
Motivation: AI/ML模型在解决新问题的同时可能放大人类偏见，需要更丰富的因果模型来指导负责任AI/ML的开发。但不同方法基于不同假设，难以整合。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于轨迹的记忆方法存在简洁性偏差，逐渐丢失重要领域知识，且只记录单模态行为轨迹，无法保留视觉注意力和逻辑推理如何共同促成解决方案，这与人类多模态整合的语义记忆不符。

Method: 引入ViLoMem双流记忆框架，分别编码视觉分心模式和逻辑推理错误，采用生长-精炼原则逐步积累和更新多模态语义知识，保持稳定可泛化策略同时避免灾难性遗忘。

Result: 在六个多模态基准测试中，ViLoMem持续提升pass@1准确率，显著减少重复的视觉和逻辑错误。消融实验证实了双流记忆和显式分心-幻觉分离的必要性。

Conclusion: ViLoMem证明了错误感知多模态记忆对于终身和跨领域智能学习的重要价值，为多模态推理提供了更符合人类认知的记忆机制。
