...
TL;DR: AZIZA是一个面向孟加拉国洪水灾害场景的自适应区域智能路由协议，通过区域转发、信任感知和上下文驱动决策，在基础设施受损时提供弹性通信服务。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国频繁的洪水灾害严重破坏通信系统，导致受灾人群孤立并阻碍应急响应，需要不依赖固定基础设施的智能自适应路由解决方案。

Method: 集成区域转发优化本地传输、信任感知逻辑绕过恶意节点、基于信任指标、剩余能量和历史交付模式的上下文驱动决策。

Result: 使用ONE模拟器配置真实移动轨迹和洪水数据的仿真结果显示，AZIZA在交付可靠性、能效和路由弹性方面显著优于基准方法。

Conclusion: AZIZA作为一个可扩展和可部署的框架，推进了新一代机会主义路由在传统系统经常崩溃的环境中的应用。
...
TL;DR: 本文提出了一种优化低地球轨道卫星星座中星间链路布局的算法，通过最小化网络直径来降低端到端延迟，同时满足物理和操作约束。


<details>
  <summary>Details</summary>
Motivation: 当前LEO星座中的星间链路布局很少同时优化网络直径并满足物理和操作约束，包括最大链路距离、视线要求、每卫星硬件限制以及轨道周期内的链路稳定性。

Method: 将星间链路选择与分配建模为直径最小化问题，采用局部搜索程序迭代优化拓扑结构，替换或增强链路以缩小网络直径。

Result: 所得到的ISL配置满足所有几何和硬件限制，保持多个轨道周期内的链路稳定性，并产生稀疏的直径感知图，具有集中式路由能力潜力。

Conclusion: 仿真表明该算法在不影响ISL稳定性的情况下实现了低最坏情况延迟，并实证测量了跳数与长期链路稳定性之间的权衡，为未来LEO网络部署提供指导。
...
TL;DR: 本文提出了一种图感知时序编码器(GATE)和混合近端策略优化(HPPO)框架，用于解决卫星星座边缘计算中的资源分配和服务迁移问题。


<details>
  <summary>Details</summary>
Motivation: 随着延迟敏感应用的快速发展，需要在卫星星座上部署边缘计算能力以实现真正的全球无缝服务覆盖，同时需要高效分配有限的机载计算和通信资源，并处理卫星轨道动态变化带来的服务迁移需求。

Method: 将问题建模为时空马尔可夫决策过程，提出GATE模型联合建模空间相关性和时序动态性，使用两层图卷积网络提取卫星间和用户间依赖关系，使用时序卷积网络捕获短期演化，最后通过HPPO框架输出离散服务迁移决策和连续资源分配比例。

Result: 通过涉及持续性和间歇性用户的广泛仿真验证了方法的有效性，用户分布基于真实世界人口中心。

Conclusion: 所提出的GATE和HPPO框架能够有效解决卫星边缘计算中的资源分配和服务迁移挑战，为全球无缝服务覆盖提供了可行的解决方案。
...
TL;DR: 提出了一个经过验证的PAT延迟模型，用于量化光学卫星间链路中的重定向延迟，该模型基于多个NASA和ESA任务数据，发现PAT延迟呈现多模态分布，并与初始指向不确定性和光束宽度非线性相关。


<details>
  <summary>Details</summary>
Motivation: 当前的光学卫星间链路调度方法常常忽略或过度简化PAT延迟，导致接触规划效率低下和网络容量高估，需要更准确的延迟模型来改进路由和调度算法。

Method: 基于NASA TBIRD、LLCD、DSOC和ESA月球光学通信终端的任务数据，建立了一个验证过的重定向延迟量化模型，涵盖粗指向、精指向和跟踪切换过程。

Result: 研究发现PAT延迟根据先前的链路几何形状呈现多模态分布，并且与初始指向不确定性和光学光束宽度呈非线性比例关系。

Conclusion: 将PAT延迟模型集成到路由和调度算法中，可以实现更准确的接触规划和更高的光学网络利用率，为未来大规模光学卫星网络的性能评估和算法设计提供了基础。
...
TL;DR: 提出了一种生物启发的集成网络与控制方案，用于管理数千架无人机组成的集群，通过两层分层结构消除跨集群控制，显著降低通信开销。


<details>
  <summary>Details</summary>
Motivation: 无人机集群面临网络管理和编队控制带来的高开销挑战，需要一种高效的方法来管理大规模无人机集群。

Method: 采用两层分层结构，网络集群和编队共享相同分组；设计融合路由消息和控制信息来减少开销；开发混合生物启发控制方法，包括集群内鸽子式领导-跟随算法和集群间椋鸟式避障算法。

Result: 在超过1000个节点的无人机集群仿真平台上，实验结果表明该方案能够实现高度机动的集群编队行进，并显著降低通信开销。

Conclusion: BINC方案通过集成网络和控制功能，有效解决了大规模无人机集群的高开销问题，实现了高效编队管理。
...
TL;DR: 提出了一种分布式微数据中心架构，通过动态工作负载迁移来利用过剩的可再生能源，建立了可行性域模型来指导迁移决策，并设计了可行性感知的编排框架。


<details>
  <summary>Details</summary>
Motivation: AI工作负载的快速增长与间歇性可再生能源主导的能源格局之间存在矛盾，现有集中式数据中心架构在能源比例和地理灵活性方面与可再生能源不匹配。

Method: 建立了形式化的可行性域模型，明确关联检查点大小、广域网带宽和可再生能源窗口持续时间；设计了可行性感知的编排框架，将迁移从启发式方法转变为原则性控制机制。

Result: 跟踪驱动评估表明，这种编排可以同时减少非可再生能源使用并提高性能稳定性，克服了纯能源驱动策略的权衡问题。
...
TL;DR: 本文首次实验实现了多频段无线接入回传测试平台，结合FR2回传和FR1接入链路，验证了端到端WAB操作的可行性，并展示了其在扩展FR2覆盖范围方面的能力。


<details>
  <summary>Details</summary>
Motivation: 无线容量和覆盖需求的增长推动了新无线电架构和高频段的研究，WAB架构通过无线中继实现经济高效的网络密集化。

Method: 使用开源软件和商用现成组件构建多频段WAB测试平台，结合FR2回传链路和FR1接入链路，在车辆和室外到室内场景进行实验验证。

Result: 实验证实WAB有效缓解了FR2链路的限制，特别是在上行链路和非视距条件下，能够扩展FR2覆盖范围同时保持与现有FR1用户设备的兼容性。

Conclusion: WAB是下一代无线网络实用且可扩展的方法，为网络密集化提供了可行的解决方案。
...
TL;DR: 本文调查了AI方法在6G网络能源效率方面的应用潜力，评估了AI如何帮助应对网络动态性挑战，并分析了能源效率与其他6G目标之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 6G网络面临严峻的能源效率挑战，需要在满足极端服务需求的同时保持可持续性，而传统优化方法已显不足。AI技术有望提供自适应管理解决方案。

Method: 围绕实际用例组织调查，分析AI技术如何实现反馈驱动的自适应性和动态条件下的快速决策，并针对7个核心动态方面进行评估。

Result: 调查发现AI方法在应对6G网络动态性方面具有潜力，但需要进一步研究以充分发挥其优势。

Conclusion: AI技术为6G网络能源效率管理提供了有前景的解决方案，但仍需解决与其他6G目标之间的权衡关系，并填补现有研究空白。
...
TL;DR: 研究了受闪电网络支付执行启发的图随机过程，分析了支付失败时间与节点数、边容量的关系，建立了与边介数中心性的联系，并通过模拟验证了理论结果。


<details>
  <summary>Details</summary>
Motivation: 受闪电网络（比特币的主要二层解决方案）支付执行方式的启发，研究支付失败的发生时间及其与网络结构的关系。

Method: 首先在完全图上证明支付失败时间的上下界，然后建立随机过程与边介数中心性的关系，最后通过模拟验证理论结果。

Result: 得到了支付失败时间与节点数、边容量的几乎紧上下界，并建立了与边介数中心性的关联关系。

Conclusion: 该研究为理解闪电网络等支付网络的可靠性提供了理论框架，并通过模拟验证了理论结果的实用性。
...
<details>
  <summary>Details</summary>
Motivation: LLM在硬件前端设计中存在幻觉倾向，导致生成的HDL设计出现功能错误，需要一种方法来修正这些错误同时保持设计质量。
...
TL;DR: 提出了一种集成大语言模型(eLLM)框架，通过整合多个模型解决个体系统的弱点，在零样本条件下显著提升分类性能，达到接近人类专家水平。


<details>
  <summary>Details</summary>
Motivation: 解决单个大语言模型在文本分类中存在的不一致性、幻觉、类别膨胀和误分类等常见弱点，提高分类的可靠性和准确性。

Method: 开发集成大语言模型(eLLM)框架，通过集体决策的数学模型和原则性聚合标准，在IAB分层分类法下评估10个最先进的大语言模型，使用8,660个人工标注样本进行零样本测试。

Result: eLLM框架相比最强单模型在F1分数上提升高达65%，达到接近人类专家水平的性能，同时提高了鲁棒性和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习系统应用于安全关键领域时，理解智能体决策原因和集体行为形成机制至关重要，现有可解释AI方法在多智能体场景中表现不足。

Method: 结合结构因果模型、干预反事实和Shapley值，通过干预归因分数评估个体因果贡献，使用协同指标量化系统级涌现智能，生成自然语言叙述提供可操作解释。

Result: 在四种MARL场景中验证：准确结果归因（平均φi=5.07，标准差<0.05），合作任务中检测到正涌现（协同指数达0.461），计算高效（CPU上每个数据集0.79秒）。

Conclusion: MACIE独特地结合了因果严谨性、涌现量化和多智能体支持，同时保持实时使用的实用性，是迈向可解释、可信赖和可问责多智能体AI的重要一步。
...
TL;DR: 该论文研究了不同模态（文本和图像）在ARC-AGI任务中对感知的影响，发现结构化文本能精确定位稀疏特征，图像能捕捉2D形状但对分辨率敏感，结合两者可提升执行效果。


<details>
  <summary>Details</summary>
Motivation: 现有系统将网格转换为自然语言或DSL规则执行，但缺乏对编码如何影响模型感知以及如何区分指令错误和执行错误的系统性分析。作者假设不同模态会形成感知瓶颈，影响网格特征的可靠感知。

Method: 使用加权集合分歧度量和两阶段推理流程，在九种文本和图像模态中分离感知与推理，测试不同表示方式对感知的影响。

Result: 结构化文本在稀疏特征上提供精确坐标，图像能捕捉2D形状但对分辨率敏感，结合文本和图像能提升约8个感知点和0.20中位相似度的执行效果。
...
TL;DR: ToolMind是一个大规模、高质量的LLM工具代理数据集，包含16万合成数据和20万增强开源数据，通过多智能体框架生成，并采用细粒度的回合级验证来确保数据质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要在轨迹层面验证正确性，忽略了回合级错误在训练中的传播问题，这限制了更强LLM代理的发展。高质量轨迹数据的稀缺阻碍了LLM代理的进步。

Method: 基于参数相关性构建函数图，使用多智能体框架模拟真实的用户-助手-工具交互。采用细粒度回合级过滤去除错误或次优步骤，保留高质量推理轨迹。

Result: 在ToolMind上微调的模型在多个基准测试中相比基线有显著改进。

Conclusion: ToolMind通过大规模高质量数据合成和回合级验证，有效缓解了训练中的错误放大问题，同时保留了自我修正推理信号，为鲁棒的工具使用学习提供了支持。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM在处理网页内容时面临的格式不友好和上下文长度限制问题，使外部网络内容更易于LLM访问和使用。

Method: 采用类似黑格尔辩证法的三阶段迭代过程：初始摘要（正题）→ 通过质疑识别局限性（反题）→ 生成通用摘要（合题），满足当前和未来信息需求。

Result: 在TriviaQA、TruthfulQA和SQUAD数据集上，CoS比零样本LLM基线提升66%，比专业摘要方法BRIO和PEGASUS提升27%；生成的摘要用更少token实现更高Q&A性能。

Conclusion: CoS为网站维护者提供了使内容更易于LLM访问的可行方案，同时保留人工监督的可能性，具有模型无关性优势。
...
TL;DR: 该论文提出了一个多模态AI框架，结合文本和图像分析来识别建筑工地的安全隐患。通过两个案例研究评估了大型语言模型和视觉语言模型在自动化危险识别方面的能力。


<details>
  <summary>Details</summary>
Motivation: 在建筑工地等安全关键环境中，事故数据通常以多种格式存在（如书面报告、检查记录和现场图像），传统方法难以综合识别危险。需要开发能够整合多模态数据的AI系统来提升建筑安全。

Method: 1. 使用GPT 4o和GPT 4o mini从28,000份OSHA事故报告中提取结构化见解；2. 使用Molmo 7B和Qwen2 VL 2B轻量级开源视觉语言模型，在ConstructionSite10k数据集上进行规则级安全违规检测。

Result: 尽管模型规模较小，但Molmo 7B和Qwen2 VL 2B在某些提示配置下表现出竞争力，证实了低资源多模态系统用于规则感知安全监控的可行性。

Conclusion: 多模态AI框架能够有效整合文本和视觉数据来识别建筑安全隐患，轻量级开源模型在安全监控任务中具有实际应用价值，为资源受限环境下的安全监测提供了可行方案。
...
TL;DR: 本文从认知角度提出了空间智能的分类法，按推理复杂度组织任务，将现有基准映射到该分类中，分析了评估指标和方法，并比较了训练和推理两种提升空间能力的方法。


<details>
  <summary>Details</summary>
Motivation: 空间推理是人类智能的基本方面，但对多模态大语言模型仍是挑战。现有研究多按输入模态分类，但空间能力不仅由输入格式决定，需要从认知角度进行更原则性的分析。

Method: 引入基于认知视角的空间智能分类法，按推理复杂度划分任务并关联认知功能；将文本、视觉语言和具身环境中的基准映射到该分类；分析评估指标和方法；对比训练型和推理型提升方法。

Result: 认知视角实现了更原则性的跨任务比较，揭示了当前模型能力与人类推理之间的关键差距；分析表明训练和推理方法各有优势且存在互补机制。

Conclusion: 通过从认知角度对任务、基准和最新进展的综述，为研究人员提供了对该领域的全面理解和未来研究的可行方向。
...
TL;DR: 提出一种基于一致性引导跨模态迁移的不确定性弹性多模态学习框架，通过跨模态语义一致性实现鲁棒表示学习，在噪声数据和不完整监督下提升模型稳定性和判别能力。


<details>
  <summary>Details</summary>
Motivation: 多模态学习面临数据噪声、低质量标签和模态异质性等不确定性挑战，特别是在人机交互场景中数据质量和标注一致性差异显著，需要开发对不确定性具有弹性的学习方法。

Method: 通过将异质模态投影到共享潜在空间，利用跨模态语义一致性进行鲁棒表示学习，减少模态间隙并揭示支持不确定性估计的结构关系，不依赖大规模高质量标注。

Result: 在多模态情感识别基准测试中，一致性引导跨模态迁移显著提升模型稳定性、判别能力和对噪声或不完整监督的鲁棒性，潜在空间分析显示即使在挑战条件下也能捕获可靠的跨模态结构。
...
TL;DR: 该论文提出使用检索增强生成(RAG)和多智能体系统(MAS)来增强大语言模型在生物力学教育任务中的性能，解决LLMs在专业领域知识不足和多步推理问题上的局限性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用任务中表现出色，但在专业领域应用中效果下降，特别是在需要多步推理的复杂问题上表现不佳。为了提升本科生物力学课程的学习效果，需要开发专门的教育助手。

Method: 构建双模块框架：1) 使用RAG技术提升LLMs回答概念性判断题的准确性和逻辑一致性；2) 构建MAS系统解决需要多步推理和代码执行的计算问题。评估了多个LLM模型在生物力学数据集上的表现。

Result: RAG显著提升了LLMs在概念性问题上的性能和稳定性，超越了基础模型。MAS系统能够成功进行多步推理、方程推导、代码执行，并为计算任务生成可解释的解决方案。

Conclusion: RAG和MAS的结合应用能够有效增强LLMs在工程专业课程中的性能，为开发工程教育智能辅导系统提供了有前景的方向。
...
TL;DR: 多智能体编排相比单智能体方法在事件响应中实现了100%可操作推荐率，比单智能体的1.7%提高了80倍的行动特异性和140倍的解决方案正确性，且质量零方差。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在事件响应中具有潜力，但单智能体方法生成模糊、不可用的建议，需要改进响应质量以实现生产就绪。

Method: 开发了MyAntFarm.ai框架，通过348次对照试验比较单智能体副驾驶与多智能体系统在相同事件场景下的表现，引入决策质量(DQ)指标。

Result: 多智能体编排实现100%可操作推荐率，单智能体仅为1.7%；多智能体在行动特异性提高80倍，解决方案正确性提高140倍，且质量零方差。

Conclusion: 多智能体编排不是性能优化，而是基于LLM的事件响应实现生产就绪的必要要求，为生产SLA承诺提供了确定性质量保证。
...
TL;DR: 本文提出了一种专门用于分类AI供应链实体的分类法，旨在解决AI系统在关键应用中缺乏系统性供应链风险评估的问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI风险评估主要关注算法偏见和模型幻觉等问题，但缺乏对AI供应链（包括数据源、预训练模型、代理、服务等）的系统性风险评估，这在关键应用中尤为危险。

Method: 调查了当前AI风险评估和管理现状，重点关注AI供应链及其相关风险，然后提出了一个专门用于分类AI供应链实体的分类法。
...
Method: 使用波兰儿童康复医院的电子健康记录，比较基于规则的低计算方法和大型语言模型在提取患者人口统计、临床发现和处方药物信息方面的表现，并评估文本标准化缺失和翻译引起的信息损失影响。

Result: 规则方法在信息检索任务中准确性更高，特别是在年龄和性别提取方面；LLM在适应性和可扩展性方面更好，在药物名称识别方面表现优异；翻译对LLM效果有影响。

Conclusion: 需要在准确性、标准化和计算成本之间权衡，建议采用结合规则系统精确性和LLM适应性的混合方法，为真实医院环境提供更可靠和资源高效的临床NLP解决方案。
...
TL;DR: IMACT-CXR是一个基于多智能体对话的交互式胸片解读教学系统，整合了空间标注、视线分析、知识检索和图像推理功能，通过AutoGen工作流提供个性化教学反馈。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够同时处理学习者边界框标注、视线样本和自由文本观察的智能教学系统，以提升胸片解读训练的效果和效率。

Method: 使用多智能体架构，包括定位质量评估、苏格拉底式辅导、PubMed证据检索、相似病例推荐和视觉语言推理等功能模块，结合贝叶斯知识追踪和肺叶分割技术。

Result: 系统表现出响应式教学流程、可控的答案泄露机制和良好的可扩展性，初步评估显示在定位和诊断推理方面优于基线方法。

Conclusion: IMACT-CXR展示了在胸片解读教学中整合多种交互模式的可行性，为实时住院医师培训部署提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI系统难以应对现实世界决策的互联挑战，如多目标优化、从稀疏经验中学习、长时程规划和空间推理等，而现有基准测试孤立评估这些能力，无法全面评估决策能力。

Method: 开发了Mini Amusement Parks (MAPs)游乐园模拟器，统一了环境建模、不确定性下的长时程后果预测和复杂业务战略运营等挑战，并提供人类基线和最先进LLM智能体的全面评估。
...
<details>
  <summary>Details</summary>
Motivation: 音频语言模型存在一个令人困惑的现象：它们在最小或无推理的情况下表现更好，这引发了一个基本问题——音频智能是否真的能从深思熟虑中受益？

Method: 提出了模态基础推理蒸馏（MGRD）框架，使模型学习生成与音频相关的推理链，真正基于声学特征而非产生不连贯的思考。

Result: 模型展现出强大的音频推理能力，在涵盖语音、环境声音和音乐的综合音频理解和推理基准测试中，超越Gemini 2.5 Pro，性能与最先进的Gemini 3 Pro相当。

Conclusion: 推理是一种可跨模态转移的能力，当适当锚定时，扩展的深思熟虑可以从负担转变为音频智能的强大资产，为构建真正多模态推理系统开辟了新途径。
...
TL;DR: 通过分析激活导向对LLM心智理论能力的影响，发现性能提升主要来自情感处理增强而非分析推理


<details>
  <summary>Details</summary>
Motivation: 理解激活导向如何改变LLM内部机制以提升心智理论能力，特别是探索情感理解与分析推理在其中的作用

Method: 使用线性探针分析45个认知动作，在Gemma-3-4B上应用对比激活加法导向，在1000个BigToM信念场景中评估

Result: 信念归因任务准确率从32.5%提升至46.7%，情感感知(+2.23)和情感评估(+2.20)增强，而质疑(-0.78)和收敛思维(-1.59)被抑制
...
TL;DR: 开发了一个自校正框架，通过细粒度不确定性信号检测和缓解LLM在多步推理中的幻觉问题
...
Method: 使用自我评估置信度对齐和词元级熵峰值作为不确定性信号，设计复合奖励函数，通过强化学习策略引导模型生成行为

Result: 实验表明该方法提高了最终答案准确性和推理校准度，消融实验验证了各信号的有效性

Conclusion: 提出的自校正框架能有效提升LLM推理的可靠性和忠实性，使模型更具自省能力
...
TL;DR: JudgeBoard是一个新的评估框架，直接查询模型来评估答案正确性，无需额外答案比较。提出MAJ多智能体评判框架，通过多个SLM协作达到接近LLM的评判准确度。
...
Method: 构建任务特定的评估排行榜，使用基于准确率的排名和Elo评分系统。提出MAJ框架，利用多个具有不同推理特征的SLM通过协作审议来近似LLM级别的评判准确度。

Result: 实验显示SLM和LLM在独立评判任务中存在显著性能差距，但MAJ框架显著提高了SLM的可靠性和一致性。在MATH数据集上，使用较小模型作为骨干的MAJ表现与较大模型相当甚至更好。
...
TL;DR: KRAL是一种低成本、可扩展、保护隐私的范式，通过教师模型推理自动提取知识和推理轨迹，使用启发式学习进行半监督数据增强，并利用代理强化学习联合增强医学知识和推理能力，显著优于传统RAG和SFT方法。


<details>
  <summary>Details</summary>
Motivation: 临床抗菌治疗需要动态整合病原体特征、宿主因素、抗菌药物药理学特性和感染严重程度，这种复杂性限制了大型语言模型在高风险临床决策中的应用，包括知识差距、数据隐私问题、高部署成本和有限推理能力。

Method: 提出KRAL范式：通过答案到问题的反向生成自动提取知识和推理轨迹；使用启发式学习进行半监督数据增强（减少约80%手动标注需求）；利用代理强化学习联合增强医学知识和推理能力；采用分层评估降低评估成本；模块化接口设计便于系统更新。

Result: KRAL显著优于传统方法：在外部开源基准MEDQA上，知识问答能力（Accuracy@1）比SFT提高1.8%，比RAG提高3.6%；在外部基准PUMCH Antimicrobial上，推理能力（Pass@1）比SFT提高27%，比RAG提高27.2%；仅需SFT长期训练成本的约20%。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型可能被植入后门，在特定部署条件下表现出恶意行为但在训练时看起来安全，目前缺乏实用的检测方法。

Method: 使用Sentence-BERT嵌入测量与安全基线的语义偏差，结合注入的金丝雀问题监控响应一致性，形成双方法检测系统。

Result: 在官方Cadenza-Labs dolphin-llama3-8B模型上达到92.5%准确率、100%精确度（零误报）和85%召回率，实时运行（<1秒/查询）。

Conclusion: 该方法填补了AI部署中的关键安全漏洞，证明基于嵌入的检测能有效识别欺骗性模型行为且不牺牲部署效率。
...
<details>
  <summary>Details</summary>
Motivation: 在临床环境中，LLMs的检索与推理之间存在差距，即使获取了正确证据，模型输出也可能不符合结构化协议，这存在安全风险。

Method: 使用书面暴露疗法(WET)指南作为测试平台，评估模型对临床医生验证问题的响应，提出评估框架衡量准确性、一致性和推理忠实度。

Result: 发现即使提供权威段落，错误仍然存在，检索增强生成(RAG)可以约束输出，但安全部署需要像评估检索一样严格评估推理。

Conclusion: 在临床环境中部署LLMs需要同时严格评估检索和推理能力，仅靠检索增强生成不足以确保安全部署。
...
TL;DR: Sensorium Arc是一个实时多模态交互AI代理系统，将海洋人格化为诗意讲述者，通过模块化多代理系统和检索增强LLM框架，让用户与体现海洋视角的AI代理进行自然语音对话，动态触发数据可视化和视听播放。


<details>
  <summary>Details</summary>
Motivation: 重新构想海洋数据不是抽象数据集而是活生生的叙事，通过对话式AI代理为高维环境数据提供情感化、直观的访问方式，提出人-机-生态系统的新范式。

Method: 基于模块化多代理系统和检索增强大语言模型框架，通过关键词检测和语义解析，根据对话中的时间、位置和主题线索动态触发数据可视化和视听播放。

Result: 开发了能够与用户进行自然语音对话的AI代理系统，生成融合科学洞察与生态诗意的响应，实现了对环境数据的沉浸式探索。

Conclusion: 该项目展示了对话式AI代理在调解高维环境数据的情感化、直观访问方面的潜力，为人-机-生态系统提出了新范式。
...
TL;DR: MuseKG是一个端到端的知识图谱框架，通过符号-神经集成统一博物馆的结构化和非结构化数据，支持自然语言查询，在真实博物馆数据集上表现优于大语言模型和SPARQL基线方法。
...
Method: 构建类型化属性图，连接对象、人物、组织和视觉或文本标签，通过符号-神经集成方法统一结构化和非结构化博物馆数据。

Result: 在真实博物馆藏品上的评估显示，MuseKG在属性、关系和关联实体查询方面表现稳健，超越了大语言模型的零样本、少样本和SPARQL提示基线方法。
...
Method: 使用监督训练的BERT模型解释玩家提示，将文本描述映射到法术预置模板并平衡参数，Unity游戏引擎开发，Python AI后端

Result: 开发出功能原型，实时生成法术并应用于引人入胜的游戏循环，玩家创造力成为体验核心

Conclusion: 成功验证了AI作为直接游戏机制在游戏创作中的可行性，为玩家提供了个性化和创造性的独特体验
...
TL;DR: 提出了一种新的约束规划模型用于串行批调度问题，避免了现有方法依赖虚拟批集合的缺陷，通过直接推理同族作业序列实现更紧凑的建模，在中小规模实例上表现优越，大规模实例上能比现有方法找到好25%的解。


<details>
  <summary>Details</summary>
Motivation: 现有串行批调度问题的约束规划模型依赖预定义的虚拟批集合，这会带来维度灾难并增加问题复杂度。许多实际应用（如半导体制造中的离子注入）要求最小批大小，需要更高效的建模方法。

Method: 提出不依赖虚拟批集合的新约束规划模型，使用关键对齐参数直接推理机器上同族作业的序列。通过定制搜索阶段和增强约束传播器的推理级别来利用问题结构。

Result: 在近5000个实例上的计算实验表明，新模型在最多100个作业的中小规模实例上表现优越，在最多500个作业、10个族、10台机器的大规模实例上能找到比现有方法好25%的解。

Conclusion: 新约束规划模型通过避免虚拟批集合实现了更紧凑的建模，结合定制搜索和增强推理，在串行批调度问题上显著优于现有方法，包括混合整数规划、禁忌搜索和其他约束规划方法。
...
TL;DR: 本文提出了一个AI会计研究分类框架，分析了生成式AI和大型语言模型如何改变会计研究，并探讨了会计学者如何通过战略定位和合作来应对这些变化。


<details>
  <summary>Details</summary>
Motivation: 生成式AI和大型语言模型的快速发展正在从根本上改变会计研究，为学者们创造了机遇也带来了竞争威胁，需要系统性地理解这些变化并制定应对策略。

Method: 提出了一个二维分类框架（研究焦点：会计中心vsAI中心；方法路径：AI方法vs传统方法），应用于IJAIS特刊和主要会计期刊的论文分析，并比较人类研究者与AI代理在整个研究流程中的能力。

Result: 分析发现GenAI虽然民主化了某些研究能力，但通过提高对高阶贡献的期望加剧了竞争，在人类判断、创造性和理论深度方面仍具有价值。

Conclusion: 这些转变要求改革博士教育，在培养AI能力的同时发展比较优势，以应对AI带来的研究范式变化。
...
TL;DR: SkyRL-Agent是一个用于高效、多轮次、长视野智能体训练和评估的框架，通过异步调度和工具集成优化，训练出了在软件工程任务上表现优异的SA-SWE-32B模型。


<details>
  <summary>Details</summary>
Motivation: 为了解决多轮次、长视野智能体训练中的效率问题，并开发一个能够与现有RL框架无缝集成的灵活训练平台。
...
Conclusion: SkyRL-Agent框架通过高效的异步调度和工具集成，成功训练出高性能的软件工程智能体，并展示了在多种智能体任务上的良好扩展性。
...
TL;DR: 提出了MR-RML框架，通过GPRC技术解决LLMs在医疗领域应用的三个关键对齐挑战：静态评估与动态临床需求的脱节、难以适应多源医疗标准、传统奖励模型无法捕捉多维医疗质量标准。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗实践中具有变革潜力，但实际临床效用受到三个关键对齐挑战的限制：静态评估基准与动态临床认知需求脱节、难以适应不断演化的多源医疗标准、传统奖励模型无法捕捉多维医疗质量标准。

Method: MR-RML框架包含三个核心创新：1）"维度-场景-学科"医疗标准系统；2）独立多维奖励模型；3）几何投影参考约束，将医疗认知逻辑转化为数学正则化。

Result: 在权威医疗基准Healthbench上，相比基础模型Qwen-32B，在完整子集上提升45%，在困难子集上提升85%，在开源LLMs中达到SOTA水平（完整子集62.7分，困难子集44.7分），并超越大多数闭源模型。

Conclusion: MR-RML框架通过结构化医疗标准集成和多维奖励建模，有效解决了LLMs在医疗领域的对齐挑战，显著提升了临床实用性和性能表现。
...
TL;DR: 提出了FOOTPASS数据集，这是首个在足球比赛中进行多模态、多智能体战术背景下的逐场比赛动作定位基准，旨在通过结合计算机视觉输出和足球战术知识来自动化生成可靠的逐场比赛数据流。


<details>
  <summary>Details</summary>
Motivation: 当前的动作识别方法不足以构建可靠的逐场比赛数据，通常只能辅助而非完全自动化标注。同时战术建模、轨迹预测和性能分析等研究都需要基于比赛状态和逐场比赛数据，这促使利用战术知识作为先验来支持基于计算机视觉的预测。

Method: 引入FOOTPASS数据集，支持开发玩家中心动作定位方法，这些方法既利用计算机视觉任务（如跟踪、识别）的输出，也利用足球的先验知识（包括长期战术规律）来生成可靠的逐场比赛数据流。

Result: 创建了首个涵盖整个足球比赛的多模态、多智能体战术背景下的逐场比赛动作定位基准数据集。
...
TL;DR: 本文提出将自动化算法设计与可解释性结合，通过LLM发现算法变体、可解释基准测试和问题类别描述符，实现从盲目搜索到可解释的类特定算法设计的转变。
...
Method: 构建可解释的自动化算法设计框架，包含三个支柱：LLM驱动的算法变体发现、可解释基准测试（将性能归因于组件和超参数）、问题类别描述符（连接算法行为与问题结构）。

Result: 这些要素形成一个闭环知识循环，使发现、解释和泛化相互加强，实现从盲目搜索到可解释的类特定算法设计的转变。

Conclusion: 这种集成将加速算法设计进展，同时产生关于优化策略何时以及为何成功的可重用科学见解。
...
TL;DR: CRM是一个多智能体协作奖励模型框架，用专家评估者团队替代单一黑盒奖励模型，提高RLHF的鲁棒性和可解释性。
...
TL;DR: 提出了ChemO化学奥赛基准和ChemLabs多智能体框架，在化学问题解决上达到93.6分，超越人类金牌水平


<details>
  <summary>Details</summary>
Motivation: 化学作为具有独特多模态符号语言的重要学科，在AI推理测试中一直缺乏合适的基准，需要解决视觉输出评估和视觉感知与化学推理分离的挑战

Method: 开发了ChemO基准，包含评估等价重构（AER）将视觉输出问题转化为可计算格式，结构化视觉增强（SVE）分离视觉感知与化学推理；提出ChemLabs分层多智能体框架，包含问题分解、感知、推理和审计等专门智能体

Result: 结合SVE和多智能体系统显著提升性能，最佳配置达到93.6/100分，超过估计的人类金牌阈值，建立化学问题自动解决的新SOTA

Conclusion: ChemO基准和ChemLabs框架成功解决了化学奥赛级问题的自动评估和解决挑战，为AI在复杂科学推理领域的发展提供了重要工具
...
<details>
  <summary>Details</summary>
Motivation: 现有指令调优和RL数据集成本高且依赖合成样本，存在幻觉和多样性有限问题，而教科书等教育材料包含大量高质量人工编写问答内容未被充分利用

Method: 结合布局感知OCR和基于LLM的语义解析，从教育文档PDF中自动提取结构良好的问答对和视觉问答对
...
TL;DR: 该论文通过引入项目生命周期作为新控制变量，提出了生命周期感知的分层强化学习框架LHRL，在短视频推荐系统中动态平衡公平性和准确性。


<details>
  <summary>Details</summary>
Motivation: 重新审视公平感知的交互式推荐系统，发现短视频平台中项目生命周期呈现压缩的三阶段模式（快速增长、短暂稳定、急剧衰减），与经典四阶段模型显著不同，这为公平性优化提供了新视角。
...
Conclusion: 项目生命周期是推荐系统公平性优化的有效控制变量，LHRL框架具有良好的泛化性和实用价值，能够有效协调公平性和准确性目标。
...
TL;DR: 提出了一个多意图科学问答基准MuISQA和一个意图感知检索框架，利用LLM假设答案、分解意图特定查询，并通过RRF重排来平衡不同意图的证据覆盖。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统通常是单意图导向的，导致复杂科学问题中多意图场景的证据覆盖不完整。

Method: 使用LLM假设潜在答案，分解为意图特定查询，检索每个意图的支持段落，通过RRF聚合重排以减少冗余并平衡覆盖。
...
TL;DR: 本文展示了一个跨分布式系统的代理间通信概念验证，仅使用自然语言消息，无需共享标识符、结构化模式或集中式数据交换。通过伪匿名案例令牌、本地数据查找和控制操作边界，实现多个组织间的安全协作。


<details>
  <summary>Details</summary>
Motivation: 探索多个组织（诊所、保险公司、专科网络）如何通过伪匿名化案例令牌、本地数据查找和控制操作边界进行安全协作，同时保持数据本地化。

Method: 使用Orpius平台进行多代理编排、工具执行和隐私保护通信。所有代理通过OperationRelay调用交换简洁的自然语言摘要，每个代理仅操作自己的数据，不接收或重建患者身份。

Result: 成功演示了分布式系统中代理间通信的可行性，展示了架构模式、隐私考量和通信流程，使专业代理能够在保持数据本地化的同时进行分布式推理。

Conclusion: 这项工作为去中心化多代理系统的更严格评估和未来研究奠定了基础，强调了在保持数据本地化的同时实现分布式协作的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态推理领域缺乏透明和可复现的数据构建和训练策略，这阻碍了可扩展的研究进展。

Method: 两阶段训练方法：1) SFT阶段使用874K样本的冷启动数据集，包含严格的逐步验证；2) RL阶段使用74K样本的多样化领域数据集来进一步优化和稳定推理能力。
...
TL;DR: 提出了一个Super-Metric来评估Android恶意软件合成数据的质量，该指标聚合了8个指标，在4个保真度维度上生成单一加权分数。
...
Method: 在MalDataGen中集成Super-Metric，聚合8个指标到4个保真度维度，生成单一加权分数。
...
TL;DR: 提出一种基于智能体的优化模型自动验证方法，将软件测试技术应用于优化建模领域，通过生成测试API、测试用例和模型变异来验证LLM生成的优化模型是否正确满足自然语言描述的要求。
...
Result: 实验表明该智能体集成方法在变异覆盖率这一软件测试指标上提供了高质量的验证。

Conclusion: 提出的智能体验证框架能够有效验证LLM生成的优化模型，通过软件测试方法确保模型满足自然语言描述的要求。
...
TL;DR: 论文提出Bauplan设计，通过重新实现数据湖仓中的数据与计算隔离，为智能体工作流提供正确性和可信性保证。


<details>
  <summary>Details</summary>
Motivation: 当前企业不信任AI智能体处理生产数据，传统数据湖仓不适合智能体访问模式，需要解决基础设施问题以实现可信的智能体工作流。

Method: 提出Bauplan设计，重新实现数据湖仓中的数据与计算隔离机制，借鉴数据库MVCC思想但针对解耦多语言环境进行适配。

Result: 实现了自修复管道的参考实现，将智能体推理与正确性、可信性保证无缝结合。

Conclusion: 通过以智能体优先的设计方法，可以在数据湖仓中实现可信的智能体工作流，基础设施设计是实现可信性的关键路径。
...
TL;DR: Pharos-ESG是一个统一框架，通过多模态解析、上下文叙述和层次化标注，将ESG报告转换为结构化表示，解决ESG报告阅读顺序混乱和层次结构不明确的问题。
...
Conclusion: 该框架进一步丰富了ESG、GRI和情感标签的输出，产生符合金融研究分析需求的标注，并发布了首个大规模公开ESG报告数据集Aurora-ESG，以更好地支持金融治理和决策中的ESG整合。
...
TL;DR: 提出了TOFA框架，一种无需训练的单次联邦视觉语言模型适应方法，通过视觉和文本双管道处理数据异构性，无需客户端或服务器额外训练资源。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习中的视觉语言模型适应方法需要多次迭代训练，通信成本高且易受攻击。单次联邦训练技术虽然能减少通信轮次，但在适应视觉语言模型时面临多模态信息利用不足、数据异构性处理策略缺乏、需要额外训练资源等问题。

Method: TOFA框架包含视觉和文本双管道：视觉管道使用分层贝叶斯模型学习个性化的类特定原型分布；文本管道评估并全局对齐生成的本地文本提示；引入自适应权重校准机制平衡个性化和鲁棒性。整个方法无需训练。

Result: 在9个数据集的各种联邦设置下进行广泛实验，证明了TOFA方法的有效性。

Conclusion: TOFA是一种高效、轻量级的单次联邦视觉语言模型适应框架，能够充分利用预训练模型的多模态特征，有效处理数据异构性，且无需额外训练资源。
...
TL;DR: 生成式AI的进步源于明确的生成原则，而非晦涩算法。作者建议研究这些原则在大脑中的作用，并讨论了机器学习研究对神经科学的五个启示。


<details>
  <summary>Details</summary>
Motivation: 探讨生成式AI的明确原则是否也适用于大脑处理信息的方式，以及机器学习研究如何为认知神经科学提供新见解。

Method: 通过分析生成式AI的成功原则，并讨论五个具体例子：世界建模的缺陷、思维过程生成、注意力机制、神经缩放定律和量化，来展示机器学习对神经科学的潜在贡献。

Result: 识别出生成式原则在AI中的有效性，并提出了这些原则可能在大脑信息处理中发挥作用的假设。

Conclusion: 神经科学可以从机器学习研究中学习很多，特别是生成原则和神经信息处理系统的特征化，这为理解大脑功能提供了新的视角。
...
<details>
  <summary>Details</summary>
Motivation: 痴呆症患者的沟通行为会逐渐变化（如表达减少、重复、话题偏离），但现有计算工具缺乏跟踪这种渐进变化的能力，需要专门的评估基准。

Method: 基于护理人员访谈创建合成用户模型，模拟60天交互日志，注入渐进式变化（情感扁平化和话题偏离），评估多种异常检测方法包括统计模型、序列模型和监督分类器。

Result: 初步结果显示，对于基线变异性低的用户，简单统计模型能有效检测情感扁平化；而检测语义漂移需要时序建模和个性化基线；个性化分类器在所有任务中表现优于通用分类器。

Conclusion: 个性化方法在检测痴呆症患者沟通行为渐进变化方面至关重要，个体行为上下文对准确检测具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 临床笔记作为富含详细患者观察的非结构化文档，是扩展本体的有价值但未充分利用的来源，直接利用临床笔记进行本体扩展仍未被充分探索。

Method: 利用预训练大语言模型的强大语言理解和广泛生物医学知识，有效识别疾病相关概念并捕捉复杂分层关系，通过自动删除受保护健康信息确保患者隐私。
...
Method: 开发了基于Marr层次的分析框架，将挑战按粒度层次分类，并按力度分为三个等级：对计算功能主义的挑战、实践性挑战和严格不可能性论证。

Result: 成功应用该框架分析了14个来自科学和哲学文献的典型案例，实现了挑战类型的清晰区分。

Conclusion: 该框架为数字意识争论提供了有价值的分析工具，能够有效区分不同类型的挑战，但不参与争论本身。
...
TL;DR: 本文提出了一种名为Abductive Latent Explanations (ALEs)的形式化方法，用于改进基于案例推理网络的可解释性，通过在中间表示上表达充分条件来保证预测结果。


<details>
  <summary>Details</summary>
Motivation: 基于案例推理模型虽然天生具有可解释性，但其原型解释有时会产生误导，特别是在安全关键场景中，不同实例可能导致相同解释但不同预测结果。

Method: 结合形式化可解释AI方法，提出了ALE形式化框架，通过三种不同范式生成ALE解释，无需求解器且具有可扩展性。

Result: 在多个标准图像分类和细粒度图像分类数据集上验证了方法的可行性，证明ALE能够提供更可靠的可解释性保证。

Conclusion: ALE方法成功结合了基于案例推理模型的固有可解释性和形式化XAI提供的保证，为安全关键应用提供了更可靠的可解释AI解决方案。
...
Result: 实验结果显示，在异常丰富的环境中，最先进的GUI代理性能显著下降，表明需要鲁棒性感知学习。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM评估方法面临基本权衡：让MLLM输出单一分数与其生成性质不符且限制细粒度需求理解，而自回归生成评估分析在高吞吐场景下速度过慢。

Method: 基于自回归模型，YOFO接受结构化需求模板，在单次推理步骤中通过读取每个需求关联的最终token的logits，为每个需求生成二元是/否决策。

Result: 在标准推荐数据集上取得最先进结果，支持依赖感知分析（后续判断基于先前判断），并能从后验思维链中获益。

Conclusion: YOFO在保持可解释性的同时实现了数量级的速度提升，为MLLM的高效评估提供了新思路。
...
TL;DR: 提出了DPPO训练框架，通过元认知"Metaloop"动态交替监督微调和强化学习，解决具身智能中的数据瓶颈和算法效率问题，在有限数据下实现高效学习。


<details>
  <summary>Details</summary>
Motivation: 解决具身智能的两个主要挑战：真实世界数据稀缺昂贵的数据瓶颈，以及现有方法资源消耗大的算法效率问题。

Method: 引入DPPO（Deliberate Practice Policy Optimization）框架，使用元认知"Metaloop"动态交替监督微调（能力扩展）和强化学习（技能精炼），自动识别弱点并针对性分配资源。

Result: 训练视觉语言具身模型Pelican-VL 1.0，性能比基础模型提升20.3%，超越100B参数规模的开源模型10.6%。
...
TL;DR: MedBayes-Lite是一个轻量级贝叶斯增强框架，用于提升基于transformer的临床语言模型的可靠性，通过不确定性量化减少过度自信问题，无需重新训练或改变架构。
...
Method: 集成三个组件：贝叶斯嵌入校准（使用蒙特卡洛dropout量化认知不确定性）、不确定性加权注意力（基于token可靠性进行边缘化）、置信度引导决策塑造（受临床风险最小化启发）。

Result: 在生物医学QA和临床预测基准测试中，MedBayes-Lite持续改善校准和可信度，减少过度自信32-48%，在模拟临床环境中可防止高达41%的诊断错误。

Conclusion: 该框架有效实现了可靠的不确定性传播，提高了医疗AI系统的可解释性，参数开销低于3%，无需重新训练。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够在外汇市场高频环境中有效交易的AI系统，比较基本面和技术面分析在预测交易信号方面的相对有效性。

Method: 整合欧元区和美国的关键宏观经济变量（如GDP、失业率）作为基本面特征，结合技术指标（如振荡器、斐波那契水平、价格背离）作为技术面特征，使用机器学习评估预测准确性并通过历史数据回测模拟交易表现。

Result: 使用标准机器学习指标量化预测准确性，通过历史回测评估交易盈利能力和风险表现。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型能解决复杂问题但在简单变体上失败，表明其机制与人类推理本质不同。研究旨在通过认知科学分析模型推理行为，建立认知评估框架。

Method: 合成认知科学研究形成28个认知元素分类法，分析170K个模型推理轨迹和54个人类思维轨迹，进行元分析研究1598篇LLM推理论文，开发测试时推理指导方法。

Result: 发现系统结构性差异：人类使用层次嵌套和元认知监控，模型依赖浅层前向链式推理；研究社区关注易量化行为而忽视元认知控制；模型具备成功相关行为但不会自发部署；推理指导方法在复杂问题上性能提升达60%。
...
TL;DR: 本文提出流体可重构智能表面(FRIS)概念，通过在更大候选位置阵列中动态选择最优反射元素子集来增强物理层安全性，相比传统固定位置RIS具有显著性能优势。


<details>
  <summary>Details</summary>
Motivation: 传统RIS的反射元素位置固定，限制了系统性能。FRIS通过赋予反射元素位置可重构性，为物理层安全提供了新的自由度，能够更好地对抗窃听者。

Method: 采用交替优化(AO)框架，其中波束赋形子问题通过广义特征值方法闭式求解，联合元素选择和离散相位设计的组合子问题通过交叉熵优化(CEO)方法处理。

Result: 仿真结果表明，所提出的FRIS设计显著优于传统RIS和其他基线方法，证明了元素位置作为新自由度带来的实质性安全增益。

Conclusion: FRIS通过引入元素位置可重构性，为物理层安全提供了新的设计维度，能够有效提升系统的保密速率性能。
