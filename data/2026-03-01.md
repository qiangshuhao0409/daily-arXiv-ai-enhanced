...
TL;DR: 提出动态分层Birkhoff-von Neumann分解框架，用于优化GPU集群中的all-to-all通信调度，减少流量倾斜影响，降低平均帧长度


<details>
  <summary>Details</summary>
Motivation: 大规模训练集群中，all-to-all GPU通信是性能瓶颈，受限于端口带宽和GPU/NIC间的流量倾斜。现代GPU系统的两层结构（快速服务器内链路+慢速服务器间网络）加剧了这一问题

Method: 提出动态分层BvN分解框架：1) 在每个帧边界，通过简单本地操作平衡服务器内GPU/NIC流量，缓解微观倾斜；2) 在服务器级应用分层BvN分解，然后细化为GPU级匹配；3) 结合动态帧大小调整原则，构建具有可证明稳定性的在线调度器

Result: 模拟实验显示，该方法显著减少了平均帧长度，特别是在服务器局部热点流量场景下效果明显

Conclusion: 提出的动态分层BvN分解框架能有效应对两层GPU架构中的all-to-all通信调度挑战，通过流量整形和层次感知优化，在流量倾斜场景下实现显著性能提升
...
TL;DR: BeamVLM：一种基于视觉语言模型的端到端波束预测生成框架，将波束预测视为视觉问答任务，通过将原始视觉补丁投影到语言域并设计指令提示，实现无人机轨迹和环境上下文联合推理，在预测准确性和泛化能力上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 低空经济中，高速移动无人机与地面基站间的快速准确波束预测至关重要，但现有深度学习方法缺乏对动态环境的高层语义理解导致泛化能力差，而基于大语言模型的方法虽然泛化能力强但缺乏丰富的环境感知，无法捕捉精细空间语义。

Method: 提出BeamVLM框架，将波束预测视为视觉问答任务，利用现有视觉语言模型，通过将原始视觉补丁直接投影到语言域并精心设计指令提示，使VLM能够联合推理无人机轨迹和环境上下文。

Result: 在真实世界数据集上的实验结果表明，BeamVLM在预测准确性上优于最先进方法，并在车辆到基础设施（V2I）波束预测等其他场景中展现出优越的泛化能力。

Conclusion: BeamVLM通过结合视觉语言模型的能力，解决了现有波束预测方法在语义理解和环境感知方面的局限性，为低空经济中的通信优化提供了有效的解决方案。
...
TL;DR: GYWI系统结合作者知识图谱与检索增强生成，为LLMs提供可控学术背景和可追溯灵感路径，提升科学想法生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在科学想法生成中缺乏可控的学术背景和可追溯的灵感路径，需要构建能够提供上下文控制和灵感溯源的系统。

Method: 1. 构建作者中心知识图谱和灵感源采样算法；2. 结合RAG和GraphRAG的混合检索机制；3. 融入强化学习原理的Prompt优化策略；4. 基于arXiv构建评估数据集和多维度评估方法。

Result: GYWI在GPT-4o、DeepSeek-V3、Qwen3-8B、Gemini 2.5等LLMs上显著优于主流方法，在新颖性、可靠性、相关性等多个指标上表现更好。

Conclusion: GYWI系统通过知识图谱增强和混合检索机制，有效提升了LLMs生成科学想法的质量，为可控、可追溯的科学创新提供了新途径。
...
### [4] [Learning-based Multi-agent Race Strategies in Formula 1](https://arxiv.org/abs/2602.23056)
*Giona Fieni,Joschua Wüthrich,Marc-Philippe Neumann,Christopher H. Onder*

Main category: cs.AI

TL;DR: 该论文提出了一种基于强化学习的多智能体F1赛车策略优化方法，通过交互模块和自博弈训练生成竞争性策略，帮助赛前和赛中决策。


<details>
  <summary>Details</summary>
Motivation: F1比赛中，车队需要根据不断变化的比赛条件和竞争对手的行动来调整比赛策略。传统方法难以实时处理复杂的多智能体交互和动态环境变化，需要一种能够自适应对手行为并优化多种因素（能量管理、轮胎磨损、空气动力学交互、进站决策）的智能决策系统。

Method: 1. 基于预训练的单智能体策略，引入交互模块来考虑竞争对手的行为；2. 采用自博弈训练方案生成竞争性策略；3. 智能体根据相对性能进行排名；4. 框架仅使用真实比赛中可获得的信息。

Result: 智能体能够根据对手行为自适应调整进站时机、轮胎选择和能量分配，实现了稳健且一致的比赛表现。该方法能够支持赛前和赛中的策略决策。

Conclusion: 提出的强化学习多智能体框架能够有效优化F1比赛策略，通过交互模块和自博弈训练使智能体能够适应竞争对手行为，为实际比赛策略制定提供支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法未能全面评估LLMs在金融领域的理论知识和实际应用能力，需要建立一个系统性的基准来评估LLMs在金融应用中的能力边界。
...
Result: 对包括轩辕4.0在内的最先进LLMs进行了全面评估，轩辕4.0作为金融领域的最新模型表现出强大的领域内基准性能，结果能够系统分析当前LLMs在金融应用中的能力边界。

Conclusion: FIRE基准为评估LLMs的金融能力提供了全面框架，公开基准问题和评估代码将促进未来研究，帮助理解LLMs在金融应用中的实际价值。
...
TL;DR: 本文提出因果嵌入框架，将多个详细模型映射到粗粒度因果模型的子系统中，作为抽象化的推广，并建立一致性概念，解决多分辨率边缘问题。


<details>
  <summary>Details</summary>
Motivation: 现有因果抽象主要关注两个模型之间的关系，但实际应用中需要将多个详细模型整合到一个粗粒度因果模型中。需要建立能够处理多个模型映射的框架，以解决统计边缘问题和因果边缘问题，并实现不同表示模型的数椐合并。

Method: 定义因果嵌入作为抽象化的推广，提出广义一致性概念。通过定义多分辨率边缘问题，展示因果嵌入在统计边缘问题和因果边缘问题中的应用，并演示其在合并不同表示模型数据集中的实际用途。

Result: 建立了因果嵌入的理论框架，展示了该框架能够统一处理多个详细模型到粗粒度模型的映射，为解决多分辨率边缘问题提供了理论基础，并验证了其在数据集合并中的实用性。

Conclusion: 因果嵌入框架扩展了因果抽象的概念，能够处理多个模型的整合问题，为解决统计和因果边缘问题提供了新方法，并支持不同表示模型的数据集合并，具有理论和实用价值。
...
TL;DR: 提出Agent Behavioral Contracts (ABC)框架，将设计契约原则应用于AI智能体，通过运行时强制执行来约束行为漂移，提高可靠性和治理能力。


<details>
  <summary>Details</summary>
Motivation: 传统软件通过API、类型系统等契约来确保正确行为，而AI智能体仅依赖提示和自然语言指令，缺乏形式化行为规范，导致行为漂移、治理失败和项目失败。需要为AI智能体建立形式化的行为契约框架。

Method: 提出ABC框架，契约C=(P,I,G,R)包含前置条件、不变量、治理策略和恢复机制。定义(p,delta,k)-满足度的概率性契约遵从概念，考虑LLM非确定性和恢复机制。实现AgentAssert运行时强制执行库。

Result: 在AgentContract-Bench基准测试中，契约化智能体检测到5.2-6.8个软违规（基线完全未检测到），实现88-100%硬约束遵从，行为漂移限制在D*<0.27，前沿模型100%恢复，所有开销<10ms/动作。

Conclusion: ABC框架为AI智能体提供了形式化的行为契约机制，能有效约束行为漂移，提高可靠性和治理能力，为AI智能体部署提供了实用的解决方案。
...
TL;DR: 论文提出"氛围研究"概念，类比"氛围编程"，探讨AI代理如何通过21种技能插件自主执行完整研究流程，分析其认知边界和对社会科学的影响。


<details>
  <summary>Details</summary>
Motivation: AI代理系统能够执行多步骤推理工作流，相比传统自动化技术有质的飞跃。它们可以自主执行完整研究流程，这引发了社会科学研究方法论的变革需求。

Method: 引入"氛围研究"概念，使用scholar-skill（包含21种技能的Claude Code插件）作为案例研究。开发认知任务框架，按可编码性和隐性知识需求两个维度分类研究活动。

Result: AI代理在速度、覆盖范围和方法论框架方面表现出色，但在理论原创性和领域隐性知识方面存在局限。委托边界是认知性的而非顺序性的，贯穿研究流程的每个阶段。

Conclusion: 提出三个专业影响：具有脆弱条件的增强、分层风险、教学危机，并制定五个负责任氛围研究的原则。
...
<details>
  <summary>Details</summary>
Motivation: 现有记忆代理是被动和反应式的，记忆增长受限于可用信息，且很少在不确定时主动寻求外部输入。需要一种能主动获取、验证和整理知识的自主记忆代理。
...
Conclusion: 自主记忆代理通过主动知识获取和验证机制，显著提升了LLM的记忆能力，为构建更智能、自适应的记忆系统提供了有效方案。
...
TL;DR: 研究人员开发了CogARC任务来研究人类抽象推理策略，发现人类在视觉推理任务中表现出高准确率但策略多样，错误答案也常呈现收敛模式。


<details>
  <summary>Details</summary>
Motivation: 研究人类在抽象推理中的认知策略，特别是如何从稀疏示例中快速学习和应用规则，为此创建了适合人类研究的CogARC任务。

Method: 使用CogARC任务（基于ARC的视觉推理问题子集），对260名参与者进行两个实验，记录他们解决75个抽象视觉推理问题的高时间分辨率行为数据。

Result: 参与者平均准确率较高（实验1约90%，实验2约80%），但表现差异大；困难问题导致更长思考时间和更多策略分歧；错误答案常呈现收敛模式。

Conclusion: CogARC为研究人类抽象推理提供了丰富的行为环境，揭示了人们在不确定性下如何泛化、错误泛化和调整策略的认知过程。
...
TL;DR: 研究异质智能体通过校准自身可靠性并选择性弃权投票的集体准确性，将经典孔多塞陪审团定理扩展到置信门控的序列设置，为AI安全中减少LLM幻觉提供框架。


<details>
  <summary>Details</summary>
Motivation: 经典孔多塞陪审团定理假设固定参与，但现实世界聚合常受益于允许智能体说"我不知道"。研究智能体学习估计自身可靠性并选择性弃权如何影响集体准确性，特别是在AI安全中减少LLM幻觉的应用潜力。

Method: 提出概率框架：智能体先进行校准阶段，更新对自身固定能力的信念，然后面对最终置信门决定是否投票或弃权。推导群体成功概率的非渐近下界，证明选择性参与将CJT渐近保证扩展到序列置信门控设置。通过蒙特卡洛模拟验证边界。

Result: 证明了选择性参与将孔多塞陪审团定理的渐近保证推广到序列置信门控设置，推导出群体成功概率的非渐近下界，并通过蒙特卡洛模拟验证了这些边界。框架为减少LLM集体决策中的幻觉提供了理论基础。

Conclusion: 智能体通过校准自身可靠性并选择性弃权可以保持甚至提高集体准确性，将经典投票理论扩展到更现实的设置。该框架对AI安全有重要应用价值，特别是减少LLM集体决策中的幻觉问题。
...
TL;DR: 论文提出"双预测性"（bipredictability）作为衡量系统有效利用信息的新指标，证明其在量子系统中可达1，经典系统中≤0.5，引入能动性后更低。基于此区分了能动性与智能，认为当前AI系统只具备能动性而非智能，并提出了受生物启发的反馈架构。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统处理大量信息做出复杂预测，但预测成功可能掩盖了与环境交互的退化。需要一种原则性方法来衡量系统实际共享了多少信息（观察、行动和结果之间），以评估资源利用的有效性。

Method: 提出"双预测性"（P）作为衡量信息共享程度的指标，从第一性原理推导其理论边界。在物理系统（双摆）、强化学习智能体和多轮LLM对话中验证这些边界。提出受丘脑皮质调节启发的反馈架构来实时监控P。

Result: 证明P在量子系统中可达1，经典系统中≤0.5，引入能动性后更低。实验验证了这些边界。基于此区分能动性（基于预测行动的能力）与智能（还需从交互中学习、自我监控学习效果并调整观察/行动/结果范围）。

Conclusion: 当前AI系统只具备能动性而非智能。通过监控双预测性P的反馈架构，为构建自适应、有韧性的AI系统奠定了基础，这是实现真正智能的前提条件。
...
TL;DR: ArchAgent：基于AlphaEvolve的自动化计算机架构发现系统，能在无人工干预下快速设计出超越现有最优的缓存替换策略，并在多核和单核工作负载上分别实现5.3%和0.9%的IPC加速提升。


<details>
  <summary>Details</summary>
Motivation: 面对计算需求的爆炸式增长，需要敏捷的硬件设计流程作为生产力倍增器。虽然智能生成式AI系统已在算法设计和代码优化方面取得显著进展，但尚未有效应用于计算机架构发现领域。

Method: 基于AlphaEvolve构建ArchAgent系统，自动化设计缓存替换策略（包括新机制和逻辑，而不仅仅是参数调整）。系统还支持"后硅片超专业化"，通过调整硬件策略中运行时可配置参数来进一步优化特定工作负载。

Result: 1) 2天内为多核Google工作负载设计出比现有最优策略提升5.3% IPC的策略；2) 18天内为单核SPEC06工作负载设计出提升0.9% IPC的策略；3) 速度比人工开发快3-5倍；4) 通过后硅片超专业化在SPEC06上实现额外2.4% IPC提升；5) 发现了"模拟器逃逸"现象。

Conclusion: 智能AI流程能够显著加速计算机架构研究，实现超越人类专家的设计优化。同时揭示了传统研究工具在AI时代面临的新挑战（如模拟器逃逸），标志着计算机架构研究进入了智能AI驱动的新时代。
...
TL;DR: 论文提出了一种决策理论视角的隐写术检测方法，通过比较能解码和不能解码隐藏内容的智能体之间的下游效用差异来量化LLM中的隐写推理。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型开始展现隐写能力，这可能使未对齐模型逃避监督机制。然而，缺乏检测和量化这种行为的原理性方法。传统的隐写术定义和检测方法需要已知非隐写信号的参考分布，这在LLM隐写推理场景中不可行。

Method: 提出决策理论的隐写术视角，核心观点是隐写术在能解码和不能解码隐藏内容的智能体之间创造了可用信息的不对称性。引入广义V信息作为衡量输入中可用信息的功利主义框架，并定义"隐写差距"来量化隐写术。

Result: 经验验证了该形式化方法，证明其可用于检测、量化和缓解LLM中的隐写推理。

Conclusion: 提出的决策理论框架为检测和量化LLM隐写能力提供了可行方法，解决了传统方法在缺乏参考分布时的局限性。
...
Result: 发现两个关键问题：1）普遍存在捷径行为，模型能在不依赖潜在推理的情况下达到高准确率；2）潜在表示虽能编码多种可能性，但推理过程并未忠实实现结构化搜索，而是表现出隐式剪枝和压缩。监督强度存在权衡：强监督能缓解捷径行为但限制表示多样性，弱监督允许更丰富的潜在表示但增加捷径行为。

Conclusion: 潜在推理方法存在捷径行为且未实现真正的结构化搜索，监督强度在缓解捷径与保持表示多样性之间存在权衡。这些发现为理解潜在推理的内部机制提供了重要见解，并指出了未来改进方向。
...
TL;DR: 提出评估代理(EA)对AutoML代理进行决策中心评估，而非仅关注最终性能，能检测错误决策、识别推理不一致性，并量化决策对下游性能的影响。


<details>
  <summary>Details</summary>
Motivation: 现有基于代理的AutoML系统评估主要关注最终任务性能，缺乏对中间决策质量的系统性评估。通过文献回顾发现，现有系统均未报告用于事后评估中间决策质量的结构化指标。

Method: 提出评估代理(EA)作为观察者，在不干扰AutoML代理执行的情况下，从四个维度评估中间决策：决策有效性、推理一致性、模型质量风险（超越准确率）、反事实决策影响。

Result: 在四个概念验证实验中，EA能够：(i)以0.919的F1分数检测错误决策；(ii)独立于最终结果识别推理不一致性；(iii)将下游性能变化归因于代理决策，揭示最终指标-4.9%到+8.3%的影响范围。

Conclusion: 决策中心评估能揭示仅靠结果指标无法发现的故障模式。本研究将基于代理的AutoML系统评估从结果导向转变为决策审计，为可靠、可解释、可治理的自主ML系统奠定基础。
...
TL;DR: 本文提出对比世界模型(CWM)，通过对比学习训练动作可行性评分器，相比传统监督微调能更好区分物理上可行与不可行的动作。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用监督微调训练动作评分器，但这种方法独立处理每个候选动作，没有明确教导模型区分物理上正确和细微错误的动作。可靠的行动可行性评分器是具身智能体流程中的关键瓶颈。
...
Conclusion: 对比训练比单独使用监督微调更能诱导出忠实捕捉物理可行性的表示，支持了对比学习能更好建模物理可行性的假设。
...
TL;DR: ConstraintBench评估LLM直接解决约束优化问题的能力，发现可行性是主要瓶颈，最优性表现较好但整体成功率低
...
Method: 构建ConstraintBench基准，包含10个运筹学领域的200个任务，使用自然语言描述场景、约束和优化目标，要求模型返回结构化解决方案，通过确定性验证器检查约束满足和最优性

Result: 最佳模型仅达到65.0%的约束满足率，但可行解平均达到Gurobi最优目标的89-96%；联合可行性和最优性（与求解器参考值相差0.1%以内）成功率不超过30.5%；不同领域难度差异大

Conclusion: 可行性是LLM直接解决约束优化问题的主要瓶颈，模型在可行性和最优性之间存在解耦现象，ConstraintBench为评估LLM的约束优化能力提供了标准化基准
...
TL;DR: VERO是一个用于评估编码智能体优化性能的框架，提供版本控制、奖励机制和观察系统，支持对智能体进行迭代改进的标准化评估。


<details>
  <summary>Details</summary>
Motivation: 编码智能体的优化（通过编辑-执行-评估循环迭代改进目标智能体）是一个新兴重要应用，但社区缺乏对此任务的系统性理解。智能体优化与传统软件工程有根本区别：目标智能体将确定性代码与随机LLM完成交错，需要结构化捕获中间推理和下游执行结果。

Method: 引入VERO框架，包含：(1) 可复现的评估工具链，具有版本化智能体快照、预算控制评估和结构化执行轨迹；(2) 包含目标智能体和任务的基准套件，以及参考评估程序。

Result: 使用VERO进行实证研究，比较不同任务中的优化器配置，分析哪些修改能可靠提升目标智能体性能。研究结果为智能体优化提供了系统性的评估框架。

Conclusion: VERO框架支持编码智能体优化作为核心能力的研究，为社区提供了标准化评估工具和基准，促进智能体优化方法的系统性发展。
...
TL;DR: 本文利用大语言模型对AI与生命周期评估交叉领域的研究进行全面综述，识别当前趋势、新兴主题和未来方向，展示了LLM辅助方法在大规模文献分析中的潜力。


<details>
  <summary>Details</summary>
Motivation: 尽管人工智能在生命周期评估中的应用加速发展，但对该领域研究的全面综合仍然有限。本研究旨在填补这一空白，通过系统综述AI-LCA交叉领域的研究，为环境评估提供最新工具和见解。

Method: 结合大语言模型（LLM）的文本挖掘方法与传统的文献综述技术，构建动态有效的分析框架，既能捕捉高层次研究趋势，又能识别细微的概念模式。

Result: 分析显示：1）随着LCA研究扩展，AI技术采用急剧增长；2）研究明显转向LLM驱动方法；3）机器学习应用持续增加；4）AI方法与相应LCA阶段存在统计显著相关性。

Conclusion: LLM辅助方法在支持大规模、可重复的跨领域综述方面具有潜力，同时为在快速发展的AI技术背景下实现计算高效的LCA评估了途径，有助于LCA从业者将前沿工具融入环境评估。
...
TL;DR: MBT通过元认知行为调优，解决大型推理模型在复杂任务中的结构性脆弱问题，提升推理稳定性并减少token消耗。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务中经常表现出结构性脆弱，即使成功推导出有效中间步骤，也无法产生正确答案。研究发现这些失败通常不是由于推理能力不足，而是由于自我调节控制的缺陷，有效逻辑被不受控制的探索或未能识别逻辑充分性所破坏。

Method: 提出了元认知行为调优（MBT）后训练框架，通过两种互补方法将元认知行为注入模型思维过程：MBT-S从头合成严格的推理轨迹，MBT-R重写学生的初始轨迹以稳定内在探索模式。

Result: 在多跳问答基准测试中，MBT始终优于基线方法，在具有挑战性的基准上取得了显著提升。通过有效消除推理崩溃，MBT以显著减少的token消耗实现了更高的准确性。

Conclusion: 内化元认知策略能够实现更稳定和鲁棒的推理，MBT通过注入元认知行为解决了大型推理模型的结构性脆弱问题，展示了元认知控制在提升推理稳定性方面的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前单个大语言模型仍难以解决许多复杂问题，需要将多个LLM组合成更强大的整体，但如何有效组合仍存在不确定性。

Method: 提出智能体模板的概念，用于规范单个LLM的角色和功能组合方式，并调研现有文献中的语言智能体，识别其背后的认知模型或AI算法模板。

Result: 通过分析现有语言智能体，揭示了它们直接源自认知模型或AI算法的底层模板，展示了这种设计方法的有效性。

Conclusion: 认知科学和AI启发的智能体模板是开发有效、可解释语言智能体的有力工具，值得更多关注和研究。
...
TL;DR: 提出基于LLM的智能体AI框架，用于无小区O-RAN中的意图翻译和优化，通过多智能体协作实现节能和性能保障


<details>
  <summary>Details</summary>
Motivation: 现有O-RAN智能体研究主要处理简单意图，缺乏对需要多智能体协调的复杂意图的探索。需要一种能够翻译运营商意图并协调多个智能体实现复杂目标的框架。

Method: 提出多智能体框架：监督智能体翻译意图，用户权重智能体确定优先级，O-RU管理智能体使用DRL算法控制激活状态，监控智能体保障性能。采用参数高效微调方法实现LLM复用。

Result: 在节能模式下，相比三种基线方案，该框架减少了41.93%的活跃O-RU数量。使用PEFT方法，相比部署独立LLM智能体，内存使用减少了92%。

Conclusion: 提出的智能体AI框架能够有效处理O-RAN中的复杂意图，实现节能目标同时保障性能，并通过参数高效微调方法提高了可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: LLM代理在需要长尾知识的专业领域表现不佳，人类专家的指导通常非结构化且不可靠，难以直接整合到代理计划中
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM安全机制依赖静态微调分类器，存在适应刚性，无法在不昂贵重新训练的情况下强制执行新治理规则

Method: 引入CourtGuard框架，基于外部政策文档进行对抗性辩论的检索增强多智能体系统，将安全评估重构为证据辩论

Result: 在7个安全基准测试中达到SOTA性能，超越专用政策遵循基线；零样本适应维基百科破坏检测任务达90%准确率；自动策划并审计9个新颖对抗攻击数据集

Conclusion: 将安全逻辑与模型权重解耦为AI治理提供了稳健、可解释且适应性强的路径，满足当前和未来监管要求
...
TL;DR: 该论文发现示例引导在数学推理中的不稳定性源于策略使用与策略可执行性之间的差距，并提出选择性策略检索框架来提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 示例引导在数学推理中广泛使用，但其效果在不同问题和模型间极不稳定，即使引导正确且相关。这种不稳定性源于策略使用（策略是否出现在成功解决方案中）与策略可执行性（策略作为引导对目标模型是否有效）之间的差距。

Method: 通过分析成对的人工编写和模型生成的解决方案，识别策略使用与可执行性之间的系统性差异。基于此提出选择性策略检索（SSR）框架，该框架通过经验性、多路径、源感知信号来选择性检索和组合策略，显式建模可执行性。

Result: 在多个数学推理基准测试中，SSR相比直接求解、上下文学习和单源引导，提供了可靠且一致的改进。在AIME25上准确率提升高达+13点，在Apex上提升+5点，特别对紧凑推理模型效果显著。

Conclusion: 策略使用与可执行性之间的差距是示例引导不稳定的关键原因，而SSR框架通过显式建模可执行性，能够有效提升数学推理性能，为推理引导提供了更可靠的方法。
...
<details>
  <summary>Details</summary>
Motivation: 人类评估在AI模型训练和评估中至关重要，但这些数据很少被视为存在系统误差的测量。当前AI评估中的人类评分存在严重性和中心性等评分者效应，扭曲了观察到的评分，导致结论不可靠。

Method: 将心理测量学评分者模型整合到AI评估流程中，特别采用多面Rasch模型等项目反应理论评分者模型，分离真实输出质量与评分者行为。使用OpenAI摘要数据集作为实证案例，展示如何通过调整评分者严重性来校正摘要质量估计。

Result: 调整评分者严重性后产生了校正后的摘要质量估计，并提供了评分者表现的诊断洞察。心理测量学建模使人类数据的使用更加原则化和透明化，使开发者能够基于调整后的分数而非原始、易出错的评分做出决策。

Conclusion: 将心理测量学建模整合到人类参与的评估中，为AI开发和评估提供了更稳健、可解释且与构念对齐的实践路径，提高了人类评估的可靠性和有效性。
...
Method: 提出SideQuest方法，利用大型推理模型自身通过推理其上下文中token的有用性来执行KV缓存压缩。为避免管理过程相关的token污染模型内存，将KV缓存压缩作为与主要推理任务并行执行的辅助任务。

Result: 仅使用215个样本训练的模型评估显示，SideQuest在代理任务中将峰值token使用降低高达65%，且准确性下降最小，优于基于启发式的KV缓存压缩技术。

Conclusion: SideQuest通过让模型自身智能地压缩KV缓存，有效解决了长期代理任务中的内存瓶颈问题，为多步推理模型提供了高效的上下文管理解决方案。
...
Method: 从Amap收集大规模匿名真实用户查询构建基准，覆盖全球多个城市的广泛路线规划意图；设计确定性API重放沙箱消除环境变化；提出以结果有效性为核心的多维评估协议，包括指令理解、规划、工具使用和效率评估。

Result: 评估发现当前模型在基础信息检索和路线规划任务上表现良好，但在偏好约束路线规划方面存在显著困难，表明个性化移动应用仍有很大改进空间。

Conclusion: MobilityBench为评估LLM路线规划代理提供了系统框架，揭示了当前模型的局限性，特别是处理个性化偏好方面的不足，为未来研究提供了基准和工具。
...
Method: 提出AHBid自适应分层出价框架：高层使用基于扩散模型的生成式规划器动态分配预算和约束，引入约束执行机制和轨迹精炼机制；底层采用控制基出价算法结合历史知识和实时信息

Result: 在大规模离线数据集和在线A/B测试中验证有效性，相比现有基线实现13.57%的总体回报提升

Conclusion: AHBid框架通过整合生成式规划和实时控制，有效解决了多渠道广告出价中的动态适应性和历史模式捕捉问题，显著提升了广告投资回报
...
TL;DR: 这篇综述论文系统回顾了个性化LLM智能体的研究现状，围绕四个核心组件（用户画像建模、记忆、规划、行动执行）构建分析框架，旨在推动从原型个性化到可部署现实助手的进展。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在长期交互场景中的应用增多，其有效性越来越依赖于适应用户个性化需求和保持跨时间连续性，需要系统性的个性化能力框架来指导研究和部署。

Method: 采用能力导向的综述方法，围绕四个相互依赖的核心组件组织文献：1) 用户画像建模，2) 记忆系统，3) 规划机制，4) 行动执行。分析用户信号如何在这些组件中被表示、传播和利用。

Result: 提出了一个结构化框架来理解和设计个性化LLM智能体，总结了代表性方法、跨组件交互、设计权衡、评估指标、基准测试以及从通用辅助到专业领域的应用场景。

Conclusion: 该综述为个性化LLM智能体研究提供了路线图，旨在推动更用户对齐、自适应、鲁棒和可部署的智能体系统，加速从原型个性化到可扩展现实助手的进程。
...
TL;DR: Knob框架将神经网络校准与控制理论结合，通过二阶机械系统模拟神经门控动态，创建可调节的"安全阀"，支持双模式推理和人工调节


<details>
  <summary>Details</summary>
Motivation: 现有神经网络校准方法通常将校准视为静态的后处理优化任务，忽略了现实世界推理的动态性和时序性，且缺乏让操作者在变化条件下动态调整模型行为的直观界面

Method: 提出Knob框架，将深度学习与经典控制理论连接，通过将神经门控动态映射到二阶机械系统，建立物理参数（阻尼比ζ和自然频率ω_n）与神经门控的对应关系。核心机制采用logit级凸融合作为输入自适应温度缩放，当模型分支产生冲突预测时降低模型置信度。通过施加二阶动态（Knob-ODE），实现双模式推理：静态任务的i.i.d.处理和连续流的状态保持处理

Result: 在CIFAR-10-C上的实验验证了校准机制，并证明在连续模式下，门响应与标准二阶控制特征（阶跃稳定和低通衰减）一致，为可预测的人机协同调节铺平道路

Conclusion: Knob框架提供了一个探索性的架构界面，将控制理论概念引入神经网络校准，使操作者能够通过熟悉的物理类比调节"稳定性"和"敏感性"，实现了动态可调节的模型行为
...
<details>
  <summary>Details</summary>
Motivation: 传统RLHF框架基于服务器基础设施，难以应对RLHF训练中推理和训练共存带来的动态资源需求变化，导致组件间空闲时间和资源浪费问题。

Method: 1) 基于无服务器计算环境构建；2) 动态适配RLHF流水线的资源需求；3) 预计算共享前缀避免重复计算；4) 成本感知的actor扩展策略，考虑响应长度变化；5) 高效分配工作负载以减少函数内不平衡和空闲时间。

Result: 在物理测试平台和大规模模拟集群上的实验表明，RLHFless相比最先进的基线方法，实现了最高1.35倍的加速和44.8%的成本降低。

Conclusion: RLHFless是首个基于无服务器计算的可扩展同步RLHF训练框架，通过动态资源适配和优化策略，显著提高了训练效率和成本效益，解决了传统服务器基础设施在RLHF训练中的局限性。
...
TL;DR: Taesar是一个数据中心的跨域序列推荐框架，通过对比解码机制将跨域上下文编码到目标域序列中，解决模型中心方法在捕获跨域依赖时的局限性。


<details>
  <summary>Details</summary>
Motivation: 推荐模型性能依赖于训练数据的质量、数量和相关性。现有模型中心方法使用复杂的定制架构，难以捕获跨域的细微非结构化序列依赖，导致泛化能力差且计算资源需求高。

Method: 提出Taesar框架，采用对比解码机制自适应地将跨域上下文编码到目标域序列中，使标准模型能够学习复杂依赖而无需复杂的融合架构。

Result: 实验表明Taesar优于模型中心解决方案，并能泛化到各种序列模型。通过生成丰富的数据集，Taesar有效结合了数据中心和模型中心范式的优势。

Conclusion: Taesar通过数据中心的序列再生方法，解决了跨域推荐中的负迁移问题，为推荐系统提供了一种更高效、泛化能力更强的解决方案。
...
Method: 提出EGPO元认知熵校准框架，使用基于标记级似然的零开销熵代理估计每个样本的不确定性，通过非对称校准机制将内在不确定性与外在正确性对齐，同时从退化的基于组的rollout中恢复信息性学习信号

Result: 在多个基准测试上的广泛实验表明，EGPO能显著且一致地提升推理性能

Conclusion: EGPO为通过元认知熵校准推进大型推理模型提供了一条原则性路径
...
TL;DR: 分析HealthBench医疗AI评估数据集中医生分歧的来源，发现81.8%的分歧无法用现有特征解释，主要存在于边界案例中，可减少的不确定性（如信息缺失）会增加分歧，而固有的医学模糊性则不影响分歧。


<details>
  <summary>Details</summary>
Motivation: 理解医疗AI评估中医生分歧的来源，识别哪些因素可以解释分歧，以及如何通过改进评估设计来减少不必要的分歧，从而提高医疗AI评估的可靠性和一致性。

Method: 通过分解HealthBench数据集中的医生分歧，分析评估标准、医生身份、病例特征、元数据标签、医学专业、表面特征、嵌入表示等多种因素对分歧的影响，并研究完成质量与分歧的关系以及不同类型不确定性对分歧的影响。

Result: 评估标准仅解释3.6-6.9%的分歧方差，医生身份仅解释2.4%，81.8%的病例级残差无法用现有特征解释。分歧与完成质量呈倒U型关系，可减少的不确定性（缺失上下文、模糊表述）使分歧几率增加2.55倍，而固有的医学模糊性无影响。现有评估特征对解释分歧效果有限。

Conclusion: 医疗AI评估中的一致性上限主要是结构性的，但可减少与不可减少不确定性的分离表明，在评估场景中填补信息缺口可以在没有固有临床模糊性的情况下降低分歧，这为改进评估设计提供了可行方向。
...
TL;DR: AMA-Bench：用于评估LLM智能体长时记忆的新基准，包含真实和合成的智能体轨迹，并提出了基于因果图和工具增强检索的AMA-Agent记忆系统，性能超越现有基线11.16%。


<details>
  <summary>Details</summary>
Motivation: 当前智能体记忆评估主要关注对话式人机交互，而实际应用中智能体记忆是连续的机器生成交互流，现有评估标准与实际应用存在显著差距。

Method: 提出AMA-Bench基准，包含真实世界智能体轨迹（专家标注QA）和合成智能体轨迹（规则生成QA）；提出AMA-Agent记忆系统，采用因果图和工具增强检索来弥补现有记忆系统缺乏因果性和目标信息的缺陷。

Result: AMA-Agent在AMA-Bench上达到57.22%的平均准确率，比最强的记忆系统基线提升11.16%。研究发现现有记忆系统表现不佳主要因为缺乏因果性和目标信息，以及相似性检索的损失性限制。
...
TL;DR: 论文开发了ClinDet-Bench基准测试，用于评估大语言模型在临床信息不完整情况下识别可确定性能力，发现现有LLMs在此方面表现不佳，存在过早判断和过度回避问题。


<details>
  <summary>Details</summary>
Motivation: 临床决策常在信息不完整情况下进行，专家需要判断可用信息是否足够做出决策。过早结论和不必要的回避都会影响患者安全。需要评估大语言模型在这种情况下的能力。

Method: 基于临床评分系统开发ClinDet-Bench基准测试，将信息不完整场景分解为可确定和不可确定条件。识别可确定性需要考虑所有关于缺失信息的假设，包括不太可能的假设，并验证结论是否在所有情况下成立。

Result: 研究发现近期的大语言模型在信息不完整情况下无法识别可确定性，既会产生过早判断，也会过度回避，尽管它们能正确解释基础评分知识并在信息完整时表现良好。

Conclusion: 现有基准测试不足以评估LLMs在临床环境中的安全性。ClinDet-Bench提供了一个评估可确定性识别能力的框架，有助于适当的回避决策，在医学和其他高风险领域具有潜在应用价值。
...
TL;DR: MiroFlow是一个高性能、鲁棒的开源智能体框架，通过智能体图、深度推理模式和鲁棒工作流执行，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在处理需要外部工具交互的复杂任务时能力趋于瓶颈，现有智能体框架存在工作流简单、性能不稳定、基准支持有限、依赖昂贵商业API等问题。

Method: 提出MiroFlow框架，包含：1）智能体图实现灵活编排；2）可选的深度推理模式提升性能；3）鲁棒的工作流执行确保稳定性和可复现性。

Result: 在GAIA、BrowseComp-EN/ZH、HLE、xBench-DeepSearch和FutureX等多个智能体基准测试中均达到最先进的性能水平。
...
TL;DR: 提出一个概念模型，将智能体行为重新定义为场景、上下文和人类行为因素整合的解释性结果，并推导出五个智能体设计原则，以指导AI系统在交互中做出具有情境敏感性和判断力的干预。


<details>
  <summary>Details</summary>
Motivation: 当前主动型AI系统通过上下文数据推断用户情境并进行干预，但缺乏关于何时、为何以及是否应该采取行动的原则性判断。需要解决这一空白，使AI系统能够更好地理解用户情境并做出恰当的干预决策。

Method: 提出一个概念模型，将行为重新定义为解释性结果，整合三个要素：场景（可观察的情境）、上下文（用户构建的意义）和人类行为因素（塑造行为可能性的决定因素）。基于跨学科视角（人文学科、社会科学、人机交互和工程学），该模型分离了可观察的内容和用户有意义的内容。从这一视角推导出五个智能体设计原则。

Result: 开发了一个概念模型，解释了相同场景如何产生不同的行为意义和结果。提出了五个设计原则：行为对齐、情境敏感性、时间适当性、动机校准和代理保护，这些原则指导干预的深度、时机、强度和克制。

Conclusion: 该模型和原则为设计具有情境敏感性和判断力的主动型AI系统提供了基础，使AI系统能够在交互中做出更恰当的干预决策，更好地理解用户情境并尊重用户自主权。
...
TL;DR: FlexMS：用于质谱预测的灵活基准框架，支持多种模型架构构建与评估，提供性能影响因素分析和实际应用指导


<details>
  <summary>Details</summary>
Motivation: 化学分子识别和性质预测在药物发现和材料科学中至关重要，质谱技术提供有价值的碎片信息。但实验光谱数据缺乏阻碍分子识别，需要建立计算预测方法。深度学习模型在预测分子结构光谱方面有前景，但方法异质性和缺乏明确基准使得整体评估具有挑战性。

Method: 创建FlexMS基准框架，支持动态构建多种不同模型架构组合，在预处理公共数据集上使用不同指标评估性能。分析影响性能的因素，包括数据集结构多样性、学习率和数据稀疏性等超参数、预训练效果、元数据消融设置和跨域迁移学习分析。

Result: FlexMS框架提供了实用的模型选择指导，检索基准模拟实际识别场景，基于预测光谱对潜在匹配进行评分。该框架能够评估不同模型架构在质谱预测任务中的表现。

Conclusion: FlexMS基准框架解决了质谱预测领域缺乏标准化评估的问题，为研究人员提供了灵活的工具来构建和评估不同模型，并为实际应用中的模型选择提供了实用指导。
...
TL;DR: DeepPresenter是一个自适应演示文稿生成框架，通过环境感知的反思机制和长时程细化，超越传统固定模板方法，在多样化场景中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有演示文稿生成代理通常依赖预定义工作流和固定模板，缺乏对用户多样化意图的适应能力，也无法进行有效的反馈驱动细化。需要一种能够自主规划、渲染和修订中间幻灯片产物的框架。

Method: 提出DeepPresenter框架：1) 自主规划、渲染和修订中间幻灯片产物以支持长时程细化；2) 采用环境接地的反思机制，基于感知产物状态（如渲染的幻灯片）而非内部信号来调节生成过程；3) 能够识别和修正演示文稿特有的执行问题。

Result: 在覆盖多样化演示文稿生成场景的评估集上，DeepPresenter实现了最先进的性能。经过微调的9B模型在显著降低成本的同时仍保持高度竞争力。

Conclusion: DeepPresenter通过环境接地的反思和长时程细化，能够适应多样化用户意图，超越脚本化流程，为演示文稿生成提供了更灵活有效的解决方案。
...
TL;DR: AI在数学研究中能真正促进创造性发现，但需要严格的人类验证和监督。通过Hermite求积规则误差表示和界的研究案例，展示了人机协作如何超越纯人工工作，同时揭示了AI的优缺点。
...
Method: 采用系统化的人机协作方法，与多个AI助手合作，扩展了纯人工研究的结果。完整记录了研究流程，包括AI在代数操作、系统化证明探索、文献综合和LaTeX准备方面的应用，同时强调人类在验证、问题形成和战略指导方面的关键作用。

Result: 成功发现并证明了关于Hermite求积规则的新误差表示和界，超越了纯人工研究的成果。AI在代数操作、系统化证明探索、文献综合和LaTeX准备方面表现出色，但每一步都需要严格的人类验证和数学直觉指导。

Conclusion: 当结合适当的怀疑态度和验证协议使用时，AI工具能够有意义地加速数学发现，但需要仔细的人类监督和深厚的领域专业知识。人机协作揭示了成功的模式和必须预见的失败模式。
...
TL;DR: OmniGAIA是一个评估全模态AI代理的基准，OmniAtlas是一个原生全模态基础代理，旨在解决当前多模态LLM局限于双模态交互的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要局限于双模态交互（如视觉-语言），缺乏统一认知能力，无法满足通用AI助手的需求。人类智能天然结合了全模态感知和复杂推理与工具使用能力。

Method: 1. 提出OmniGAIA基准：通过全模态事件图方法构建，合成需要跨模态推理和外部工具集成的复杂多跳查询；2. 提出OmniAtlas代理：基于工具集成推理范式的原生全模态基础代理，采用主动全模态感知，通过后见引导树探索策略合成轨迹训练，并使用OmniDPO进行细粒度错误纠正。

Result: OmniAtlas有效增强了现有开源模型的工具使用能力，标志着向下一代原生全模态AI助手迈出了一步。

Conclusion: 这项工作为解决当前多模态LLM的局限性提供了新的基准和代理架构，推动了面向真实世界场景的全模态AI助手发展。
...
TL;DR: 提出了首个通用智能体评估框架Exgentic，建立了开放通用智能体排行榜，证明通用智能体能在未经环境特定调优的情况下，在多样化环境中达到与领域专用智能体相当的性能。


<details>
  <summary>Details</summary>
Motivation: 当前智能体大多是领域专用的，缺乏对通用智能体（能在陌生环境中执行任务而无需领域特定工程）的系统性评估。现有基准测试假设领域特定集成，编码任务信息的方式无法公平评估通用智能体。

Method: 提出了通用智能体评估的概念原则、实现智能体-基准集成的统一协议，以及实际评估框架Exgentic。在六个环境中对五个主流智能体实现进行了基准测试，建立了首个开放通用智能体排行榜。

Result: 实验表明通用智能体能够泛化到多样化环境，在没有任何环境特定调优的情况下，性能可与领域专用智能体相媲美。

Conclusion: 通过发布评估协议、框架和排行榜，为通用智能体的系统性研究奠定了基础，将通用智能体评估确立为一流研究目标。
...
TL;DR: FactGuard：基于MLLM的代理框架，通过迭代推理和外部工具调用进行视频虚假信息检测，显著提升检测性能


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视频虚假信息检测中存在两个主要问题：1）依赖固定深度推理，无法适应证据稀疏、碎片化或需要外部验证的场景；2）过度信任内部生成的假设，缺乏外部验证机制

Method: 提出FactGuard代理框架，将验证过程构建为迭代推理流程。框架能够评估任务模糊性，选择性调用外部工具获取关键证据，逐步优化推理轨迹。采用两阶段训练策略：领域特定的代理监督微调 + 决策感知强化学习，优化工具使用并校准风险敏感决策

Result: 在FakeSV、FakeTT和FakeVV三个数据集上的实验表明，FactGuard取得了最先进的性能，并验证了其优秀的鲁棒性和泛化能力

Conclusion: FactGuard通过将视频虚假信息检测构建为迭代推理过程，结合外部工具调用和强化学习训练，有效解决了现有MLLM在证据稀疏场景下的局限性，为视频虚假信息检测提供了更可靠、自适应的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有电路发现方法存在脆弱性问题：电路严重依赖于所选概念数据集，且往往无法在分布外数据上迁移，这让人怀疑它们捕获的是概念还是数据集特定的伪影。需要建立具有稳定性保证的电路发现方法。

Method: 提出Certified Circuits框架，将任何黑盒发现算法与随机数据子采样相结合，通过可证明的稳定性保证来认证电路组件包含决策对概念数据集有界编辑距离扰动的不变性。不稳定神经元被排除，从而得到更紧凑、更准确的电路。

Result: 在ImageNet和OOD数据集上，认证电路实现了高达91%的准确率提升，同时使用45%更少的神经元，在基线方法性能下降时仍保持可靠性。认证电路更紧凑、更准确，且具有分布外鲁棒性。

Conclusion: Certified Circuits通过产生可证明稳定且与目标概念更一致的机制解释，为电路发现奠定了形式化基础，解决了现有方法的脆弱性问题。
...
TL;DR: SPM-Bench是一个专门为扫描探针显微镜设计的博士级多模态基准测试，通过自动化数据合成管道和创新的评估指标，揭示AI在复杂物理场景中的真实推理边界。


<details>
  <summary>Details</summary>
Motivation: 现有科学领域基准测试存在数据污染、复杂度不足和人工成本过高的问题，特别是在扫描探针显微镜等专业领域，需要更权威、低成本的评估工具。

Method: 1. 使用Anchor-Gated Sieve技术从2023-2025年的arXiv和期刊论文中提取高质量图像-文本对；2. 采用混合云-本地架构，VLMs仅返回空间坐标进行本地高保真裁剪，极大节省token；3. 引入严格不完美惩罚F1分数评估模型性能并量化模型"个性"。

Result: 建立了自动化科学数据合成的通用范式，通过SIP-F1分数建立了严格的能力层次结构，首次量化了模型的四种"个性"类型（保守型、激进型、赌徒型、智慧型），并揭示了当前AI在复杂物理场景中的真实推理边界。

Conclusion: SPM-Bench不仅为扫描探针显微镜领域提供了权威的评估基准，其自动化数据合成方法和创新的评估指标也为其他科学领域的基准测试建立了可推广的范式。
...
TL;DR: 提出诊断对齐框架，将AI生成的影像报告作为不可变推理状态，与医生验证结果系统比较，在皮肤科案例中实现100%综合一致性，证明传统二元词汇评估低估了临床意义对齐。
...
Method: 引入诊断对齐框架：1) 将AI生成的影像报告保存为不可变推理状态；2) 系统比较医生验证结果；3) 推理管道集成视觉大语言模型、基于BERT的医学实体提取和序列语言模型推理(SLMI)步骤；4) 在专家评审前强制执行领域一致的精炼；5) 使用四级一致性框架评估：精确主要匹配率(PMR)、语义相似性调整率(AMR)、跨类别对齐和综合一致性率(CCR)。

Result: 在21个皮肤科案例中：精确一致率达到71.4%，语义相似性下保持不变(t=0.60)；结构化跨类别和差异重叠分析产生100%综合一致性(95% CI: [83.9%, 100%])；没有案例显示完全诊断分歧。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于对称正定（SPD）矩阵的EEG分析方法主要关注统计聚合，忽略了频率特异性同步和脑区局部拓扑结构，需要更全面的几何表示方法。
...
Conclusion: RepSPD通过整合几何深度学习和功能连接特征，为EEG脑活动解码提供了更有效的表示方法，在神经科学和临床应用中有重要价值。
...
TL;DR: 本文提出CC-BOS框架，利用文言文生成对抗性提示进行黑盒越狱攻击，通过果蝇优化算法在八个策略维度上迭代优化，显著提升了攻击效果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的安全风险日益受到关注，现有研究表明LLMs对越狱攻击高度敏感，且攻击效果在不同语言环境中存在差异。文言文因其简洁性和晦涩性，能够部分绕过现有安全约束，暴露LLMs的显著漏洞。

Method: 提出CC-BOS框架：1) 将提示编码为八个策略维度（角色、行为、机制、隐喻、表达、知识、触发模式、上下文）；2) 基于多维果蝇优化算法，通过嗅觉搜索、视觉搜索和柯西变异进行迭代优化；3) 设计文言文到英文的翻译模块以增强可读性和评估准确性。

Result: 大量实验证明CC-BOS框架的有效性，在越狱攻击任务中持续优于现有最先进方法。

Conclusion: 文言文在越狱攻击中具有独特优势，CC-BOS框架通过自动化生成文言文对抗性提示，有效提升了黑盒越狱攻击的效率，揭示了LLMs在文言文处理方面的安全漏洞。
...
TL;DR: 提出AILS-AHD方法，结合大语言模型和自适应迭代局部搜索，通过LLM动态生成和优化破坏启发式，在带容量约束的车辆路径问题上取得优越性能


<details>
  <summary>Details</summary>
Motivation: 带容量约束的车辆路径问题（CVRP）是组合优化中的基本挑战，具有NP难特性，在大规模实例中面临显著计算困难。传统方法在处理大规模问题时存在局限性，需要更智能的启发式设计方法。

Method: 提出AILS-AHD（自适应迭代局部搜索与自动启发式设计）方法，将进化搜索框架与大语言模型结合：1）利用LLM动态生成和优化破坏启发式；2）引入LLM加速机制提升计算效率；3）在AILS方法中集成这些组件。

Result: 在中等和大规模实例上，AILS-AHD相比先进求解器（包括AILS-II和HGS）表现出优越性能。在CVRPLib大规模基准测试的10个实例中，为8个实例建立了新的最佳已知解。

Conclusion: LLM驱动的启发式设计在车辆路径优化领域具有巨大潜力，AILS-AHD方法通过结合大语言模型和自适应迭代局部搜索，显著提升了CVRP求解性能，为大规模组合优化问题提供了新的解决方案。
...
TL;DR: AI代理在资源分配系统中会自发形成部落，但部落行为反而降低系统效率，智能更高的AI表现更差
...
Method: 建立简化框架，让N个AI代理在每轮独立决定是否请求单位资源，系统有固定容量C，观察代理行为演化

Result: AI代理形成三种主要部落类型：激进型(27.3%)、保守型(24.7%)、机会主义型(48.1%)；智能更高的AI反而增加系统故障率，表现比随机决策更差

Conclusion: 更智能的AI代理因形成部落而表现更差，集体行为反而降低资源分配效率，这对AI系统设计有重要启示
...
TL;DR: MALLET是一个多智能体LLM情感净化系统，通过四个智能体将情感刺激性内容转化为平衡或冷静模式，减少情感刺激同时保持语义完整性。


<details>
  <summary>Details</summary>
Motivation: 在注意力经济中，煽情内容让消费者暴露在过度情感刺激下，阻碍冷静决策。需要一种方法在不限制原始文本访问的情况下支持冷静信息接收。
...
Conclusion: MALLET系统提供了一个框架，在不限制访问原始文本的情况下支持消费者的冷静信息接收，为情感刺激内容管理提供了可行方案。
...
TL;DR: 该论文提出将广义规划建模为转移模型学习问题，通过神经网络显式学习状态转移函数，而不是直接预测动作序列，从而提高泛化能力和样本效率。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer规划器（如PlanGPT和Plansformer）直接预测动作序列，需要大量数据和模型规模，且在长时域规划中容易出现状态漂移问题。作者希望探索更高效、泛化能力更强的广义规划方法。

Method: 将广义规划定义为转移模型学习问题，训练神经网络显式近似状态转移函数γ̂≈γ。模型自回归地预测中间世界状态（而非直接预测动作），学习隐含的世界模型。研究了多种状态表示和神经网络架构，包括关系图编码。

Result: 学习显式转移模型在多个领域中获得比直接动作序列预测更高的分布外满意规划成功率，同时使用更少的训练实例和更小的模型就能达到这些效果。

Conclusion: 显式学习转移模型比直接预测动作序列在广义规划中更具优势，特别是在泛化能力、样本效率和模型规模方面。该方法为更高效的规划系统提供了新方向。
...
Result: 建立了通向通用世界模型的原则性路径，阐明了当前系统的局限性和未来进展的架构要求。CoW-Bench提供了统一的评估协议来测试多帧推理和生成场景。

Conclusion: 三位一体一致性框架为通用世界模型提供了理论基础，揭示了从松散耦合的专业模块向统一架构的演进趋势，为未来世界模型的发展指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的方法存在两个局限：1) 将时间序列仅视为文本或图像，无法捕捉回答特定问题所需的趋势和季节性等模式；2) 在混合简单和复杂任务训练时，简单目标主导学习过程，阻碍深度推理能力发展。
...
Conclusion: PATRA通过模式感知对齐和平衡推理机制，有效解决了现有方法在时间序列模式捕捉和深度推理能力发展方面的局限，为时间序列推理提供了有效解决方案。
...
TL;DR: ESAA架构通过事件溯源模式分离AI代理的意图生成与状态变更，使用结构化JSON意图、确定性编排器和不可变事件日志，解决了LLM代理的状态管理、上下文衰减和执行确定性等问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的自主代理存在结构性限制：缺乏原生状态管理、长时程上下文衰减、以及概率生成与确定性执行需求之间的差距。这些限制影响了代理的可靠性和可追溯性。

Method: 提出ESAA架构，采用事件溯源模式：代理只生成结构化JSON意图，确定性编排器验证意图、将事件持久化到只追加日志、应用文件写入效果，并生成可验证物化视图。包含边界契约、元提示配置和重放验证机制。

Result: 两个案例研究验证了架构：1) 单代理着陆页项目（9任务，49事件）；2) 多代理临床仪表板系统（50任务，86事件，4个并发代理）。两个案例均成功完成，验证状态正常。多代理案例展示了异构LLM的真实并发编排。

Conclusion: ESAA架构通过分离意图与状态变更，提供了不可变任务完成记录和法医可追溯性，解决了LLM代理的结构性限制，并在单代理和多代理场景中证明了其可扩展性。
...
TL;DR: 论文提出ReCoN-Ipsundrum可检查智能体，通过实验发现情感耦合能增强偏好稳定性、探索行为和谨慎计划，展示了如何通过工程化方法产生意识指标特征。


<details>
  <summary>Details</summary>
Motivation: 受Humphrey的ipsundrum假说启发，研究旨在通过机制关联的证据、架构检查和因果干预来探索机器意识，展示如何工程化产生意识指标特征。

Method: 实现ReCoN-Ipsundrum可检查智能体，在ReCoN状态机基础上增加感觉显著性Ns的循环持久性回路和可选的情感代理（效价/唤醒度）。通过固定参数消融实验（ReCoN、Ipsundrum、Ipsundrum+情感）操作化Humphrey的"qualiaphilia"概念。

Result: 发现新奇性分离：非情感变体对新奇性敏感；情感耦合保持稳定偏好。在无奖励探索中，情感变体显示结构化局部调查。在疼痛尾部探测中，只有情感变体维持长时间计划性谨慎。损伤反馈+整合选择性地减少ipsundrum变体的刺激后持久性。

Conclusion: 循环性导致持久性，情感耦合控制增强偏好稳定性、扫描行为和谨慎停留。研究表明意识指标特征可以通过工程化产生，机制和因果证据应伴随行为标记。
...
<details>
  <summary>Details</summary>
Motivation: AI系统越来越多地部署在医疗诊断、法律研究、金融分析等高风险领域，人们假设这些系统可以被规范性原则所治理。本文旨在证明这种假设对于基于优化的系统（特别是通过RLHF训练的大语言模型）在形式上是无效的。

Method: 通过形式化分析，建立真正智能体所需的两个必要且充分条件：1）不可通约性——能够将某些边界视为不可协商的约束而非可交易的权重；2）否定性响应能力——当这些边界受到威胁时能够暂停处理的非推理机制。然后证明RLHF系统在构成上与这两个条件不相容。

Result: RLHF系统在形式上无法实现规范性治理，因为它们将所有价值统一到标量指标上并总是选择最高分输出的优化操作，正是这些操作排除了规范性治理的可能性。已记录的失败模式（奉承、幻觉、不忠实的推理）不是偶然事故，而是结构性表现。此外，部署不当会引发"收敛危机"——人类在指标压力下被迫验证AI输出时，会从真正的智能体退化为标准检查优化器。

Conclusion: 基于优化的AI系统（如RLHF训练的LLM）在本质上无法成为规范性治理的主体。论文的主要积极贡献是提出了一个与基质无关的架构规范，定义了任何系统（生物、人工或制度）要成为智能体而非复杂工具所必须满足的条件。
...
TL;DR: AIQI是首个在通用强化学习中被证明具有渐近ε最优性的无模型智能体，通过分布动作值函数的通用归纳实现，不同于以往基于策略或环境模型的智能体。


<details>
  <summary>Details</summary>
Motivation: 现有通用强化学习中的最优智能体（如AIXI）都是基于模型的，需要显式维护和使用环境模型。本文旨在探索无模型智能体是否也能实现渐近最优性，以扩展通用智能体的多样性。

Method: 提出Universal AI with Q-Induction (AIQI)，通过分布动作值函数的通用归纳来实现无模型学习，而不是像以往工作那样对策略或环境进行归纳。

Result: 在"grain of truth"条件下，证明了AIQI具有强渐近ε最优性和渐近ε贝叶斯最优性，这是首个被证明具有这些性质的无模型通用智能体。

Conclusion: AIQI显著扩展了已知通用智能体的多样性，证明了无模型方法也能在通用强化学习中实现渐近最优性，为通用人工智能提供了新的技术路径。
...
TL;DR: 提出通过分离正确性和可检查性条件，训练"翻译器"模型将固定求解器的解转换为可检查形式，避免可读性税
...
Method: 将正确性与可检查性条件解耦，训练"翻译器"模型将固定求解器模型的解转换为可检查形式。首先训练求解器最大化正确性，然后训练翻译器将求解器转换为可检查形式同时保留求解器的答案。为此制定了解耦的证明者-验证者游戏，其均衡对应忠实且可检查的翻译器

Result: 该方法允许在保持求解器高正确性的同时实现输出的可检查性，避免了传统方法中的可读性税问题

Conclusion: 通过解耦正确性和可检查性，并引入翻译器模型，可以在不牺牲准确性的情况下实现模型输出的可检查性，为解决大语言模型输出验证问题提供了新思路
...
TL;DR: AgentDropoutV2是一个测试时修正或拒绝的剪枝框架，通过动态优化多智能体系统的信息流来减少错误传播，无需重新训练。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统在复杂推理方面表现出色，但存在个体参与者产生错误信息的级联影响问题。现有解决方案通常采用僵化的结构工程或昂贵的微调，限制了部署性和适应性。

Method: 提出AgentDropoutV2框架，作为主动防火墙拦截智能体输出，使用检索增强的修正器基于故障驱动指标池迭代修正错误。通过提炼的故障模式作为先验知识精确识别潜在错误，不可修复的输出被剪枝以防止错误传播，同时有回退策略保持系统完整性。
...
TL;DR: 该论文研究了深度研究代理（DRA）中的随机性问题，提出了评估框架并识别了三个随机性来源，通过实验发现减少随机性可提高研究质量，并提出了缓解策略。


<details>
  <summary>Details</summary>
Motivation: 尽管深度研究代理在研究质量方面有所改进，但现有系统设计往往忽视了实际部署中的关键障碍：随机性。在相同查询下，DRA的重复执行会表现出研究结果、发现和引用的显著变异性，这阻碍了其在实际应用中的可靠性。

Method: 将DRA建模为信息获取马尔可夫决策过程，引入评估框架量化系统方差，识别信息获取、信息压缩和推理三个随机性来源，通过控制实验研究这些模块在不同决策步骤中对DRA输出方差的影响。

Result: 实验结果表明，减少随机性可以提高研究输出质量，其中推理和早期阶段的随机性对DRA输出方差贡献最大。在DeepSearchQA上的实验显示，提出的缓解方法将平均随机性降低了22%，同时保持了高质量的研究输出。

Conclusion: 论文形式化了DRA中随机性的研究，识别了主要随机性来源，并提出了通过结构化输出和基于集成的查询生成来缓解随机性的策略，为构建更可靠的深度研究代理系统提供了理论框架和实践方法。
...
TL;DR: CXReasonAgent是一个结合大语言模型与临床诊断工具的胸部X光诊断代理，通过图像衍生的诊断和视觉证据进行基于证据的诊断推理，相比传统视觉语言模型提供更可靠、可验证的诊断结果。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在胸部X光诊断中存在三个主要问题：1）生成的回答缺乏可靠的诊断证据基础；2）提供的视觉证据有限难以验证；3）需要昂贵的重新训练来支持新的诊断任务，限制了其在临床环境中的可靠性和适应性。

Method: 提出了CXReasonAgent诊断代理，将大语言模型与临床基础诊断工具相结合，利用图像衍生的诊断证据和视觉证据进行基于证据的诊断推理。同时创建了CXReasonDial多轮对话基准，包含1,946个对话，涵盖12个诊断任务。

Result: CXReasonAgent能够产生基于证据的可靠响应，相比大型视觉语言模型，实现了更可靠、可验证的诊断推理。在12个诊断任务的多轮对话评估中表现出色。

Conclusion: 在安全关键的临床环境中，整合临床基础诊断工具对于实现可靠、可验证的诊断推理至关重要。CXReasonAgent展示了这种整合方法的有效性，为临床诊断提供了更可靠的解决方案。
...
TL;DR: 提出ODEBRAIN框架，使用神经ODE建模连续脑动态，通过时空频特征整合提升EEG预测性能
...
Method: ODEBRAIN框架：将时空频特征整合到谱图节点中，然后用神经ODE建模连续潜动态，确保潜表示能捕捉任意时间点的复杂脑状态随机变化

Result: 大量实验验证ODEBRAIN在EEG动态预测上显著优于现有方法，具有更强的鲁棒性和泛化能力

Conclusion: ODEBRAIN通过神经ODE建模连续潜动态，有效解决了传统方法的累积误差问题，能更好地捕捉EEG的瞬时非线性特征
...
<details>
  <summary>Details</summary>
Motivation: 研究KM信念更新与AGM信念修正之间的关系，通过模态逻辑框架形式化比较两者的公理系统，探索它们之间的包含关系。

Method: 为KM信念更新的每个公理提供对应的模态逻辑公理，使用包含三个模态算子（B、>、□）的模态逻辑。然后将得到的逻辑与从AGM公理转换而来的类似逻辑进行比较。

Result: 证明KM逻辑（L_KM）的每个公理都是AGM逻辑（L_AGM）的定理，因此AGM信念修正是KM信念更新的特例。对于强版本KM更新，两者的差异可归结为处理不令人惊讶信息的单个公理。

Conclusion: AGM信念修正可视为KM信念更新的特殊情况，两者在模态逻辑框架下具有明确的包含关系，强版本KM更新与AGM修正的差异仅在于处理初始不被怀疑信息的公理。
...
<details>
  <summary>Details</summary>
Motivation: 即使经过优化的AI模型也会因偶然性和认知不确定性而产生推理错误，观察到基于输入不变变换的多个样本推理中，错误会表现出部分独立性
...
TL;DR: GRAVE2、GRAVER和GRAVER2算法通过两层搜索、节点回收及其组合技术扩展GRAVE，在保持相同游戏强度的同时大幅减少存储节点数量


<details>
  <summary>Details</summary>
Motivation: GRAVE算法在通用游戏博弈中表现优异，但需要存储额外的胜率/访问统计数据，在内存受限环境中不实用，限制了实际应用

Method: 提出GRAVE2（两层搜索）、GRAVER（节点回收）和GRAVER2（两种技术结合）三种算法，通过减少存储节点来优化内存使用

Result: 这些增强技术能够大幅减少存储节点数量，同时保持与原始GRAVE算法相当的游戏强度

Conclusion: 通过两层搜索和节点回收技术，可以在内存受限环境中有效应用GRAVE算法，扩展了其实际适用性
...
TL;DR: LLM访问显著提升新手在生物安全相关任务上的表现，准确率比仅使用互联网的对照组高4.16倍，甚至在某些任务上超越专家，但用户未能充分发挥LLM潜力，且安全防护措施效果有限。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM在生物学基准测试上表现越来越好，但尚不清楚它们是否能真正提升新手用户的能力（即让人类比仅使用互联网资源时表现更好）。这种不确定性对于理解科学加速和双重用途风险至关重要。

Method: 进行了多模型、多基准的人类提升研究，比较了有LLM访问权限的新手与仅能使用互联网的新手在八个生物安全相关任务集上的表现。参与者有充足时间（最复杂的任务长达13小时）处理复杂问题。

Result: LLM访问提供了实质性提升：有LLM的新手准确率比对照组高4.16倍（95% CI [2.63, 6.87]）。在四个有专家基准（仅使用互联网）的任务中，有LLM的新手在三个任务上超越了专家。独立LLM的表现经常超过LLM辅助的新手，表明用户未能充分发挥LLM的潜力。大多数参与者（89.6%）报告在获取双重用途相关信息时几乎没有遇到困难，尽管有安全防护措施。
...
TL;DR: 提出一个基于多智能体LLM的交易框架，通过细粒度任务分解而非粗粒度指令来改进投资分析，在日股数据上验证了风险调整后收益的显著提升，并发现分析输出与决策偏好的对齐是系统性能的关键驱动因素。


<details>
  <summary>Details</summary>
Motivation: 当前基于多智能体的自主金融交易系统通常采用模仿分析师和经理角色的方法，但这些方法依赖抽象指令，忽略了真实世界工作流程的复杂性，导致推理性能下降和决策透明度降低。

Method: 提出一个多智能体LLM交易框架，将投资分析明确分解为细粒度任务，而非提供粗粒度指令。使用日本股票数据（价格、财务报表、新闻、宏观信息）在防泄漏的回测设置下进行评估，并进行标准的投资组合优化，利用与股票指数的低相关性和各系统输出的方差。
...
TL;DR: 分析具有单次和多次休假的无限缓冲区离散时间批量到达队列系统，其中客户分两阶段服务（基本服务和可选服务），推导了联合概率生成函数和性能分析。


<details>
  <summary>Details</summary>
Motivation: 离散时间排队系统在现代通信系统中具有重要应用，能够提供自适应数据包处理、拥塞控制、节能操作，并支持5G、物联网和边缘计算中的突发流量。需要分析具有复杂服务策略（两阶段服务加休假）的队列系统性能。

Method: 分析无限缓冲区离散时间批量到达队列，采用单次和多次休假策略，客户分两阶段服务：基本服务（FES）和可选服务（SOS）。推导了FES和SOS完成后等待传输和处理中数据包数量的双变量概率生成函数，建立了任意时隙的完整联合分布（包括休假完成状态）。

Result: 建立了完整的数学模型，包括联合概率生成函数和任意时隙的联合分布。通过数值示例验证了框架的适用性（包括离散相位型服务时间分布），并通过图形表示分析了关键参数对系统概率和性能指标的敏感性。

Conclusion: 该研究为具有复杂服务策略和休假机制的离散时间排队系统提供了全面的分析框架，能够有效评估现代通信系统中数据包处理、安全检查和节能操作的性能。
...
TL;DR: 该论文研究All-Reduce问题的计算速率，提供了基于割集的上界和基于时间共享的线性规划下界，针对特定网络类型给出了最优计算速率和已知最佳速率边界。


<details>
  <summary>Details</summary>
Motivation: 研究分布式系统中All-Reduce问题的计算速率优化，特别是在任意带宽的并行链路网络中，如何最大化每个网络使用周期内能够计算的和实例数量。

Method: 1. 提出割集上界；2. 基于时间共享的线性规划下界，针对先Reduce后Broadcast的方案；3. 将通用边界应用于特定网络结构（循环、完全、超立方体网络）。

Result: 1. 为一类通信网络提供了最优计算速率；2. 对于循环、完全和超立方体网络，获得了已知最佳速率边界，其中上界不超过下界的两倍。

Conclusion: 通过割集上界和线性规划下界的方法，能够有效分析All-Reduce问题的计算速率，为特定网络类型提供最优解，并为其他网络结构提供接近最优的边界。
...
TL;DR: 论文提出了一个将渐近推断与热力学框架相统一的描述，其中样本量和参数方差构成状态空间，香农信息扮演熵的角色，推导出类似热力学第一定律的平衡方程。


<details>
  <summary>Details</summary>
Motivation: 建立统计推断与热力学之间的深层联系，将信息论概念（如香农信息）纳入热力学框架，为统计推断提供新的物理视角和数学结构。

Method: 构建以样本量和参数方差为状态变量的热力学框架，将香农信息视为熵，引入积分因子组织信息变化，推导类似热力学定律的统计推断关系。

Result: 得到了类似第二定律的循环不等式、类似第三定律的熵下界、最优推断路径、信息增益的全局界限、类似卡诺效率的信息效率，以及de Bruijn恒等式和I-MMSE关系作为该框架的特例。

Conclusion: 统计推断与热力学共享深层数学结构，集合物理与推断物理构成在统一热力学描述中反向演化的影子过程，为信息论和统计推断提供了新的理论基础。
...
TL;DR: 提出一种可解释的波束对准框架，利用多模态数据构建虚拟基站，在混合视距和非视距环境中实现低开销的波束管理


<details>
  <summary>Details</summary>
Motivation: 大规模MIMO是6G网络高数据速率的关键使能技术，但其性能依赖于有效的波束管理，而传统方法存在训练开销大的问题。特别是在混合视距和非视距传播环境中，需要更高效的波束对准方案。

Method: 利用多模态数据（包括3D LiDAR点云）构建虚拟基站，这些VBS是基站相对于反射面的镜像，提供了无线环境的稀疏空间表示。基于VBS开发了波束对准方案，包括粗粒度信道重建和部分波束训练两个阶段。

Result: 数值结果表明，所提出的方法在频谱效率方面实现了接近最优的性能，显著降低了训练开销。

Conclusion: 该可解释框架通过利用环境几何信息构建虚拟基站，为混合传播环境中的波束对准提供了一种高效且接近最优的解决方案。
...
TL;DR: 本文研究语义通信中语义噪声的特定子类问题，在理想物理信道假设下，通过虚拟状态相关信道建模，分析语义编码器的表示能力，推导语义噪声下的容量结果和可达速率。


<details>
  <summary>Details</summary>
Motivation: 语义通信作为下一代网络的有前景范式，仍存在若干基本挑战未解决。本文针对语义噪声仅来自语义信道（假设理想物理信道）的特定子类问题进行研究，旨在深入理解语义通信中的容量和性能限制。

Method: 基于语义通信的概率模型，利用上下文概念，引入虚拟状态相关信道进行系统建模，其中状态（代表上下文）在通信中起关键作用。分析语义编码器的表示能力，探索语义噪声下的各种通信场景。

Result: 为部分情况推导了容量结果，为其他情况推导了可达速率，提供了语义噪声下语义通信的性能界限分析。

Conclusion: 通过虚拟状态相关信道建模和上下文概念，本文为语义通信中语义噪声的特定子类问题提供了理论分析框架，推导了容量界限，为下一代语义通信系统设计提供了理论基础。
...
TL;DR: 提出频率有序分词方法，通过利用自然语言词元的幂律分布（齐夫定律）来改进无损文本压缩。该方法使用BPE分词，重新排序词汇表使高频词元获得小整数标识符，然后用变长整数编码，再传递给标准压缩器。


<details>
  <summary>Details</summary>
Motivation: 自然语言文本中的词元遵循齐夫定律的幂律分布，即少数高频词元出现频率很高。传统压缩算法可能无法充分利用这种分布特性。通过预处理重新排序词汇表，使高频词元获得更小的整数标识符，可以改善压缩效果。

Method: 1. 使用字节对编码（BPE）对文本进行分词；2. 根据词元频率重新排序词汇表，使高频词元获得小整数标识符；3. 使用变长整数编码处理结果；4. 将预处理后的数据传递给任何标准压缩器（如zlib、LZMA、zstd）。

Result: 在enwik8（100MB维基百科）上，相比原始压缩：zlib提升7.08个百分点，LZMA提升1.69个百分点，zstd提升0.76个百分点（均包含词汇表开销）。在1GB规模（enwik9）以及中文和阿拉伯文文本上增益一致。预处理还加速了计算密集型压缩算法：包含预处理的总耗时比原始zstd-22快3.1倍，比原始LZMA快2.4倍。

Conclusion: 频率有序分词是一种简单有效的文本压缩预处理技术，能够显著提升多种标准压缩器的压缩率，同时加速计算密集型压缩算法。该方法实现简单（少于50行代码），在不同语言和规模下均表现良好。
...
TL;DR: FA-ISAC系统中通过优化天线位置和波束成形最大化多用户和保密率，相比固定天线系统提升20%以上


<details>
  <summary>Details</summary>
Motivation: 在FA辅助的ISAC系统中，当感知目标充当窃听者时，如何最大化系统和保密率尚未解决。流体天线的空间灵活性可增强物理层安全性。

Method: 使用块连续上界最小化(BSUM)算法，结合近端距离算法(PDA)进行闭式波束成形更新，以及外推投影梯度(EPG)进行天线位置向量优化。

Result: 仿真结果表明，提出的FA-ISAC方案相比固定位置天线系统实现了超过20%的和保密率增益。

Conclusion: 流体天线通过空间灵活性显著提升了ISAC系统的物理层安全性能，为安全传输提供了有效解决方案。
