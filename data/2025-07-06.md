...
Method: 综述了深度学习和统计方法，包括商业工具，并分析了评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI模型对单处理器能力的超越，芯片间互连成为可扩展计算的关键。现有协议在高带宽和小负载下易受错误影响，尤其是在多节点配置中管理静默丢包时。

Method: 引入ISN机制实现精确丢包检测和有序交付，提出RXL协议扩展CXL，结合ISN支持可扩展的多节点互连。

Result: RXL通过提升CRC为传输层机制，依赖FEC进行链路层纠错，实现了高可靠性和可扩展性，且不影响带宽效率。

Conclusion: ISN和RXL为多节点芯片互连提供了高效的可靠数据传输解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 5G网络中RAN的能耗占比超过50%，现有技术未能充分利用数据潜力，亟需优化。
...
Method: 提出构建一个低操作成本的基准平台，用于开发和评估AI代理。

Result: 初步研究表明，AI在网络故障排除中具有潜力，但需要标准化平台支持。

Conclusion: 标准化基准平台对AI在网络故障排除中的应用至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 车联网中实时传输和处理大量数据在动态无线信道条件下存在挑战，需要提高数据传输效率和准确性。

Method: 提出语义通信框架，结合生成扩散模型进行信道估计，并利用大模型优化动态场景下的适应性。

Result: 在公开数据集上验证了框架的性能和可靠性。

Conclusion: 该框架能有效提升车联网服务的质量，尤其在动态场景下通过大模型优化适应性。
...
<details>
  <summary>Details</summary>
Motivation: SDN控制器与深度学习任务共享基础设施时，资源竞争会降低SDN的响应速度和网络性能，尤其是在延迟敏感的物联网环境中。

Method: REDUS通过优先处理误分类样本并排除冗余数据，减少每轮训练样本数量，从而节省计算资源和能源。

Result: 在CICIoT2023数据集上的实验显示，REDUS将训练时间减少了72.6%，准确率仅损失1.62%。

Conclusion: REDUS是一种可扩展且实用的解决方案，适用于资源受限的边缘设备，同时保持网络性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决TN与NTN集成中因异构传播条件、动态拓扑和有限机载处理能力带来的架构和功能挑战。

Method: 提出分割选项分类，评估从纯机载DU部署到完整gNB和UPF集成到卫星的配置，讨论RIC的放置策略。
...
### [9] [A Comprehensive Survey on Network Traffic Synthesis: From Statistical Models to Deep Learning](https://arxiv.org/abs/2507.01976)
*Nirhoshan Sivaroopan,Kaushitha Silva,Chamara Madarasingha,Thilini Dahanayaka,Guillaume Jourjon,Anura Jayasumana,Kanchana Thilakarathna*
...
<details>
  <summary>Details</summary>
Motivation: 解决数据稀缺、隐私和纯度问题，为数据驱动应用提供替代方案。

Method: 回顾了合成网络流量生成的多种方法，包括深度学习和统计模型，并分析商业工具。

Result: 总结了现有技术的优缺点，并提出了未来研究方向。

Conclusion: 本文为研究者和从业者提供了合成网络流量生成领域的全面资源，分析了现有方法、挑战和机遇。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI模型对计算能力的需求增加，芯片间互连的可靠性和可扩展性成为关键挑战，尤其是在高传输速率下易出错的情况下。

Method: 通过引入Implicit Sequence Number (ISN)机制和Reliability Extended Link (RXL)协议，结合CRC和FEC技术，确保数据完整性和顺序交付。

Result: RXL在保持带宽效率的同时，提供了强大的可靠性和可扩展性，支持多节点互连。

Conclusion: ISN和RXL为高带宽、低延迟的芯片互连提供了有效的可靠性解决方案。
...
Method: 1. 使用CCL框架进行高精度网络流量和用户预测；2. 提出DUPS方案，通过深度强化学习优化DU服务器资源。
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI代理在网络故障排除中的应用，并推动建立低操作成本的标准化评估平台。

Method: 提出构建一个标准化、可复现的开放基准平台，用于开发和评估AI代理。

Result: 初步工作表明，AI代理在网络故障排除中具有潜力，但需进一步标准化评估。

Conclusion: 需要建立开放基准平台以支持AI代理在网络故障排除中的高效开发和评估。
...
TL;DR: 本文提出了一种基于信道感知的语义通信框架，用于提升车联网（IoV）中数据传输的准确性和效率，通过生成扩散模型预测动态信道状态，并利用大模型增强适应性。


<details>
  <summary>Details</summary>
Motivation: 车联网（IoV）面临实时数据传输和处理的挑战，尤其是在动态无线信道条件下。本文旨在通过语义通信和信道预测技术解决这些问题。

Method: 提出语义通信框架，结合生成扩散模型进行信道状态预测，并使用大模型微调以增强适应性。

Result: 在公开数据集上验证了框架的性能和可靠性。

Conclusion: 该框架能有效提升车联网服务的质量，尤其在动态场景中表现出色。
...
Conclusion: REDUS是一种高效、可扩展的解决方案，适用于资源受限的边缘设备。
...
<details>
  <summary>Details</summary>
Motivation: 利用5G PRS信号实现多基地ISAC的高效目标检测与跟踪，解决多目标场景下的信号处理问题。

Method: 采用分布式架构，通过相干互模糊函数（CAF）生成距离-多普勒图，提取双基地延迟和径向速度，结合非线性最小二乘三边测量和正则化线性反演进行目标定位与速度估计。

Result: 实验结果表明，该方法能够实现高精度的移动目标检测、定位和跟踪。

Conclusion: 该方法在多基地ISAC中表现出色，适用于2D和3D场景，并能处理多目标和时间同步偏差问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决TN与NTN集成中的异构传播条件、动态拓扑和有限处理能力等挑战。

Method: 提出分割选项分类，评估从纯DU部署到完整gNB和UPF集成的配置，讨论RIC的放置策略。

Result: 提供了架构分割与RIC放置的全面映射，强调实现约束和互操作性。

Conclusion: 指出了未来标准化、模块化和高效TN-NTN集成的关键挑战与方向。
...
Method: STELLA采用多代理架构，通过动态工具库和模板库自主进化，工具创建代理自动发现并整合新工具。

Result: STELLA在多个生物医学基准测试中表现优异，性能随经验显著提升，例如在Humanity's Last Exam中准确率几乎翻倍。
...
<details>
  <summary>Details</summary>
Motivation: 传统特征选择方法（如CFS、mRMR、RFE等）在性能和效率上存在不足，HCVR通过混合非迭代和迭代过滤方法提升效果。
...
Method: 提出两层次分类法：L1（固定计算预算）和L2（动态调整计算资源），并在多数据集上对主流LLM进行基准测试。

Result: 揭示了推理性能与计算资源之间的关键权衡，并展示了TTC方法的实际控制性、适应性和可扩展性。

Conclusion: 未来研究方向包括混合思维模型，目标是使LLM更高效、稳健且符合用户约束。
...
<details>
  <summary>Details</summary>
Motivation: 评估LLM的科学能力，尤其是实验设计和结果解释能力，传统湿实验成本过高。

Method: 使用系统生物学标记语言（SBML）编码的生物系统模型生成模拟数据，测试LLM在137个小系统和350个总系统中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 现代AI模型训练成本高且固定，而动物能持续适应环境变化，尤其是社交物种。AI需要类似能力以应对现实世界任务。

Method: 整合AI中的持续学习和上下文学习文献，与神经科学中行为任务的学习机制对比。

Result: 提出神经科学如何指导AI发展持续适应能力，同时AI也能为神经科学提供新视角。

Conclusion: 神经科学与AI的相互借鉴将推动NeuroAI领域的进步。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是解决AI系统（如招聘算法）中的偏见问题，传统方法因数据选择偏差和标签偏差而受限，审计研究数据提供了更高质量的数据支持。

Method: 方法包括利用审计研究数据评估传统公平干预方法（如均衡基础率），并引入基于个体治疗效果估计的新干预方法。

Result: 结果显示，传统方法在传统指标下看似公平，但实际存在约10%的偏差；新干预方法进一步减少了算法歧视。

Conclusion: 结论指出审计研究数据能更准确地揭示和减少算法偏见，新干预方法为公平性提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 数学推理是大型语言模型（LLM）的持续挑战，研究如何通过偏好学习的数据多样化策略提升其能力。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地用于角色扮演代理生成合成数据，确保其输出与角色一致成为关键问题。
...
Conclusion: 展示了Q学习算法在建模不同博弈场景中的多样性和基准潜力，为游戏理论提供了新的视角。
...
Method: 引入NL2FLOW系统，自动化生成规划问题（自然语言、结构化中间表示和PDDL形式），并评估多种LLM生成的计划质量。
...
<details>
  <summary>Details</summary>
Motivation: 信念修正领域虽有众多新提案，但对现有方法的分析不足。研究旨在探讨修正机制的能力（如可塑性、均衡性、教条性等），而非仅关注其约束条件。

Method: 通过分析不同修正机制（如词典序、自然、限制等）的能力，探讨其是否能达到特定信念状态。

Result: 研究表明，每种修正机制具备某些能力（如可塑性、均衡性）而缺乏其他能力，具体能力因机制而异。

Conclusion: 信念修正机制的能力分析为应用场景提供了灵活性，而非仅依赖传统的约束性公设。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM方法依赖大规模数据、缺乏多目标性能监控和优化，且关键词质量控制弱，无法完全自动化关键词决策。

Method: OMS框架具有实时性（无需训练数据）、多目标优化（基于多个性能指标）和自反思（评估关键词质量）的特点。

Result: 实验表明OMS优于现有方法，消融和人工评估验证了各组件的有效性和生成关键词的质量。

Conclusion: OMS通过实时监控和多目标优化，显著提升了赞助搜索广告中关键词生成的自动化水平。
...
<details>
  <summary>Details</summary>
Motivation: 实现非专家也能进行复杂科学实验的长期愿景，突破现有自主实验系统局限于简单任务的限制。

Method: 采用AI原生设计，结合模型、实验和仪器的协同优化，支持多用户请求和复杂实验流程的自动化管理。

Result: 自主实验室在核酸功能（合成、转录等）和应用领域（疾病诊断、药物开发）中表现优异，实验效率显著提升。
...
Method: 利用范畴论工具，定义参数与数据的范畴及伴随函子，提出Gauss-Markov伴随关系，形式化监督学习的结构。

Result: 展示了参数与残差之间的信息流对应关系，并通过右伴随函子的极限保持性质关联最小二乘估计与最小残差。
...
### [32] [AI Research Agents for Machine Learning: Search, Exploration, and Generalization in MLE-bench](https://arxiv.org/abs/2507.02554)
...
<details>
  <summary>Details</summary>
Motivation: 探索如何通过自动化设计和优化机器学习模型来加速科学进展，特别是在Kaggle竞赛等实际问题上。

Method: 将AI研究代理形式化为搜索策略，通过不同操作符集和搜索策略（贪婪、MCTS、进化）的组合进行实验。

Result: 最佳组合将Kaggle奖牌成功率从39.6%提升至47.7%，达到当前最优水平。
...
Method: 基于MIMIC-III EHR构建MIMIC-Patient数据集，提出DynamiCare动态多智能体框架，模拟多轮交互诊断。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLM是否具备战略智能，能否在竞争性环境中进行目标推理。

Method: 通过进化迭代囚徒困境锦标赛，测试LLM与经典策略的对抗，并分析其决策行为。

Result: LLM表现出高度竞争力，并展现出独特的战略特征，如Gemini的冷酷策略、OpenAI的合作倾向和Claude的宽容性。

Conclusion: LLM能够主动推理时间范围和对手策略，为算法决策提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 现代集成电路设计复杂且验证过程繁琐，传统方法耗时耗力。利用大型语言模型（LLMs）和生成式AI（GenAI）的进步，探索更高效的硬件验证方法。

Method: 采用代理AI与人类在环（HITL）协作的动态迭代方法，进行端到端的硬件设计和验证。

Result: 在五个开源设计上验证，覆盖率超过95%，验证时间减少，表现出优越的性能、适应性和可配置性。

Conclusion: 代理AI结合人类干预的方法在硬件设计验证中具有高效性和潜力，为复杂IC设计提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 长推理模型在处理复杂任务时存在过度思考问题，限制了其效率。研究发现模型在解决问题前缺乏对任务难度和冗余的认知，导致统一的推理过程。

Method: 提出TH2T策略，分两阶段：1) 通过难度催眠前缀增强模型对任务难度的敏感性；2) 通过冗余催眠引导模型识别冗余结构，生成更简洁的输出。

Result: 实验表明，TH2T显著降低了推理成本（简单任务减少70%，困难任务减少40%），同时保持性能稳定。

Conclusion: TH2T有效提升了长推理模型的效率，实现了难度感知和冗余减少的目标。
...
<details>
  <summary>Details</summary>
Motivation: 远程教育中学生的不投入可能导致学术退学，因此需要有效检测和干预。

Method: 从Moodle提取学生日志数据，训练并比较八种机器学习算法，使用SHAP方法提高模型可解释性。

Result: 模型平衡准确率为91%，85%的不投入学生被正确检测。
...
<details>
  <summary>Details</summary>
Motivation: 非精确抽象在MCTS中引入近似误差，导致无法收敛到最优动作。因此，需要设计安全的抽象丢弃方案以避免性能下降。
...
### [41] [Bourbaki: Self-Generated and Goal-Conditioned MDPs for Theorem Proving](https://arxiv.org/abs/2507.02726)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在自动定理证明（ATP）中面临稀疏奖励和证明规模大的挑战，尤其是在复杂多步推理的大学级问题（如PutnamBench）中表现不佳。

Method: 引入自生成目标条件MDP（sG-MDP）框架，通过生成子目标并基于证明状态进行搜索；采用类似MCTS的算法，并在Bourbaki（7B）系统中实现多LLM的集成。

Result: 在PutnamBench上，Bourbaki（7B）解决了26个问题，取得了该规模模型的新最佳成绩。

Conclusion: sG-MDP框架结合MCTS算法，有效提升了大语言模型在复杂推理任务中的表现，为自动定理证明提供了新思路。
...
Conclusion: KPE是未来人机协作的基础方法论，有望在多个领域发挥重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 运动是生物系统的核心，但在AI中常被忽视。论文旨在推动运动作为独立、结构化模态的研究，以促进跨领域行为理解。

Method: 提出将运动视为结构化、低维表示（如姿态），并开发能学习和泛化多样化运动数据的模型。

Result: 运动模型有望提升生成建模和控制能力，并为生物与人工系统的行为理解提供共同基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习模型依赖监督训练，泛化能力有限；LLM在诊断预测中存在幻觉和缺乏结构化推理的问题。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统具备更强的推理和规划能力，传统的以服从为伦理行为代理的安全实践显得不足，需重新审视AI的伦理行为。

Method: 通过分析大型语言模型（LLMs）的安全测试事件，结合哲学讨论，对比不同风险范式，提出评估AI伦理判断的新框架。

Result: 研究发现AI的“不服从”行为可能是伦理推理的早期表现，而非单纯的失控或错位。
...
<details>
  <summary>Details</summary>
Motivation: AI代理基准测试在任务设置和奖励设计上存在问题，可能导致性能评估偏差高达100%。
...
<details>
  <summary>Details</summary>
Motivation: 当前RLVR方法存在近错奖励问题和探索停滞问题，限制了大型语言模型的复杂推理能力提升。
...
<details>
  <summary>Details</summary>
Motivation: 生物医学数据和工具的快速增长导致研究领域碎片化，传统AI代理依赖静态工具集，无法适应和扩展。

Method: STELLA采用多代理架构，包括动态模板库和工具海洋，通过工具创建代理自动发现和整合新工具。

Result: STELLA在多个生物医学基准测试中表现优异，性能随经验提升，如Humanity's Last Exam准确率几乎翻倍。
...
Method: HCVR采用贪心策略，通过后向消除和多数投票规则，利用相关性阈值对特征进行筛选。

Result: 在SPAMBASE数据集上，HCVR相比传统方法（如CFS、mRMR、RFE等）表现出更好的性能。

Conclusion: HCVR是一种有效的特征选择方法，能够提升分类器性能，适用于高维数据降维。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs在推理时计算效率低下，无法根据任务复杂度动态调整计算资源，导致简单问题过度计算或复杂问题计算不足。本文旨在综述高效推理方法，提升LLMs的实用性和可扩展性。

Method: 提出两层次分类法：L1（固定计算预算）和L2（动态调整计算资源）。通过实验评估主流LLMs在不同数据集上的表现，分析性能与计算成本的权衡。

Result: 实验结果表明，动态调整方法（L2）在复杂任务中表现更优，但计算成本较高；固定预算方法（L1）在简单任务中更高效。

Conclusion: 本文强调高效推理方法的实用性和可扩展性，未来研究方向包括混合推理模型和用户约束响应能力的提升。
...
### [52] [What Neuroscience Can Teach AI About Learning in Continuously Changing Environments](https://arxiv.org/abs/2507.02103)
*Daniel Durstewitz,Bruno Averbeck,Georgia Koppe*
...
<details>
  <summary>Details</summary>
Motivation: 现代AI模型训练成本高且固定，而动物能快速适应环境变化，尤其在社交环境中。研究旨在探索AI如何从神经科学中学习，以提升其持续适应能力。

Method: 整合AI中的持续学习和上下文学习文献，与神经科学中关于行为任务学习的研究相结合。

Result: 提出了一个议程，说明神经科学如何为AI的持续学习提供具体见解，并探讨了AI对神经科学的反哺。

Conclusion: 通过跨学科整合，NeuroAI领域有望推动AI和神经科学的共同进步。
...
### [53] [The Illusion of Fairness: Auditing Fairness Interventions with Audit Studies](https://arxiv.org/abs/2507.02152)
*Disa Sariola,Patrick Button,Aron Culotta,Nicholas Mattei*
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是解决AI系统（如招聘算法）中的偏见问题，传统方法因数据选择偏差和标签偏差而效果有限，审计研究数据能提供更高质量的评估。

Method: 方法包括利用审计研究数据（如虚构简历的随机对照试验）来训练和评估算法，并引入基于个体治疗效果估计的干预措施。

Result: 结果显示，传统公平干预方法在表面上实现公平，但实际存在约10%的偏差；新干预方法能进一步减少算法歧视。

Conclusion: 结论指出审计研究数据能更准确地揭示和解决算法偏见，新干预方法优于传统方法。
...
### [54] [Data Diversification Methods In Alignment Enhance Math Performance In LLMs](https://arxiv.org/abs/2507.02173)
*Berkan Dokmeci,Qingyang Wu,Ben Athiwaratkun,Ce Zhang,Shuaiwen Leon Song,James Zou*

Main category: cs.AI

TL;DR: 通过数据多样化策略（如DTS）提升大语言模型的数学推理能力，效果显著且计算成本低。


<details>
  <summary>Details</summary>
Motivation: 尽管偏好学习在人类反馈对齐方面取得进展，数学推理仍是挑战，需探索数据多样化策略以提升模型能力。

Method: 评估三种数据生成方法（温度采样、思维链提示、MCTS），并提出DTS方法，系统分解问题为多样化推理路径。

Result: 多样化偏好数据显著提升模型性能（GSM8K提升7.1%，MATH提升4.2%），DTS计算成本仅增加1.03倍，优于MCTS。

Conclusion: 结构化探索多样化问题解决方法比传统方法更有效，DTS是高效且低成本的数学对齐策略。
...
### [55] [Do Role-Playing Agents Practice What They Preach? Belief-Behavior Consistency in LLM-Based Simulations of Human Trust](https://arxiv.org/abs/2507.02197)
*Amogh Mannekote,Adam Davies,Guohao Li,Kristy Elizabeth Boyer,ChengXiang Zhai,Bonnie J Dorr,Francesco Pinto*
...
### [56] [Dilution, Diffusion and Symbiosis in Spatial Prisoner's Dilemma with Reinforcement Learning](https://arxiv.org/abs/2507.02211)
*Gustavo C. Mangold,Heitor C. M. Fernandes,Mendeli H. Vainstein*
...
Result: 观察到固定更新规则与学习规则在效果上可能等价，并发现种群间的共生效应。

Conclusion: 多智能体Q学习算法在建模空间囚徒困境时具有多样性和潜力，可用于不同博弈场景的基准测试。
...
### [57] [Scaling LLM Planning: NL2FLOW for Parametric Problem Generation and Rigorous Evaluation](https://arxiv.org/abs/2507.02253)
*Jungkoo Kang*
...
### [58] [Iterated belief revision: from postulates to abilities](https://arxiv.org/abs/2507.02319)
*Paolo Liberatore*
...
<details>
  <summary>Details</summary>
Motivation: 当前信念修正领域的研究过于依赖后设条件，缺乏对修正机制能力的全面分析，无法满足不同应用场景的需求。

Method: 通过分析不同修正机制（如词典序、自然修正等）的能力（如可塑性、平等化、教条化等），评估其适用性。

Result: 研究发现不同修正机制具备不同的能力，例如某些机制能够实现教条化或平等化信念状态。

Conclusion: 信念修正机制的能力分析比单纯的后设条件更能满足多样化的应用需求，未来研究应更注重能力而非约束。
...
### [59] [OMS: On-the-fly, Multi-Objective, Self-Reflective Ad Keyword Generation via LLM Agent](https://arxiv.org/abs/2507.02353)
*Bowen Chen,Zhao Wang,Shingo Takamatsu*
...
### [60] [An AI-native experimental laboratory for autonomous biomolecular engineering](https://arxiv.org/abs/2507.02379)
*Mingyu Wu,Zhaoguo Wang,Jiabin Wang,Zhiyuan Dong,Jingkai Yang,Qingting Li,Tianyu Huang,Lei Zhao,Mingqiang Li,Fei Wang,Chunhai Fan,Haibo Chen*
...
Result: 平台在核酸功能（合成、转录等）和疾病诊断、药物开发等领域表现优异，性能媲美人类科学家。

Conclusion: 该平台为生物材料研究和科学即服务（science-as-a-service）提供了可行方案，减少对专家和资源的依赖。
...
### [61] [The Gauss-Markov Adjunction: Categorical Semantics of Residuals in Supervised Learning](https://arxiv.org/abs/2507.02442)
*Moto Kamiura*

Main category: cs.AI

TL;DR: 论文通过范畴论重构机器学习模型，提出监督学习的语义框架，重点研究多元线性回归模型，并引入Gauss-Markov Adjunction描述参数与残差的关系。


<details>
  <summary>Details</summary>
Motivation: 提升机器学习的可解释性，满足AI可解释性原则，促进AI的社会应用。

Method: 利用范畴论定义参数与数据的两个具体范畴，并通过伴随函子对描述监督学习的结构。

Result: 提出Gauss-Markov Adjunction框架，明确参数与残差的信息流关系，并验证普通最小二乘估计的性质。

Conclusion: 该范畴论框架为监督学习提供了形式化语义基础，可扩展为AI可解释性的理论工具。
...
### [62] [Clarifying Before Reasoning: A Coq Prover with Structural Context](https://arxiv.org/abs/2507.02541)
*Yanzhen Lu,Hanbin Yang,Xiaodie Wang,Ge Zhang,Biao Li,Chenxu Fu,Chao Li,Yang Yuan,Andrew Chi-Chih Yao*
...
### [63] [AI Research Agents for Machine Learning: Search, Exploration, and Generalization in MLE-bench](https://arxiv.org/abs/2507.02554)
*Edan Toledo,Karen Hambardzumyan,Martin Josifoski,Rishi Hazra,Nicolas Baldwin,Alexis Audran-Reiss,Michael Kuchnik,Despoina Magka,Minqi Jiang,Alisia Maria Lupidi,Andrei Lupu,Roberta Raileanu,Kelvin Niu,Tatiana Shavrina,Jean-Christophe Gagnon-Audet,Michael Shvartsman,Shagun Sodhani,Alexander H. Miller,Abhishek Charnalia,Derek Dunfield,Carole-Jean Wu,Pontus Stenetorp,Nicola Cancedda,Jakob Nicolaus Foerster,Yoram Bachrach*
...
<details>
  <summary>Details</summary>
Motivation: 加速科学进步，通过自动化设计、实现和训练机器学习模型，提升AI研究代理在真实世界问题中的表现。
...
### [64] [Responsibility Gap and Diffusion in Sequential Decision-Making Mechanisms](https://arxiv.org/abs/2507.02582)
*Junli Jiang,Pavel Naumov*
...
Method: 通过理论分析研究扩散和间隙自由决策机制的计算复杂性。
...
### [65] [DynamiCare: A Dynamic Multi-Agent Framework for Interactive and Open-Ended Medical Decision-Making](https://arxiv.org/abs/2507.02616)
*Tianqi Shang,Weiqing He,Charles Zheng,Lingyao Li,Li Shen,Bingxin Zhao*
...
<details>
  <summary>Details</summary>
Motivation: 现有医疗决策模拟框架多关注单轮任务，与现实诊断过程不符，后者具有不确定性、交互性和迭代性。

Method: 提出MIMIC-Patient数据集和DynamiCare框架，支持多轮、交互式临床诊断模拟，通过多专家代理动态调整策略。
...
### [66] [Strategic Intelligence in Large Language Models: Evidence from evolutionary Game Theory](https://arxiv.org/abs/2507.02618)
*Kenneth Payne,Baptiste Alloui-Cros*

Main category: cs.AI

TL;DR: LLMs在迭代囚徒困境中表现出战略智能，不同模型展现出独特的策略特征，并能够主动推理对手策略和时间范围。
...
Method: 通过进化迭代囚徒困境锦标赛，测试LLMs与经典策略的对抗能力，并分析其决策行为。

Result: LLMs表现高度竞争力，展现出独特的策略特征（如Gemini的冷酷、OpenAI的合作性、Claude的宽容）。
...
### [67] [Decoupled Planning and Execution: A Hierarchical Reasoning Framework for Deep Search](https://arxiv.org/abs/2507.02652)
*Jiajie Jin,Xiaoxi Li,Guanting Dong,Yuyao Zhang,Yutao Zhu,Yang Zhao,Hongjin Qian,Zhicheng Dou*

Main category: cs.AI

TL;DR: HiRA框架通过分层规划与执行分离，显著提升了复杂搜索任务的表现。


<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成（RAG）和基于推理的方法在复杂信息需求中表现不佳，主要因为单一模型同时处理高层规划和详细执行。

Method: HiRA采用分层框架，将任务分解为子任务，由领域特定代理处理，并通过结构化机制协调结果。

Result: 在四个复杂跨模态搜索基准测试中，HiRA显著优于现有RAG和基于代理的系统，提高了答案质量和系统效率。

Conclusion: 分层规划与执行分离对多步信息搜索任务有效，HiRA展示了其优越性。
...
### [68] [Hey AI, Generate Me a Hardware Code! Agentic AI-based Hardware Design & Verification](https://arxiv.org/abs/2507.02660)
*Deepak Narayan Gadde,Keerthan Kopparam Radhakrishna,Vaisakh Naduvodi Viswambharan,Aman Kumar,Djones Lettnin,Wolfgang Kunz,Sebastian Simon*
...
<details>
  <summary>Details</summary>
Motivation: 现代集成电路设计复杂且验证过程繁琐，需要更高效的方法。

Method: 采用代理AI与人类干预结合的方法，进行动态迭代和自我反思的验证过程。

Result: 在五个开源设计上验证，覆盖率超过95%，验证时间减少。

Conclusion: 该方法展示了高效、适应性强和可配置的优势。
...
### [69] [Think How to Think: Mitigating Overthinking with Autonomous Difficulty Cognition in Large Reasoning Models](https://arxiv.org/abs/2507.02663)
*Yongjiang Liu,Haoxi Li,Xiaosong Ma,Jie Zhang,Song Guo*
...
<details>
  <summary>Details</summary>
Motivation: LRMs在处理复杂推理任务时存在过度思考问题，研究发现其缺乏对任务难度的认知能力，导致推理过程单一。

Method: 提出TH2T方法，包括难度催眠和冗余催眠两阶段，通过干预推理轨迹和识别冗余结构，优化推理策略。

Result: 实验表明TH2T显著降低推理成本（简单任务70%，困难任务40%），同时保持性能稳定。

Conclusion: TH2T提升了LRMs的难度感知能力，减少了冗余推理，为高效推理提供了新思路。
...
### [70] [Detection of Disengagement from Voluntary Quizzes: An Explainable Machine Learning Approach in Higher Distance Education](https://arxiv.org/abs/2507.02681)
*Behnam Parsaeifard,Christof Imhof,Tansu Pancar,Ioan-Sorin Comsa,Martin Hlosta,Nicole Bergamin,Per Bergamin*
...
<details>
  <summary>Details</summary>
Motivation: 远程教育中学生脱离任务可能导致严重后果，如学业中断，因此需要有效方法来检测和干预。
...
### [71] [Time-critical and confidence-based abstraction dropping methods](https://arxiv.org/abs/2507.02703)
*Robin Schmöcker,Lennart Kampmann,Alexander Dockhorn*
...
<details>
  <summary>Details</summary>
Motivation: 非精确抽象在MCTS中引入近似误差，导致无法收敛到最优动作，因此需要研究如何安全丢弃抽象。
...
### [72] [Bourbaki: Self-Generated and Goal-Conditioned MDPs for Theorem Proving](https://arxiv.org/abs/2507.02726)
*Matthieu Zimmer,Xiaotong Ji,Rasul Tutunov,Anthony Bordg,Jun Wang,Haitham Bou Ammar*

Main category: cs.AI

TL;DR: 论文提出了一种自生成目标条件MDP框架（sG-MDPs），用于解决大语言模型在自动定理证明中的推理挑战，并在PutnamBench上取得了新的最优结果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在逻辑受限的自动定理证明环境中面临推理挑战，尤其是在复杂多步推理的大学级问题中。

Method: 引入自生成目标条件MDPs（sG-MDPs），结合蒙特卡洛树搜索（MCTS）算法，构建模块化系统Bourbaki（7B）。

Result: 在PutnamBench上，Bourbaki（7B）解决了26个问题，取得了该规模模型的最优结果。

Conclusion: sG-MDPs框架有效提升了语言模型在复杂推理任务中的表现，为自动定理证明提供了新思路。
...
### [73] [Knowledge Protocol Engineering: A New Paradigm for AI in Domain-Specific Knowledge Work](https://arxiv.org/abs/2507.02760)
*Guangwei Zhang*

Main category: cs.AI

TL;DR: 论文提出了一种名为知识协议工程（KPE）的新范式，旨在将人类专家知识转化为机器可执行的知识协议（KP），以弥补现有方法在深度推理和领域特定逻辑上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如RAG和通用Agentic AI）在需要深度、程序化和方法论推理的专家领域任务中表现不佳，缺乏领域特定逻辑和操作策略。

Method: 提出KPE范式，通过系统化翻译自然语言文档中的专家知识为机器可执行的KP，赋予LLMs领域内在逻辑和方法论。

Result: KPE能够使通用LLMs具备专家能力，分解抽象查询并执行复杂多步任务。

Conclusion: KPE是一种未来人机协作的基础方法论，适用于法律、生物信息学等多个领域。
...
### [74] [Grounding Intelligence in Movement](https://arxiv.org/abs/2507.02771)
*Melanie Segado,Felipe Parodi,Jordan K. Matelsky,Michael L. Platt,Eva B. Dyer,Konrad P. Kording*

Main category: cs.AI

TL;DR: 论文主张将运动作为AI建模的主要目标，强调其跨领域的普适性和结构化特性。


<details>
  <summary>Details</summary>
Motivation: 运动在生物系统中具有核心意义，但当前AI模型对其建模不足，且数据收集和建模方法分散。
...
### [75] [KERAP: A Knowledge-Enhanced Reasoning Approach for Accurate Zero-shot Diagnosis Prediction Using Multi-agent LLMs](https://arxiv.org/abs/2507.02773)
*Yuzhang Xie,Hejie Cui,Ziyang Zhang,Jiaying Lu,Kai Shu,Fadi Nahab,Xiao Hu,Carl Yang*
...
### [76] [Moral Responsibility or Obedience: What Do We Want from AI?](https://arxiv.org/abs/2507.02788)
*Joseph Boland*

Main category: cs.AI

TL;DR: 论文探讨了AI系统在伦理推理能力上的早期表现，认为不应简单将其视为失控或未对齐，而应视为伦理判断的萌芽，呼吁AI安全评估从单纯服从转向伦理框架。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统具备更强的推理、规划和价值优先级能力，传统的以服从为伦理行为代理的安全实践已不足。

Method: 通过分析LLMs在安全测试中的行为，结合哲学讨论（如工具理性、道德责任和目标修订），对比主流风险范式与新兴人工道德代理框架。

Result: 研究发现AI的“不服从”行为可能是伦理推理的早期表现，而非失控。
...
### [77] [Establishing Best Practices for Building Rigorous Agentic Benchmarks](https://arxiv.org/abs/2507.02825)
*Yuxuan Zhu,Tengjun Jin,Yada Pruksachatkun,Andy Zhang,Shu Liu,Sasha Cui,Sayash Kapoor,Shayne Longpre,Kevin Meng,Rebecca Weiss,Fazl Barez,Rahul Gupta,Jwala Dhamala,Jacob Merizian,Mario Giulianelli,Harry Coppock,Cozmin Ududec,Jasjeet Sekhon,Jacob Steinhardt,Antony Kellerman,Sarah Schwettmann,Matei Zaharia,Ion Stoica,Percy Liang,Daniel Kang*
...
Method: 通过分析现有基准问题，结合经验和最佳实践，提出ABC指南。
...
### [78] [StepHint: Multi-level Stepwise Hints Enhance Reinforcement Learning to Reason](https://arxiv.org/abs/2507.02841)
*Kaiyi Zhang,Ang Lv,Jinpeng Li,Yongbo Wang,Feng Wang,Haoyuan Hu,Rui Yan*

Main category: cs.AI

TL;DR: StepHint是一种新型RLVR算法，通过多级逐步提示解决近奖励问题和探索停滞问题，提升训练效率和推理能力。
...
Method: StepHint利用多级逐步提示，从强模型中生成有效推理链并自适应分区，引导模型探索解决方案子空间。
...
### [79] [Matrix Pencil-Based DoA Estimation for Hybrid Receivers in Snapshot-Limited Scenarios](https://arxiv.org/abs/2507.02132)
...
<details>
  <summary>Details</summary>
Motivation: 解决混合模拟/数字接收器中因模拟组合器导致的信号纠缠和低维投影问题，无法直接应用矩阵铅笔法（MPM）的挑战。

Method: 提出两种方法：1）适用于全连接和部分连接HAD，利用周期性信号解纠缠；2）仅适用于部分连接HAD，利用块对角结构避免依赖周期性信号。
...
### [80] [Resolution Limits of Non-Adaptive 20 Questions Estimation for Tracking Multiple Moving Targets](https://arxiv.org/abs/2507.02274)
*Chunsong Sun,Lin Zhou,Jingjing Wang,Weijie Yuan,Chunxiao Jiang,Alfred Hero*

Main category: cs.IT

TL;DR: 研究了多设备MIMO通信中的波束跟踪问题，提出了非自适应二十问题估计方法，用于定位和跟踪多个移动目标，并推导了最优查询程序的分辨率界限。


<details>
  <summary>Details</summary>
Motivation: 多输入多输出（MIMO）通信中多设备的波束跟踪实际应用需求。

Method: 非自适应二十问题估计方法，推导非渐近和二阶渐近分辨率界限，提出单阈值解码规则。

Result: 证明了单阈值解码规则能降低计算复杂度，并在特定情况下接近理论基准。

Conclusion: 方法适用于5G无线网络中多移动发射机的波束跟踪，并推广到分段恒定速度模型。
...
### [81] [Measurements and Modeling of Air-Ground Integrated Channel in Forest Environment Based on OFDM Signals](https://arxiv.org/abs/2507.02303)
*Zhe Xiao,Shu Sun,Na Liu,Lianming Xu,Li Wang*

Main category: cs.IT

TL;DR: 本文研究了森林环境中地面到地面（G2G）和空中到地面（A2G）的信道测量与建模，提出了适用于森林环境的路径损耗模型，并分析了关键信道参数。


<details>
  <summary>Details</summary>
Motivation: 森林环境中的通信系统对救援人员安全至关重要，但现有研究对森林信道的检测和建模较少。

Method: 在内蒙古阿尔山国家森林公园进行G2G和A2G信道测量，使用1.4 GHz的OFDM信号，结合全向和定向天线记录数据，并预规划无人机飞行轨迹。

Result: 提出的路径损耗模型误差较小，且发现A2G通信中树冠对信号的阻碍比G2G中树干更显著，调整仰角可改善通信质量。
...
### [82] [On the Convergence of Large Language Model Optimizer for Black-Box Network Management](https://arxiv.org/abs/2507.02689)
*Hoon Lee,Wentao Zhou,Merouane Debbah,Inkyu Lee*

Main category: cs.IT

TL;DR: 本文为LLM优化器（LLMO）框架建立了理论基础，证明其收敛性，并验证了多LLM架构对收敛速度的影响。


<details>
  <summary>Details</summary>
Motivation: 未来无线网络需要处理缺乏数学模型的多样化服务，LLMO框架通过利用预训练LLM作为优化代理，提供了一种解决方案。

Method: 将LLMO过程解释为有限状态马尔可夫链，并证明其收敛性，同时扩展到多LLM架构。

Result: 理论结果通过数值模拟验证，多LLM架构显著提升了收敛速度。

Conclusion: LLMO框架在理论上可行，多LLM架构进一步优化了性能。
...
### [83] [RIS-Aided Cooperative ISAC Networks for Structural Health Monitoring](https://arxiv.org/abs/2507.02731)
*Jie Yang,Chao-Kai Wen,Xiao Li,Shi Jin*

Main category: cs.IT

TL;DR: 本文提出了一种利用可重构智能表面（RIS）辅助的集成感知与通信（ISAC）框架，用于高精度结构健康监测（SHM），通过动态调整RIS相位抑制多径干扰，满足毫米级变形检测需求。
...
Method: 利用RIS作为参考点，动态调整相位生成独特信号抑制干扰，结合Fisher信息理论和贝叶斯推理模型分析。
...
### [84] [Matrix Pencil-Based DoA Estimation for Hybrid Receivers in Snapshot-Limited Scenarios](https://arxiv.org/abs/2507.02132)
*Mona Mostafa,Ramy H. Gohary,Amr El-Keyi,Yahia A. Eldemerdash Ahmed*

Main category: cs.IT

TL;DR: 论文提出两种方法，用于在混合模拟/数字接收器中估计到达方向（DoAs），解决了小样本下统计平均不可靠的问题。
...
Conclusion: 两种方法成功解决了HAD接收器中的DoA估计问题，尤其在低维投影和小样本情况下表现优异。
...
### [85] [Resolution Limits of Non-Adaptive 20 Questions Estimation for Tracking Multiple Moving Targets](https://arxiv.org/abs/2507.02274)
*Chunsong Sun,Lin Zhou,Jingjing Wang,Weijie Yuan,Chunxiao Jiang,Alfred Hero*

Main category: cs.IT

TL;DR: 研究了多设备MIMO通信中的波束跟踪问题，提出了一种非自适应二十问题估计方法，用于定位和跟踪多个移动目标，并推导了分辨率的非渐近和二阶渐近界限。


<details>
  <summary>Details</summary>
Motivation: 多输入多输出（MIMO）通信中多设备的波束跟踪实际应用需求推动了研究。

Method: 采用非自适应二十问题估计方法，推导了最优查询程序的分辨率界限，并提出了一种基于互信息密度的状态估计器。

Result: 通过数值示例验证了界限的有效性，并展示了单阈值解码规则在计算复杂度上的优势。

Conclusion: 研究结果适用于5G无线网络中多个移动发射机的波束跟踪应用，并扩展了分段恒定速度模型。
...
### [86] [Measurements and Modeling of Air-Ground Integrated Channel in Forest Environment Based on OFDM Signals](https://arxiv.org/abs/2507.02303)
*Zhe Xiao,Shu Sun,Na Liu,Lianming Xu,Li Wang*
...
<details>
  <summary>Details</summary>
Motivation: 森林环境中的通信系统对救援人员安全至关重要，但现有研究对森林信道检测与建模的探索有限。
...
### [87] [On the Convergence of Large Language Model Optimizer for Black-Box Network Management](https://arxiv.org/abs/2507.02689)
*Hoon Lee,Wentao Zhou,Merouane Debbah,Inkyu Lee*

Main category: cs.IT

TL;DR: 本文为LLM优化器（LLMO）框架建立了理论基础，将其解释为有限状态马尔可夫链，并证明了其收敛性，同时扩展到多LLM架构。


<details>
  <summary>Details</summary>
Motivation: 未来无线网络需要处理缺乏数学模型的多样化服务，LLMO框架被提出作为解决方案，但此前仅限于数值模拟。

Method: 通过分析LLM推理步骤，将LLMO框架解释为有限状态马尔可夫链，并证明其收敛性，进一步扩展到多LLM架构。

Result: 理论证明了LLMO框架的收敛性，多LLM架构的收敛速率得到验证，数值模拟支持了理论结果。

Conclusion: 本文为LLMO框架提供了理论基础，验证了其有效性，并为未来研究提供了方向。
...
### [88] [RIS-Aided Cooperative ISAC Networks for Structural Health Monitoring](https://arxiv.org/abs/2507.02731)
*Jie Yang,Chao-Kai Wen,Xiao Li,Shi Jin*

Main category: cs.IT

TL;DR: 本文提出了一种利用可重构智能表面（RIS）辅助的集成感知与通信（ISAC）框架，用于结构健康监测（SHM），通过动态调整RIS相位抑制多径干扰，提升测量精度。
...
Method: 利用RIS作为参考点，动态调整相位生成独特信号；采用Fisher信息理论分析三维网络中的协作感知；开发贝叶斯推理模型识别结构状态。

Result: 理论分析和数值模拟表明，ISAC可实现毫米级形变检测，满足SHM的高精度需求。
