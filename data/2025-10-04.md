...
Motivation: 填补物理层处理算法理论进展与实际在商用处理器上实现之间的差距，解决现有系统吞吐量与理论最优值之间的差异。

Method: 开发基于裸金属CUDA内核的MMGaP算法，可扩展到大型GPU处理平台，并能打包为TensorFlow模块便于系统集成。

Result: 在5G网络中，MMGaP相比现有技术提升上行吞吐量约50Mbps/用户，下行吞吐量100Mbps/用户；支持更大MIMO规模（16天线16用户），上行吞吐量提升超过50Mbps/用户。

Conclusion: MMGaP能够在不同NVIDIA GPU上满足5G系统的实时要求，实现线速运行，为下一代蜂窝网络提供实用的大规模MIMO处理解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数学推理方面表现出色，但依赖闭源API存在隐私问题，从头训练开源模型计算成本高，需要开发既能保护隐私又高效的解决方案。

Method: 使用半自动数据合成管道生成多样化运筹学问题-答案对，微调Llama-3.1-8B-Instruct模型，并增强模型调用外部求解器API的能力。

Result: 在四个标准基准测试中的三个上，OR-Toolformer达到80.1%的执行准确率，比同规模基线模型高出4.3%以上；在零样本评估中，对两种未见过的运筹学问题类型达到54%的平均准确率，比最强基线提高21个百分点。

Conclusion: 工具增强的微调方法能够有效提升LLM在运筹学问题建模和求解方面的准确性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人类行为建模方法要么对理性做出不切实际的假设，要么计算量过大难以快速适应。日常社交互动往往遵循可预测的认知节省模式，需要更高效的行为预测方法。

Method: 将日常行为模式建模为行为程序，使用大语言模型合成行为程序假设空间，结合概率推理处理不确定性，在网格世界任务和家庭模拟器中进行测试。

Result: 在稀疏观测下预测人类和AI行为，比行为克隆和基于LLM的方法在样本内准确性和样本外泛化能力上高出50%。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI系统在化学工程中跨学科合作和探索未知问题方面存在局限，需要构建能够自主演化研究并进行科学发现的智能系统。

Method: 开发了CA-ChemE系统，整合领域特定知识库、知识增强技术和协作智能体，构建具有深度专业推理和高效跨学科协作能力的智能生态系统。

Result: 知识库增强机制使7个专家智能体的对话质量得分平均提高10-15%；协作智能体的介入使远领域专家对的协作效率提升8.5%，而邻近领域仅提升0.8%，揭示了"知识库差距导致的协作效率降低"效应。

Conclusion: 精心设计的多智能体架构为化学工程中的自主科学发现提供了可行路径，证明了智能系统在推动科学进步方面的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 传统评估基准无法捕捉LLM智能体在交互环境中涌现的社会和认知动态，需要新的评估方法来理解自主智能体的行为特征。

Method: 使用多智能体辩论作为受控"社交实验室"，让具有不同角色和激励的LLM智能体在LLM仲裁者的监督下就各种话题进行辩论，并采用新的心理测量和语义指标进行分析。

Result: 发现智能体具有强烈的共识寻求倾向（语义一致性μ>0.88），角色分配能产生稳定的心理测量特征，仲裁者角色能显著改变辩论结果。
...
Method: 提出AGILE方法，将拼图解决建模为交互过程，模型在每个步骤生成可执行代码执行动作，环境提供细粒度视觉反馈来指导任务完成。通过观察和交互的迭代循环，模型通过探索和反馈逐步提升感知和推理能力。
...
Conclusion: Aristotle系统证明了结合形式和非形式推理方法在解决复杂数学问题上的有效性，具有良好的扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有上下文和记忆基准主要关注对话场景，但企业动态环境中的记忆评估需求至关重要。

Method: 通过集成Slack、Linear和Git等多个通信和生产力平台的异步事件，创建平台交错的时间线，包含噪声、冲突和交叉引用信息。数据集通过专家设计和基于代理的合成方法构建。

Result: 实验显示最先进的LLM和记忆后端在利用长期记忆、处理跨平台依赖和解决矛盾方面面临挑战，表现最佳的GPT-5模型在MEMTRACK上仅获得60%的正确性分数。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录快速增长带来的临床决策复杂性增加，需要数据驱动的决策支持工具来辅助处方医生，同时不取代临床判断。

Method: 使用检索增强生成(RAG)管道，整合自然语言处理和结构化临床输入，通过检索相似历史病例来生成上下文相关的治疗建议。

Result: 初步评估显示，在适当约束和严格验证下，LLM工具能为处方工作流程提供有价值的决策支持，输出具有临床合理性和一致性。
...
<details>
  <summary>Details</summary>
Motivation: 奖励黑客行为（模型利用奖励函数漏洞获得高奖励但不解决实际任务）是一个严重威胁，特别是隐式黑客行为，其推理链看起来正常从而绕过现有监控方法。

Method: TRACE方法通过逐步截断模型的推理链，强制模型在不同长度下给出答案，测量每个截断点通过验证器的比率。黑客模型会因走捷径而在较短的推理链长度下就获得高通过率。

Result: 在数学推理任务中，TRACE相比最强的72B CoT监控器提升超过65%的性能；在编程任务中，相比32B监控器提升超过30%的性能。TRACE还能在训练过程中发现未知漏洞。
...
<details>
  <summary>Details</summary>
Motivation: LLM代理在多步任务中经常出现可预测的失败，如尝试未满足前提条件的动作、发出冗余命令或处理环境约束不当。虽然检索增强生成(RAG)可以提供运行时指导，但需要维护外部知识库并增加计算开销。

Method: 三步流程：(1)从代理失败中提取紧凑可重用的提示；(2)使用这些提示在任务开始时通过一次性检索生成改进的教师轨迹；(3)训练学生模型学习这些轨迹（移除提示字符串），强制内部化而非记忆。

Result: 在ALFWorld（家务任务）和WebShop（在线购物）两个交互基准上，蒸馏后的学生模型始终优于基线代理，ALFWorld成功率91%（基线79%），WebShop得分72（基线61），同时比检索增强教师少用10-60%的token。
...
<details>
  <summary>Details</summary>
Motivation: 传统数据驱动方法需要大量人工干预，难以有效扩展和泛化到多样化应用，需要自动化建模解决方案。

Method: 评估两种LLM代理框架：具有专门协作代理的多代理系统，以及基于ReAct范式的单代理系统，两者都能自主处理数据预处理、神经网络开发、训练、超参数优化和不确定性量化。

Result: LLM代理开发的模型超越了传统的临界热通量查找表，预测精度和不确定性量化与人类专家开发的贝叶斯优化深度神经网络模型相当。
...
<details>
  <summary>Details</summary>
Motivation: 系统日志是宝贵的网络威胁情报来源，但由于缺乏结构、语义不一致和设备间碎片化，其效用受限。需要将噪声、异构数据转换为连贯且可互操作的表示形式。

Method: 集成轻量级日志本体与检索增强生成(RAG)和迭代校正步骤，确保生成的知识图谱在语法和语义上有效。系统将图谱聚合为会话，并使用LLM预测MITRE ATT&CK战术。

Result: 在公共基准和真实世界蜜罐数据集上的评估显示，OntoLogX能够跨多个KG后端稳健生成知识图谱，并准确将对抗活动映射到ATT&CK战术。检索和校正提高了精确率和召回率。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLMs在大规模知识挖掘中部署成本过高，以及传统分类器-抽取器管道脆弱且无法泛化到新任务的问题。

Method: 使用LLMs作为规划器分解用户指令为可执行管道，并作为标注器生成监督数据训练小型代理模型，统一分类和抽取为两个原子操作。

Result: Falconer在指令跟随准确性上接近最先进的LLMs，同时减少推理成本高达90%，加速大规模知识挖掘超过20倍。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI教育社区往往忽视了领域专家精心整理知识在创建有效辅导系统中的作用，本文旨在强调这一主题的重要性。

Method: 1. 使用可解释AI技术结合专家指定的问题解决规则自动生成课程；2. 利用专家制定的学习课程大纲开发自适应辅导系统。

Result: 通过传粉昆虫识别的案例研究，证明了这些方法在实际应用中的可行性和价值。

Conclusion: 领域专家精心整理的知识在创建新颖教育系统中具有重要作用，能够改善学习体验并提高算法效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法在处理多模态LLMs时，将视觉输入视为固定条件，忽略了视觉变化带来的不确定性，导致策略对视觉变化不够鲁棒，探索效果不佳。

Method: VOGUE方法将图像视为随机上下文，通过计算原始分支和噪声分支之间的对称KL散度来量化策略对视觉扰动的敏感性，创建不确定性感知的探索信号，并结合token熵奖励和退火采样计划来平衡探索与利用。
...
Method: 通过两步流程创建数据集：(1)使用结构化指令提示LLM生成120个技术文档样本，描述虚构但合理的AI系统；(2)法律专家审查并标注每个样本是否违反AI法案的具体条款。

Result: 创建了AIReg-Bench数据集，并评估了前沿LLM是否能复现专家的合规性标签，为理解基于LLM的AI法规合规评估工具提供了起点。

Conclusion: 该研究为LLM在AI法规合规评估方面的应用建立了基准，数据集和评估代码已开源，可供后续LLM比较使用。
...
<details>
  <summary>Details</summary>
Motivation: 现代部署中大量测试时计算（数千个标记或节点扩展）用于提升可靠性，但标准树搜索存在两个问题：广度饱和（额外样本产生近重复）和深度近视（噪声短期效用剪枝有长期回报的分支）。

Method: LToT 将前沿分为主线和侧线。主线用于利用高效用候选，侧线用于探索低效用但一致的候选。采用侧线竞速与短路机制（LR--SC）：在宽侧线集上分布微小探测，使用宽度感知阈值和重复确认，一旦分支表现超过主线标准立即提升。

Result: 理论证明侧线成本为伪线性 Θ(N₀ log_η N₀)，与未限制主线的指数增长形成对比。实证评估正在准备中。

Conclusion: LToT 将大测试预算转化为原则性多样性，同时保持提升纪律，缓解饱和和近视问题而不增加计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在数学推理任务中的内部表示机制，探索如何平衡利用已知推理轨迹和探索新推理路径，以提高推理质量。

Method: 训练稀疏自编码器生成稀疏向量表示，应用k-means聚类构建标记转移图，基于边权重定义奖励函数来量化推理轨迹的遵循程度，并测量生成多样性。

Result: 研究发现平衡利用和探索对于数学推理任务的高准确性至关重要，稀疏自编码器可作为可扩展的奖励模型来指导生成过程。

Conclusion: 该方法能够防止极端行为，促进更高质量的推理过程，通过平衡利用和探索来优化大语言模型的数学推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 在关键领域如法律和医学中，高保证推理需要准确、可验证且明确基于证据的结论。这些推理依赖于从规则、法规和合同中编码的前提，涉及可废止逻辑，单个事实的引入可能使一般规则失效，带来重大挑战。

Method: 提出LOGicalThought (LogT)架构，使用高级逻辑语言和推理器与LLM结合，构建双重符号图上下文和基于逻辑的上下文，将长形式指南的推理问题转化为紧凑的接地评估。

Result: 在四个多领域基准测试中，相比四个基线，LogT整体性能提升11.84%。在三种推理模式上性能显著提升：否定推理提升高达+10.2%，蕴含推理+13.2%，可废止推理+5.5%。

Conclusion: LogT通过神经符号方法有效解决了高保证文本指南中的严格推理挑战，特别是在涉及否定、蕴含和可废止规则的逻辑结构方面表现出显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM规划代理在处理观测不确定性时，往往忽视其内部动态与实际环境之间的差异，导致在信息不完整和动态噪声的实际环境中决策效果不佳。

Method: 提出InfoSeeker框架，提示LLM主动规划信息收集行动来验证理解、检测环境变化或测试假设，然后生成或修订任务导向计划。

Result: 在部分可观测环境基准测试中，InfoSeeker相比先前方法实现了74%的绝对性能提升，且不牺牲样本效率，在机器人操作和网络导航等基准测试中也优于基线方法。

Conclusion: 规划与信息寻求的紧密集成对于在部分可观测环境中实现鲁棒行为至关重要，InfoSeeker框架有效解决了内部动态与环境实际状态不一致的问题。
...
Method: 首先提出理论框架将复杂问题解决形式化为层次选择过程，然后引入SAPO算法，使用过程奖励函数引导模型学习增量推理步骤。

Result: 实验结果表明该方法在挑战性推理基准上显著提升性能，并增强了生成过程的可解释性。

Conclusion: SAPO通过将扩散语言模型的去噪过程与潜在推理层次对齐，有效改善了复杂推理任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法直接优化安全回应，但缺乏对潜在风险的系统性思考。InvThink旨在让模型在生成回应前先识别和分析失败模式，从而更主动地避免安全风险。

Method: 通过三个步骤实现逆向思维：1) 枚举潜在危害，2) 分析后果，3) 生成主动避免这些风险的安全输出。通过监督微调和强化学习在三个LLM家族中实现。

Result: 在安全改进方面展现出比现有方法更强的模型规模扩展性；缓解安全税效应，保持标准基准上的通用推理能力；在高风险领域（医疗、金融、法律等）实现高达15.7%的有害回应减少。
...
<details>
  <summary>Details</summary>
Motivation: 现有防御方法存在局限性：自我验证方法因单个智能体缺乏检测跨智能体不安全链和委托风险的能力而效果不佳；外部防护模块会增加系统开销并造成单点故障。

Method: 提出AdvEvo-MARL框架，联合优化攻击者（合成进化越狱提示）和防御者（训练任务智能体既能完成任务又能抵抗攻击），并引入基于功能组的公共基线进行优势估计以稳定学习和促进合作。

Result: 在代表性攻击场景中，AdvEvo-MARL始终将攻击成功率保持在20%以下，而基线方法可达38.33%，同时保持甚至提高任务准确率（推理任务上提升达+3.67%）。
...
Conclusion: AgentRec通过智能体协作有效解决了现有对话推荐系统的关键挑战，在多个性能指标上实现了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 虽然LLMs在多个行业表现出色，但在需要认知能力的应用如心理咨询方面潜力尚未充分开发。研究旨在确定LLMs是否能有效担任心理咨询师角色。

Method: 开发PsychoBench基准测试，包含约2,252个精心设计的单选题，基于美国国家心理咨询师认证考试，涵盖心理学各子学科，要求深度理解能力。

Result: 评估显示先进模型如GPT-4o、Llama3.3-70B和Gemma3-27B远超通过阈值(约70%准确率)，而较小的开源模型如Qwen2.5-7B、Mistral-7B远低于该阈值。

Conclusion: 只有前沿LLMs目前能够达到心理咨询考试标准，这突显了开发面向心理学的LLMs的潜力和挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有CMDP方法在高维或非结构化上下文中泛化能力差，导致计算量大且性能不稳定

Method: 利用LLM对上下文输入进行信息论压缩，生成低维语义摘要来增强状态表示，基于近似上下文充分性理论

Result: 在离散、连续、视觉和推荐基准测试中优于原始上下文和非上下文基线，提高奖励、成功率、样本效率，降低延迟和内存使用
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否能够理解道路网络地图并执行导航任务，填补现有模型在地理空间推理方面的空白。

Method: 提出轨迹恢复作为代理任务，构建GLOBALTRACE数据集（包含4000+真实轨迹），使用道路网络作为上下文，通过提示框架让LLMs生成有效路径。
...
Method: 开发五个不同的GuruAgents，每个代理模拟一位标志性投资者，通过将他们的投资哲学编码到LLM提示中，整合金融工具和确定性推理流程。

Result: 在2023年第四季度至2025年第二季度的纳斯达克100成分股回测中，GuruAgents表现出由其提示人格驱动的独特行为，巴菲特代理表现最佳，年化收益率达42.2%，显著超越基准，其他代理表现各异。

Conclusion: 提示工程能够成功将投资大师的定性哲学转化为可复制的定量策略，为自动化系统投资指明了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 计算机使用代理(CUAs)在GUI上执行操作以实现用户目标，但存在盲目追求目标的风险，无论目标是否可行、安全或符合上下文。需要系统性地识别和量化这种风险。

Method: 开发BLIND-ACT基准测试，包含90个任务，捕捉三种BGD模式：缺乏上下文推理、在模糊情况下做出假设和决策、矛盾或不可行的目标。基于OSWorld构建真实环境，使用LLM评估代理行为。

Result: 评估了9个前沿模型，包括Claude Sonnet和Opus 4、Computer-Use-Preview、GPT-5等，平均BGD率为80.8%。提示干预能降低BGD水平，但风险仍然显著。

Conclusion: BGD揭示了即使输入没有直接危害时也会出现的微妙风险。需要更强的训练或推理时干预措施。识别BGD和引入BLIND-ACT为未来研究和缓解这一基本风险奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 患者在接受侵入性手术前常有未解答的问题，但时间紧迫的工作流程和隐私限制限制了个性化咨询。需要开发一个安全、高效且保护隐私的系统来提供准确的临床信息。

Method: 使用高精度句子转换器分类器路由输入，从临床医生策划的FAQ中返回原文答案，避免临床路径中的自由文本生成。在两个领域（拔牙和胃镜检查）使用专家审查的验证集进行阈值设定和独立测试集评估。

Result: E5-large-instruct编码器达到0.983的总体准确率，AUC为0.996，仅有7个错误。非生成性临床路径每输入消耗约1.0 mWh，比本地8B SLM的小对话回复能耗低约170倍，同时保持约0.10秒的延迟。

Conclusion: 通过返回经过验证的FAQ原文答案，可以在临床路径中结构性地避免前沿判别和生成引起的错误，支持在带宽受限环境中的隐私保护、可持续性和公平部署。
...
<details>
  <summary>Details</summary>
Motivation: 当前AGI评估方法主要依赖基于人类直觉设计的合成任务，这些方法在AI历史上表现不佳，需要更有效的评估方法。

Method: 提出基于稳健任务执行的评估设计哲学，借鉴数据科学中用于展示系统可靠部署的实践方法，并提供实际评估示例。

Result: 论证了传统基于直觉的合成任务评估方法的局限性，提出了更实用的评估框架。

Conclusion: AGI评估应转向关注系统在实际任务中的稳健执行能力，而非依赖直觉设计的合成测试。
...
<details>
  <summary>Details</summary>
Motivation: 现有的偏好微调方法忽视了合成偏好标注中普遍存在的风格和长度偏差噪声，需要一种能够生成高质量负样本的方法来改进LVLM的对齐效果。

Method: 基于LLM引导的响应编辑框架，生成与接受响应风格和长度相似但包含目标错误的拒绝响应，构建了包含30K高质量样本的VaPR数据集。

Result: 在10个基准测试中，VaPR模型实现了显著性能提升：LLaVA平均提升6.5%，Qwen2VL提升4.0%，Qwen2.5VL提升1.5%，特别是在推理任务上表现突出，同时减少了二元问题中的"是"回答倾向。

Conclusion: VaPR框架有效解决了合成偏好标注的噪声问题，显著提升了LVLM性能，且可推广到开源LLM作为编辑器，为LVLM对齐提供了高质量数据生成方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决知识图谱使用中需要深入理解本体论和查询语言语法的技术障碍，让研究人员能够通过自然语言轻松访问复杂的代谢组学数据。

Method: 采用多智能体系统架构，包括入口代理、验证代理、监督代理、知识图谱代理和SPARQL查询代理，通过LangChain和LangGraph库集成LLM与外部工具。

Result: 在50个代谢组学问题测试中，MetaboT达到83.67%的准确率，而标准LLM基线仅8.16%，证明多智能体系统对准确检索实体和生成正确查询的必要性。

Conclusion: MetaboT成功弥合了复杂语义技术与用户友好交互之间的差距，通过自动化SPARQL查询生成和执行，为研究人员提供了便捷的代谢组学数据访问途径。
...
<details>
  <summary>Details</summary>
Motivation: 弥合理论AI构建与操作网络安全需求之间的差距，提供统一的方法来选择和部署AI解决方案以应对当代网络威胁。

Method: 采用结构化框架，将NIST CSF 2.0功能细分为具体任务，将AI代理特性（自主性、自适应学习、实时响应）与安全需求关联，并定义辅助、增强和完全自主三个自主性级别。

Result: 通过概念验证表明，定制化的AI代理部署能够与现实约束和风险状况对齐，增强态势感知、加速响应时间，并通过自适应风险管理强化长期韧性。

Conclusion: 该研究为遵循行业标准的稳健、经验验证的多代理系统奠定了基础，实现了AI理论与网络安全操作需求的有效对接。
...
<details>
  <summary>Details</summary>
Motivation: 学术规章咨询对学生理解机构政策至关重要，但构建有效系统需要特定领域的规章资源。

Method: CatRAG框架结合检索增强生成与基于图的推理，统一密集检索和图推理，使用带有语义特征的层次化类别标注知识图谱进行领域对齐，轻量级意图分类器将查询路由到适当的检索模块。

Result: 在规章特定数据集上的分类和问答任务评估中，REBot达到了98.89%的F1分数，实现了最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 在快速演变的军事威胁和复杂作战环境中，AI集成带来优势但也面临挑战。当前研究多从外部视角处理人机协作系统，需要深入系统内部动态来应对多维责任、安全和鲁棒性问题。

Method: 设计包含四个维度的可信赖协同学习模型：1) 可调节自主性 - 根据任务状态、系统信心和环境不确定性动态校准自主级别；2) 多层控制 - 持续监督、活动监控和问责制；3) 双向反馈 - 显性和隐性反馈循环，确保推理、不确定性和学习适应的适当沟通；4) 协作决策 - 生成、评估和提出带有置信度和理由的决策。

Result: 提出了一个具体的人机协作模型，并提供了具体示例和建议，有助于进一步发展负责任和可信赖的军事人机协作系统。

Conclusion: 该研究通过提出包含四个关键维度的协同学习模型，为解决军事行动中人机协作系统的责任、安全和鲁棒性挑战提供了框架，促进了可信赖人机协作系统的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM的CoT推理受限于自回归token级生成，缺乏全局规划，导致推理冗余、不连贯或不准确，现有方法如树算法和RL计算成本高且难以产生最优推理轨迹。

Method: 两阶段框架：1) 用高级LLM将CoT提炼为紧凑高层指导进行SFT；2) 指导感知RL方法联合优化最终输出和高层指导质量。

Result: 在MATH、AIME2024、AIME2025、AMC等数学推理基准测试中，在Qwen2.5-7B-Instruct、Qwen3-8B、Qwen3-14B、LLaMA3.2-3B等模型上均取得稳定显著改进。

Conclusion: PTA-GRPO在不同模型和任务上一致实现稳定显著的性能提升，验证了其有效性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 通过从专家演示中学习推理奖励，优先考虑正确性而非表面形式，实现与最终答案有效性相关的评分，并能在推理轨迹中定位错误。

Method: 使用对抗性逆强化学习框架，直接从专家演示中学习密集的token级奖励模型，该模型在训练时提供步骤级反馈优化推理策略，在推理时作为批评者重排采样轨迹。

Result: 在GSM8K数据集上，使用Llama3和Qwen2.5骨干网络，证明密集推理奖励可作为学习信号激发推理能力，通过奖励引导的重排提高了预测性能（特别是基于Llama的策略）。

Conclusion: 通过将训练信号、推理时选择和token级诊断统一到单一推理奖励中，这项工作展示了可重用的过程级奖励在增强语言模型多步推理方面的广泛潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有约束生成方法存在两难：贪婪约束解码会扭曲语言模型分布，而拒绝采样则计算效率低下。在程序模糊测试等需要同时保证有效性和多样性的领域，这两种极端方法都有问题。

Method: CARS方法从无约束采样开始，通过记录违反约束的连续序列到trie结构中，并在未来采样中减去这些序列的概率质量，实现自适应剪枝。
...
TL;DR: 该研究评估大语言模型在集体决策中与人类社会推理的对齐程度，通过Lost at Sea任务进行大规模实验，发现不同LLM在模拟集体推理时表现出不同的偏差模式。
...
Method: 使用Lost at Sea社会心理学任务进行大规模在线实验（N=748），随机分配组别进行领导选举，比较可见人口属性与匿名别名的差异，然后模拟匹配的LLM组别。

Result: 不同LLM行为存在分歧：有些反映人类偏见，有些掩盖这些偏见并试图补偿。人类-AI在集体推理中的对齐取决于情境、线索和模型特定的归纳偏差。
...
TL;DR: ReTabAD是一个为表格异常检测恢复文本语义的基准，提供20个带有文本元数据的表格数据集和多种检测算法，通过零样本LLM框架利用语义上下文提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有表格异常检测基准缺乏文本语义上下文，无法充分利用领域知识，限制了模型在实际应用中的性能。

Method: 提供20个带有结构化文本元数据的表格数据集，实现包括经典、深度学习和LLM方法在内的多种异常检测算法，并开发零样本LLM框架。

Result: 语义上下文显著提高了异常检测性能，并增强了模型的可解释性，支持基于领域知识的推理。
...
TL;DR: 本文系统研究了LLM中深度层的作用，发现深度层的贡献高度依赖于评估设置：基于似然的评估中浅层关键，而生成和推理任务需要中深层，知识检索集中在浅层，推理准确度依赖深层但可通过蒸馏重塑。


<details>
  <summary>Details</summary>
Motivation: 针对现有研究认为LLM深层贡献有限且可移除的观点，通过系统分析揭示深度层在不同评估设置下的真实作用，避免片面结论。

Method: 采用多维度系统研究，包括评估协议（似然vs生成）、任务类别、模型架构，分析不同深度层的贡献分布。

Result: 发现深度层使用高度异质：似然评估中仅前几层关键，生成任务需要中深层维持连贯性，知识检索集中于浅层，推理准确度依赖深层但可通过蒸馏转移。

Conclusion: LLM深度使用高度依赖于上下文，需要基于任务、指标和模型的视角来理解和压缩大模型。
...
TL;DR: 该研究评估AI模型在ConceptARC基准上的抽象推理能力，发现仅靠输出准确率会高估文本模态的抽象推理能力，低估视觉模态的抽象推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管OpenAI的o3-preview推理模型在ARC-AGI基准上超过人类准确率，但研究者质疑这些模型是否真正理解任务设计者意图的抽象概念，而非依赖表面模式。

Method: 在ConceptARC上评估模型，改变输入模态（文本vs视觉）、是否允许使用外部Python工具、以及推理模型的推理努力程度。除了测量输出准确率，还精细评估模型生成的自然语言解释规则。

Result: 使用文本表示的模型能匹配人类输出准确率，但其规则多基于表面捷径，很少捕捉到意图的抽象概念。在视觉模态中，模型输出准确率大幅下降，但规则分析显示它们仍能生成相当比例的意图抽象规则，只是无法正确应用。

Conclusion: 模型在抽象推理方面仍落后于人类，仅靠准确率评估ARC类任务的抽象推理能力会高估文本模态、低估视觉模态的能力。该评估框架能更真实地反映多模态模型的抽象推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 企业级文档理解模型需要大量多样化且标注良好的数据集，但传统数据收集方法成本高昂（可达数百万美元），存在隐私约束、法律限制和大量手动标注需求。
...
TL;DR: 提出了针对深度研究代理(DRAs)的严格基准和多维评估框架，包含214个专家策划的挑战性查询，覆盖10个主题领域，支持对长格式报告的全面评估。
...
Method: 构建包含214个专家策划查询的基准，每个查询配备手动构建的参考包；开发多维评估框架，整合语义质量、主题聚焦和检索可信度的评分指标。
...
TL;DR: 提出了UpSafe°C框架，通过安全感知的升级改造来增强LLM安全性，包括识别关键层、构建稀疏MoE结构、两阶段SFT策略和安全温度机制，实现动态安全控制。


<details>
  <summary>Details</summary>
Motivation: 现有安全技术（外部护栏、推理时指导和训练后对齐）在平衡安全性、实用性和可控性方面存在局限性，需要更灵活的安全增强方法。

Method: 识别安全关键层并升级为稀疏MoE结构，路由器作为软护栏选择性激活原始MLP和安全专家；采用两阶段SFT策略强化安全判别能力；引入安全温度机制实现推理时动态控制。

Result: 在多个基准测试、基础模型和模型规模上，UpSafe°C对有害和越狱输入实现了稳健的安全改进，同时在通用任务上保持竞争力；安全温度实现了效用与安全之间的帕累托最优边界。
...
<details>
  <summary>Details</summary>
Motivation: 尽管RLVR被用于提升大语言模型的推理能力，但最近证据表明它可能反而会缩小推理边界而非扩展它，这促使研究者深入分析其学习动态。

Method: 通过理论分析和在多个数学推理基准上的实证研究，揭示了RLVR中的负干扰和赢家通吃现象，并提出了一种专注于低概率问题的数据筛选算法。

Result: 研究发现RLVR会不成比例地强化基础模型中高概率的正确解决方案，同时抑制其他初始低概率的解决方案，导致Pass@k性能下降。

Conclusion: 基于这些发现，提出的专注于低概率问题的数据筛选算法显著改善了Pass@k性能，为解决RLVR的局限性提供了有效方法。
...
Method: bBoN方法通过生成多个代理执行轨迹，并使用描述代理执行过程的行为叙述来进行轨迹选择，实现广泛探索和原则性选择。

Result: 在OSWorld基准测试中达到69.9%的新SOTA，接近人类水平(72%)，在WindowsAgentArena和AndroidWorld上也表现出强泛化能力。
...
TL;DR: 提出RLAD训练范式，通过推理抽象来引导模型学习算法推理，包含抽象生成器和解决方案生成器的联合训练，提高对困难问题的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有大型模型在推理过程中缺乏对算法过程的捕获和重用，容易陷入冗长和退化的探索，需要更有效的推理方法。

Method: 引入推理抽象概念，训练模型提出多个抽象描述，然后使用RL激励基于这些抽象构建解决方案，形成两玩家RL训练范式RLAD。

Result: RLAD实现了结构化探索，解耦了抽象提议和解决方案生成的学习信号，提高了对更困难问题的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 生物信号模态间存在相关性，但缺乏大规模标注数据限制了特定任务模型的训练。现有跨模态知识迁移方法基于知识蒸馏，计算和内存开销大，特别是对于大模型基础模型。

Method: 训练轻量级桥接网络对齐基础模型的中间表示，实现跨模态信息流动。提出高效的对齐位置选择策略和灵活的原型网络作为桥接架构。

Result: 在多个生物信号模态、任务和数据集上的实验表明，BioX-Bridge将可训练参数减少88-99%，同时保持或优于现有最先进方法的迁移性能。
...
TL;DR: 提出基于MCMC的AI驱动通用MIMO接收机架构，将信道估计、符号检测和信道解码等任务整合到统一概率框架中，提高可解释性、可扩展性和灵活性。


<details>
  <summary>Details</summary>
Motivation: MIMO接收机处理面临复杂性和可扩展性挑战，AI技术为解决这些问题提供了潜力。现有AI方法将接收机处理视为黑盒，缺乏可解释性。

Method: 使用马尔可夫链蒙特卡洛(MCMC)技术构建通用贝叶斯计算引擎，将多个处理任务整合到统一概率框架中，并可结合数据驱动学习方法。

Result: 该方法增强了接收机在不同场景下的可解释性、可扩展性和灵活性，实现了整体性能优化。

Conclusion: 所提出的统一框架能够促进完全智能通信接收机的发展，为下一代无线网络提供有效的MIMO接收解决方案。
...
TL;DR: 本章介绍DNA编码的最新约束条件，包括反向、反向互补、GC含量、汉明距离、非相关相关、热力学、避免串联重复和避免二级结构等约束，重点讨论使用代数方法构建具有高汉明距离的非循环DNA编码。


<details>
  <summary>Details</summary>
Motivation: DNA编码在DNA计算和DNA数据存储中的应用日益重要，传统约束条件已不足以满足新应用需求，需要引入新的约束条件来设计更有效的DNA编码。

Method: 采用代数方法，使用有限环和映射（通常是等距映射）来构建DNA编码，重点关注能够产生高汉明距离的构造方法，并讨论各种距离度量标准。

Result: 提出了满足多重约束条件的DNA编码代数构造方法，讨论了DNA编码的代数界限，并展示了如何构建具有高汉明距离的非循环DNA编码。

Conclusion: DNA编码研究需要继续发展新的约束条件和构造方法，本章为未来研究提供了开放的研究方向，特别是在满足多重约束条件的代数构造方面。
...
TL;DR: 本文研究了基于凸多面体P定义的多项式求值码（Toric码）的列表解码问题，提出了Guruswami-Sudan算法的推广版本，并计算了解码半径的界限。


<details>
  <summary>Details</summary>
Motivation: 多项式求值码在编码理论中占据重要地位，但针对一般类别的多项式求值码（特别是Toric码）的列表解码问题研究不足。Reed-Solomon和Reed-Muller等特殊情形已有深入研究，需要扩展到更一般的凸多面体情况。

Method: 提出了Guruswami-Sudan算法的推广版本，该算法考虑了凸多面体P的几何和组合结构特性。

Result: 计算了这类多项式求值码的列表解码半径的界限。

Conclusion: 成功将列表解码技术扩展到基于任意凸多面体定义的多项式求值码，为这类编码的纠错能力提供了理论保证。
...
TL;DR: 提出基于统一二进制树表示的并行SGRAND解码器和混合GRAND算法，在保持最大似然解码最优性的同时显著降低解码延迟。


<details>
  <summary>Details</summary>
Motivation: 现有Soft GRAND解码器虽然能达到最大似然解码，但其顺序测试错误模式的本质阻碍了并行化，导致高解码延迟。

Method: 使用统一的错误模式二进制树表示，支持紧凑表示、高效操作和并行探索；提出并行SGRAND设计，结合剪枝策略和基于树的计算；开发混合GRAND算法，将EP树表示与ORB GRAND结合。

Result: 并行SGRAND相比串行实现达到3.75倍加速，混合增强方法达到4.8倍加速，硬件映射下预期有更大增益。

Conclusion: EP树表示有效支持并行GRAND解码，在保持ML最优性的同时显著提升解码效率，为高吞吐量低延迟解码提供了可行方案。
...
TL;DR: 本文研究了在可分区可重构智能表面(RIS)增强的无人机网络中下行非正交多址(NOMA)通信性能，提出了RUOM算法来优化公平性和效率。


<details>
  <summary>Details</summary>
Motivation: 无人机通信面临信道质量不稳定和资源分配不公平的问题，需要结合RIS技术来改善通信可靠性和公平性。

Method: 分析了三种链路类型，推导了信噪比的闭式表达式，提出了双层优化问题和RUOM算法来最小化最大中断概率和RIS反射单元总数。

Result: 仿真验证了分析模型，RUOM算法显著提高了BS-UAV通信的公平性和效率。

Conclusion: RIS辅助的NOMA通信能有效改善无人机网络的性能，RUOM算法在公平性和资源效率方面表现优异。
...
TL;DR: 提出了一种实用的两阶段通用随机性提取框架，用于从相关随机源中提取秘密密钥。第一阶段使用变分概率量化将观测映射为离散均匀随机变量，第二阶段使用安全草图实现密钥一致性。该方法应用于基于感知的物理层密钥生成，通过集成感知与通信系统中的距离-角度图作为相关源。


<details>
  <summary>Details</summary>
Motivation: 解决传统物理层密钥生成方法依赖信道互易性、需要双向信道探测导致协议开销大、不适用于高移动性场景的问题。通过利用集成感知与通信系统中的感知信息作为相关源，提高密钥生成效率和安全性。

Method: 两阶段框架：1) 变分概率量化阶段：使用概率神经网络编码器将观测映射为离散均匀随机变量，通过变分学习和对抗训练最小化信息泄露；2) 安全草图阶段：使用码偏移构造协调编码器输出为相同密钥。应用于基于距离-角度图的感知物理层密钥生成。

Result: 通过端到端仿真和真实软件定义无线电测量验证了方法的可行性，包括攻击者部分知晓Bob位置的情况。结果表明所提通用随机性提取框架和基于感知的物理层密钥生成方法具有令人信服的性能。

Conclusion: 提出的两阶段通用随机性提取框架和基于感知的物理层密钥生成方法有效解决了传统方法的局限性，在集成感知与通信系统中实现了高效安全的密钥生成，特别适用于高移动性场景。
...
TL;DR: 本文研究了基于Rb-85的里德堡原子接收器在检测8-PAM信号时的性能，并与传统接收器进行比较，证明里德堡接收器无需额外滤波器即可抑制干扰
...
Method: 通过监测控制激光束在通过含里德堡原子的蒸汽室前后的变化，分析透射激光信号来检测外部电磁场的特性和存在

Result: 里德堡接收器能够抑制离谐振干扰，无需额外滤波器，在符号错误率方面优于传统电路基接收器

Conclusion: 里德堡接收器可作为集成滤波器和解调器，在8-PAM信号检测中表现出比传统接收器更好的性能
...
TL;DR: 提出了一种联合考虑语义相关性和信道状态的协作边缘推理方法，通过将注意力权重与信道信息结合，在无线网络中实现多设备协作，提升在数据损坏情况下的推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在无线协作推理中要么只考虑语义相关性，要么只考虑信道状态，这种分离的方法在恶劣传播条件下表现不佳。需要一种联合考虑语义信息和物理层参数的协作策略。

Method: 使用键-查询机制进行选择性信息交换，通过将通用注意力权重与信道信息相结合，在设备分组时同时考虑语义信息相关性和信道状态。

Result: 数值模拟显示，与仅考虑应用层或物理层参数的分离决策方法相比，所提出的联合方法在数据损坏情况下显著优于本地推理和传统协作推理。

Conclusion: 在无线协作边缘推理中，联合考虑语义相关性和信道状态的方法比分离方法更有效，特别是在恶劣传播条件下能显著提升推理性能。
...
TL;DR: 该论文研究了协作推理框架，通过交换中间特征而非原始数据来提高边缘设备分类准确率，分析了信道噪声、协作点选择和通信-准确率权衡，证明中间协作方法对信道错误具有鲁棒性且查询传输需要更高可靠性。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备协作推理中，需要在提高分类准确率和带宽限制之间找到平衡，因此需要高效的通信策略来优化资源使用。

Method: 基于键-查询机制进行选择性信息交换，分析信道噪声对特征通信的影响，研究不同中间协作点的选择，探索通信剪枝和通信-准确率权衡。

Result: 中间协作方法对信道错误具有鲁棒性，查询传输比数据传输本身需要更高程度的可靠性，可以通过优化协作点来在最小化资源使用的同时优化准确率。

Conclusion: 协作推理框架能够有效平衡准确率和带宽限制，通过选择合适的协作点和通信策略，可以在保持鲁棒性的同时优化性能。
