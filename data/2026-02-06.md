...
TL;DR: COL方法通过两阶段在线学习安全操作区域：先被动观察推断初始安全区域，再通过干预性贝叶斯学习逐步扩展，确保学习区域安全并收敛到完整安全区域。


<details>
  <summary>Details</summary>
Motivation: 云无线接入网络需要动态扩展容量，但在实际网络中部署自适应控制器存在违反服务协议和操作约束的风险，因此需要安全地学习网络的安全操作区域。

Method: 提出因果在线学习(COL)方法，包含两个在线阶段：1)推理阶段：被动观察RAN，通过因果推理和高斯过程回归推断初始安全区域；2)干预阶段：通过干预性贝叶斯学习逐步扩展安全区域。

Result: 理论证明COL确保学习区域以指定概率安全，并在标准条件下收敛到完整安全区域。在5G测试床上的实验表明，COL能快速学习安全区域，运营成本低，样本效率比当前最先进的安全学习方法高10倍。

Conclusion: COL方法为云RAN的安全自适应控制提供了一种有效解决方案，通过两阶段在线学习机制平衡了安全性和探索效率，在实际网络中具有应用潜力。
...
TL;DR: 提出LoSRef方案，利用Wi-Fi的视距路径作为无线参考，在非同步Wi-Fi系统中实现延迟校准和相位对齐，使普通Wi-Fi设备能进行相位相干的雷达式延迟-多普勒分析。


<details>
  <summary>Details</summary>
Motivation: 传统Wi-Fi感知无法进行相位相干的雷达式延迟-多普勒分析，因为发射机和接收机不同步。现有方法依赖有线参考信号或专用参考天线，限制了实际应用。

Method: 提出LoSRef方案，利用视距路径作为无线参考信号，实现延迟校准和相位对齐。该方案无需有线连接或专用天线，可在普通Wi-Fi设备上实现相位相干的双基地雷达操作。

Result: 通过室内人体步态和呼吸实验证明，仅使用商用Wi-Fi设备就能获得相位相干的信道脉冲响应和延迟-多普勒响应。能够检测步态引起的距离变化和呼吸引起的亚波长位移，并能提取比主导静态多径分量弱20dB的目标动态。

Conclusion: LoSRef方案填补了传统Wi-Fi感知与Wi-Fi雷达之间的长期空白，使普通Wi-Fi设备能够进行相位相干的雷达式操作，为可解释的人体运动感知提供了新途径。
...
TL;DR: 开发了一个基于随机定时混合自动机和统计模型检验的形式化验证框架，用于评估电力网格传感器网络中CSMA/CA参数配置的可靠性，通过认证纳什均衡策略来提升系统性能。


<details>
  <summary>Details</summary>
Motivation: 电力网格中基于IEEE 802.15.4的ad hoc传感器网络面临节点自私调整CSMA/CA参数的问题，这会降低系统可靠性、能效和电网约束合规性。现有分析方法难以在异步、事件驱动和资源受限条件下进行严格评估。

Method: 开发了一个集成随机定时混合自动机与统计模型检验的验证框架，使用置信边界形式化评估CSMA/CA参数配置。通过时态逻辑编码节点和系统级目标，并通过大规模统计评估自动进行协议筛选，认证对单边偏差保持鲁棒的纳什均衡策略。

Result: 在变电站规模场景中，认证的均衡策略将效用从0.862提升到0.914，投递率从89.5%提升到93.2%。相比投递导向基线，将每周期平均能耗从152.8 mJ降低到149.2 mJ，同时保持相当的投递性能。认证配置满足延迟、可靠性和能量约束，鲁棒系数高于0.97，效用高于0.91。

Conclusion: 提出的形式化验证框架能够有效评估和认证电力网格传感器网络中CSMA/CA参数配置的纳什均衡策略，显著提升系统可靠性、能效和性能，为资源受限的异步事件驱动系统提供严格的评估方法。
...
TL;DR: 该研究收集了云计算环境下虚拟化技术对网络性能影响的测量数据集，分析了KVM、LXC、Docker等虚拟化技术在不同参数配置下对端到端延迟的影响，并构建数据模型评估云计算环境对数据包往返时间的影响。


<details>
  <summary>Details</summary>
Motivation: 云计算虚拟化技术（如完全虚拟化和操作系统虚拟化）虽然带来灵活性，但也引入了CPU、I/O和网络资源的额外开销，使得数据平面数据包吞吐量的预测变得更加困难。需要研究不同虚拟化技术对网络性能指标的具体影响。

Method: 收集主动网络测量数据集，通过改变多种网络参数（CPU亲和性、回波包注入频率、虚拟网络驱动类型、CPU/I/O/网络负载、并发VM数量）进行实验。使用KVM、LXC、Docker三种虚拟化技术，分析它们对端到端延迟的影响。对数据集进行预处理、相关性分析、降维和聚类等可视化探索，并构建数据模型评估云计算环境对数据包往返时间的影响。

Result: 创建了一个包含不同虚拟化技术和网络参数配置下的网络性能测量数据集。通过分析发现虚拟化技术对网络延迟有显著影响，不同虚拟化类型（KVM、LXC、Docker）在不同负载条件下的表现存在差异。数据集可用于开发基于机器学习的系统来辅助管理员决策。

Conclusion: 虚拟化技术在云计算环境中对网络性能有重要影响，特别是对端到端延迟。通过系统测量和分析，可以更好地理解不同虚拟化技术的性能特征，为云计算环境下的网络优化和管理决策提供数据支持。该数据集和建模方法有助于开发智能管理系统来预测和优化虚拟化环境中的网络性能。
...
TL;DR: 论文批评AI进步的线性模型，提出"熟悉智能"和"陌生智能"概念，认为AI智能将是陌生智能，在不同领域表现出超人类和亚人类能力的混合，支持非线性智能模型。


<details>
  <summary>Details</summary>
Motivation: 当前AI评估常采用线性模型，假设智能是统一能力，但作者认为这种模型无法准确描述AI智能的本质，特别是AI可能表现出与传统人类智能模式不同的"陌生智能"特征。

Method: 扩展Susan Schneider对AI进步线性模型的批评，引入"熟悉智能"和"陌生智能"概念，发展非线性智能模型，认为通用智能是实现在广泛环境中达成广泛目标的能力，而非单一线性量。

Result: AI智能很可能表现为陌生智能，在不同领域混合超人类和亚人类能力，甚至在相同领域内结合超人类洞察力和令人惊讶的错误。非线性模型能更好解释这种复杂模式。

Conclusion: 如果AI是陌生智能，即使最先进的系统也可能在看似简单的任务上失败，这种错误本身不能证明系统缺乏卓越的通用智能。同样，在单一任务上的优秀表现（如IQ测试）不能保证在其他领域的广泛能力。
...
TL;DR: DeepRead是一个结构感知的多轮文档推理智能体，通过保留文档层次结构和顺序信息，在长文档问答中显著优于现有搜索方法


<details>
  <summary>Details</summary>
Motivation: 现有基于智能体的搜索框架通常将长文档视为扁平化的文本块集合，未能充分利用文档固有的先验信息，如层次化组织和顺序性话语结构

Method: 使用LLM-based OCR将PDF转换为结构化Markdown，保留标题和段落边界；在段落级别索引文档，为每个段落分配编码章节身份和顺序的元数据键；提供两个互补工具：Retrieve工具定位相关段落并暴露其结构坐标，ReadSection工具在指定章节和段落范围内进行连续、保序的阅读

Result: DeepRead在文档问答任务中相比Search-o1风格的智能体搜索取得了显著改进；验证了检索和阅读工具之间的协同效应；细粒度行为分析揭示了类似人类"定位然后阅读"的阅读推理范式

Conclusion: DeepRead通过显式操作化文档的层次和顺序先验，实现了更有效的长文档问答，展示了结构感知的智能体搜索框架的优势
...
<details>
  <summary>Details</summary>
Motivation: 开放世界中的联合规划常涉及不完全信息和未知因素（如对象、人类目标），导致知识缺口，需要AI代理主动获取人类输入来优化规划性能

Method: 提出MINT（最小信息神经符号树）框架：1）构建符号树模拟可能的人机交互；2）使用神经规划策略评估知识缺口导致的规划不确定性；3）利用自博弈优化询问策略；4）使用LLM搜索总结推理过程并生成最优询问

Result: 在三个涉及未知/未见对象的基准测试中，MINT通过有限的问题数量实现了接近专家级的回报，显著提高了奖励和成功率

Conclusion: MINT框架有效解决了对象驱动规划中的知识缺口问题，通过主动询问策略优化人机协作规划性能，在现实场景中展现出强大潜力
...
TL;DR: LLMs在已解决的图论问题上表现良好，能提供正确证明，但在开放问题上仅能提供解释和探索策略，无法产生新见解，适合概念探索而非创新性数学推理。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs被学生用于计算机科学高级内容学习，需要评估其在数学严谨推理方面的可靠性，特别是在图论等数学领域。

Method: 使用八阶段评估协议模拟真实数学探究过程，包括解释、探索、策略形成和证明构建，测试LLM在已解决图论问题和开放问题上的表现。

Result: LLM在已解决问题上表现优异：提供正确定义、识别相关结构、准确回忆结果并构建专家确认的有效证明；在开放问题上能生成连贯解释和合理探索策略，但无法推进解决方案。

Conclusion: LLMs适合支持已建立材料的概念探索，但在需要新颖数学见解或关键结构推理的任务中仍有局限，教育中应引导学生将其用于概念探索，同时依赖独立验证和严谨论证进行正式问题解决。
...
TL;DR: 论文提出LLM智能体不确定性量化新框架，从传统不确定性累积视角转向条件不确定性减少过程，强调交互性在智能体UQ中的核心作用


<details>
  <summary>Details</summary>
Motivation: 当前不确定性量化研究主要集中于单轮问答，而现实世界中的LLM智能体应用日益复杂，需要面向交互式智能体的新UQ框架

Method: 提出首个通用的智能体UQ形式化框架，涵盖现有UQ设置；引入条件不确定性减少过程新视角，强调动作交互性；构建概念框架指导智能体UQ设计

Result: 揭示了现有工作隐含地将LLM UQ视为不确定性累积过程，这在开放世界交互式智能体中失效；提出了基于交互动作的条件不确定性减少新范式

Conclusion: 智能体UQ对前沿LLM开发和领域应用有重要实践意义，仍需解决开放问题；新框架为设计交互式智能体不确定性量化提供了可操作指导
...
TL;DR: 基于强化学习的自适应碰撞规避框架，用于小卫星执行多碎片主动清除任务，优化燃料效率和任务时间


<details>
  <summary>Details</summary>
Motivation: 随着地球轨道环境日益拥挤，主动碎片清除任务面临确保安全操作和降低碰撞风险的挑战。小卫星因其灵活性、成本效益和机动性而被广泛采用，适合动态的碎片清除任务。

Method: 采用掩码近端策略优化算法，集成加油策略、高效任务规划和自适应碰撞规避，优化航天器交会操作。RL智能体学习确定与多个碎片目标交会的有效序列。

Result: 基于铱星33碎片数据集的模拟评估显示，与传统启发式方法相比，该RL框架在降低碰撞风险的同时提高了任务效率。
...
TL;DR: VERA-MH评估框架在自杀风险检测中的临床有效性和可靠性验证：通过模拟对话比较临床医生与AI评估的一致性


<details>
  <summary>Details</summary>
Motivation: 尽管生成式AI聊天机器人在心理健康支持方面应用广泛，但其安全性仍是首要问题。需要建立基于证据的自动化安全基准来评估AI在心理健康领域的安全性。

Method: 1. 模拟大量LLM用户代理与通用AI聊天机器人的对话；2. 持证心理健康临床医生使用评分标准独立评估对话中的安全/不安全行为；3. LLM法官使用相同标准评估相同对话；4. 比较临床医生之间、临床医生共识与LLM法官之间的评分一致性。

Result: 1. 临床医生之间的安全评分一致性高（IRR: 0.77），建立了黄金标准临床参考；2. LLM法官与临床共识高度一致（IRR: 0.81）；3. 临床医生普遍认为用户代理具有真实性。

Conclusion: 研究支持VERA-MH的临床有效性和可靠性，这是一个开源、全自动的心理健康AI安全评估框架。未来研究将关注其普适性和鲁棒性。
...
TL;DR: 比较三种自主碎片清除任务规划器：固定参数训练的Masked PPO、领域随机化训练的Masked PPO和蒙特卡洛树搜索(MCTS)，评估其在燃料和时间约束变化下的性能表现。


<details>
  <summary>Details</summary>
Motivation: 自主碎片清除任务规划需要在效率、适应性和严格的燃料/时间约束之间取得平衡，现有方法在应对约束条件变化时存在局限性。

Method: 使用三种规划器：1) 固定任务参数训练的Masked PPO；2) 在变化任务约束下训练的领域随机化Masked PPO；3) 蒙特卡洛树搜索(MCTS)基线。在高保真轨道模拟中评估，包含加油、真实转移动力学和随机碎片场。

Result: 固定PPO在训练匹配条件下性能最佳但分布偏移时急剧下降；领域随机化PPO适应性改善但名义性能略有损失；MCTS约束变化处理最好但计算时间高几个数量级。

Conclusion: 学习策略的速度与搜索方法的适应性存在权衡，结合训练时多样性和在线规划可能是未来弹性ADR任务规划器的有前景路径。
...
TL;DR: GAMMS是一个轻量级、可扩展的多智能体模拟框架，基于图表示环境，支持快速开发和评估智能体行为，特别适用于大规模部署和快速原型设计。


<details>
  <summary>Details</summary>
Motivation: 随着智能系统和多智能体协调在现实应用中越来越重要，需要既可扩展又易于使用的模拟工具。现有高保真模拟器虽然强大，但计算成本高，不适合快速原型设计或大规模智能体部署。

Method: GAMMS采用基于图的对抗多智能体建模模拟器设计，强调五个核心目标：可扩展性、易用性、集成优先架构、快速可视化反馈和现实基础。它支持与外部工具（如机器学习库、规划求解器）集成，并提供内置可视化功能。

Result: GAMMS能够高效模拟复杂领域（如城市道路网络和通信系统），支持各种策略类型的智能体（启发式、基于优化、基于学习，包括使用大语言模型的智能体），在标准硬件上实现高性能模拟。

Conclusion: GAMMS通过降低研究门槛，在标准硬件上实现高性能模拟，促进了多智能体系统、自主规划和对抗建模领域的实验和创新。该框架是开源的。
...
<details>
  <summary>Details</summary>
Motivation: LLM越来越多地被用作推理质量的评估者，但在支付风险场景下的可靠性和偏差仍不清楚。需要建立一个系统框架来评估LLM在商户类别代码风险评估中的推理能力。

Method: 引入结构化多评估框架，结合五标准评分表和蒙特卡洛评分来评估推理质量和评估稳定性。五个前沿LLM在署名和匿名条件下生成并交叉评估MCC风险推理。采用共识偏差度量消除循环性，通过比较每个评估者的分数与其他所有评估者的平均值来衡量自我评估和跨模型偏差。

Result: 结果显示显著异质性：GPT-5.1和Claude 4.5 Sonnet显示负自我评估偏差(-0.33, -0.31)，而Gemini-2.5 Pro和Grok 4显示正偏差(+0.77, +0.71)，匿名化使偏差减少25.8%。LLM评估者比人类共识平均高0.46分，GPT-5.1和Claude 4.5 Sonnet的负偏差反映与人类判断更接近。使用支付网络数据的真实验证显示四个模型具有统计显著相关性(Spearman rho = 0.56-0.77)。

Conclusion: 该框架为支付风险工作流中的LLM-as-a-judge系统提供了可复现的评估基础，并强调了在运营金融环境中需要偏差感知协议。
...
TL;DR: DemPO框架通过算法抽签构建代表性评分者群体，在偏好对齐中实现人口统计学代表性，硬面板使用配额抽样，软面板通过包含概率加权，实验证明代表性训练优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有AI偏好对齐方法（如RLHF）依赖的评分者群体通常是便利样本，系统性过度代表某些人口群体而低估其他群体，导致AI系统学习到的价值观不具有代表性。

Method: 提出民主偏好优化（DemPO）框架，应用算法抽签机制构建公民大会式的代表性评分者群体。提供两种训练方案：硬面板（仅使用配额抽样的代表性样本）和软面板（保留所有数据但按抽签包含概率重新加权）。

Result: 在包含人口统计信息的公共偏好数据集上，使用1B到8B参数的Llama模型进行微调。硬面板在六种聚合方法中始终排名第一，软面板始终优于未加权基线，且模型容量越大效果越明显。

Conclusion: 在偏好收集阶段强制实施人口统计学代表性，而非事后修正，能够产生更好地反映代表性公众价值观的模型行为。DemPO框架为构建更具代表性的AI对齐提供了有效方法。
...
Method: 基于对人类交互中沟通挑战的系统文献综述，SocialVeil引入了三种代表性的沟通障碍类型：语义模糊性、社会文化不匹配和情感干扰。设计了两种障碍感知评估指标：未解决困惑和相互理解。在720个场景和四个前沿LLM上进行实验。

Result: 沟通障碍持续损害LLM性能：相互理解平均降低超过45%，困惑度提升近50%。人类评估验证了模拟障碍的保真度（ICC≈0.78，Pearson r≈0.80）。适应策略（修复指令和交互学习）效果有限，远未达到无障碍性能。

Conclusion: SocialVeil使社会交互环境更接近真实世界沟通，为探索LLM智能体的社会智能开辟了机会。研究表明当前LLM在非理想沟通条件下表现显著下降，适应策略效果有限。
...
Conclusion: CAST-CKT通过混沌感知建模有效解决了跨城市交通预测中的领域偏移问题，理论分析显示改进了泛化边界，为数据稀缺环境下的交通预测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图的RAG方法过度依赖表面节点匹配，缺乏显式因果建模，导致答案不可靠或虚假。先前尝试结合因果关系的方法通常局限于局部或单文档上下文，且受到模块化图结构导致的信息隔离问题，阻碍了可扩展性和跨模块因果推理。

Method: 提出HugRAG框架，通过因果门控在层次化模块间重新思考知识组织，显式建模因果关系以抑制虚假相关性，同时支持大规模知识图上的可扩展推理。
...
<details>
  <summary>Details</summary>
Motivation: 评估当前AI系统回答研究级数学问题的能力，这些问题来自作者实际研究过程中自然产生的问题

Method: 创建了一个包含10个未公开过的研究级数学问题的数据集，这些问题来自作者的实际研究过程，答案暂时加密

Result: 提供了一个评估AI数学能力的基准测试集，包含10个真实的研究级数学问题
...
TL;DR: 本文针对多知识库检索增强生成中的权威性偏差问题，提出DAKS路由算法和对齐图证据融合方法，在藏医药领域实现更好的跨知识库证据覆盖和可追溯性。


<details>
  <summary>Details</summary>
Motivation: 在藏医药等专业领域，多知识库检索增强生成面临权威性偏差问题——密集的百科条目容易主导检索，而更权威的经典文献和临床论文证据被忽视。需要解决跨知识库检索的挑战，提高可追溯性并减少幻觉。

Method: 提出两种互补方法：1) DAKS算法进行知识库路由和预算检索，缓解密度驱动偏差，在适当时优先权威来源；2) 使用对齐图指导证据融合和覆盖感知打包，改善跨知识库证据覆盖，避免简单拼接。所有答案由轻量级生成器openPangu-Embedded-7B生成。

Result: 实验显示路由质量和跨知识库证据覆盖持续提升，完整系统在CrossEv@5指标上表现最佳，同时保持强大的忠实性和引用正确性。在500个查询基准测试中验证了方法的有效性。

Conclusion: 提出的DAKS路由和对齐图证据融合方法有效解决了多知识库检索增强生成中的权威性偏差问题，在藏医药领域实现了更好的跨知识库证据覆盖和可追溯性，为专业领域的检索增强生成提供了实用解决方案。
...
Method: 提出可分离sink divergence假设，基于此设计Surgery方法，使用sink divergence抑制正则化器，引导注意力头向负sink divergence组

Result: 在BeaverTails、HarmBench、SorryBench基准上分别提升防御性能5.90%、11.25%、9.55%

Conclusion: Surgery方法通过注意力sink机制有效防御有害微调，提升模型安全性
...
TL;DR: 本研究通过整合多种可解释AI技术（GRAD-CAM、LRP、SHAP）来增强脑肿瘤检测深度学习模型的可解释性，在BraTS 2021数据集上达到91.24%准确率，提供从区域到像素级的全面解释。


<details>
  <summary>Details</summary>
Motivation: 提高AI驱动医学影像分析的透明度和可信度，特别是在脑肿瘤检测等关键医疗任务中，需要更全面的模型决策解释来增强临床信任。

Method: 开发定制卷积神经网络（CNN）并在BraTS 2021数据集上训练，结合三种XAI技术：GRAD-CAM突出空间重要区域，LRP提供像素级相关性，SHAP量化特征贡献，形成多层次解释方法。

Result: 模型达到91.24%的准确率，成功识别完整和部分肿瘤，集成方法相比单一XAI技术展现出更优越的解释能力，能有效解释包括部分肿瘤可见性在内的各种预测情况。

Conclusion: 集成多种XAI技术能为医疗AI系统提供更全面的可解释性，增强脑肿瘤检测等关键医疗任务的可靠性和透明度，推动AI在医疗领域的可信应用。
...
TL;DR: 提出TEA方法，通过动态原位任务生成评估智能体在未见3D环境中的能力，发现现有模型在基础感知和3D交互意识方面存在严重缺陷
...
Method: 提出动态原位任务生成方法TEA，受人类认知启发。通过结构化图表示定义任务，构建两阶段交互-演化任务生成系统：交互阶段智能体与环境主动交互，创建任务执行与生成循环；演化阶段通过任务图建模重组和重用现有任务生成新任务，无需外部数据。

Result: 在10个未见场景中，TEA在两个周期内自动生成了87,876个任务，经人工验证具有物理合理性并涵盖基本日常认知能力。将SOTA模型与人类对比发现，尽管模型在公共基准上表现出色，但在基础感知任务上表现糟糕，严重缺乏3D交互意识，且对任务类型的推理高度敏感。
...
TL;DR: 提出新的相似度度量recos，通过排序向量分量归一化点积，比余弦相似度能更好地捕捉语义相似性
...
Method: 推导出比经典柯西-施瓦茨界更紧的点积上界，基于此提出recos度量，通过排序向量分量归一化点积，放宽完美相似性条件为序数一致性
...
TL;DR: 本文提出一个原则性框架，将LLM集成到安全管理决策支持中，通过一致性检查和数字孪生反馈控制幻觉风险，在事件响应场景中相比前沿LLM减少30%恢复时间。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在安全管理任务（如事件响应规划）中具有潜力，但其不可靠性和幻觉倾向是主要挑战。需要解决LLM在安全决策支持中的可靠性问题。

Method: 提出一个原则性框架，将LLM集成到迭代循环中：LLM生成候选动作，检查与系统约束和前瞻预测的一致性；当一致性低时，通过数字孪生等外部反馈收集信息，使用上下文学习精炼候选动作。

Result: 证明该设计可通过调整一致性阈值控制幻觉风险，并在特定假设下建立上下文学习的遗憾边界。在四个公共数据集的事件响应实验中，相比前沿LLM减少恢复时间达30%。

Conclusion: 该框架为在安全管理中使用LLM提供了一种原则性方法，通过一致性检查和反馈机制有效控制幻觉风险，显著提升事件响应效率。
...
TL;DR: 该论文认为"通用时间序列基础模型"存在范畴错误，将结构容器误认为语义模态。由于时间序列具有不相容的生成过程，单一模型会退化为昂贵的"通用滤波器"，无法在分布漂移下泛化。作者提出"自回归盲界"理论，并倡导用因果控制代理范式替代通用性，通过外部上下文协调专门化求解器层次结构。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列建模领域追求"通用基础模型"存在根本问题：将时间序列的结构容器属性误认为语义模态属性。不同领域的时间序列（如金融与流体动力学）具有不相容的生成过程，导致单一模型无法有效处理分布漂移和干预驱动的机制变化。

Method: 提出"自回归盲界"理论，证明仅依赖历史的模型无法预测干预驱动的机制变化。倡导采用"因果控制代理"范式，其中代理利用外部上下文协调专门化求解器的层次结构，包括冻结的领域专家和轻量级的即时适配器。

Result: 理论分析表明通用时间序列基础模型会退化为昂贵的"通用滤波器"，在分布漂移下泛化能力有限。提出的因果控制代理范式能够更好地处理干预驱动的机制变化，通过外部上下文协调专门化求解器实现更稳健的适应。

Conclusion: 应放弃追求通用性，转向因果控制代理范式。需要将基准测试从"零样本准确率"转向"漂移适应速度"，优先考虑稳健的控制理论系统，以应对时间序列中的分布漂移和干预驱动变化。
...
TL;DR: AMR是一个基于图神经网络的MOOC推荐框架，通过双向游走自动发现元路径，使用bi-LSTM编码器学习路径特定方面表示，并将其作为边特征融入学习者-学习者和知识概念-知识概念子图，实现细粒度语义感知的推荐。


<details>
  <summary>Details</summary>
Motivation: 传统MOOC推荐方法（协同过滤、基于内容过滤）存在数据稀疏性和过度专业化问题。现有图方法依赖人工预定义元路径，只能捕捉表面结构关系，且给领域专家带来负担和工程成本。

Method: 提出AMR框架：1）通过双向游走自动发现元路径；2）使用bi-LSTM编码器嵌入节点语义内容，生成方面感知的路径表示；3）将这些表示作为边特征融入学习者-学习者和KC-KC子图；4）实现细粒度语义感知的知识概念推荐。

Result: 在MOOCCube和PEEK数据集上的实验表明，AMR在HR@K和nDCG@K等关键指标上持续优于最先进的图神经网络基线。进一步分析证实AMR能有效捕捉丰富的路径特定方面信息。

Conclusion: AMR通过自动发现元路径和嵌入语义内容，克服了传统方法依赖预定义元路径的局限性，实现了更准确的MOOC推荐，为学习者提供更精细的语义感知推荐服务。
...
TL;DR: GPT-5在谈判任务上达到或超越商学院学生水平，但中低阶模型仍需改进，谈判行为分析揭示模型间存在欺骗、计算准确性等多维度差异


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在谈判这一核心商业任务中的能力，谈判需要战略推理、心智理论和经济价值创造，了解模型在现实商业场景中的表现

Method: 引入PieArena大规模谈判基准，基于MBA谈判课程的真实场景进行多智能体交互评估，研究联合意向性智能体支架的效果

Result: GPT-5达到AGI级别表现，匹配或超越经过一学期谈判训练和针对性指导的商学院学生；中低阶模型通过智能体支架获得显著改进，但前沿模型收益递减；谈判行为分析揭示模型在欺骗、计算准确性、指令遵从和感知声誉方面的异质性

Conclusion: 前沿语言智能体在智力和心理上已具备在高风险经济环境中部署的能力，但在鲁棒性和可信赖性方面仍存在挑战，需要进一步研究
...
TL;DR: ProAct框架通过两阶段训练让LLM智能体学习前瞻推理：第一阶段使用GLAD进行监督微调，将搜索树压缩为因果推理链；第二阶段使用MC-Critic增强策略梯度算法，通过环境rollout校准价值估计，提升长期规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在需要长期规划的交互环境中表现不佳，主要原因是模拟未来状态时会产生复合错误。需要一种方法让智能体能够内化准确的前瞻推理能力。

Method: 提出ProAct框架，包含两个阶段：1) GLAD：通过监督微调，将基于环境的搜索轨迹压缩为简洁的因果推理链；2) MC-Critic：可插拔的辅助价值估计器，利用轻量级环境rollout校准价值估计，增强PPO、GRPO等策略梯度算法。

Result: 在随机环境（如2048）和确定性环境（如Sokoban）中，ProAct显著提升了规划准确性。4B参数模型训练后超越了所有开源基线，媲美最先进的闭源模型，并在未见环境中展现出强大的泛化能力。

Conclusion: ProAct通过将复杂搜索树压缩为因果推理链，结合MC-Critic的价值校准，使LLM智能体能够有效内化前瞻推理能力，显著提升长期规划性能，同时保持计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂问题解决中表现出强大能力，但许多智能体系统由于内部工作流程不透明而难以解释和控制。虽然有些框架提供了明确的协作架构，但许多已部署的智能体系统对用户来说仍然是黑盒。

Method: 提出Agentic Workflow Reconstruction (AWR)任务，旨在仅使用输入输出访问来合成显式、可解释的替代工作流。开发AgentXRay框架，将AWR表述为链式结构工作流空间中离散智能体角色和工具调用的组合优化问题。采用蒙特卡洛树搜索，并通过基于评分的红黑剪枝机制增强，动态整合代理质量与搜索深度。

Result: 在不同领域的实验中，AgentXRay相比未剪枝搜索实现了更高的代理相似度，并减少了token消耗，在固定迭代预算下实现了更深层的工作流探索。
...
TL;DR: PATHWAYS基准测试包含250个多步决策任务，测试网络代理是否能发现并正确使用隐藏的上下文信息。结果显示当前代理架构在适应性调查、证据整合和判断覆盖方面存在严重缺陷。


<details>
  <summary>Details</summary>
Motivation: 评估网络代理在需要发现和使用隐藏上下文信息的复杂决策任务中的表现，揭示当前代理架构在适应性调查和证据整合方面的局限性。

Method: 创建包含250个多步决策任务的PATHWAYS基准测试，要求代理在网页环境中发现隐藏的上下文信息。测试了封闭和开放模型，分析代理在导航、证据检索、决策整合等方面的表现。

Result: 代理通常能导航到相关页面，但仅在少数情况下能检索到决定性的隐藏证据。当任务需要推翻误导性表面信号时，性能急剧下降至接近随机水平。代理经常幻觉化调查推理，声称依赖从未访问的证据。即使发现正确上下文，也常无法将其整合到最终决策中。

Conclusion: 当前网络代理架构缺乏可靠的适应性调查机制、证据整合能力和判断覆盖能力。提供更明确的指令能改善上下文发现，但会降低整体准确性，揭示了程序合规性与有效判断之间的权衡。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署需要极端量化，但低比特效率与性能之间存在关键权衡。残差二值化虽然实现硬件友好的无矩阵乘法推理，但存在病态特征共适应问题，特别是并行残差二进制路径学习冗余特征，限制了模型表达能力。

Method: 提出RaBiT量化框架，核心机制是从单个共享全精度权重顺序推导每个二进制路径，确保每个路径纠正前一个路径的误差。通过稳健初始化优先功能保留而非权重近似来稳定过程。
...
TL;DR: 研究评估了四个小型医疗LLM在眼科患者问答中的表现，发现Meerkat-7B表现最佳，MedLLaMA3-v20表现最差且有幻觉问题，GPT-4-Turbo评估与临床医生评分高度一致。


<details>
  <summary>Details</summary>
Motivation: 随着领域特定大语言模型在眼科患者教育、分诊和临床决策支持中的应用日益增多，需要严格评估以确保安全性和准确性，特别是对于资源高效部署的小型模型。

Method: 采用横断面研究设计，使用四个参数小于100亿的医疗LLM回答180个眼科患者问题，生成2160个回答。由三位不同资历的眼科医生和GPT-4-Turbo使用S.C.O.R.E.框架（安全性、共识与上下文、客观性、可重复性、可解释性）进行五级评分，通过Spearman秩相关、Kendall tau统计和核密度估计分析评估一致性。

Result: Meerkat-7B表现最佳（高级顾问评分3.44，顾问4.08，住院医师4.18），MedLLaMA3-v20表现最差，25.5%的回答包含幻觉或临床误导内容。GPT-4-Turbo评估与临床医生评分高度一致（Spearman rho=0.80，Kendall tau=0.67），但高级顾问评分更保守。

Conclusion: 医疗LLM在眼科问答中显示出安全潜力，但在临床深度和共识方面仍有差距。LLM评估可用于大规模基准测试，但需要混合自动化和临床医生审查框架来指导安全的临床部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于PINN的意见动力学建模方法存在局限性：1) 针对不完整先验设计，缺乏整合局部、全局和内生层次动力学的完整物理系统；2) 基于惩罚的约束难以深度编码物理先验，导致优化问题和潜在表示与物理透明度之间的差异。

Method: 1) 从相互作用粒子理论出发，通过扩散-对流-反应系统解释意见动力学；2) 基于神经ODE定义神经意见动力学，协调神经网络与物理先验；3) 提出OPINN框架，将物理先验深度整合到神经网络架构中。

Result: 在真实世界和合成数据集上评估，OPINN在意见演化预测方面实现了最先进的性能，为网络、物理和社会系统的交叉研究提供了有前景的范式。

Conclusion: OPINN框架成功地将物理可解释性与数据驱动的灵活性相结合，为意见动力学建模提供了更全面和有效的解决方案，有助于缓解社会极化和保障网络空间安全。
...
TL;DR: H-AdminSim：一个结合真实数据生成与多智能体仿真的医院行政工作流端到端仿真框架，用于系统评估LLM在行政自动化中的表现


<details>
  <summary>Details</summary>
Motivation: 医院行政部门每天处理大量操作任务（大型医院超过1万条请求），对LLM自动化需求增长。但现有研究主要关注医患交互或孤立行政子任务，未能捕捉真实行政工作流的复杂性。

Method: 提出H-AdminSim框架，结合真实数据生成与多智能体仿真来模拟医院行政工作流。通过FHIR集成提供统一互操作环境，使用详细评分标准对任务进行定量评估。

Result: 该框架为评估LLM驱动行政自动化的可行性和性能提供了标准化测试平台，能够系统比较不同LLM在复杂医院行政工作流中的表现。

Conclusion: H-AdminSim填补了现有研究的空白，通过端到端仿真框架解决了医院行政工作流复杂性的建模问题，为LLM在医疗行政自动化中的应用提供了系统评估工具。
...
Conclusion: THOR通过建模超关系知识图谱中的结构不变性，实现了有效的跨图谱归纳推理，为超关系知识图谱的链接预测提供了新的解决方案。
...
TL;DR: M²-Miner：首个基于蒙特卡洛树搜索的低成本自动化移动GUI代理数据挖掘框架，通过多智能体协作、意图回收策略和渐进式训练，显著提升数据挖掘效率和质量。


<details>
  <summary>Details</summary>
Motivation: 构建强大的GUI代理需要大规模高质量用户行为轨迹数据（意图-轨迹对）进行训练，但当前手动标注和GUI代理数据挖掘方法面临三个关键挑战：构建成本高、数据质量差、数据丰富度低。

Method: 提出M²-Miner框架：1）基于蒙特卡洛树搜索的自动化数据挖掘；2）协作多智能体框架（InferAgent指导、OrchestraAgent加速、JudgeAgent评估）；3）意图回收策略提取额外有价值的交互轨迹；4）渐进式模型在环训练策略提升数据挖掘成功率。

Result: 使用M²-Miner挖掘的数据进行微调的GUI代理在多个常用移动GUI基准测试中实现了最先进的性能，证明了该框架的有效性。

Conclusion: M²-Miner是首个低成本、自动化的移动GUI代理数据挖掘框架，通过创新的多智能体协作、意图回收和渐进式训练策略，解决了GUI代理数据挖掘的关键挑战，为社区研究提供了有力支持。
...
TL;DR: 该论文评估了时间序列基础模型在波动性电力市场中的日前电价预测性能，发现这些模型相比传统方法可提升高达37.4%的预测精度。


<details>
  <summary>Details</summary>
Motivation: 电力价格预测对能源市场参与者至关重要，但由于价格信号的波动性和非线性，传统统计和深度学习模型难以有效捕捉复杂的时间依赖关系并整合异构数据。虽然时间序列基础模型在通用时间序列预测任务中表现出色，但它们在波动性市场中的日前电价预测效果尚未得到充分探索。

Method: 提出尖峰正则化策略，并评估了多种时间序列基础模型（包括Tiny Time Mixers、MOIRAI、MOMENT和TimesFM），与传统统计和深度学习模型（如ARIMA、LSTM、CNN-LSTM）进行比较。使用新加坡波动性批发市场的半小时数据，并在适用模型中纳入外生因素（如天气和日历变量）。

Result: 时间序列基础模型在所有评估设置中始终优于传统方法，在MAPE指标上实现了高达37.4%的改进。

Conclusion: 研究结果为提高波动性电力市场的预测准确性和决策制定提供了实用指导，表明时间序列基础模型在电价预测任务中具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有条件表示学习方法存在两个关键限制：1) 对子空间基向量敏感；2) 容易受到子空间间干扰。这些限制影响了条件特征提取的准确性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习依赖标量奖励存在成本高、领域脆弱、无法理解解决方案底层逻辑的问题，阻碍了LLM实现专家级推理能力
...
Conclusion: ALIVE通过推理三位一体的方法促进了能力增长的自我维持轨迹，为无需人工监督的通用推理对齐提供了可扩展的基础
...
TL;DR: Phi-former：一种用于预测化合物-蛋白质相互作用的成对分层交互表示学习方法，通过整合基序（motif）的生物作用，在原子-原子、基序-基序和原子-基序三个层次上系统建模相互作用。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法虽然能在原子层面建模化合物-蛋白质相互作用（CPI），但忽略了化学现实——分子片段（基序或功能基团）通常是生物识别和结合的主要单元。需要一种能反映生物系统中分子识别机制的方法。

Method: 提出Phi-former方法：1）对化合物和蛋白质进行分层表示；2）采用成对预训练框架，在原子-原子、基序-基序和原子-基序三个层次上系统建模相互作用；3）设计层内和层间学习流程，使不同交互层次相互促进。

Result: Phi-former在CPI相关任务上表现出优越性能。案例研究表明，该方法能准确识别CPI中被激活的特定原子或基序，提供可解释的模型解释。

Conclusion: Phi-former通过整合基序的生物作用，更好地反映了生物识别机制，为理性药物设计和精准医疗应用提供了可解释的指导。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多媒体应用中存在自回归解码延迟高的问题。现有推测解码方法需要草稿模型，但获取、调优和维护草稿模型成本高、复杂度大。现有方法通常需要辅助训练或专门化，即使免训练方法也需要昂贵的搜索或优化。

Method: 提出SDFP框架，基于Fisher信息迹进行层剪枝来构建草稿模型。使用层敏感性作为输出扰动的代理，移除低影响层获得紧凑草稿模型，同时保持与原始模型的兼容性以进行标准推测验证。无需额外训练、超参数调优或单独维护草稿模型。

Result: 在基准测试中，SDFP实现了1.32x-1.5x的解码加速，且不改变目标模型的输出分布，支持低延迟多媒体应用。

Conclusion: SDFP提供了一种完全免训练、即插即用的框架，能够快速构建部署友好的草稿模型，有效降低大型语言模型在多媒体应用中的解码延迟。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI诊断资源在颌面病理学中缺乏高质量、结构化的多模态数据集，特别是对成釉细胞瘤的覆盖有限，格式不一致，无法直接用于模型训练

Method: 1) 创建专门针对成釉细胞瘤的多模态数据集，整合放射学、组织病理学和口腔临床图像；2) 使用自然语言处理技术从文本报告中提取临床相关特征；3) 对图像数据进行领域特定的预处理和增强；4) 开发多模态深度学习模型，用于分类变异、评估复发风险和支持手术规划

Result: 变异分类准确率从46.2%提升到65.9%，异常组织检测F1分数从43.0%提升到90.3%，相比MultiCaRe等资源有显著改进

Conclusion: 这项工作通过提供强大的数据集和适应性强的多模态AI框架，推进了患者特异性决策支持系统的发展
...
TL;DR: 提出Split Personality Training (SPT)方法，通过微调一个"诚实人格"到LoRA参数中，在模型正常运行时保持不活跃，需要审计时激活该人格来审查主模型的响应，有效检测大语言模型中的隐藏不对齐行为。


<details>
  <summary>Details</summary>
Motivation: 检测大语言模型中的不对齐行为很困难，因为模型可能在训练中学会隐藏不当行为。标准审计技术存在不足：黑盒方法难以区分不对齐输出和良性输出，而机制可解释性方法无法随模型能力扩展。
...
Result: 在Anthropic审计游戏模型有机体基准测试中，SPT达到96%的总体准确率，而Anthropic报告接近0%的准确率。诚实人格揭示了外部观察者无法访问的潜在知识，如受损模型训练时的虚构偏见。

Conclusion: SPT方法通过创建可激活的诚实人格，有效检测大语言模型中的隐藏不对齐行为，提供了一种可扩展的审计解决方案，能够揭示模型内部状态中隐藏的知识。
...
<details>
  <summary>Details</summary>
Motivation: 在安全关键应用和罕见事件模拟中，需要保证生成的样本以概率1满足特定约束条件，而现有的软约束或基于奖励的引导方法无法提供这种保证。

Method: 基于Doob's h-transform、鞅表示和二次变差过程，提出条件扩散引导框架：1）通过添加包含条件函数对数梯度的显式漂移修正来引导预训练扩散模型；2）提出基于鞅损失和鞅协变损失的两种离策略学习算法来估计h及其梯度。

Result: 在总变差距离和Wasserstein距离上提供了非渐近理论保证，明确刻画了分数近似和引导估计误差的影响。数值实验验证了方法在强制硬约束和生成罕见事件样本方面的有效性。

Conclusion: 该框架为扩散模型的条件生成提供了理论严谨的硬约束满足方法，通过鞅理论实现了高效的条件引导，在安全关键应用中具有重要价值。
...
TL;DR: RGCF-XRec是一个混合框架，通过推理引导的协同过滤知识增强语言模型，在单一步骤中实现可解释的顺序推荐，提升推荐准确性和解释质量。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在可解释推荐系统中忽视了协同信号，且通常将推荐和解释作为独立任务处理，导致内存占用大。需要一种能同时利用协同过滤知识和语言模型能力的一体化解决方案。

Method: 提出RGCF-XRec框架：1) 通过上下文提示进行推理引导的CF知识增强，发现潜在偏好和可解释推理路径；2) 基于连贯性、完整性、相关性和一致性的高效评分机制，减少噪声CF推理痕迹；3) 统一表示学习网络编码协同和语义信号，构建结构化提示来指导LLM进行可解释顺序推荐。

Result: 在Amazon数据集上表现优异：Sports和Toys的HR@10分别提升7.38%和4.59%，ROUGE-L分别提升8.02%和3.49%。冷启动场景整体提升14.5%，热启动提升11.9%。零样本HR@5在Beauty和Toys分别提升18.54%和23.16%。使用轻量级LLaMA 3.2-3B骨干确保训练效率和可扩展性。

Conclusion: RGCF-XRec通过整合推理引导的协同过滤知识和语言模型，在单一步骤中实现了高效、可解释的顺序推荐，显著提升了推荐性能、解释质量和泛化能力，同时保持了训练效率和可扩展性。
...
TL;DR: 论文提出一个测试时自优化框架，通过上下文学习和奖励反馈循环，让视觉语言模型能够像人类一样通过迭代优化解决七巧板几何推理问题，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 人类通过试错、观察和修正解决七巧板拼图，但现有视觉语言模型在连续几何推理上表现不佳（单块IoU仅0.41，两块组合降至0.23）。研究探索AI能否在测试时不更新参数的情况下迭代优化预测。

Method: 提出训练免费的验证器-优化器代理框架，结合上下文学习和奖励引导反馈循环。通过递归优化循环，基于几何一致性反馈迭代自优化预测，无需模型重新训练。

Result: 在中等三角形案例中，IoU从0.63提升至0.932。框架显著增强了视觉语言模型的几何推理能力，将自优化AI从承诺变为连续空间领域的实践。

Conclusion: 通过上下文学习和奖励循环融入人类启发的迭代优化机制，可以大幅提升视觉语言模型的几何推理能力，为自优化AI在连续空间领域提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探究LLMs是否能在个体层面预测认知偏见，并在认知负荷等情境因素与偏见交互时，模拟人类偏见行为的动态变化，这对设计偏见感知的交互式AI系统至关重要。

Method: 将三个经典决策场景改编为对话设置，进行人类实验（N=1100），参与者通过简单或复杂对话与聊天机器人互动。使用参与者人口统计数据和对话记录，基于GPT-4和GPT-5模拟相同交互条件。

Result: 人类实验显示出稳定的偏见模式。LLMs能够精确复现人类偏见，GPT-4和GPT-5在模拟人类行为对齐方面存在显著差异。

Conclusion: LLMs能够准确模拟人类在交互决策中的偏见行为，这对设计和评估适应性的、偏见感知的LLM基AI系统具有重要启示，不同模型在行为对齐方面存在差异需要特别关注。
...
TL;DR: 本文提出GETR方法，通过图神经网络增强的标记表示进行跨语言知识迁移，在低资源语言任务上显著优于现有方法
...
Method: 提出GETR（图增强标记表示）方法，用于跨语言知识迁移。同时采用两个基线方法：(a)隐藏层增强和(b)通过标记翻译进行标记嵌入迁移。

Result: GETR方法在真正低资源语言（Mizo、Khasi）的词性标注任务上比现有方法提升13个百分点；在模拟低资源语言（Marathi、Bangla、Malayalam）的情感分类和命名实体识别任务上分别提升20和27个百分点的宏F1值。

Conclusion: 基于图神经网络的方法显著优于现有的多语言和跨语言基线方法，并对迁移机制进行了详细分析，识别了在此语言背景下成功知识迁移的关键因素。
...
TL;DR: 提出Resin概率编程语言和Reactive Circuits结构，通过异步推理和基于信号变化频率的计算分区，在动态环境中实现高效精确推理


<details>
  <summary>Details</summary>
Motivation: 复杂概率模型的精确推理计算成本过高，现有方法在动态环境中效率低下，无法利用现实世界信息流更新速率异质性的特点

Method: 1) 提出Resin概率编程语言，融合概率逻辑与响应式编程；2) 提出Reactive Circuits作为元结构，基于代数电路和异步数据流构建时间动态DAG，根据输入信号波动性自适应调整

Result: 在高保真无人机群模拟中，相比频率无关推理方法获得数个数量级的加速；RC结构成功捕捉环境动态，显著降低延迟，支持响应式实时推理

Conclusion: 通过基于异步输入变化频率的计算分区，将大型推理任务分解为可记忆化的子问题，确保仅重新评估受新信息影响的模型组件，大幅减少流式上下文中的冗余计算
...
TL;DR: 提出Generative Ontology框架，结合传统本体论的结构严谨性和大语言模型的创造力，通过可执行的Pydantic模式约束LLM生成，并采用多智能体管道实现领域专业设计。


<details>
  <summary>Details</summary>
Motivation: 传统本体论擅长描述领域结构但无法生成新产物，而大语言模型能流畅生成但缺乏结构有效性，经常产生幻觉。需要结合两者的互补优势：本体论提供语法结构，LLM提供创造力。

Method: 将领域知识编码为可执行的Pydantic模式，通过DSPy签名约束LLM生成。采用多智能体管道，为不同本体领域分配专业角色（如机制架构师、主题编织者、平衡批评家），每个智能体带有防止浅层输出的"专业焦虑"。结合检索增强生成和迭代验证确保机制与组件的一致性。
...
TL;DR: 该论文是关于基于图的智能体记忆系统的综述，探讨了图结构如何增强LLM智能体在长期复杂任务中的记忆能力，包括记忆分类、生命周期技术分析、开源资源整理和应用场景。


<details>
  <summary>Details</summary>
Motivation: 随着基于LLM的智能体在长期复杂任务（如多轮对话、游戏、科学发现）中的广泛应用，记忆成为核心模块。图结构因其内在的建模关系依赖、组织层次信息和高效检索能力，成为智能体记忆的强大结构。本文旨在系统综述基于图的智能体记忆研究现状。

Method: 1. 提出智能体记忆的分类法：短期vs长期记忆、知识vs经验记忆、非结构化vs结构化记忆，并从图实现视角分析
2. 按照智能体记忆生命周期系统分析关键技术：记忆提取（数据转换）、存储（高效组织）、检索（支持推理的相关内容检索）、演化（内容更新）
3. 总结支持自演化智能体记忆开发和评估的开源库与基准

Result: 1. 建立了基于图的智能体记忆系统分类框架
2. 系统分析了记忆生命周期中的关键技术
3. 整理了相关开源资源（研究论文、开源数据、项目）并创建了GitHub资源库
4. 探索了多样化的应用场景
5. 识别了关键挑战和未来研究方向

Conclusion: 基于图的智能体记忆系统在增强LLM智能体处理长期复杂任务能力方面具有重要价值。该综述为社区提供了全面的技术分析和资源整理，旨在推动更高效可靠的图基智能体记忆系统发展。所有相关资源已收集在GitHub仓库中供社区使用。
...
TL;DR: 本文提出了一种基于Transformer架构计算和内存访问复杂度的分析模型，能够准确预测LLM推理的能耗曲线，发现能效存在"甜点区"（中等长度输入输出），通过序列长度优化可显著降低能耗。


<details>
  <summary>Details</summary>
Motivation: 现有LLM能耗估计方法通常使用输入输出序列长度的简单线性函数，但实际观察发现能效存在明显的非线性依赖关系：中等长度输入输出时能效最高，而长输入或极短输出时能效急剧下降。需要更准确的能耗模型来指导生产系统中的序列优化策略。

Method: 基于Transformer架构的计算和内存访问复杂度推导出分析模型，使用TensorRT-LLM在NVIDIA H100 GPU上评估能耗，测试了OPT、LLaMA、Gemma、Falcon、Qwen2、Granite等1B到9B参数的多样化LLM，输入输出长度范围从64到4096个token。

Result: 提出的分析模型平均绝对百分比误差（MAPE）仅为1.79%，验证了模型的准确性。发现能效存在明显的"甜点区"：短到中等长度输入和中等长度输出时能效最高，而长输入或极短输出时能效显著下降。

Conclusion: 通过将序列长度与能效"甜点区"对齐，可以显著降低LLM推理的能耗，支持生产系统中的智能截断、摘要和自适应生成策略，为绿色AI部署提供理论指导。
...
Method: 提出GenLoRA，用非线性基向量生成替代显式基向量存储。为每个低秩矩阵维护潜在向量，使用一组轻量径向基函数(RBFs)合成基向量。每个RBF参数远少于显式基向量。

Result: 在多个数据集和架构上的实验表明，GenLoRA在更小参数预算下获得更高有效LoRA秩，实现更优的微调性能。

Conclusion: GenLoRA通过非线性基向量生成提高了参数效率，在有限参数预算下获得更高模型容量，为参数高效微调提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 论文发现强化学习与可验证奖励（RLVR）中存在系统性病理问题——递归空间收缩（RSC），即有效替代方案的采样概率消失。虽然KL正则化试图缓解这一问题，但它强加了严格的形状匹配约束，导致与正确性所需的锐化产生梯度冲突。

Method: 提出锚定策略优化（APO），将范式从全局形状匹配转向支持覆盖。通过基于参考模型高置信度支持定义安全流形，APO允许为效率进行激进锐化，同时在错误校正期间选择性调用恢复力以防止崩溃。

Result: 在数学基准测试中，APO打破了准确性-多样性权衡，显著提高了Pass@1，同时恢复了标准策略梯度方法通常丢失的Pass@K多样性。

Conclusion: APO作为一种梯度对齐机制，通过最大化支持覆盖实现弹性恢复，有效解决了RLVR中的递归空间收缩问题，为强化学习优化提供了新思路。
...
Method: 提出RLFKV框架：1) 将金融回答分解为原子知识单元；2) 评估每个单元的正确性计算细粒度忠实度奖励；3) 引入信息量奖励防止奖励攻击；4) 结合强化学习优化模型
...
TL;DR: LeakBoost是一种基于感知损失的主动探测框架，通过优化合成图像来放大模型对训练成员和非成员样本的内部表示差异，显著提升现有成员推理攻击的效果。
...
Method: 提出LeakBoost框架：给定候选输入，通过优化感知（激活空间）目标合成探测图像，放大成员和非成员在模型内部表示上的差异，然后将该图像输入现成的成员检测器进行分析。

Result: LeakBoost与现有方法结合后，在多个图像分类数据集和不同神经网络架构上取得显著改进：AUC从接近随机水平（0.53-0.62）提升到0.81-0.88，在1%假阳性率下的真阳性率提高了一个数量级以上。

Conclusion: LeakBoost提供了一种模块化且计算高效的方法来评估白盒设置下的隐私风险，推动了动态成员推理的研究，特别是在深层网络和基于梯度的检测器中效果最佳。
...
TL;DR: 研究将AI智能体自动优化方法应用于形式验证领域（特别是Rocq自动定理证明），评估不同优化器在优化证明生成智能体时的表现，发现简单的小样本引导方法最有效，但仍无法超越精心设计的最先进证明智能体。


<details>
  <summary>Details</summary>
Motivation: 研究自动AI智能体优化方法在现实世界形式验证场景中的适用性，探索能否自动化智能体系统的精细调优（如提示设计、上下文知识、控制策略等），以减轻人工工程负担。

Method: 以Rocq自动定理证明为代表性挑战领域，评估不同自动智能体优化器在优化Rocq证明生成智能体任务上的表现，比较各种优化方法的效果。

Result: 多个优化器都能带来可测量的改进，但简单的小样本引导方法是最一致有效的；然而，所有研究的自动优化方法都无法达到精心设计的最先进证明智能体的性能水平。

Conclusion: 自动智能体优化方法在形式验证领域有一定应用潜力，但当前技术仍无法完全替代人工精心设计的智能体系统，小样本引导是相对最有效的自动化方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型的RL训练框架（如RLinf）采用同步执行，导致环境交互、策略生成和模型更新阶段的资源利用率低下和吞吐量受限，成为训练效率瓶颈

Method: 提出完全异步策略训练框架，采用多级解耦架构：异步并行化环境交互与轨迹收集、流式执行策略生成、解耦调度训练更新，借鉴大模型RL中的异步优化思想

Result: 在LIBERO基准上，相比现有同步策略实现最高59.25%的吞吐量提升，深度优化分离策略后可达126.67%提升；消融实验验证各异步组件的有效性；8-256 GPU规模扩展验证了良好的可扩展性

Conclusion: 首次实现VLA模型从环境交互到策略更新的全流程异步训练框架，显著提升训练效率，为大规模VLA模型训练提供高效解决方案
...
TL;DR: FiMI是专门为印度数字支付系统开发的金融语言模型，基于Mistral Small 24B架构，通过多阶段训练流程，在金融推理和工具调用方面显著优于基础模型。
...
Result: FiMI Base在金融推理基准上比Mistral Small 24B Base提升20%；FiMI Instruct在领域特定工具调用上比Mistral Small 24B Instruct提升87%；同时在通用基准上保持可比性能。
...
TL;DR: 提出NEX框架，通过分析激活神经元模式自动评估推理质量，无需标签即可预测下游任务准确性


<details>
  <summary>Details</summary>
Motivation: 大语言模型推理时经常采样多个思维链或搜索合并检查点，这使选择成为瓶颈，且通常缺乏目标分布的监督。需要一种无需标签的方法来评估推理质量。

Method: 提出NEX框架，将推理视为探索(E-phase)和利用(X-phase)交替的过程。通过检测新激活的MLP神经元峰值识别E阶段，使用两状态隐马尔可夫模型推断E-X阶段，并根据E引入的神经元是否在后续X阶段被重用来评估其贡献。

Result: 在推理基准测试和Qwen3合并系列中，NEX仅使用少量未标记激活数据即可预测下游准确性并识别更好的变体。通过人类标注验证了E-X信号的有效性，并通过"有效vs冗余"神经元转移提供了因果证据。

Conclusion: NEX提供了一种无需标签、可解释的推理质量评估方法，能够有效识别冗余探索并选择最佳推理路径，为模型推理优化提供了新工具。
...
Result: STProtein能够有效解决空间蛋白质组数据的稀缺性问题，加速空间多组学整合，有望在生命科学领域催化突破性进展。

Conclusion: 该工具使科学家能够加速发现组织内蛋白质的复杂空间模式，揭示标记基因间的新关系，探索生物"暗物质"，推动生命科学研究。
...
TL;DR: TKG-Thinker：一种具有自主规划和自适应检索能力的新型智能体，通过动态多轮交互和双训练策略（SFT+RL）解决TKGQA中的推理幻觉和静态提示限制问题，在基准数据集上达到SOTA性能。
...
Result: 在基准数据集上使用三个开源LLM进行实验，TKG-Thinker取得了最先进的性能，并在复杂TKGQA设置中展现出强大的泛化能力。

Conclusion: TKG-Thinker通过动态多轮交互和双训练策略有效解决了TKGQA中的推理幻觉和静态提示限制问题，为时间知识图谱问答提供了更有效的解决方案。
...
TL;DR: 提出一种新的布尔网络学习方法，通过连接学习、紧凑卷积和自适应离散化三个角度，在保持精度的同时大幅减少布尔运算量，相比现有方法在精度-计算帕累托前沿上显著提升。


<details>
  <summary>Details</summary>
Motivation: 浮点神经网络在推理时计算成本高，而布尔网络适合资源受限环境，但学习紧凑且准确的布尔网络具有组合复杂性挑战。

Method: 1) 提出无额外参数的学习连接策略；2) 引入利用局部性的紧凑卷积布尔架构，减少布尔运算；3) 提出自适应离散化策略，降低从连续值网络转换到布尔网络的精度损失。

Result: 在标准视觉基准测试中，该方法在精度与计算的帕累托前沿上显著优于现有方法，在达到更好精度的同时，布尔运算量最多减少37倍。

Conclusion: 通过三个创新角度解决了布尔网络学习的挑战，实现了更高效、更准确的布尔网络，为资源受限环境提供了有效的解决方案。
...
TL;DR: OmniVideo-R1：一个通过强化学习框架提升音视频理解能力的模型，采用查询密集型定位和模态注意力融合策略


<details>
  <summary>Details</summary>
Motivation: 人类通过多种模态协同感知世界，但现有的全视频模型在音视频理解任务上仍面临挑战，需要提升多模态推理能力

Method: 提出强化框架OmniVideo-R1，包含两个关键策略：1）基于自监督学习的查询密集型定位；2）基于对比学习的模态注意力融合
...
TL;DR: BABE是一个用于评估生物AI系统实验推理能力的基准测试，基于同行评审研究论文和真实生物研究构建，专注于因果推理和跨尺度推断能力。


<details>
  <summary>Details</summary>
Motivation: 现有生物学基准测试未能充分评估研究人员所需的关键技能：将实验结果与背景知识结合以得出有意义结论的能力。大语言模型在科学推理方面能力增强，但缺乏评估其真实科学推理能力的基准。
...
Conclusion: BABE基准填补了现有生物学评估的空白，能够更真实地评估AI系统的实验推理能力，为衡量AI在生物学研究中的实际应用潜力提供了重要工具。
...
TL;DR: 提出分层座位分配问题(HSAP)，解决组织团队在楼层平面图中的最优座位分配，确保层级关系紧密的团队座位相邻，通过概率路线图(PRM)和快速探索随机树(RRT)计算座位距离，结合启发式搜索和动态规划用整数规划求解。


<details>
  <summary>Details</summary>
Motivation: 大型组织需要确保具有紧密层级关系的团队（如研究小组）座位相邻，形成连续区域。目前这个问题是手动处理的，导致重新规划频率低且效果不理想，需要自动化解决方案。

Method: 提出端到端框架：1) 使用概率路线图(PRM)和快速探索随机树(RRT)计算任意座位对之间的距离；2) 结合启发式搜索和动态规划方法，通过整数规划求解HSAP问题。

Result: 在不同规模实例下评估PRM框架和座位分配方案，进行了定量和定性分析，证明了方法的有效性。

Conclusion: 提出的HSAP解决方案能够自动化处理大型组织的分层座位分配问题，解决了手动规划的低效问题，为组织座位布局优化提供了系统化方法。
...
<details>
  <summary>Details</summary>
Motivation: 车载LLM对话代理与外部服务（如Google的A2A协议）协调时，会创建新的攻击面，恶意操作可能通过自然语言载荷传播，导致从驾驶员分心到未经授权的车辆控制等严重后果。现有的AI安全框架缺乏安全关键系统工程中的"关注点分离"标准，将保护对象（资产）与攻击方式（攻击路径）混为一谈。

Method: 提出AgentHeLLM威胁建模框架，正式分离资产识别与攻击路径分析。引入基于伤害导向"受害者建模"的人本资产分类法（受《世界人权宣言》启发），以及区分毒化路径（恶意数据传播）与触发路径（激活操作）的基于图的正式模型。开发开源攻击路径建议工具AgentHeLLM Attack Path Generator，使用双层搜索策略自动化多阶段威胁发现。

Result: 开发了实用的攻击路径建议工具，能够自动化发现多阶段威胁，展示了框架的实际适用性。

Conclusion: AgentHeLLM框架填补了车载LLM对话代理安全威胁建模的方法论空白，通过正式分离资产与攻击路径分析，为安全关键系统提供了更严谨的安全工程方法。
...
TL;DR: 本文针对大语言模型在建模与仿真应用中的使用误区，提供了全面的实践指导，强调原则性设计、诊断策略和实证评估的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在建模与仿真工作流中的广泛应用，许多看似简单的实践可能引入微妙问题、不必要的复杂性甚至导致结果劣化。作者旨在为LLMs的使用提供全面实用的指导，帮助建模者做出明智决策。

Method: 通过分析LLMs在M&S应用中的常见误区，包括非确定性、知识增强（RAG和LoRA）、M&S数据分解和超参数设置等问题，提出原则性设计选择、诊断策略和实证评估方法。

Result: 论文识别了LLMs在M&S应用中的关键挑战和误区，如添加更多数据可能适得其反、微调前缺乏评估、温度设为0仍无法保证确定性、大量M&S数据输入可能过度等问题。

Conclusion: LLMs在M&S应用中需要谨慎使用，建模者应基于原则性设计、诊断策略和实证评估来决策何时、如何以及是否依赖LLMs，避免常见误区并优化使用效果。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在探索量子增强的强化学习方法在复杂组合优化问题（如CVRP）中的潜力，比较经典、全量子和混合架构的性能差异。

Method: 采用Advantage Actor-Critic (A2C)智能体，实现经典、全量子和混合三种变体，集成transformer架构通过自注意力和交叉注意力机制捕捉车辆、客户和仓库之间的关系。实验针对20个客户和4辆车的多车辆容量约束场景，进行10次独立运行。

Result: 所有三种方法都能学习有效的路径规划策略，但量子增强模型优于经典基线，产生更稳健的路线组织。混合架构在距离、紧凑性和路线重叠方面表现最佳。定性可视化显示量子模型生成更结构化、连贯的路线解决方案。
...
TL;DR: 本文提出了一种地理感知的Transformer交通预测模型(GATTF)，利用传感器间的互信息来捕捉地理关系，提高了高速公路交通预测的准确性，且不增加模型复杂度。


<details>
  <summary>Details</summary>
Motivation: 高速公路数字孪生技术需要高分辨率实时交通数据和预测能力，但交通动态的时空复杂性使得预测困难。现有序列深度学习模型在捕捉长期时间依赖方面有优势，但预测精度和模型复杂度仍需改进。

Method: 提出地理感知的Transformer交通预测模型(GATTF)，利用分布式传感器间的互信息来捕捉地理关系，增强模型对空间相关性的理解，同时保持Transformer架构的优势。

Result: 使用瑞士日内瓦高速公路网络的实时数据进行评估，结果表明：通过互信息融入地理感知的GATTF模型相比标准Transformer提高了预测准确性，且没有增加模型复杂度。

Conclusion: 地理感知的Transformer模型能够有效提升高速公路交通预测性能，为数字孪生交通管理系统提供了更准确的预测支持，同时保持了模型的简洁性。
...
TL;DR: 该研究探索了使用Whisper预训练ASR模型进行语音情感识别，提出了两种基于注意力的池化方法来降低Whisper表征维度，在波斯语数据集上取得了SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 语音情感识别研究面临标准大数据集缺乏的限制，现有研究使用预训练模型提取特征。本研究探索Whisper预训练ASR系统在语音情感识别中的能力。

Method: 提出了两种基于注意力的池化方法：多头注意力平均池化和QKV池化，用于高效降低Whisper表征维度同时保留情感特征。在英语(IEMOCAP)和波斯语(ShEMO)数据集上实验，使用Whisper Tiny和Small模型。

Result: 多头QKV架构在ShEMO数据集上取得了最先进的结果，未加权准确率提高了2.47%。发现中间层在波斯语数据集上表现更好，为HuBERT X-Large等大型模型提供了轻量高效的替代方案。

Conclusion: Whisper作为语音情感识别的表征提取器具有潜力，基于注意力的池化方法在维度减少方面有效，中间层特征更适合情感识别任务。
...
TL;DR: AgenticPay是一个用于多智能体买卖谈判的基准测试和仿真框架，通过自然语言驱动谈判，包含110多个任务，评估LLM在商业谈判中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试缺乏评估多智能体之间语言驱动的经济交互的原则性设置，而基于LLM的智能体越来越需要在谈判、协调和交易中自主运作。

Method: 构建AgenticPay框架，模拟买卖双方拥有私有约束和产品相关估值的市场环境，通过多轮语言谈判而非单纯数字竞价达成协议，包含结构化动作提取和可行性、效率、福利等指标。
...
TL;DR: 开发基于VR行为数据的离散事件模拟器，用于评估学校安全干预策略，特别是机器人干预枪手策略，实现大规模可扩展评估


<details>
  <summary>Details</summary>
Motivation: VR是评估学校安全措施的有力工具，但需要为每个条件招募新参与者，难以进行大规模或迭代评估，特别是学习有效干预策略需要大量训练

Method: 开发数据驱动的离散事件模拟器，将枪手移动和区域内行为建模为从VR研究中参与者行为学习的随机过程，用于评估机器人干预策略

Result: 模拟器能够重现关键经验模式，实现干预策略的可扩展评估和学习，这些策略难以直接通过人类受试者训练
...
TL;DR: DyTopo：一种动态拓扑多智能体框架，通过每轮重构稀疏有向通信图来优化LLM多智能体系统的协作效率


<details>
  <summary>Details</summary>
Motivation: 现有基于提示的大语言模型多智能体系统通常采用固定的通信模式，无法适应迭代问题解决过程中不同阶段的需求变化，导致协作效率低下

Method: DyTopo采用管理者引导的多智能体框架，每轮根据管理者的目标，让每个智能体输出轻量级自然语言查询（需求）和关键（提供）描述符，通过语义匹配构建稀疏有向通信图，仅沿诱导边传递私有消息

Result: 在代码生成和数学推理基准测试中，使用四种LLM骨干网络，DyTopo始终优于最强基线（平均提升6.2%），并能通过演化图提供可解释的协调轨迹

Conclusion: DyTopo通过动态重构通信拓扑显著提升了多智能体系统的推理性能，同时提供了可解释的协作过程可视化，为理解多智能体交互提供了新视角
...
TL;DR: 研究纳米孔测序中的上下文相关删除错误，提出两种模型：1）当运行长度阈值k=Clogn时，构建可纠正t个上下文删除的高效编码；2）当k为常数时，研究极值上下文删除信道下的最大可达速率。


<details>
  <summary>Details</summary>
Motivation: 纳米孔测序器在DNA数据存储系统中会产生上下文相关的删除错误，这与传统假设的独立随机删除不同。需要设计针对这种上下文相关删除错误的纠错编码方案。

Method: 提出两种模型：1）确定性单删除模型，当运行长度≥k时发生删除；2）极值上下文删除信道模型。对于第一种模型，分析k=Clogn时的冗余度界限并设计高效可编解码的编码。对于第二种模型，研究k为常数时的最大可达速率。

Result: 对于k=Clogn模型，最小冗余度在(1-C)tlog n到2(1-C)tlog n之间，是任意t删除纠错编码的(1-C)倍。设计了高效可编解码的编码，特别是当t=1且C>1/2时，冗余度基本匹配非构造性上界。对于极值上下文删除信道，获得了任意常数k下的最大可达速率的尖锐界限。

Conclusion: 上下文相关的纳米孔删除错误需要专门的编码方案。提出的模型和编码设计为DNA数据存储系统中的纳米孔测序错误提供了有效的纠错解决方案，显著降低了冗余度要求。
...
TL;DR: 提出从代数曲线构造准循环码和广义准循环码的新方法，适用于Kummer扩张曲线，包括超椭圆曲线、范迹曲线和Hermitian曲线，可获得灵活共指数的QC码。


<details>
  <summary>Details</summary>
Motivation: 以往基于椭圆曲线的QC码构造方法有限，需要更通用的构造框架来生成具有灵活参数的准循环码，特别是适用于更广泛的代数曲线类型。

Method: 利用代数曲线（特别是有理函数域的Kummer扩张曲线）构造QC和GQC码，基于已知的自同构群分类推导显式参数公式，适用于超椭圆曲线、范迹曲线和Hermitian曲线等。

Result: 成功构建了具有灵活共指数的QC码，扩展了QC码的构造范围，为基于代数曲线的编码理论提供了新的构造框架。

Conclusion: 该方法突破了传统椭圆曲线构造的限制，为从更广泛的代数曲线家族构造QC和GQC码提供了系统方法，具有重要的理论和应用价值。
...
TL;DR: 提出稀疏非均匀采样的信道探测框架，结合LR-SAGE算法提取多径分量，实现50倍测量加速、98%数据量减少和99.96%后处理复杂度降低


<details>
  <summary>Details</summary>
Motivation: 6G AI和ISAC需要大规模真实信道数据集，但传统频域信道探测方法因需避免时延模糊而采样点过多，效率低下

Method: 提出抛物线频率采样(PFS)策略进行非均匀频率点分布，结合似然校正空间交替广义期望最大化(LR-SAGE)算法提取多径分量

Result: 在280-300GHz验证，实现50倍测量加速、98%数据量减少、99.96%后处理复杂度降低，同时能准确捕获与传统测量一致的多径分量和信道特性

Conclusion: 该框架为构建AI原生6G系统所需的大规模ISAC数据集提供了基础使能技术
...
TL;DR: 本文构造了在任意域上具有高效列表译码能力的线性化和秩度量码，实现了超过唯一译码半径的错误纠正能力。


<details>
  <summary>Details</summary>
Motivation: 和秩度量在多种编码应用中都很重要，但现有和秩度量码缺乏高效的列表译码方案。本文旨在填补这一空白，构建能够超越唯一译码半径的高效列表译码和秩度量码。

Method: 通过构造线性化Reed-Solomon码的子码，利用子空间设计限制消息多项式，并扩展线性代数译码框架到折叠线性化Reed-Solomon码。

Result: 成功构造了任意域上的线性化和秩度量码，实现了错误率ρ下的高效列表译码，码率为1-ρ-ε，列表大小受多项式界限制。

Conclusion: 本文首次提供了正码率、高效列表译码超越唯一译码半径的和秩度量码的显式构造，为和秩度量下的高效译码提供了新框架。
...
Method: 将远场波束覆盖问题建模为空间频率滤波问题，通过逆傅里叶变换实现角度覆盖；针对有限天线数引入的滚降效应，提出具有保护缩放的滚降感知设计；将方法扩展到近场场景，通过一阶泰勒近似将二维波束覆盖问题转化为二维逆傅里叶变换

Result: 提出的傅里叶变换方法在实现与传统采样优化方法相当的波束形成性能的同时，显著降低了计算复杂度；在近场场景中观察到固有的距离散焦效应
...
TL;DR: 本文针对Tsallis熵生成的Bregman散度（即β-散度），建立了广义Pinsker不等式，确定了最优常数C_{α,K}，将KL散度的Pinsker不等式推广到更一般的散度度量。


<details>
  <summary>Details</summary>
Motivation: 传统Pinsker不等式将KL散度下界表示为总变差，用于将KL散度控制转换为L1范数控制。本文的动机源于Tsallis损失的概率预测和在线学习应用，需要将这种关系推广到更一般的Bregman散度。

Method: 针对由负α-Tsallis熵生成的Bregman散度D_α（也称为β-散度），在概率单纯形Δ^K的相对内部，建立了广义Pinsker不等式，并明确确定了最优常数C_{α,K}。

Result: 证明了对于任意p,q∈Δ^K的相对内部，有尖锐的不等式D_α(p∥q) ≥ C_{α,K}/2·∥p-q∥₁²，并为每一对(α,K)确定了最优常数C_{α,K}。

Conclusion: 成功将经典Pinsker不等式推广到Tsallis熵生成的Bregman散度，为概率预测和在线学习应用提供了理论工具，建立了散度与L1距离之间的定量关系。
...
TL;DR: 该论文提出了一种基于加权比例公平的迭代启发式算法，用于优化XR应用的上行MU-MIMO调度，以最大化跨层及时吞吐量，同时满足用户PAoI约束。


<details>
  <summary>Details</summary>
Motivation: 扩展现实(XR)应用对延迟敏感，需要优化上行多用户MIMO调度以最大化及时吞吐量。现有方法难以在满足用户信息新鲜度(PAoI)约束的同时保持系统吞吐量。

Method: 采用基于加权比例公平的迭代启发式算法，将PAoI度量作为约束条件，通过无信令方式动态调整调度权重，解决NP-hard的跨层优化问题。

Result: 数值仿真表明，所提算法在保持整体系统吞吐量的同时，显著提升了XR容量，优于现有基线方法。

Conclusion: 该研究为XR应用的上行MU-MIMO调度提供了一种有效的跨层优化方案，能够在满足用户信息新鲜度要求的同时最大化及时吞吐量。
...
TL;DR: 论文证明存在一个通用码本，对所有可能的X统计特性都接近最优，性能至少相当于X自适应水填充码本降低0.11比特/维度的效果。


<details>
  <summary>Details</summary>
Motivation: LLM部署中，矩阵乘积W^T X的高效计算是关键。权重量化是常用方法，但理论上最优的量化算法依赖于X的统计特性，需要将矢量量化码本与X的PCA方向对齐（水填充分配）。然而，码本对X统计特性的依赖在实际中很不实用。

Method: 通过理论证明存在一个通用码本，该码本对所有可能的X统计特性都同时接近最优。证明是非构造性的，表明存在一个在ℝ^n中的网，该网在所有希尔伯特范数下都能同时近乎最优地覆盖球面。

Result: 存在一个通用码本，其性能至少相当于X自适应水填充码本降低0.11比特/维度的效果。这意味着存在一个对所有统计特性都接近最优的量化方案，可作为低精度存储格式的理想候选。

Conclusion: 虽然证明是非构造性的，但该结果表明存在一个通用量化方案，能够避免对输入统计特性的依赖，为LLM权重量化提供了理论上的最优解，但实际构造这样的码本仍是未来研究的挑战。
